!*==chetf2_rook.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!                 (used to handle NaN and Inf)
!
                     IF ( ABS(REAL(A(imax,imax)))>=alpha*rowmax ) THEN
!
...
!
                        d11 = ONE/REAL(A(k,k))
                        CALL CHER(Uplo,N-k,-d11,A(k+1,k),1,A(k+1,k+1),  &
     &                            Lda)
!
!                    Store L(k) in column k
!
