!*==chetrd_he2hb.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Parameters ..
      ...
      PARAMETER (RONE=1.0E+0,ZERO=(0.0E+0,0.0E+0),ONE=(1.0E+0,0.0E+0),  &
     &           HALF=(0.5E+0,0.0E+0))
!     ..
!     .. Local Scalars ..
...
!     .. External Subroutines ..
      EXTERNAL XERBLA , CHER2K , CHEMM , CGEMM , CCOPY , CLARFT ,       &
     &         CGELQF , CGEQRF , CLASET
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MIN , MAX
!     ..
...
!
            CALL CGELQF(Kd,pn,A(i,i+Kd),Lda,Tau(i),Work(s2pos),ls2,     &
     &                  iinfo)
!
...
!            Form the matrix T
!
            CALL CLARFT('Forward','Rowwise',pn,pk,A(i,i+Kd),Lda,Tau(i), &
     &                  Work(tpos),ldt)
!
!            Compute W:
!
            CALL CGEMM('Conjugate','No transpose',pk,pn,pk,ONE,         &
     &                 Work(tpos),ldt,A(i,i+Kd),Lda,ZERO,Work(s2pos),   &
     &                 lds2)
!
            CALL CHEMM('Right',Uplo,pk,pn,ONE,A(i+Kd,i+Kd),Lda,         &
     &                 Work(s2pos),lds2,ZERO,Work(wpos),ldw)
!
            CALL CGEMM('No transpose','Conjugate',pk,pk,pn,ONE,         &
     &                 Work(wpos),ldw,Work(s2pos),lds2,ZERO,Work(s1pos),&
     &                 lds1)
!
            CALL CGEMM('No transpose','No transpose',pk,pn,pk,-HALF,    &
...
!
            CALL CHER2K(Uplo,'Conjugate',pn,pk,-ONE,A(i,i+Kd),Lda,      &
...
!
            CALL CGEQRF(pn,Kd,A(i+Kd,i),Lda,Tau(i),Work(s2pos),ls2,     &
     &                  iinfo)
!
!            Copy the upper portion of A into AB
!
...
!            Form the matrix T
!
            CALL CLARFT('Forward','Columnwise',pn,pk,A(i+Kd,i),Lda,     &
     &                  Tau(i),Work(tpos),ldt)
!
!            Compute W:
!
            CALL CGEMM('No transpose','No transpose',pn,pk,pk,ONE,      &
     &                 A(i+Kd,i),Lda,Work(tpos),ldt,ZERO,Work(s2pos),   &
     &                 lds2)
!
            CALL CHEMM('Left',Uplo,pn,pk,ONE,A(i+Kd,i+Kd),Lda,          &
     &                 Work(s2pos),lds2,ZERO,Work(wpos),ldw)
!
            CALL CGEMM('Conjugate','No transpose',pk,pk,pn,ONE,         &
     &                 Work(s2pos),lds2,Work(wpos),ldw,ZERO,Work(s1pos),&
     &                 lds1)
!
            CALL CGEMM('No transpose','No transpose',pn,pk,pk,-HALF,    &
...
!            an update of the form:  A := A - V*W' - W*V'
!
            CALL CHER2K(Uplo,'No transpose',pn,pk,-ONE,A(i+Kd,i),Lda,   &
     &                  Work(wpos),ldw,RONE,A(i+Kd,i+Kd),Lda)
!            ==================================================================
