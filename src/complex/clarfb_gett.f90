!*==clarfb_gett.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b CLARFB_GETT
!
!  =========== DOCUMENTATION ===========
!
...
!
!
         IF ( lnotident ) CALL CTRMM('L','L','C','U',K,N-K,CONE,A,Lda,  &
     &                               Work,Ldwork)
!
!        col2_(3) Compute W2: = W2 + (V2**H) * B2 = W2 + (B1**H) * B2
!        V2 stored in B1.
!
         IF ( M>0 ) CALL CGEMM('C','N',K,N-K,M,CONE,B,Ldb,B(1,K+1),Ldb, &
     &                         CONE,Work,Ldwork)
!
!        col2_(4) Compute W2: = T * W2,
!        T is upper-triangular.
!
         CALL CTRMM('L','U','N','N',K,N-K,CONE,T,Ldt,Work,Ldwork)
!
!        col2_(5) Compute B2: = B2 - V2 * W2 = B2 - B1 * W2,
!        V2 stored in B1.
!
         IF ( M>0 ) CALL CGEMM('N','N',M,N-K,K,-CONE,B,Ldb,Work,Ldwork, &
     &                         CONE,B(1,K+1),Ldb)
!
!
...
!           V1 stored in A1 (diagonal ones are not stored).
!
         IF ( lnotident ) CALL CTRMM('L','L','N','U',K,N-K,CONE,A,Lda,  &
     &                               Work,Ldwork)
!
...
!
      IF ( lnotident ) CALL CTRMM('L','L','C','U',K,K,CONE,A,Lda,Work,  &
     &                            Ldwork)
!
...
!     W1 is upper-triangular with zeroes below the diagonal.
!
      CALL CTRMM('L','U','N','N',K,K,CONE,T,Ldt,Work,Ldwork)
!
!     col1_(4) Compute B1: = - V2 * W1 = - B1 * W1,
!     V2 = B1, W1 is upper-triangular with zeroes below the diagonal.
!
      IF ( M>0 ) CALL CTRMM('R','U','N','N',M,K,-CONE,Work,Ldwork,B,Ldb)
!
      IF ( lnotident ) THEN
!
...
!
         CALL CTRMM('L','L','N','U',K,K,CONE,A,Lda,Work,Ldwork)
!
