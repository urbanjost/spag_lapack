!*==clantb.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
            ...
            IF ( LSAME(Diag,'U') ) THEN
               ...
               IF ( K>0 ) THEN
                  DO j = 2 , N
                     colssq(1) = ZERO
                     colssq(2) = ONE
                     CALL CLASSQ(MIN(j-1,K),Ab(MAX(K+2-j,1),j),1,       &
     &                           colssq(1),colssq(2))
            ...
            ELSE
               ...
               DO j = 1 , N
                  colssq(1) = ZERO
                  colssq(2) = ONE
                  CALL CLASSQ(MIN(j,K+1),Ab(MAX(K+2-j,1),j),1,colssq(1),&
     &                        colssq(2))
                  CALL SCOMBSSQ(ssq,colssq)
               ENDDO
            ENDIF
         ...
         ELSE
            ...
            DO j = 1 , N
               colssq(1) = ZERO
               colssq(2) = ONE
               CALL CLASSQ(MIN(N-j+1,K+1),Ab(1,j),1,colssq(1),colssq(2))
               CALL SCOMBSSQ(ssq,colssq)
            ENDDO
         ENDIF
