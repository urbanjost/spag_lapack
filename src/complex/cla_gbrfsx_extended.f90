!*==cla_gbrfsx_extended.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Array Arguments ..
      INTEGER Ipiv(*)
      COMPLEX Ab(Ldab,*) , Afb(Ldafb,*) , B(Ldb,*) , Y(Ldy,*) , Res(*) ,&
     &        Dy(*) , Y_tail(*)
...
!
            ...
            IF ( y_prec_state==BASE_RESIDUAL ) THEN
               CALL CGBMV(trans,m,N,Kl,Ku,(-1.0E+0,0.0E+0),Ab,Ldab,     &
     &                    Y(1,j),1,(1.0E+0,0.0E+0),Res,1)
            ELSEIF ( y_prec_state==EXTRA_RESIDUAL ) THEN
               CALL BLAS_CGBMV_X(Trans_type,N,N,Kl,Ku,(-1.0E+0,0.0E+0), &
     &                           Ab,Ldab,Y(1,j),1,(1.0E+0,0.0E+0),Res,1,&
     &                           Prec_type)
            ELSE
               CALL BLAS_CGBMV2_X(Trans_type,N,N,Kl,Ku,(-1.0E+0,0.0E+0),&
...
!
         CALL CCOPY(N,B(1,j),1,Res,1)
         CALL CGBMV(trans,N,N,Kl,Ku,(-1.0E+0,0.0E+0),Ab,Ldab,Y(1,j),1,  &
     &              (1.0E+0,0.0E+0),Res,1)
...
!
         CALL CLA_GBAMV(Trans_type,N,N,Kl,Ku,1.0E+0,Ab,Ldab,Y(1,j),1,   &
     &                  1.0E+0,Ayb,1)
