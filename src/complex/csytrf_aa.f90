!*==csytrf_aa.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b CSYTRF_AA
!
!  =========== DOCUMENTATION ===========
!
...
!        Panel factorization
!
            CALL CLASYF_AA(Uplo,2-k1,N-j,jb,A(MAX(1,j),j+1),Lda,        &
...
!
                     j3 = j2
                     DO mj = nj - 1 , 1 , -1
                        CALL CGEMV('No transpose',mj,jb+1,-ONE,         &
...
!
                     CALL CGEMM('Transpose','Transpose',nj,N-j3+1,jb+1, &
...
!        Panel factorization
!
            CALL CLASYF_AA(Uplo,2-k1,N-j,jb,A(j+1,MAX(1,j)),Lda,        &
     &                     Ipiv(j+1),Work,N,Work(N*nb+1))
!
!        Adjust IPIV and apply it back (J-th step picks (J+1)-th pivot)
!
...
!
                     CALL CGEMM('No transpose','Transpose',N-j3+1,nj,   &
