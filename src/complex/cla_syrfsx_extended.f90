!*==cla_syrfsx_extended.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Array Arguments ..
      INTEGER Ipiv(*)
      COMPLEX A(Lda,*) , Af(Ldaf,*) , B(Ldb,*) , Y(Ldy,*) , Res(*) ,    &
     &        Dy(*) , Y_tail(*)
...
!     .. External Subroutines ..
     ...
     &         BLAS_CSYMV2_X , CLA_SYAMV , CLA_WWADDW , CLA_LIN_BERR
      REAL SLAMCH
!     ..
!     .. Intrinsic Functions ..
...
!
            ...
            IF ( y_prec_state==BASE_RESIDUAL ) THEN
               CALL CSYMV(Uplo,N,CMPLX(-1.0),A,Lda,Y(1,j),1,CMPLX(1.0), &
     &                    Res,1)
            ELSEIF ( y_prec_state==EXTRA_RESIDUAL ) THEN
               CALL BLAS_CSYMV_X(uplo2,N,CMPLX(-1.0),A,Lda,Y(1,j),1,    &
     &                           CMPLX(1.0),Res,1,Prec_type)
            ELSE
               CALL BLAS_CSYMV2_X(uplo2,N,CMPLX(-1.0),A,Lda,Y(1,j),     &
     &                            Y_tail,1,CMPLX(1.0),Res,1,Prec_type)
            ENDIF
...
!
         CALL CCOPY(N,B(1,j),1,Res,1)
         CALL CSYMV(Uplo,N,CMPLX(-1.0),A,Lda,Y(1,j),1,CMPLX(1.0),Res,1)
