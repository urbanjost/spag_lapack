!*==cgerfsx.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CGERFSX
!
!  =========== DOCUMENTATION ===========
!
...
!
         IF ( colequ .AND. notran ) THEN
            rcond_tmp = CLA_GERCOND_C(Trans,N,A,Lda,Af,Ldaf,Ipiv,C,     &
     &                  .TRUE.,Info,Work,Rwork)
         ELSEIF ( rowequ .AND. .NOT.notran ) THEN
            rcond_tmp = CLA_GERCOND_C(Trans,N,A,Lda,Af,Ldaf,Ipiv,R,     &
     &                  .TRUE.,Info,Work,Rwork)
         ELSE
            rcond_tmp = CLA_GERCOND_C(Trans,N,A,Lda,Af,Ldaf,Ipiv,C,     &
...
!
         ...
         DO j = 1 , Nrhs
            IF ( Err_bnds_comp(j,LA_LINRX_ERR_I)<cwise_wrong ) THEN
               rcond_tmp = CLA_GERCOND_X(Trans,N,A,Lda,Af,Ldaf,Ipiv,    &
