!*==clals0.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
         DO i = 1 , Givptr
            CALL CSROT(Nrhs,B(Givcol(i,2),1),Ldb,B(Givcol(i,1),1),Ldb,  &
...
!
         IF ( K==1 ) THEN
            ...
            IF ( Z(1)<ZERO ) CALL CSSCAL(Nrhs,NEGONE,B,Ldb)
         ELSE
            DO j = 1 , K
               diflj = Difl(j)
               dj = Poles(j,1)
...
!
               ...
               DO jcol = 1 , Nrhs
                  ...
                  ENDDO
               ENDDO
               CALL SGEMV('T',K,Nrhs,ONE,Rwork(1+K+Nrhs*2),K,Rwork(1),1,&
     &                    ZERO,Rwork(1+K),1)
               i = K + Nrhs*2
               DO jcol = 1 , Nrhs
                  DO jrow = 1 , K
...
!
         IF ( K<MAX(m,n) ) CALL CLACPY('A',n-K,Nrhs,Bx(K+1,1),Ldbx,     &
     &                                 B(K+1,1),Ldb)
      ELSE
!
     ...
     &                          /(dsigj+Poles(i,1))/Difr(i,2)
               ...
               DO i = j + 1 , K
                  IF ( Z(j)==ZERO ) THEN
                     Rwork(i) = ZERO
                  ELSE
                     Rwork(i) = Z(j)/(SLAMC3(dsigj,-Poles(i,2))-Difl(i))&
     &                          /(dsigj+Poles(i,1))/Difr(i,2)
                  ENDIF
               ENDDO
!
...
!
         ...
         ENDIF
         IF ( K<MAX(m,n) ) CALL CLACPY('A',n-K,Nrhs,B(K+1,1),Ldb,       &
     &                                 Bx(K+1,1),Ldbx)
!
!        Step (3R): permute rows of B.
!
         CALL CCOPY(Nrhs,Bx(1,1),Ldbx,B(nlp1,1),Ldb)
         IF ( Sqre==1 ) CALL CCOPY(Nrhs,Bx(m,1),Ldbx,B(m,1),Ldb)
         DO i = 2 , n
