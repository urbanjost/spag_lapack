!*==cpstrf.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
                     ...
                     IF ( pvt<N )                                       &
     &                    CALL CSWAP(N-pvt,A(j,pvt+1),Lda,A(pvt,pvt+1), &
     &                    Lda)
                     DO i = j + 1 , pvt - 1
                        ctemp = CONJG(A(j,i))
                        A(j,i) = CONJG(A(i,pvt))
...
!
                  IF ( j<N ) THEN
                     ...
                     CALL CGEMV('Trans',j-k,N-j,-CONE,A(k,j+1),Lda,     &
...
!
               IF ( k+jb<=N ) CALL CHERK('Upper','Conj Trans',N-j+1,jb, &
...
!
                     ...
                     IF ( pvt<N )                                       &
     &                    CALL CSWAP(N-pvt,A(pvt+1,j),1,A(pvt+1,pvt),1)
                     DO i = j + 1 , pvt - 1
                        ctemp = CONJG(A(i,j))
                        A(i,j) = CONJG(A(pvt,i))
                        A(pvt,i) = ctemp
...
!
                  IF ( j<N ) THEN
                     ...
                     CALL CGEMV('No Trans',N-j,j-k,-CONE,A(j+1,k),Lda,  &
...
!              Update trailing matrix, J already incremented
!
               IF ( k+jb<=N ) CALL CHERK('Lower','No Trans',N-j+1,jb,   &
