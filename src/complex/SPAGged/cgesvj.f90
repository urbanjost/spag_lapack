!*==cgesvj.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief <b> CGESVJ </b>
!
!  =========== DOCUMENTATION ===========
!
...
!
     ...
     &        bigtheta , cs , ctol , epsln , mxaapq , mxsinj , rootbig ,&
     &        rooteps , rootsfmin , roottol , sfmin , skl , small , sn ,&
     &        t , temp1 , theta , thsign , tol
      COMPLEX :: aapq , ompq
...
!
      IF ( N==1 ) THEN
         IF ( lsvec ) CALL CLASCL('G',0,0,Sva(1),skl,M,1,A(1,1),Lda,    &
     &                            ierr)
         Rwork(1) = ONE/skl
         IF ( Sva(1)>=sfmin ) THEN
            Rwork(2) = ONE
...
!     [+ + x x]                    [x x].             [x x]
!
            CALL CGSVJ0(Jobv,M-n34,N-n34,A(n34+1,n34+1),Lda,Cwork(n34+1)&
     &                  ,Sva(n34+1),mvl,V(n34*q+1,n34+1),Ldv,epsln,     &
     &                  sfmin,tol,2,Cwork(N+1),Lwork-N,ierr)
 
            CALL CGSVJ0(Jobv,M-n2,n34-n2,A(n2+1,n2+1),Lda,Cwork(n2+1),  &
     &                  Sva(n2+1),mvl,V(n2*q+1,n2+1),Ldv,epsln,sfmin,   &
     &                  tol,2,Cwork(N+1),Lwork-N,ierr)
 
            CALL CGSVJ1(Jobv,M-n2,N-n2,n4,A(n2+1,n2+1),Lda,Cwork(n2+1), &
     &                  Sva(n2+1),mvl,V(n2*q+1,n2+1),Ldv,epsln,sfmin,   &
     &                  tol,1,Cwork(N+1),Lwork-N,ierr)
!
            CALL CGSVJ0(Jobv,M-n4,n2-n4,A(n4+1,n4+1),Lda,Cwork(n4+1),   &
     &                  Sva(n4+1),mvl,V(n4*q+1,n4+1),Ldv,epsln,sfmin,   &
     &                  tol,1,Cwork(N+1),Lwork-N,ierr)
!
            CALL CGSVJ0(Jobv,M,n4,A,Lda,Cwork,Sva,mvl,V,Ldv,epsln,sfmin,&
     &                  tol,1,Cwork(N+1),Lwork-N,ierr)
!
            CALL CGSVJ1(Jobv,M,n2,n4,A,Lda,Cwork,Sva,mvl,V,Ldv,epsln,   &
...
!
!
            CALL CGSVJ0(Jobv,n4,n4,A,Lda,Cwork,Sva,mvl,V,Ldv,epsln,     &
     &                  sfmin,tol,2,Cwork(N+1),Lwork-N,ierr)
!
            CALL CGSVJ0(Jobv,n2,n4,A(1,n4+1),Lda,Cwork(n4+1),Sva(n4+1), &
     &                  mvl,V(n4*q+1,n4+1),Ldv,epsln,sfmin,tol,1,       &
     &                  Cwork(N+1),Lwork-N,ierr)
!
            CALL CGSVJ1(Jobv,n2,n2,n4,A,Lda,Cwork,Sva,mvl,V,Ldv,epsln,  &
     &                  sfmin,tol,1,Cwork(N+1),Lwork-N,ierr)
!
            CALL CGSVJ0(Jobv,n2+n4,n4,A(1,n2+1),Lda,Cwork(n2+1),        &
     &                  Sva(n2+1),mvl,V(n2*q+1,n2+1),Ldv,epsln,sfmin,   &
     &                  tol,1,Cwork(N+1),Lwork-N,ierr)
...
!
     ...
     &                                  /aaqq)/aapp
                              ELSE
                                 CALL CCOPY(M,A(1,p),1,Cwork(N+1),1)
                                 CALL CLASCL('G',0,0,aapp,ONE,M,1,      &
     &                              Cwork(N+1),Lda,ierr)
                                 aapq = CDOTC(M,Cwork(N+1),1,A(1,q),1)  &
     &                                  /aaqq
                              ENDIF
                           ELSE
     ...
     &                                  /aapp)/aaqq
                              ELSE
                                 CALL CCOPY(M,A(1,q),1,Cwork(N+1),1)
                                 CALL CLASCL('G',0,0,aaqq,ONE,M,1,      &
     &                              Cwork(N+1),Lda,ierr)
                                 aapq = CDOTC(M,A(1,p),1,Cwork(N+1),1)  &
     &                                  /aapp
                              ENDIF
                           ENDIF
!
...
!              .. have to use modified Gram-Schmidt like transformation
     ...
     &                              ,Lda,ierr)
                                 CALL CAXPY(M,-aapq,Cwork(N+1),1,A(1,q),&
     &                              1)
...
!              .. have to use modified Gram-Schmidt like transformation
     ...
     &                              ,Lda,ierr)
                                 CALL CAXPY(M,-CONJG(aapq),Cwork(N+1),1,&
     &                              A(1,p),1)
