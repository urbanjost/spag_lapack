!*==cgejsv.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CGEJSV
!
!  =========== DOCUMENTATION ===========
!
...
!
      REAL :: aapp , aaqq , aatmax , aatmin , big , big1 , condr1 ,     &
     &        condr2 , cond_ok , entra , entrat , epsln , maxprj ,      &
     &        scalem , sconda , sfmin , small , temp1 , uscal1 ,        &
     &        uscal2 , xsc
      LOGICAL :: almort , defr , errest , goscal , jracc , kill ,       &
     &           l2aber , l2kill , l2pert , l2rank , l2tran , lquery ,  &
     &           lsvec , noscal , rowpiv , rsvec , transp
      COMPLEX , DIMENSION(1) :: cdummy
      COMPLEX :: ctemp
      INTEGER :: ierr , iwoff , lrwcon , lrwqp3 , lrwsvdj , lwcon ,     &
     &           lwlqf , lwqp3 , lwqrf , lwrk_cgelqf , lwrk_cgeqp3 ,    &
     &           lwrk_cgeqp3n , lwrk_cgeqrf , lwrk_cgesvj ,             &
     &           lwrk_cgesvju , lwrk_cgesvjv , lwrk_cunmlq ,            &
     &           lwrk_cunmqr , lwrk_cunmqrm , lwsvdj , lwsvdjv ,        &
     &           lwunmlq , lwunmqr , lwunmqrm , miniwrk , minrwrk ,     &
     &           minwrk , n1 , nr , numrank , optwrk , p , q , warning
...
!             only the singular values are requested
            ...
            ENDIF
            IF ( lquery ) THEN
               CALL CGESVJ('L','N','N',N,N,A,Lda,Sva,N,V,Ldv,cdummy,-1, &
     &                     rdummy,-1,ierr)
               lwrk_cgesvj = cdummy(1)
               IF ( errest ) THEN
                  optwrk = MAX(N+lwrk_cgeqp3,N**2+lwcon,N+lwrk_cgeqrf,  &
     &                     lwrk_cgesvj)
               ELSE
...
!            singular values and the right singular vectors are requested
     ...
     &                  N+lwunmlq)
            ENDIF
            IF ( lquery ) THEN
               CALL CGESVJ('L','U','N',N,N,U,Ldu,Sva,N,A,Lda,cdummy,-1, &
     &                     rdummy,-1,ierr)
               lwrk_cgesvj = cdummy(1)
               CALL CUNMLQ('L','C',N,N,N,A,Lda,cdummy,V,Ldv,cdummy,-1,  &
     &                     ierr)
               lwrk_cunmlq = cdummy(1)
               IF ( errest ) THEN
                  optwrk = MAX(N+lwrk_cgeqp3,lwcon,lwrk_cgesvj,         &
     &                     N+lwrk_cgelqf,2*N+lwrk_cgeqrf,N+lwrk_cgesvj, &
     &                     N+lwrk_cunmlq)
               ELSE
                  optwrk = MAX(N+lwrk_cgeqp3,lwrk_cgesvj,N+lwrk_cgelqf, &
     &                     2*N+lwrk_cgeqrf,N+lwrk_cgesvj,N+lwrk_cunmlq)
               ENDIF
            ENDIF
            IF ( l2tran .OR. rowpiv ) THEN
               IF ( errest ) THEN
...
!            full SVD is requested
     ...
     &                     ierr)
               lwrk_cunmqrm = cdummy(1)
               CALL CUNMQR('L','N',N,N,N,A,Lda,cdummy,U,Ldu,cdummy,-1,  &
     &                     ierr)
               lwrk_cunmqr = cdummy(1)
               IF ( .NOT.jracc ) THEN
                  CALL CGEQP3(N,N,A,Lda,Iwork,cdummy,cdummy,-1,rdummy,  &
     &                        ierr)
                  lwrk_cgeqp3n = cdummy(1)
                  CALL CGESVJ('L','U','N',N,N,U,Ldu,Sva,N,V,Ldv,cdummy, &
     &                        -1,rdummy,-1,ierr)
                  lwrk_cgesvj = cdummy(1)
                  CALL CGESVJ('U','U','N',N,N,U,Ldu,Sva,N,V,Ldv,cdummy, &
     &                        -1,rdummy,-1,ierr)
                  lwrk_cgesvju = cdummy(1)
                  CALL CGESVJ('L','U','V',N,N,U,Ldu,Sva,N,V,Ldv,cdummy, &
     &                        -1,rdummy,-1,ierr)
                  lwrk_cgesvjv = cdummy(1)
                  CALL CUNMLQ('L','C',N,N,N,A,Lda,cdummy,V,Ldv,cdummy,  &
     &                        -1,ierr)
                  lwrk_cunmlq = cdummy(1)
                  IF ( errest ) THEN
                     optwrk = MAX(N+lwrk_cgeqp3,N+lwcon,2*N+N**2+lwcon, &
     &                        2*N+lwrk_cgeqrf,2*N+lwrk_cgeqp3n,         &
     &                        2*N+N**2+N+lwrk_cgelqf,2*N+N**2+N+N**2+   &
     &                        lwcon,2*N+N**2+N+lwrk_cgesvj,             &
     &                        2*N+N**2+N+lwrk_cgesvjv,                  &
     &                        2*N+N**2+N+lwrk_cunmqr,                   &
     &                        2*N+N**2+N+lwrk_cunmlq,                   &
     &                        N+N**2+lwrk_cgesvju,N+lwrk_cunmqrm)
                  ELSE
                     optwrk = MAX(N+lwrk_cgeqp3,2*N+N**2+lwcon,         &
     &                        2*N+lwrk_cgeqrf,2*N+lwrk_cgeqp3n,         &
     &                        2*N+N**2+N+lwrk_cgelqf,2*N+N**2+N+N**2+   &
     &                        lwcon,2*N+N**2+N+lwrk_cgesvj,             &
     &                        2*N+N**2+N+lwrk_cgesvjv,                  &
     ...
     &                        -1,rdummy,-1,ierr)
                  lwrk_cgesvjv = cdummy(1)
                  CALL CUNMQR('L','N',N,N,N,cdummy,N,cdummy,V,Ldv,      &
     &                        cdummy,-1,ierr)
                  lwrk_cunmqr = cdummy(1)
                  CALL CUNMQR('L','N',M,N,N,A,Lda,cdummy,U,Ldu,cdummy,  &
     &                        -1,ierr)
                  lwrk_cunmqrm = cdummy(1)
                  IF ( errest ) THEN
                     optwrk = MAX(N+lwrk_cgeqp3,N+lwcon,2*N+lwrk_cgeqrf,&
...
!              .. V is available as workspace
               ...
               ENDDO
               IF ( lsvec ) THEN
                  CALL CPOCON('U',N,V,Ldv,ONE,temp1,Cwork(N+1),Rwork,   &
     &                        ierr)
               ELSE
...
![]     $              CWORK(N+N*N+1), RWORK, IERR )
               CALL CPOCON('U',N,Cwork,N,ONE,temp1,Cwork(N*N+1),Rwork,  &
     &                     ierr)
!
            ENDIF
            IF ( temp1/=ZERO ) THEN
...
!
         CALL CGESVJ('L','N','N',nr,nr,A,Lda,Sva,N,V,Ldv,Cwork,Lwork,   &
     &               Rwork,Lrwork,Info)
!
         scalem = Rwork(1)
...
!
            CALL CGESVJ('L','U','N',N,nr,V,Ldv,Sva,nr,A,Lda,Cwork,Lwork,&
     &                  Rwork,Lrwork,Info)
            scalem = Rwork(1)
            numrank = NINT(Rwork(2))
...
!
            ...
            CALL CLASET('U',nr-1,nr-1,CZERO,CZERO,V(1,2),Ldv)
            CALL CGEQRF(nr,nr,V,Ldv,Cwork(N+1),Cwork(2*N+1),Lwork-2*N,  &
     &                  ierr)
            DO p = 1 , nr
            ...
            CALL CLASET('U',nr-1,nr-1,CZERO,CZERO,V(1,2),Ldv)
!
            CALL CGESVJ('L','U','N',nr,nr,V,Ldv,Sva,nr,U,Ldu,Cwork(N+1),&
     &                  Lwork-N,Rwork,Lrwork,Info)
            ...
            ENDIF
!
            CALL CUNMLQ('L','C',N,N,nr,A,Lda,Cwork,V,Ldv,Cwork(N+1),    &
     &                  Lwork-N,ierr)
!
         ENDIF
...
!
         IF ( transp ) CALL CLACPY('A',N,N,V,Ldv,U,Ldu)
!
      ELSEIF ( jracc .AND. (.NOT.lsvec) .AND. (nr==N) ) THEN
!
         CALL CLASET('L',N-1,N-1,CZERO,CZERO,A(2,1),Lda)
!
         CALL CGESVJ('U','N','V',N,N,A,Lda,Sva,N,V,Ldv,Cwork,Lwork,     &
     &               Rwork,Lrwork,Info)
         scalem = Rwork(1)
         numrank = NINT(Rwork(2))
         CALL CLAPMR(.FALSE.,N,N,V,Ldv,Iwork)
...
!
         ...
         CALL CLASET('U',nr-1,nr-1,CZERO,CZERO,U(1,2),Ldu)
!
         CALL CGESVJ('L','U','N',nr,nr,U,Ldu,Sva,nr,A,Lda,Cwork(N+1),   &
     &               Lwork-N,Rwork,Lrwork,Info)
         scalem = Rwork(1)
         numrank = NINT(Rwork(2))
!
         ...
         ENDIF
!
         CALL CUNMQR('L','N',M,n1,N,A,Lda,Cwork,U,Ldu,Cwork(N+1),       &
     &               Lwork-N,ierr)
!
...
!     $                V(p,q) = TEMP1 * ( V(p,q) / ABS(V(p,q)) )
     ...
     &                    V(p,q) = ctemp
            ...
            CALL CGEQRF(N,nr,V,Ldv,Cwork(N+1),Cwork(2*N+1),Lwork-2*N,   &
     &                  ierr)
...
!                  U(p,q) = - TEMP1 * ( U(q,p) / ABS(U(q,p)) )
            ...
            CALL CGESVJ('L','U','V',nr,nr,U,Ldu,Sva,N,V,Ldv,            &
     ...
     &                  Info)
            ...
            CALL CUNMQR('L','N',N,N,nr,Cwork(2*N+1),N,Cwork(N+1),V,Ldv, &
...
!              R1^* * P2 = Q2 * R2
               DO p = 1 , nr
                  Iwork(N+p) = 0
               ENDDO
               CALL CGEQP3(N,nr,V,Ldv,Iwork(N+1),Cwork(N+1),Cwork(2*N+1)&
...
!                 factorization is then built in explicitly.
                  CALL CTRSM('L','U','C','N',nr,nr,CONE,Cwork(2*N+1),N, &
     &                       V,Ldv)
                  IF ( nr<N ) THEN
                     CALL CLASET('A',N-nr,nr,CZERO,CZERO,V(nr+1,1),Ldv)
                     CALL CLASET('A',nr,N-nr,CZERO,CZERO,V(1,nr+1),Ldv)
...
!              R2=L3*Q3), pre-multiplied with the transposed Q3.
     ...
     &                     Lrwork,Info)
               ...
               ENDDO
               CALL CTRSM('L','U','N','N',nr,nr,CONE,Cwork(2*N+1),N,U,  &
     &                    Ldu)
!              .. apply the permutation from the second QR factorization
               ...
               IF ( nr<N ) THEN
                  ...
                  CALL CLASET('A',N-nr,N-nr,CZERO,CONE,V(nr+1,nr+1),Ldv)
               ENDIF
               CALL CUNMQR('L','N',N,N,nr,Cwork(2*N+1),N,Cwork(N+1),V,  &
...
!              accumulation of Jacobi rotations.
               CALL CGESVJ('L','U','V',nr,nr,V,Ldv,Sva,nr,U,Ldu,        &
     &                     Cwork(2*N+N*nr+nr+1),Lwork-2*N-N*nr-nr,Rwork,&
     &                     Lrwork,Info)
               scalem = Rwork(1)
               numrank = NINT(Rwork(2))
...
!     $                                        ABS(CWORK(N+(p-1)*N+q)) )
            ...
            ENDIF
!
            CALL CGESVJ('U','U','N',N,N,Cwork(N+1),N,Sva,N,U,Ldu,       &
