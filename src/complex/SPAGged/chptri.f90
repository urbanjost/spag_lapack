!*==chptri.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CHPTRI
!
!  =========== DOCUMENTATION ===========
!
...
!
               IF ( k>1 ) THEN
                  ...
                  CALL CHPMV(Uplo,k-1,-CONE,Ap,Work,1,ZERO,Ap(kc),1)
                  Ap(kc+k-1) = Ap(kc+k-1)                               &
...
!
     ...
     &                             - CDOTC(k-1,Ap(kc),1,Ap(kcnext),1)
                  ...
                  CALL CHPMV(Uplo,k-1,-CONE,Ap,Work,1,ZERO,Ap(kcnext),1)
...
!
               IF ( k<N ) THEN
                  ...
                  CALL CHPMV(Uplo,N-k,-CONE,Ap(kc+N-k+1),Work,1,ZERO,   &
     &                       Ap(kc+1),1)
                  Ap(kc) = Ap(kc) - REAL(CDOTC(N-k,Work,1,Ap(kc+1),1))
               ENDIF
               kstep = 1
            ELSE
!
...
!
     ...
     &                           - CDOTC(N-k,Ap(kc+1),1,Ap(kcnext+2),1)
                  CALL CCOPY(N-k,Ap(kcnext+2),1,Work,1)
                  CALL CHPMV(Uplo,N-k,-CONE,Ap(kc+(N-k+1)),Work,1,ZERO, &
     &                       Ap(kcnext+2),1)
                  Ap(kcnext) = Ap(kcnext)                               &
     &                         - REAL(CDOTC(N-k,Work,1,Ap(kcnext+2),1))
               ENDIF
               kstep = 2
               kcnext = kcnext - (N-k+3)
            ENDIF
