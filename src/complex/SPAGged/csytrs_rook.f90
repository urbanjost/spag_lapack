!*==csytrs_rook.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CSYTRS_ROOK
!
!  =========== DOCUMENTATION ===========
!
...
!
               IF ( k>2 ) THEN
                  CALL CGERU(k-2,Nrhs,-CONE,A(1,k),1,B(k,1),Ldb,B(1,1), &
     &                       Ldb)
                  CALL CGERU(k-2,Nrhs,-CONE,A(1,k-1),1,B(k-1,1),Ldb,    &
     &                       B(1,1),Ldb)
               ENDIF
!
!           Multiply by the inverse of the diagonal block.
...
!           stored in column K of A.
!
               IF ( k>1 ) CALL CGEMV('Transpose',k-1,Nrhs,-CONE,B,Ldb,  &
...
!
               IF ( k>1 ) THEN
                  CALL CGEMV('Transpose',k-1,Nrhs,-CONE,B,Ldb,A(1,k),1, &
     &                       CONE,B(k,1),Ldb)
                  CALL CGEMV('Transpose',k-1,Nrhs,-CONE,B,Ldb,A(1,k+1), &
     &                       1,CONE,B(k+1,1),Ldb)
               ENDIF
...
!           stored in column K of A.
!
               IF ( k<N ) CALL CGERU(N-k,Nrhs,-CONE,A(k+1,k),1,B(k,1),  &
     &                               Ldb,B(k+1,1),Ldb)
!
!           Multiply by the inverse of the diagonal block.
!
...
!
               IF ( k<N-1 ) THEN
                  CALL CGERU(N-k-1,Nrhs,-CONE,A(k+2,k),1,B(k,1),Ldb,    &
     &                       B(k+2,1),Ldb)
                  CALL CGERU(N-k-1,Nrhs,-CONE,A(k+2,k+1),1,B(k+1,1),Ldb,&
     &                       B(k+2,1),Ldb)
               ENDIF
!
!           Multiply by the inverse of the diagonal block.
...
!           stored in column K of A.
!
               IF ( k<N ) CALL CGEMV('Transpose',N-k,Nrhs,-CONE,B(k+1,1)&
