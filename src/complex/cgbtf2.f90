!*==cgbtf2.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
            IF ( jp/=1 ) CALL CSWAP(ju-j+1,Ab(kv+jp,j),Ldab-1,Ab(kv+1,j)&
     &                              ,Ldab-1)
            IF ( km>0 ) THEN
!
!              Compute multipliers.
...
!
               IF ( ju>j ) CALL CGERU(km,ju-j,-ONE,Ab(kv+2,j),1,        &
