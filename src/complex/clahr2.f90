!*==clahr2.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL CAXPY , CCOPY , CGEMM , CGEMV , CLACPY , CLARFG , CSCAL ,&
     &         CTRMM , CTRMV , CLACGV
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MIN
...
!        Compute  Y(K+1:N,I)
!
         CALL CGEMV('NO TRANSPOSE',N-K,N-K-i+1,ONE,A(K+1,i+1),Lda,      &
     &              A(K+i,i),1,ZERO,Y(K+1,i),1)
         CALL CGEMV('Conjugate transpose',N-K-i+1,i-1,ONE,A(K+i,1),Lda, &
     &              A(K+i,i),1,ZERO,T(1,i),1)
         CALL CGEMV('NO TRANSPOSE',N-K,i-1,-ONE,Y(K+1,1),Ldy,T(1,i),1,  &
     &              ONE,Y(K+1,i),1)
...
!
      ...
      CALL CTRMM('RIGHT','Lower','NO TRANSPOSE','UNIT',K,Nb,ONE,A(K+1,1)&
     &           ,Lda,Y,Ldy)
      IF ( N>K+Nb ) CALL CGEMM('NO TRANSPOSE','NO TRANSPOSE',K,Nb,      &
     &                         N-K-Nb,ONE,A(1,2+Nb),Lda,A(K+1+Nb,1),Lda,&
     &                         ONE,Y,Ldy)
      CALL CTRMM('RIGHT','Upper','NO TRANSPOSE','NON-UNIT',K,Nb,ONE,T,  &
     &           Ldt,Y,Ldy)
!
!
!     End of CLAHR2
