!*==ctgsyl.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b CTGSYL
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Scalar Arguments ..
      CHARACTER Trans
      INTEGER Ijob , Info , Lda , Ldb , Ldc , Ldd , Lde , Ldf , Lwork , &
     &        M , N
      REAL Dif , Scale
!     ..
!     .. Array Arguments ..
...
!
            ...
            dsum = ONE
            pq = M*N
            CALL CTGSY2(Trans,ifunc,M,N,A,Lda,B,Ldb,C,Ldc,D,Ldd,E,Lde,F,&
...
!
            ...
            DO j = p + 2 , q
               ...
               DO i = p , 1 , -1
                  ...
                  ie = Iwork(i+1) - 1
                  mb = ie - is + 1
                  CALL CTGSY2(Trans,ifunc,mb,nb,A(is,is),Lda,B(js,js),  &
     ...
     &                        Lde,F(is,js),Ldf,scaloc,dsum,dscale,linfo)
                  ...
                  IF ( scaloc/=ONE ) THEN
                     ...
                     ENDDO
                     DO k = js , je
                        CALL CSCAL(is-1,CMPLX(scaloc,ZERO),C(1,k),1)
                        CALL CSCAL(is-1,CMPLX(scaloc,ZERO),F(1,k),1)
                     ENDDO
                     DO k = js , je
                        CALL CSCAL(M-ie,CMPLX(scaloc,ZERO),C(ie+1,k),1)
                        CALL CSCAL(M-ie,CMPLX(scaloc,ZERO),F(ie+1,k),1)
                     ENDDO
                     DO k = je + 1 , N
                        CALL CSCAL(M,CMPLX(scaloc,ZERO),C(1,k),1)
                        CALL CSCAL(M,CMPLX(scaloc,ZERO),F(1,k),1)
...
!
                  IF ( i>1 ) THEN
                     CALL CGEMM('N','N',is-1,nb,mb,CMPLX(-ONE,ZERO),    &
     &                          A(1,is),Lda,C(is,js),Ldc,CMPLX(ONE,ZERO)&
     &                          ,C(1,js),Ldc)
                     CALL CGEMM('N','N',is-1,nb,mb,CMPLX(-ONE,ZERO),    &
     &                          D(1,is),Ldd,C(is,js),Ldc,CMPLX(ONE,ZERO)&
     &                          ,F(1,js),Ldf)
                  ENDIF
                  IF ( j<q ) THEN
                     CALL CGEMM('N','N',mb,N-je,nb,CMPLX(ONE,ZERO),     &
     &                          F(is,js),Ldf,B(js,je+1),Ldb,            &
     &                          CMPLX(ONE,ZERO),C(is,je+1),Ldc)
                     CALL CGEMM('N','N',mb,N-je,nb,CMPLX(ONE,ZERO),     &
     &                          F(is,js),Ldf,E(js,je+1),Lde,            &
     &                          CMPLX(ONE,ZERO),F(is,je+1),Ldf)
                  ENDIF
               ENDDO
            ENDDO
            IF ( dscale/=ZERO ) THEN
...
!
         ...
         DO i = 1 , p
            ...
            DO j = q , p + 2 , -1
               ...
               je = Iwork(j+1) - 1
               nb = je - js + 1
               CALL CTGSY2(Trans,ifunc,mb,nb,A(is,is),Lda,B(js,js),Ldb, &
...
!
               IF ( j>p+2 ) THEN
                  CALL CGEMM('N','C',mb,js-1,nb,CMPLX(ONE,ZERO),C(is,js)&
     &                       ,Ldc,B(1,js),Ldb,CMPLX(ONE,ZERO),F(is,1),  &
     &                       Ldf)
                  CALL CGEMM('N','C',mb,js-1,nb,CMPLX(ONE,ZERO),F(is,js)&
     &                       ,Ldf,E(1,js),Lde,CMPLX(ONE,ZERO),F(is,1),  &
     &                       Ldf)
               ENDIF
               IF ( i<p ) THEN
                  CALL CGEMM('C','N',M-ie,nb,mb,CMPLX(-ONE,ZERO),       &
     &                       A(is,ie+1),Lda,C(is,js),Ldc,CMPLX(ONE,ZERO)&
     &                       ,C(ie+1,js),Ldc)
                  CALL CGEMM('C','N',M-ie,nb,mb,CMPLX(-ONE,ZERO),       &
     &                       D(is,ie+1),Ldd,F(is,js),Ldf,CMPLX(ONE,ZERO)&
     &                       ,C(ie+1,js),Ldc)
               ENDIF
            ENDDO
         ENDDO
