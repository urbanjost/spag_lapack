!*==csptri.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b CSPTRI
!
!  =========== DOCUMENTATION ===========
!
...
!
               IF ( k>1 ) THEN
                  ...
                  CALL CSPMV(Uplo,k-1,-ONE,Ap,Work,1,ZERO,Ap(kc),1)
                  Ap(kc+k-1) = Ap(kc+k-1) - CDOTU(k-1,Work,1,Ap(kc),1)
               ENDIF
               kstep = 1
            ELSE
!
...
!
     ...
     &                             - CDOTU(k-1,Ap(kc),1,Ap(kcnext),1)
                  CALL CCOPY(k-1,Ap(kcnext),1,Work,1)
                  CALL CSPMV(Uplo,k-1,-ONE,Ap,Work,1,ZERO,Ap(kcnext),1)
...
!
               IF ( k<N ) THEN
                  ...
                  CALL CSPMV(Uplo,N-k,-ONE,Ap(kc+N-k+1),Work,1,ZERO,    &
     &                       Ap(kc+1),1)
                  Ap(kc) = Ap(kc) - CDOTU(N-k,Work,1,Ap(kc+1),1)
               ENDIF
               kstep = 1
            ELSE
!
...
!
     ...
     &                           - CDOTU(N-k,Ap(kc+1),1,Ap(kcnext+2),1)
...
!
               ...
               IF ( kp<N ) CALL CSWAP(N-kp,Ap(kc+kp-k+1),1,Ap(kpc+1),1)
               kx = kc + kp - k
               DO j = k + 1 , kp - 1
                  kx = kx + N - j + 1
                  temp = Ap(kc+j-k)
