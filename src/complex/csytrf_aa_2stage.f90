!*==csytrf_aa_2stage.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL CCOPY , CGBTRF , CGEMM , CGETRF , CLACPY , CLASET ,      &
     &         CTRSM , CSWAP , XERBLA
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MIN , MAX
...
!                  H(I,J) = T(I,I)*U(I,J) + T(I+1,I)*U(I+1,J)
                  ...
                  ELSE
                     jb = 2*nb
                  ENDIF
                  CALL CGEMM('NoTranspose','NoTranspose',nb,kb,jb,CONE, &
...
!
     ...
     &                  Tb(td+1+(j*nb)*ldtb),ldtb-1)
            IF ( j>1 ) THEN
!              T(J,J) = U(1:J,J)'*H(1:J)
               CALL CGEMM('Transpose','NoTranspose',kb,kb,(j-1)*nb,     &
     &                    -CONE,A(1,j*nb+1),Lda,Work(nb+1),N,CONE,      &
     &                    Tb(td+1+(j*nb)*ldtb),ldtb-1)
!              T(J,J) += U(J,J)'*T(J,J-1)*U(J-1,J)
               CALL CGEMM('Transpose','NoTranspose',kb,nb,kb,CONE,      &
     &                    A((j-1)*nb+1,j*nb+1),Lda,                     &
     &                    Tb(td+nb+1+((j-1)*nb)*ldtb),ldtb-1,CZERO,     &
     &                    Work(1),N)
               CALL CGEMM('NoTranspose','NoTranspose',kb,kb,nb,-CONE,   &
...
!
                  CALL CGEMM('Transpose','NoTranspose',nb,N-(j+1)*nb,   &
...
!
               DO k = 1 , nb
                  CALL CCOPY(N-(j+1)*nb,Work(1+(k-1)*N),1,              &
...
!
     ...
     &                     ldtb-1)
               IF ( j>0 ) CALL CTRSM('R','U','N','U',kb,nb,CONE,        &
     &                               A((j-1)*nb+1,j*nb+1),Lda,          &
     &                               Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
!
...
!
     ...
     &                  = Tb(td+nb+i-k+1+(j*nb+k-1)*ldtb)
                  ENDDO
               ENDDO
               CALL CLASET('Lower',kb,nb,CZERO,CONE,A(j*nb+1,(j+1)*nb+1)&
     &                     ,Lda)
!
...
!                    > Swap A(I1+1:M, I1) with A(I2, I1+1:M)
                     IF ( i2>(i1+1) )                                   &
     &                    CALL CSWAP(i2-i1-1,A(i1,i1+1),Lda,A(i1+1,i2), &
     &                    1)
...
!                  H(I,J) = T(I,I)*L(J,I)' + T(I+1,I)'*L(J,I+1)'
                  ...
                  ELSE
                     jb = 2*nb
                  ENDIF
                  CALL CGEMM('NoTranspose','Transpose',nb,kb,jb,CONE,   &
     &                       Tb(td+1+(i*nb)*ldtb),ldtb-1,               &
...
!              T(J,J) += L(J,J)*T(J,J-1)*L(J,J-1)'
               CALL CGEMM('NoTranspose','NoTranspose',kb,nb,kb,CONE,    &
     &                    A(j*nb+1,(j-1)*nb+1),Lda,                     &
     &                    Tb(td+nb+1+((j-1)*nb)*ldtb),ldtb-1,CZERO,     &
     &                    Work(1),N)
               CALL CGEMM('NoTranspose','Transpose',kb,kb,nb,-CONE,     &
...
!
                  IF ( j==1 ) THEN
                     CALL CGEMM('NoTranspose','Transpose',kb,kb,kb,CONE,&
     &                          Tb(td+1+(j*nb)*ldtb),ldtb-1,            &
     &                          A(j*nb+1,(j-1)*nb+1),Lda,CZERO,         &
     &                          Work(j*nb+1),N)
                  ELSE
                     CALL CGEMM('NoTranspose','Transpose',kb,kb,nb+kb,  &
     &                          CONE,Tb(td+nb+1+((j-1)*nb)*ldtb),ldtb-1,&
...
!
     ...
     &                     Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
               IF ( j>0 ) CALL CTRSM('R','L','T','U',kb,nb,CONE,        &
     &                               A(j*nb+1,(j-1)*nb+1),Lda,          &
     &                               Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
!
...
!
     ...
     &                  = Tb(td+nb+i-k+1+(j*nb+k-1)*ldtb)
                  ENDDO
               ENDDO
               CALL CLASET('Upper',kb,nb,CZERO,CONE,A((j+1)*nb+1,j*nb+1)&
     &                     ,Lda)
!
!              Apply pivots to trailing submatrix of A
!
...
!                    > Swap A(I1+1:M, I1) with A(I2, I1+1:M)
                     IF ( i2>(i1+1) )                                   &
     &                    CALL CSWAP(i2-i1-1,A(i1+1,i1),1,A(i2,i1+1),   &
     &                    Lda)
!                    > Swap A(I2+1:M, I1) with A(I2+1:M, I2)
                     IF ( i2<N )                                        &
