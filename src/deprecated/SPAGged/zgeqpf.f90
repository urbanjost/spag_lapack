!*==zgeqpf.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b ZGEQPF
!
!  =========== DOCUMENTATION ===========
!
...
!
      IF ( itemp>0 ) THEN
         ...
         IF ( ma<N ) CALL ZUNM2R('Left','Conjugate transpose',M,N-ma,ma,&
...
!
               aii = A(i,i)
               A(i,i) = DCMPLX(ONE)
               CALL ZLARF('Left',M-i+1,N-i,A(i,i),1,DCONJG(Tau(i)),     &
     &                    A(i,i+1),Lda,Work)
               A(i,i) = aii
            ENDIF
!
