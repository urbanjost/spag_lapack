!*==sgetrf.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SGETRF VARIANT: left-looking Level 3 BLAS version of the algorithm.
!
!  =========== DOCUMENTATION ===========
!
...
!
               CALL STRSM('Left','Lower','No transpose','Unit',nb,jb,   &
     &                    ONE,A(k,k),Lda,A(k,j),Lda)
!
!              Update trailing submatrix.
!
               CALL SGEMM('No transpose','No transpose',M-k-nb+1,jb,nb, &
...
!
         IF ( N>M ) THEN
            ...
            DO k = 1 , M , nb
               ...
               jb = MIN(M-k+1,nb)
!
               CALL STRSM('Left','Lower','No transpose','Unit',jb,N-M,  &
     &                    ONE,A(k,k),Lda,A(k,M+1),Lda)
 
!
               IF ( k+nb<=M ) CALL SGEMM('No transpose','No transpose', &
