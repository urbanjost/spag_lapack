!*==zgetrf.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZGETRF VARIANT: left-looking Level 3 BLAS version of the algorithm.
!
!  =========== DOCUMENTATION ===========
!
...
!
               CALL ZTRSM('Left','Lower','No transpose','Unit',nb,jb,   &
     &                    ONE,A(k,k),Lda,A(k,j),Lda)
!
!              Update trailing submatrix.
!
               CALL ZGEMM('No transpose','No transpose',M-k-nb+1,jb,nb, &
...
!
         DO k = 1 , MIN(M,N) , nb
            CALL ZLASWP(k-1,A(1,1),Lda,k,MIN(k+nb-1,MIN(M,N)),Ipiv,1)
         ENDDO
!
...
!
         IF ( N>M ) THEN
            ...
            DO k = 1 , M , nb
               ...
               jb = MIN(M-k+1,nb)
!
               CALL ZTRSM('Left','Lower','No transpose','Unit',jb,N-M,  &
     &                    ONE,A(k,k),Lda,A(k,M+1),Lda)
 
!
               IF ( k+nb<=M ) CALL ZGEMM('No transpose','No transpose', &
