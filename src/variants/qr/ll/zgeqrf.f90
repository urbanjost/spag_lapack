!*==zgeqrf.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZGEQRF VARIANT: left-looking Level 3 BLAS of the algorithm.
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Local Scalars ..
      LOGICAL lquery
      INTEGER i , ib , iinfo , iws , j , k , lwkopt , nb , nbmin , nx , &
     &        lbwork , nt , llwork
!     ..
...
!
      IF ( nb>1 .AND. nb<k ) nx = MAX(0,ILAENV(3,'ZGEQRF',' ',M,N,-1,-1)&
     &                            )
!
...
!
               CALL ZLARFB('Left','Transpose','Forward','Columnwise',   &
...
!
!
            IF ( i+ib<=N ) CALL ZLARFT('Forward','Columnwise',M-i+1,ib, &
...
!        Use unblocked code to factor the last or only block.
!
            CALL ZGEQR2(M-i+1,N-i+1,A(i,i),Lda,Tau(i),Work,iinfo)
...
!
         IF ( nt<=nb ) THEN
            CALL ZLARFT('Forward','Columnwise',M-i+1,k-i+1,A(i,i),Lda,  &
     &                  Tau(i),Work(i),lbwork)
         ELSE
            CALL ZLARFT('Forward','Columnwise',M-i+1,k-i+1,A(i,i),Lda,  &
...
!
         DO j = 1 , k - nx , nb
            ...
            CALL ZLARFB('Left','Transpose','Forward','Columnwise',M-j+1,&
