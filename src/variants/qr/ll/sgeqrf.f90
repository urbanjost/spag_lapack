!*==sgeqrf.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SGEQRF VARIANT: left-looking Level 3 BLAS version of the algorithm.
!
!  =========== DOCUMENTATION ===========
!
...
!
      IF ( nb>1 .AND. nb<k ) nx = MAX(0,ILAENV(3,'SGEQRF',' ',M,N,-1,-1)&
     &                            )
!
...
!
!
            IF ( i+ib<=N ) CALL SLARFT('Forward','Columnwise',M-i+1,ib, &
...
!
         IF ( nt<=nb ) THEN
            CALL SLARFT('Forward','Columnwise',M-i+1,k-i+1,A(i,i),Lda,  &
     &                  Tau(i),Work(i),lbwork)
         ELSE
            CALL SLARFT('Forward','Columnwise',M-i+1,k-i+1,A(i,i),Lda,  &
...
!
         DO j = 1 , k - nx , nb
            ...
            CALL SLARFB('Left','Transpose','Forward','Columnwise',M-j+1,&
