!*==sgesvdx.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL SBDSVDX , SGEBRD , SGELQF , SGEQRF , SLACPY , SLASCL ,   &
     &         SLASET , SORMBR , SORMLQ , SORMQR , SCOPY , XERBLA
!     ..
!     .. External Functions ..
      LOGICAL LSAME
      INTEGER ILAENV
...
!
            itau = 1
            itemp = itau + N
            CALL SGEQRF(M,N,A,Lda,Work(itau),Work(itemp),Lwork-itemp+1, &
     &                  Info)
!
...
!
            ...
            CALL SGEBRD(N,N,Work(iqrf),N,Work(id),Work(ie),Work(itauq), &
...
!
            itgkz = itemp
            itemp = itgkz + N*(N*2+1)
            CALL SBDSVDX('U',jobz,rngtgk,N,Work(id),Work(ie),Vl,Vu,     &
...
!
               CALL SORMBR('Q','L','N',N,Ns,N,Work(iqrf),N,Work(itauq), &
     &                     U,Ldu,Work(itemp),Lwork-itemp+1,Info)
!
!              Call SORMQR to compute Q*(QB*UB).
!              (Workspace in WORK( ITEMP ): need N, prefer N*NB)
!
               CALL SORMQR('L','N',M,Ns,N,A,Lda,Work(itau),U,Ldu,       &
...
!              (Workspace in WORK( ITEMP ): need N, prefer N*NB)
!
               CALL SORMBR('P','R','T',Ns,N,N,Work(iqrf),N,Work(itaup), &
...
!
            ...
            itaup = itauq + N
            itemp = itaup + N
            CALL SGEBRD(M,N,A,Lda,Work(id),Work(ie),Work(itauq),        &
     &                  Work(itaup),Work(itemp),Lwork-itemp+1,Info)
!
!           Solve eigenvalue problem TGK*Z=Z*S.
!           (Workspace: need 14*N + 2*N*(N+1))
...
!              (Workspace in WORK( ITEMP ): need N, prefer N*NB)
!
               CALL SORMBR('Q','L','N',M,Ns,N,A,Lda,Work(itauq),U,Ldu,  &
...
!              (Workspace in WORK( ITEMP ): need N, prefer N*NB)
!
               CALL SORMBR('P','R','T',Ns,N,N,A,Lda,Work(itaup),Vt,Ldvt,&
     &                     Work(itemp),Lwork-itemp+1,ierr)
            ENDIF
         ENDIF
!
...
!
         itau = 1
         itemp = itau + M
         CALL SGELQF(M,N,A,Lda,Work(itau),Work(itemp),Lwork-itemp+1,    &
     &               Info)
...
!
         ...
         CALL SGEBRD(M,M,Work(ilqf),M,Work(id),Work(ie),Work(itauq),    &
     &               Work(itaup),Work(itemp),Lwork-itemp+1,Info)
!
!           Solve eigenvalue problem TGK*Z=Z*S.
...
!
         itgkz = itemp
         itemp = itgkz + M*(M*2+1)
         CALL SBDSVDX('U',jobz,rngtgk,M,Work(id),Work(ie),Vl,Vu,iltgk,  &
...
!
            CALL SORMBR('Q','L','N',M,Ns,M,Work(ilqf),M,Work(itauq),U,  &
...
!              (Workspace in WORK( ITEMP ): need M, prefer M*NB)
!
            CALL SORMBR('P','R','T',Ns,M,M,Work(ilqf),M,Work(itaup),Vt, &
     &                  Ldvt,Work(itemp),Lwork-itemp+1,Info)
!
!              Call SORMLQ to compute ((VB**T)*(PB**T))*Q.
!              (Workspace in WORK( ITEMP ): need M, prefer M*NB)
!
            CALL SORMLQ('R','N',Ns,N,M,A,Lda,Work(itau),Vt,Ldvt,        &
     &                  Work(itemp),Lwork-itemp+1,Info)
         ENDIF
      ELSE
!
...
!
         ...
         itaup = itauq + M
         itemp = itaup + M
         CALL SGEBRD(M,N,A,Lda,Work(id),Work(ie),Work(itauq),Work(itaup)&
     &               ,Work(itemp),Lwork-itemp+1,Info)
!
!           Solve eigenvalue problem TGK*Z=Z*S.
!           (Workspace: need 2*M*M+14*M)
!
         itgkz = itemp
         itemp = itgkz + M*(M*2+1)
         CALL SBDSVDX('L',jobz,rngtgk,M,Work(id),Work(ie),Vl,Vu,iltgk,  &
     &                iutgk,Ns,S,Work(itgkz),M*2,Work(itemp),Iwork,Info)
!
!           If needed, compute left singular vectors.
!
...
!              (Workspace in WORK( ITEMP ): need M, prefer M*NB)
!
            CALL SORMBR('P','R','T',Ns,N,M,A,Lda,Work(itaup),Vt,Ldvt,   &
     &                  Work(itemp),Lwork-itemp+1,Info)
         ENDIF
      ENDIF
!
