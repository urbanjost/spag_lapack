!*==sggesx.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Local Scalars ..
     ...
     &        ilo , ip , iright , irows , itau , iwrk , liwmin , lwrk , &
     &        maxwrk , minwrk
      REAL anrm , anrmto , bignum , bnrm , bnrmto , eps , pl , pr ,     &
     &     safmax , safmin , smlnum
!     ..
!     .. Local Arrays ..
      REAL dif(2)
!     ..
...
!
      IF ( Info==0 ) THEN
         IF ( N>0 ) THEN
            ...
            maxwrk = minwrk - N + N*ILAENV(1,'SGEQRF',' ',N,1,N,0)
            maxwrk = MAX(maxwrk,                                        &
     &               minwrk-N+N*ILAENV(1,'SORMQR',' ',N,1,N,-1))
...
!
         CALL STGSEN(ijob,ilvsl,ilvsr,Bwork,N,A,Lda,B,Ldb,Alphar,Alphai,&
