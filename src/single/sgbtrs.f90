!*==sgbtrs.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b SGBTRS
!
!  =========== DOCUMENTATION ===========
!
...
!
         IF ( lnoti ) THEN
            DO j = 1 , N - 1
               ...
               CALL SGER(lm,Nrhs,-ONE,Ab(kd+1,j),1,B(j,1),Ldb,B(j+1,1), &
     &                   Ldb)
            ENDDO
         ENDIF
!
...
!
            CALL STBSV('Upper','No transpose','Non-unit',N,Kl+Ku,Ab,    &
     &                 Ldab,B(1,i),1)
         ENDDO
!
      ELSE
...
!
            CALL STBSV('Upper','Transpose','Non-unit',N,Kl+Ku,Ab,Ldab,  &
     &                 B(1,i),1)
         ENDDO
!
...
!
         IF ( lnoti ) THEN
            DO j = N - 1 , 1 , -1
               lm = MIN(Kl,N-j)
               CALL SGEMV('Transpose',lm,Nrhs,-ONE,B(j+1,1),Ldb,        &
