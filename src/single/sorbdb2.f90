!*==sorbdb2.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
      DO i = 1 , P
!
         IF ( i>1 ) CALL SROT(Q-i+1,X11(i,i),Ldx11,X21(i-1,i),Ldx21,c,s)
         CALL SLARFGP(Q-i+1,X11(i,i),X11(i,i+1),Ldx11,Tauq1(i))
         c = X11(i,i)
         X11(i,i) = ONE
         CALL SLARF('R',P-i,Q-i+1,X11(i,i),Ldx11,Tauq1(i),X11(i+1,i),   &
     &              Ldx11,Work(ilarf))
         CALL SLARF('R',M-P-i+1,Q-i+1,X11(i,i),Ldx11,Tauq1(i),X21(i,i), &
     &              Ldx21,Work(ilarf))
         s = SQRT(SNRM2(P-i,X11(i+1,i),1)**2+SNRM2(M-P-i+1,X21(i,i),1)  &
     &       **2)
         Theta(i) = ATAN2(s,c)
!
     ...
     &                lorbdb5,childinfo)
         ...
         IF ( i<P ) THEN
            ...
            s = SIN(Phi(i))
            X11(i+1,i) = ONE
            CALL SLARF('L',P-i,Q-i,X11(i+1,i),1,Taup1(i),X11(i+1,i+1),  &
     &                 Ldx11,Work(ilarf))
         ENDIF
         X21(i,i) = ONE
