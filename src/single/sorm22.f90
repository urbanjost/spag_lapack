!*==sorm22.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
               CALL SLACPY('All',N1,len,C(N2+1,i),Ldc,Work,ldwork)
               CALL STRMM('Left','Lower','No Transpose','Non-Unit',N1,  &
     &                    len,ONE,Q(1,N2+1),Ldq,Work,ldwork)
!
!              Multiply top part of C by Q11.
!
               CALL SGEMM('No Transpose','No Transpose',N1,len,N2,ONE,Q,&
     &                    Ldq,C(1,i),Ldc,ONE,Work,ldwork)
!
!              Multiply top part of C by Q21.
!
               CALL SLACPY('All',N2,len,C(1,i),Ldc,Work(N1+1),ldwork)
               CALL STRMM('Left','Upper','No Transpose','Non-Unit',N2,  &
     &                    len,ONE,Q(N1+1,1),Ldq,Work(N1+1),ldwork)
!
!              Multiply bottom part of C by Q22.
!
               CALL SGEMM('No Transpose','No Transpose',N2,len,N1,ONE,  &
...
!
               CALL SLACPY('All',N2,len,C(N1+1,i),Ldc,Work,ldwork)
               CALL STRMM('Left','Upper','Transpose','Non-Unit',N2,len, &
     &                    ONE,Q(N1+1,1),Ldq,Work,ldwork)
!
!              Multiply top part of C by Q11**T.
!
               CALL SGEMM('Transpose','No Transpose',N2,len,N1,ONE,Q,   &
     &                    Ldq,C(1,i),Ldc,ONE,Work,ldwork)
!
!              Multiply top part of C by Q12**T.
!
               CALL SLACPY('All',N1,len,C(1,i),Ldc,Work(N2+1),ldwork)
               CALL STRMM('Left','Lower','Transpose','Non-Unit',N1,len, &
     &                    ONE,Q(1,N2+1),Ldq,Work(N2+1),ldwork)
!
!              Multiply bottom part of C by Q22**T.
!
               CALL SGEMM('Transpose','No Transpose',N1,len,N2,ONE,     &
...
!
            CALL SLACPY('All',len,N2,C(i,N1+1),Ldc,Work,ldwork)
            CALL STRMM('Right','Upper','No Transpose','Non-Unit',len,N2,&
     &                 ONE,Q(N1+1,1),Ldq,Work,ldwork)
!
!              Multiply left part of C by Q11.
!
            CALL SGEMM('No Transpose','No Transpose',len,N2,N1,ONE,     &
     &                 C(i,1),Ldc,Q,Ldq,ONE,Work,ldwork)
!
!              Multiply left part of C by Q12.
!
            CALL SLACPY('All',len,N1,C(i,1),Ldc,Work(1+N2*ldwork),      &
     &                  ldwork)
            CALL STRMM('Right','Lower','No Transpose','Non-Unit',len,N1,&
     &                 ONE,Q(1,N2+1),Ldq,Work(1+N2*ldwork),ldwork)
!
!              Multiply right part of C by Q22.
!
            CALL SGEMM('No Transpose','No Transpose',len,N1,N2,ONE,     &
...
!
            CALL SLACPY('All',len,N1,C(i,N2+1),Ldc,Work,ldwork)
            CALL STRMM('Right','Lower','Transpose','Non-Unit',len,N1,   &
     &                 ONE,Q(1,N2+1),Ldq,Work,ldwork)
!
!              Multiply left part of C by Q11**T.
!
            CALL SGEMM('No Transpose','Transpose',len,N1,N2,ONE,C(i,1), &
     &                 Ldc,Q,Ldq,ONE,Work,ldwork)
!
!              Multiply left part of C by Q21**T.
!
            CALL SLACPY('All',len,N2,C(i,1),Ldc,Work(1+N1*ldwork),      &
     &                  ldwork)
            CALL STRMM('Right','Upper','Transpose','Non-Unit',len,N2,   &
     &                 ONE,Q(N1+1,1),Ldq,Work(1+N1*ldwork),ldwork)
!
!              Multiply right part of C by Q22**T.
!
            CALL SGEMM('No Transpose','Transpose',len,N2,N1,ONE,        &
