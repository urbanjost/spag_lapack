!*==sbdsqr.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b SBDSQR
!
!  =========== DOCUMENTATION ===========
!
...
!     .. External Subroutines ..
      EXTERNAL SLARTG , SLAS2 , SLASQ1 , SLASR , SLASV2 , SROT , SSCAL ,&
     &         SSWAP , XERBLA
!     ..
!     .. Intrinsic Functions ..
...
!
         IF ( Nru>0 ) CALL SLASR('R','V','F',Nru,N,Work(1),Work(N),U,   &
     &                           Ldu)
         IF ( Ncc>0 ) CALL SLASR('L','V','F',N,Ncc,Work(1),Work(N),C,   &
     &                           Ldc)
      ENDIF
!
...
!
         IF ( Ncvt>0 ) CALL SROT(Ncvt,Vt(m-1,1),Ldvt,Vt(m,1),Ldvt,cosr, &
     &                           sinr)
...
!           Update singular vectors if desired
!
         IF ( Ncvt>0 ) CALL SLASR('L','V','B',m-ll+1,Ncvt,Work(nm12+1), &
     &                            Work(nm13+1),Vt(ll,1),Ldvt)
         IF ( Nru>0 ) CALL SLASR('R','V','B',Nru,m-ll+1,Work(1),Work(N),&
     &                           U(1,ll),Ldu)
         IF ( Ncc>0 ) CALL SLASR('L','V','B',m-ll+1,Ncc,Work(1),Work(N),&
     &                           C(ll,1),Ldc)
      ENDIF
!
