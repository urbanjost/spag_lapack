!*==slaqr5.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     ..
!     .. Array Arguments ..
      REAL H(Ldh,*) , Si(*) , Sr(*) , U(Ldu,*) , V(Ldv,*) , Wh(Ldwh,*) ,&
     &     Wv(Ldwv,*) , Z(Ldz,*)
!     ..
!
!  ================================================================
...
!     ..
!     .. Local Scalars ..
      REAL alpha , beta , h11 , h12 , h21 , h22 , refsum , safmax ,     &
...
!
               k = krcol + 2*(m22-1)
               IF ( k==Ktop-1 ) THEN
                  CALL SLAQR1(2,H(k+1,k+1),Ldh,Sr(2*m22-1),Si(2*m22-1), &
...
!
            DO m = mbot , mtop , -1
               k = krcol + 2*(m-1)
               IF ( k==Ktop-1 ) THEN
                  CALL SLAQR1(3,H(Ktop,Ktop),Ldh,Sr(2*m-1),Si(2*m-1),   &
     &                        Sr(2*m),Si(2*m),V(1,m))
                  alpha = V(1,m)
...
!
                     CALL SLAQR1(3,H(k+1,k+1),Ldh,Sr(2*m-1),Si(2*m-1),  &
     &                           Sr(2*m),Si(2*m),vt)
                     alpha = vt(1)
...
!
            DO jrow = jtop , MAX(Ktop,incol) - 1 , Nv
               ...
               CALL SGEMM('N','N',jlen,nu,nu,ONE,H(jrow,incol+k1),Ldh,  &
