!*==ssysvxx.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b SSYSVXX
!
!  =========== DOCUMENTATION ===========
!
...
!     .. External Subroutines ..
      EXTERNAL SSYEQUB , SSYTRF , SSYTRS , SLACPY , SLAQSY , XERBLA ,   &
     &         SLASCL2 , SSYRFSX
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MAX , MIN
...
!
            IF ( N>0 ) Rpvgrw = SLA_SYRPVGRW(Uplo,N,Info,A,Lda,Af,Ldaf, &
     &                          Ipiv,Work)
            RETURN
         ENDIF
      ENDIF
...
!
      IF ( N>0 ) Rpvgrw = SLA_SYRPVGRW(Uplo,N,Info,A,Lda,Af,Ldaf,Ipiv,  &
     &                    Work)
!
...
!
      CALL SSYRFSX(Uplo,Equed,N,Nrhs,A,Lda,Af,Ldaf,Ipiv,S,B,Ldb,X,Ldx,  &
     &             Rcond,Berr,N_err_bnds,Err_bnds_norm,Err_bnds_comp,   &
     &             Nparams,Params,Work,Iwork,Info)
!
!     Scale solutions.
!
      IF ( rcequ ) CALL SLASCL2(N,Nrhs,S,X,Ldx)
!
!
!     End of SSYSVXX
!
