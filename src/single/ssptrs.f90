!*==ssptrs.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b SSPTRS
!
!  =========== DOCUMENTATION ===========
!
...
!
               ...
               CALL SGER(k-2,Nrhs,-ONE,Ap(kc-(k-1)),1,B(k-1,1),Ldb,     &
     &                   B(1,1),Ldb)
!
!           Multiply by the inverse of the diagonal block.
!
...
!           stored in column K of A.
!
               CALL SGEMV('Transpose',k-1,Nrhs,-ONE,B,Ldb,Ap(kc),1,ONE, &
     &                    B(k,1),Ldb)
!
!           Interchange rows K and IPIV(K).
!
...
!
               CALL SGEMV('Transpose',k-1,Nrhs,-ONE,B,Ldb,Ap(kc),1,ONE, &
     &                    B(k,1),Ldb)
               CALL SGEMV('Transpose',k-1,Nrhs,-ONE,B,Ldb,Ap(kc+k),1,   &
     &                    ONE,B(k+1,1),Ldb)
!
!           Interchange rows K and -IPIV(K).
!
...
!           stored in column K of A.
!
               IF ( k<N ) CALL SGER(N-k,Nrhs,-ONE,Ap(kc+1),1,B(k,1),Ldb,&
     &                              B(k+1,1),Ldb)
!
!           Multiply by the inverse of the diagonal block.
!
...
!
               IF ( k<N-1 ) THEN
                  CALL SGER(N-k-1,Nrhs,-ONE,Ap(kc+2),1,B(k,1),Ldb,      &
     &                      B(k+2,1),Ldb)
                  CALL SGER(N-k-1,Nrhs,-ONE,Ap(kc+N-k+2),1,B(k+1,1),Ldb,&
     &                      B(k+2,1),Ldb)
               ENDIF
!
!           Multiply by the inverse of the diagonal block.
