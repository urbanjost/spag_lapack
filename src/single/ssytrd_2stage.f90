!*==ssytrd_2stage.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
      ...
      abpos = 1
      wpos = abpos + ldab*N
      CALL SSYTRD_SY2SB(Uplo,N,kd,A,Lda,Work(abpos),ldab,Tau,Work(wpos),&
     &                  lwrk,Info)
      IF ( Info/=0 ) THEN
         CALL XERBLA('SSYTRD_SY2SB',-Info)
         RETURN
      ENDIF
      CALL SSYTRD_SB2ST('Y',Vect,Uplo,N,kd,Work(abpos),ldab,D,E,Hous2,  &
