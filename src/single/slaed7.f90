!*==slaed7.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
      CALL SLAED8(Icompq,k,N,Qsiz,D,Q,Ldq,Indxq,Rho,Cutpnt,Work(iz),    &
...
!
      IF ( k/=0 ) THEN
         CALL SLAED9(k,1,k,N,D,Work(is),k,Rho,Work(idlmda),Work(iw),    &
     &               Qstore(Qptr(curr)),k,Info)
         IF ( Info==0 ) THEN
            IF ( Icompq==1 ) CALL SGEMM('N','N',Qsiz,k,k,ONE,Work(iq2), &
