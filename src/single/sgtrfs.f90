!*==sgtrfs.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b SGTRFS
!
!  =========== DOCUMENTATION ===========
!
...
!
            ...
            CALL SLAGTM(Trans,N,1,-ONE,Dl,D,Du,X(1,j),Ldx,ONE,Work(N+1),&
     &                  N)
!
...
!
               CALL SGTTRS(Trans,N,1,Dlf,Df,Duf,Du2,Ipiv,Work(N+1),N,   &
     &                     Info)
...
!
                  CALL SGTTRS(transt,N,1,Dlf,Df,Duf,Du2,Ipiv,Work(N+1), &
     &                        N,Info)
                  DO i = 1 , N
...
!
                  DO i = 1 , N
                     Work(N+i) = Work(i)*Work(N+i)
                  ENDDO
                  CALL SGTTRS(transn,N,1,Dlf,Df,Duf,Du2,Ipiv,Work(N+1), &
     &                        N,Info)
               ENDIF
               CYCLE
            ENDIF
