!*==ssytrd_sb2st.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      ...
      USE S_ILAENV2STAGE
      USE S_LSAME
      USE S_OMP_GET_THREAD_NUM
      USE S_SLACPY
      USE S_SLASET
      USE S_SSB2ST_KERNELS
      USE S_XERBLA
      IMPLICIT NONE
!*--SSYTRD_SB2ST246
!
...
!
      INTEGER :: abdpos , abofdpos , apos , awpos , blklastind , colpt ,&
     &           debug , dpos , ed , edind , grsiz , i , ib , inda ,    &
     &           indtau , indv , indw , k , lda , ldv , lhmin , lwmin , &
     &           m , myid , nbtiles , nthreads , ofdpos , shift ,       &
     &           sisev , sizea , sizetau , st , stepercol , stind ,     &
     &           stt , sweepid , thed , thgrid , thgrnb , thgrsiz ,     &
     &           tid , ttype
      LOGICAL :: afters1 , lquery , upper , wantq
!
...
!$OMP$     DEPEND(out:WORK(MYID))
                        ...
                        CALL SSB2ST_KERNELS(Uplo,wantq,ttype,stind,     &
...
!$OMP END TASK
                     ENDIF
#else
                     CALL SSB2ST_KERNELS(Uplo,wantq,ttype,stind,edind,  &
