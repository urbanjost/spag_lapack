!*==strevc3.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b STREVC3
!
!  =========== DOCUMENTATION ===========
!
...
!
     ...
     &        smlnum , ulp , unfl , vcrit , vmax , wi , wr , xnorm
      INTEGER :: i , ierr , ii , ip , is , iv , j , j1 , j2 , jnxt , k ,&
     &           ki , ki2 , maxwrk , nb
      INTEGER , DIMENSION(NBMAX) :: iscomplex
...
!
                        CALL SLALN2(.FALSE.,1,1,smin,ONE,T(j,j),Ldt,ONE,&
...
!
                        CALL SLALN2(.FALSE.,2,1,smin,ONE,T(j-1,j-1),Ldt,&
...
!                    Update right-hand side
!
                        CALL SAXPY(j-2,-x(1,1),T(1,j-1),1,Work(1+iv*N), &
     &                             1)
                        CALL SAXPY(j-2,-x(2,1),T(1,j),1,Work(1+iv*N),1)
                     ENDIF
                  ENDIF
               ENDDO
!
...
!                 version 1: back-transform each vector with GEMV, Q*x.
                  IF ( ki>1 )                                           &
     &                 CALL SGEMV('N',N,ki-1,ONE,Vr,Ldvr,Work(1+iv*N),1,&
...
!                    1-by-1 diagonal block
!
                        CALL SLALN2(.FALSE.,1,2,smin,ONE,T(j,j),Ldt,ONE,&
...
!
                        CALL SAXPY(j-1,-x(1,1),T(1,j),1,Work(1+(iv-1)*N)&
     &                             ,1)
                        CALL SAXPY(j-1,-x(1,2),T(1,j),1,Work(1+(iv)*N), &
     &                             1)
!
                     ELSE
!
!                    2-by-2 diagonal block
!
                        CALL SLALN2(.FALSE.,2,2,smin,ONE,T(j-1,j-1),Ldt,&
...
!
     ...
     &                             Work(1+(iv-1)*N),1)
                        CALL SAXPY(j-2,-x(2,1),T(1,j),1,Work(1+(iv-1)*N)&
     &                             ,1)
                        CALL SAXPY(j-2,-x(1,2),T(1,j-1),1,Work(1+(iv)*N)&
     &                             ,1)
                        CALL SAXPY(j-2,-x(2,2),T(1,j),1,Work(1+(iv)*N), &
     &                             1)
                     ENDIF
                  ENDIF
               ENDDO
...
!                 version 1: back-transform each vector with GEMV, Q*x.
                  IF ( ki>2 ) THEN
                     CALL SGEMV('N',N,ki-2,ONE,Vr,Ldvr,Work(1+(iv-1)*N),&
     &                          1,Work(ki-1+(iv-1)*N),Vr(1,ki-1),1)
                     CALL SGEMV('N',N,ki-2,ONE,Vr,Ldvr,Work(1+(iv)*N),1,&
     &                          Work(ki+(iv)*N),Vr(1,ki),1)
                  ELSE
...
!              or if this was last vector, do the GEMM
               IF ( (iv<=2) .OR. (ki2==1) ) THEN
                  CALL SGEMM('N','N',N,nb-iv+1,ki2+nb-iv,ONE,Vr,Ldvr,   &
...
!                       reuse same REMAX as previous K
                  ...
                  ENDDO
                  CALL SLACPY('F',N,nb-iv+1,Work(1+(nb+iv)*N),N,        &
     &                        Vr(1,ki2),Ldvr)
                  iv = nb
               ELSE
                  iv = iv - 1
...
!
     ...
     &                     - SDOT(j-ki-1,T(ki+1,j+1),1,Work(ki+1+iv*N), &
     &                     1)
!
!                    Solve
...
!
                        CALL SLALN2(.TRUE.,2,1,smin,ONE,T(j,j),Ldt,ONE, &
     &                              ONE,Work(j+iv*N),N,wr,ZERO,x,2,     &
     &                              scale,xnorm,ierr)
!
!                    Scale if necessary
...
!
                        Work(j+(iv)*N) = Work(j+(iv)*N)                 &
     &                     - SDOT(j-ki-2,T(ki+2,j),1,Work(ki+2+(iv)*N), &
     &                     1)
                        Work(j+(iv+1)*N) = Work(j+(iv+1)*N)             &
     &                     - SDOT(j-ki-2,T(ki+2,j),1,Work(ki+2+(iv+1)*N)&
     &                     ,1)
!
...
!
                        Work(j+1+(iv)*N) = Work(j+1+(iv)*N)             &
     &                     - SDOT(j-ki-2,T(ki+2,j+1),1,Work(ki+2+(iv)*N)&
     &                     ,1)
!
...
!                    [ (T(j+1,j) T(j+1,j+1))                  ]
!
                        CALL SLALN2(.TRUE.,2,2,smin,ONE,T(j,j),Ldt,ONE, &
     &                              ONE,Work(j+iv*N),N,wr,-wi,x,2,scale,&
     &                              xnorm,ierr)
!
!                    Scale if necessary
...
!                 no back-transform: copy x to VL and normalize.
                  CALL SCOPY(N-ki+1,Work(ki+(iv)*N),1,Vl(ki,is),1)
                  CALL SCOPY(N-ki+1,Work(ki+(iv+1)*N),1,Vl(ki,is+1),1)
!
                  emax = ZERO
                  DO k = ki , N
...
!              or if this was last vector, do the GEMM
               IF ( (iv>=nb-1) .OR. (ki2==N) ) THEN
                  CALL SGEMM('N','N',N,iv,N-ki2+iv,ONE,Vl(1,ki2-iv+1),  &
...
!                       reuse same REMAX as previous K
                     ...
                     CALL SSCAL(N,remax,Work(1+(nb+k)*N),1)
                  ENDDO
                  CALL SLACPY('F',N,iv,Work(1+(nb+1)*N),N,Vl(1,ki2-iv+1)&
     &                        ,Ldvl)
                  iv = 1
               ELSE
                  iv = iv + 1
