!*==ssytri2x.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b SSYTRI2X
!
!  =========== DOCUMENTATION ===========
!
...
!
      REAL :: ak , akkp1 , akp1 , d , t , u01_ip1_j , u01_i_j ,         &
     &        u11_ip1_j , u11_i_j
...
!       U11**T*invD1*U11->U11
!
            CALL STRMM('L','U','T','U',nnb,nnb,ONE,A(cut+1,cut+1),Lda,  &
     &                 Work(u11+1,1),N+Nb+1)
!
            DO i = 1 , nnb
               DO j = i , nnb
...
!
            CALL SGEMM('T','N',nnb,nnb,cut,ONE,A(1,cut+1),Lda,Work,     &
...
!        U01 =  U00**T*invD0*U01
!
            CALL STRMM('L',Uplo,'T','U',cut,nnb,ONE,A,Lda,Work,N+Nb+1)
...
!
         ...
         DO WHILE ( i<=N )
            ...
            ELSE
               ip = -Ipiv(i)
               i = i + 1
               IF ( (i-1)<ip ) CALL SSYSWAPR(Uplo,N,A,Lda,i-1,ip)
               IF ( (i-1)>ip ) CALL SSYSWAPR(Uplo,N,A,Lda,ip,i-1)
            ENDIF
            i = i + 1
         ENDDO
      ELSE
...
!       L11**T*invD1*L11->L11
!
            CALL STRMM('L',Uplo,'T','U',nnb,nnb,ONE,A(cut+1,cut+1),Lda, &
     &                 Work(u11+1,1),N+Nb+1)
