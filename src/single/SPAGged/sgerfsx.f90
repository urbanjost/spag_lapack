!*==sgerfsx.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b SGERFSX
!
!  =========== DOCUMENTATION ===========
!
...
!
         IF ( colequ .AND. notran ) THEN
            rcond_tmp = SLA_GERCOND(Trans,N,A,Lda,Af,Ldaf,Ipiv,-1,C,    &
     &                  Info,Work,Iwork)
         ELSEIF ( rowequ .AND. .NOT.notran ) THEN
            rcond_tmp = SLA_GERCOND(Trans,N,A,Lda,Af,Ldaf,Ipiv,-1,R,    &
     &                  Info,Work,Iwork)
         ELSE
            rcond_tmp = SLA_GERCOND(Trans,N,A,Lda,Af,Ldaf,Ipiv,0,R,Info,&
     &                  Work,Iwork)
         ENDIF
         DO j = 1 , Nrhs
!
...
!
         ...
         DO j = 1 , Nrhs
            IF ( Err_bnds_comp(j,LA_LINRX_ERR_I)<cwise_wrong ) THEN
               rcond_tmp = SLA_GERCOND(Trans,N,A,Lda,Af,Ldaf,Ipiv,1,    &
