!*==sgesvd.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
     ...
     &           lwork_sgebrd , lwork_sgelqf , lwork_sgeqrf ,           &
     &           lwork_sorgbr_p , lwork_sorgbr_q , lwork_sorglq_m ,     &
     &           lwork_sorglq_n , lwork_sorgqr_m , lwork_sorgqr_n ,     &
...
!
               CALL SGEBRD(M,N,A,Lda,S,dum(1),dum(1),dum(1),dum(1),-1,  &
     &                     ierr)
               lwork_sgebrd = INT(dum(1))
               ...
               IF ( wntua ) THEN
                  ...
                  maxwrk = MAX(maxwrk,3*N+lwork_sorgbr_q)
               ENDIF
               IF ( .NOT.wntvn ) maxwrk = MAX(maxwrk,3*N+lwork_sorgbr_p)
...
!                Compute space needed for SORGBR P
               ...
               IF ( wntva ) THEN
                  ...
                  maxwrk = MAX(maxwrk,3*M+lwork_sorgbr_p)
               ENDIF
               IF ( .NOT.wntun ) maxwrk = MAX(maxwrk,3*M+lwork_sorgbr_q)
               maxwrk = MAX(maxwrk,bdspac)
...
!
               ...
               IF ( wntus ) ncu = N
               IF ( wntua ) ncu = M
               CALL SORGBR('Q',M,ncu,N,U,Ldu,Work(itauq),Work(iwork),   &
     &                     Lwork-iwork+1,ierr)
            ENDIF
            IF ( wntvas ) THEN
!
...
!
            IF ( wntuo ) CALL SORGBR('Q',M,N,N,A,Lda,Work(itauq),       &
     &                               Work(iwork),Lwork-iwork+1,ierr)
!
...
!              (Workspace: need 4*N-1, prefer 3*N+(N-1)*NB)
!
            IF ( wntvo ) CALL SORGBR('P',N,N,N,A,Lda,Work(itaup),       &
     &                               Work(iwork),Lwork-iwork+1,ierr)
            iwork = ie + N
...
!
               CALL SBDSQR('U',N,ncvt,nru,0,S,Work(ie),Vt,Ldvt,U,Ldu,   &
...
!              (Workspace: need BDSPAC)
!
               CALL SBDSQR('U',N,ncvt,nru,0,S,Work(ie),A,Lda,U,Ldu,dum, &
     &                     1,Work(iwork),Info)
            ELSE
!
!              Perform bidiagonal QR iteration, if desired, computing
...
!              (Workspace: need BDSPAC)
!
               CALL SBDSQR('U',N,ncvt,nru,0,S,Work(ie),Vt,Ldvt,A,Lda,   &
     &                     dum,1,Work(iwork),Info)
            ENDIF
!
         ELSEIF ( wntun ) THEN
...
!
     ...
     &        CALL SLASCL('G',0,0,smlnum,anrm,minmn-1,1,Work(2),minmn,  &
     &        ierr)
      ENDIF
!
