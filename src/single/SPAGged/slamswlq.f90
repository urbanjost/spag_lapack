!*==slamswlq.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b SLAMSWLQ
!
!  Definition:
!  ===========
...
!
         IF ( kk>0 ) THEN
            ii = M - kk + 1
            CALL STPMLQT('L','T',kk,N,K,0,Mb,A(1,ii),Lda,T(1,ctr*K+1),  &
...
!
            ctr = ctr - 1
            CALL STPMLQT('L','T',Nb-K,N,K,0,Mb,A(1,i),Lda,T(1,ctr*K+1), &
     &                   Ldt,C(1,1),Ldc,C(i,1),Ldc,Work,Info)
         ENDDO
!
!         Multiply Q to the first block of C (1:M,1:NB)
!
         CALL SGEMLQT('L','T',Nb,N,K,Mb,A(1,1),Lda,T,Ldt,C(1,1),Ldc,    &
     &                Work,Info)
!
...
!
         ...
         ii = M - kk + 1
         ctr = 1
         CALL SGEMLQT('L','N',Nb,N,K,Mb,A(1,1),Lda,T,Ldt,C(1,1),Ldc,    &
     &                Work,Info)
!
...
!
            CALL STPMLQT('L','N',Nb-K,N,K,0,Mb,A(1,i),Lda,T(1,ctr*K+1), &
     &                   Ldt,C(1,1),Ldc,C(i,1),Ldc,Work,Info)
            ctr = ctr + 1
!
         ENDDO
...
!
!
         IF ( ii<=M ) CALL STPMLQT('L','N',kk,N,K,0,Mb,A(1,ii),Lda,     &
...
!
         ...
         IF ( kk>0 ) THEN
            ii = N - kk + 1
            CALL STPMLQT('R','N',M,kk,K,0,Mb,A(1,ii),Lda,T(1,ctr*K+1),  &
...
!
            ctr = ctr - 1
            CALL STPMLQT('R','N',M,Nb-K,K,0,Mb,A(1,i),Lda,T(1,ctr*K+1), &
...
!
         CALL SGEMLQT('R','N',M,Nb,K,Mb,A(1,1),Lda,T,Ldt,C(1,1),Ldc,    &
     &                Work,Info)
!
...
!
         ...
         ii = N - kk + 1
         ctr = 1
         CALL SGEMLQT('R','T',M,Nb,K,Mb,A(1,1),Lda,T,Ldt,C(1,1),Ldc,    &
     &                Work,Info)
!
         DO i = Nb + 1 , ii - Nb + K , (Nb-K)
!
!         Multiply Q to the current block of C (1:M,I:I+MB)
!
            CALL STPMLQT('R','T',M,Nb-K,K,0,Mb,A(1,i),Lda,T(1,ctr*K+1), &
     &                   Ldt,C(1,1),Ldc,C(1,i),Ldc,Work,Info)
            ctr = ctr + 1
!
         ENDDO
...
!
!
         IF ( ii<=N ) CALL STPMLQT('R','T',M,kk,K,0,Mb,A(1,ii),Lda,     &
