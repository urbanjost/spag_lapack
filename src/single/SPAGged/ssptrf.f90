!*==ssptrf.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b SSPTRF
!
!  =========== DOCUMENTATION ===========
!
...
!
                     kp = k
                  ELSEIF ( ABS(Ap(kpc+imax-1))>=alpha*rowmax ) THEN
!
...
!
                     kp = k
                  ELSEIF ( ABS(Ap(kpc))>=alpha*rowmax ) THEN
!
!                 interchange rows and columns K and IMAX, use 1-by-1
!                 pivot block
!
...
!
                  IF ( kp<N )                                           &
     &                 CALL SSWAP(N-kp,Ap(knc+kp-kk+1),1,Ap(kpc+1),1)
                  kx = knc + kp - kk
                  DO j = kk + 1 , kp - 1
                     kx = kx + N - j + 1
                     t = Ap(knc+j-kk)
