!*==sspgst.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b SSPGST
!
!  =========== DOCUMENTATION ===========
!
...
!
               ...
               IF ( k<N ) THEN
                  ...
                  CALL SSPR2(Uplo,N-k,-ONE,Ap(kk+1),1,Bp(kk+1),1,       &
     &                       Ap(k1k1))
                  CALL SAXPY(N-k,ct,Bp(kk+1),1,Ap(kk+1),1)
                  CALL STPSV(Uplo,'No transpose','Non-unit',N-k,Bp(k1k1)&
     &                       ,Ap(kk+1),1)
               ENDIF
               kk = k1k1
            ENDDO
...
!
            ...
            CALL SSPMV(Uplo,N-j,ONE,Ap(j1j1),Bp(jj+1),1,ONE,Ap(jj+1),1)
            CALL STPMV(Uplo,'Transpose','Non-unit',N-j+1,Bp(jj),Ap(jj), &
     &                 1)
            jj = j1j1
         ENDDO
      ENDIF
