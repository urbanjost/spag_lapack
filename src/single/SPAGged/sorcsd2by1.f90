!*==sorcsd2by1.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
     ...
     &           lorgqrmin , lorgqropt , lworkmin , lworkopt , r
      REAL , DIMENSION(1) :: dum1
      REAL , DIMENSION(1,1) :: dum2
...
!
      IF ( Info==0 ) THEN
         ...
         lorglqopt = 1
         IF ( r==Q ) THEN
            CALL SORBDB1(M,P,Q,X11,Ldx11,X21,Ldx21,Theta,dum1,dum1,dum1,&
     &                   dum1,Work,-1,childinfo)
            ...
            IF ( wantv1t .AND. Q>0 ) THEN
               CALL SORGLQ(Q-1,Q-1,Q-1,V1t,Ldv1t,dum1,Work(1),-1,       &
     &                     childinfo)
               lorglqmin = MAX(lorglqmin,Q-1)
               lorglqopt = MAX(lorglqopt,INT(Work(1)))
            ENDIF
            CALL SBBCSD(Jobu1,Jobu2,Jobv1t,'N','N',M,P,Q,Theta,dum1,U1, &
     &                  Ldu1,U2,Ldu2,V1t,Ldv1t,dum2,1,dum1,dum1,dum1,   &
     &                  dum1,dum1,dum1,dum1,dum1,Work(1),-1,childinfo)
            lbbcsd = INT(Work(1))
         ELSEIF ( r==P ) THEN
            CALL SORBDB2(M,P,Q,X11,Ldx11,X21,Ldx21,Theta,dum1,dum1,dum1,&
     &                   dum1,Work(1),-1,childinfo)
            lorbdb = INT(Work(1))
            IF ( wantu1 .AND. P>0 ) THEN
               CALL SORGQR(P-1,P-1,P-1,U1(2,2),Ldu1,dum1,Work(1),-1,    &
     &                     childinfo)
            ...
            IF ( wantv1t .AND. Q>0 ) THEN
               ...
               lorglqopt = MAX(lorglqopt,INT(Work(1)))
            ENDIF
            CALL SBBCSD(Jobv1t,'N',Jobu1,Jobu2,'T',M,Q,P,Theta,dum1,V1t,&
     &                  Ldv1t,dum2,1,U1,Ldu1,U2,Ldu2,dum1,dum1,dum1,    &
     &                  dum1,dum1,dum1,dum1,dum1,Work(1),-1,childinfo)
            lbbcsd = INT(Work(1))
         ELSEIF ( r==M-P ) THEN
            CALL SORBDB3(M,P,Q,X11,Ldx11,X21,Ldx21,Theta,dum1,dum1,dum1,&
     &                   dum1,Work(1),-1,childinfo)
            lorbdb = INT(Work(1))
            IF ( wantu1 .AND. P>0 ) THEN
               CALL SORGQR(P,P,Q,U1,Ldu1,dum1,Work(1),-1,childinfo)
            ...
            ENDIF
            IF ( wantu2 .AND. M>P ) THEN
               CALL SORGQR(M-P-1,M-P-1,M-P-1,U2(2,2),Ldu2,dum1,Work(1), &
     &                     -1,childinfo)
            ...
            IF ( wantv1t .AND. Q>0 ) THEN
               ...
               lorglqopt = MAX(lorglqopt,INT(Work(1)))
            ENDIF
            CALL SBBCSD('N',Jobv1t,Jobu2,Jobu1,'T',M,M-Q,M-P,Theta,dum1,&
     &                  dum2,1,V1t,Ldv1t,U2,Ldu2,U1,Ldu1,dum1,dum1,dum1,&
     &                  dum1,dum1,dum1,dum1,dum1,Work(1),-1,childinfo)
            lbbcsd = INT(Work(1))
         ELSE
            CALL SORBDB4(M,P,Q,X11,Ldx11,X21,Ldx21,Theta,dum1,dum1,dum1,&
     &                   dum1,dum1,Work(1),-1,childinfo)
            ...
            ENDIF
            IF ( wantu2 .AND. M>P ) THEN
               CALL SORGQR(M-P,M-P,M-Q,U2,Ldu2,dum1,Work(1),-1,         &
     &                     childinfo)
               lorgqrmin = MAX(lorgqrmin,M-P)
               lorgqropt = MAX(lorgqropt,INT(Work(1)))
            ENDIF
            IF ( wantv1t .AND. Q>0 ) THEN
               ...
               lorglqopt = MAX(lorglqopt,INT(Work(1)))
            ENDIF
            CALL SBBCSD(Jobu2,Jobu1,'N',Jobv1t,'N',M,M-P,M-Q,Theta,dum1,&
...
!
         CALL SORBDB1(M,P,Q,X11,Ldx11,X21,Ldx21,Theta,Work(iphi),       &
...
!
         IF ( wantu1 .AND. P>0 ) THEN
            ...
            CALL SORGQR(P,P,Q,U1,Ldu1,Work(itaup1),Work(iorgqr),lorgqr, &
     &                  childinfo)
         ENDIF
         IF ( wantu2 .AND. M>P ) THEN
     ...
     &                  lorgqr,childinfo)
         ...
         IF ( wantv1t .AND. Q>0 ) THEN
            ...
            DO j = 2 , Q
               ...
               V1t(j,1) = ZERO
            ENDDO
            CALL SLACPY('U',Q-1,Q-1,X21(1,2),Ldx21,V1t(2,2),Ldv1t)
...
!
         CALL SBBCSD(Jobu1,Jobu2,Jobv1t,'N','N',M,P,Q,Theta,Work(iphi), &
...
!        Simultaneously bidiagonalize X11 and X21
!
         CALL SORBDB2(M,P,Q,X11,Ldx11,X21,Ldx21,Theta,Work(iphi),       &
     &                Work(itaup1),Work(itaup2),Work(itauq1),           &
     &                Work(iorbdb),lorbdb,childinfo)
!
!        Accumulate Householder reflectors
!
     ...
     &                  lorgqr,childinfo)
         ...
         IF ( wantv1t .AND. Q>0 ) THEN
            ...
            CALL SORGLQ(Q,Q,r,V1t,Ldv1t,Work(itauq1),Work(iorglq),      &
     &                  lorglq,childinfo)
         ENDIF
!
!        Simultaneously diagonalize X11 and X21.
!
         CALL SBBCSD(Jobv1t,'N',Jobu1,Jobu2,'T',M,Q,P,Theta,Work(iphi), &
     &               V1t,Ldv1t,dum1,1,U1,Ldu1,U2,Ldu2,Work(ib11d),      &
     &               Work(ib11e),Work(ib12d),Work(ib12e),Work(ib21d),   &
     &               Work(ib21e),Work(ib22d),Work(ib22e),Work(ibbcsd),  &
     &               lbbcsd,childinfo)
!
...
!        Simultaneously bidiagonalize X11 and X21
!
         CALL SORBDB3(M,P,Q,X11,Ldx11,X21,Ldx21,Theta,Work(iphi),       &
     &                Work(itaup1),Work(itaup2),Work(itauq1),           &
     &                Work(iorbdb),lorbdb,childinfo)
!
!        Accumulate Householder reflectors
     ...
     &                  childinfo)
         ...
         IF ( wantu2 .AND. M>P ) THEN
            ...
            DO j = 2 , M - P
               ...
               U2(j,1) = ZERO
            ENDDO
            CALL SLACPY('L',M-P-1,M-P-1,X21(2,1),Ldx21,U2(2,2),Ldu2)
            CALL SORGQR(M-P-1,M-P-1,M-P-1,U2(2,2),Ldu2,Work(itaup2),    &
     &                  Work(iorgqr),lorgqr,childinfo)
         ENDIF
         IF ( wantv1t .AND. Q>0 ) THEN
...
!        Simultaneously diagonalize X11 and X21.
!
         CALL SBBCSD('N',Jobv1t,Jobu2,Jobu1,'T',M,M-Q,M-P,Theta,        &
     &               Work(iphi),dum1,1,V1t,Ldv1t,U2,Ldu2,U1,Ldu1,       &
...
!
         IF ( Q>r ) THEN
            ...
            DO i = r + 1 , Q
               Iwork(i) = i - r
            ENDDO
            IF ( wantu1 ) CALL SLAPMT(.FALSE.,P,Q,U1,Ldu1,Iwork)
            IF ( wantv1t ) CALL SLAPMR(.FALSE.,Q,Q,V1t,Ldv1t,Iwork)
         ENDIF
      ELSE
!
!        Case 4: R = M-Q
...
!        Simultaneously bidiagonalize X11 and X21
!
         CALL SORBDB4(M,P,Q,X11,Ldx11,X21,Ldx21,Theta,Work(iphi),       &
     &                Work(itaup1),Work(itaup2),Work(itauq1),           &
     &                Work(iorbdb),Work(iorbdb+M),lorbdb-M,childinfo)
!
!        Accumulate Householder reflectors
!
         IF ( wantu2 .AND. M>P ) CALL SCOPY(M-P,Work(iorbdb+P),1,U2,1)
         IF ( wantu1 .AND. P>0 ) THEN
            CALL SCOPY(P,Work(iorbdb),1,U1,1)
            DO j = 2 , P
               U1(1,j) = ZERO
            ENDDO
            CALL SLACPY('L',P-1,M-Q-1,X11(2,1),Ldx11,U1(2,2),Ldu1)
            CALL SORGQR(P,P,M-Q,U1,Ldu1,Work(itaup1),Work(iorgqr),      &
     &                  lorgqr,childinfo)
         ENDIF
         IF ( wantu2 .AND. M>P ) THEN
            DO j = 2 , M - P
               U2(1,j) = ZERO
            ENDDO
            CALL SLACPY('L',M-P-1,M-Q-1,X21(2,1),Ldx21,U2(2,2),Ldu2)
            CALL SORGQR(M-P,M-P,M-Q,U2,Ldu2,Work(itaup2),Work(iorgqr),  &
     &                  lorgqr,childinfo)
         ENDIF
         IF ( wantv1t .AND. Q>0 ) THEN
     ...
     &                  V1t(M-Q+1,M-Q+1),Ldv1t)
            CALL SLACPY('U',-P+Q,Q-P,X21(M-Q+1,P+1),Ldx21,V1t(P+1,P+1), &
     &                  Ldv1t)
            CALL SORGLQ(Q,Q,Q,V1t,Ldv1t,Work(itauq1),Work(iorglq),      &
     &                  lorglq,childinfo)
         ENDIF
!
!        Simultaneously diagonalize X11 and X21.
!
         CALL SBBCSD(Jobu2,Jobu1,'N',Jobv1t,'N',M,M-P,M-Q,Theta,        &
     &               Work(iphi),U2,Ldu2,U1,Ldu1,dum1,1,V1t,Ldv1t,       &
     &               Work(ib11d),Work(ib11e),Work(ib12d),Work(ib12e),   &
     &               Work(ib21d),Work(ib21e),Work(ib22d),Work(ib22e),   &
     &               Work(ibbcsd),lbbcsd,childinfo)
!
...
!
         IF ( P>r ) THEN
            ...
            DO i = r + 1 , P
               Iwork(i) = i - r
            ENDDO
            IF ( wantu1 ) CALL SLAPMT(.FALSE.,P,P,U1,Ldu1,Iwork)
            IF ( wantv1t ) CALL SLAPMR(.FALSE.,P,Q,V1t,Ldv1t,Iwork)
         ENDIF
      ENDIF
!
!
