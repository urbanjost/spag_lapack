!*==stftri.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b STFTRI
!
!  =========== DOCUMENTATION ===========
!
...
!
               ...
               IF ( Info>0 ) RETURN
               CALL STRMM('R','L','N',Diag,n2,n1,-ONE,A(0),N,A(n1),N)
               CALL STRTRI('U',Diag,n2,A(N),N,Info)
               IF ( Info>0 ) Info = Info + n1
               IF ( Info>0 ) RETURN
               CALL STRMM('L','U','T',Diag,n2,n1,ONE,A(N),N,A(n1),N)
!
            ELSE
!
...
!
               ...
               IF ( Info>0 ) RETURN
               CALL STRMM('L','L','T',Diag,n1,n2,-ONE,A(n2),N,A(0),N)
               CALL STRTRI('U',Diag,n2,A(n1),N,Info)
               IF ( Info>0 ) Info = Info + n1
               IF ( Info>0 ) RETURN
               CALL STRMM('R','U','N',Diag,n1,n2,ONE,A(n1),N,A(0),N)
!
            ENDIF
!
!
...
!
            ...
            IF ( Info>0 ) RETURN
            CALL STRMM('L','U','N',Diag,n1,n2,-ONE,A(0),n1,A(n1*n1),n1)
            CALL STRTRI('L',Diag,n2,A(1),n1,Info)
            IF ( Info>0 ) Info = Info + n1
            IF ( Info>0 ) RETURN
            CALL STRMM('R','L','T',Diag,n1,n2,ONE,A(1),n1,A(n1*n1),n1)
!
         ELSE
!
...
!
            ...
            IF ( Info>0 ) RETURN
            CALL STRMM('R','U','T',Diag,n2,n1,-ONE,A(n2*n2),n2,A(0),n2)
            CALL STRTRI('L',Diag,n2,A(n1*n2),n2,Info)
            IF ( Info>0 ) Info = Info + n1
            IF ( Info>0 ) RETURN
            CALL STRMM('L','L','N',Diag,n2,n1,ONE,A(n1*n2),n2,A(0),n2)
!
         ENDIF
!
!
...
!
            ...
            IF ( Info>0 ) RETURN
            CALL STRMM('R','L','N',Diag,k,k,-ONE,A(1),N+1,A(k+1),N+1)
            CALL STRTRI('U',Diag,k,A(0),N+1,Info)
            IF ( Info>0 ) Info = Info + k
            IF ( Info>0 ) RETURN
            CALL STRMM('L','U','T',Diag,k,k,ONE,A(0),N+1,A(k+1),N+1)
!
         ELSE
!
...
!
            ...
            IF ( Info>0 ) RETURN
            CALL STRMM('L','L','T',Diag,k,k,-ONE,A(k+1),N+1,A(0),N+1)
            CALL STRTRI('U',Diag,k,A(k),N+1,Info)
            IF ( Info>0 ) Info = Info + k
            IF ( Info>0 ) RETURN
            CALL STRMM('R','U','N',Diag,k,k,ONE,A(k),N+1,A(0),N+1)
         ENDIF
!
...
!
         ...
         IF ( Info>0 ) RETURN
         CALL STRMM('L','U','N',Diag,k,k,-ONE,A(k),k,A(k*(k+1)),k)
         CALL STRTRI('L',Diag,k,A(0),k,Info)
         IF ( Info>0 ) Info = Info + k
         IF ( Info>0 ) RETURN
         CALL STRMM('R','L','T',Diag,k,k,ONE,A(0),k,A(k*(k+1)),k)
      ELSE
!
...
!
         ...
         IF ( Info>0 ) RETURN
         CALL STRMM('R','U','T',Diag,k,k,-ONE,A(k*(k+1)),k,A(0),k)
         CALL STRTRI('L',Diag,k,A(k*k),k,Info)
         IF ( Info>0 ) Info = Info + k
         IF ( Info>0 ) RETURN
         CALL STRMM('L','L','N',Diag,k,k,ONE,A(k*k),k,A(0),k)
      ENDIF
!
!
!     End of STFTRI
