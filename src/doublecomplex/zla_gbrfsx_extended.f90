!*==zla_gbrfsx_extended.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Array Arguments ..
      INTEGER Ipiv(*)
      COMPLEX*16 Ab(Ldab,*) , Afb(Ldafb,*) , B(Ldb,*) , Y(Ldy,*) ,      &
     &           Res(*) , Dy(*) , Y_tail(*)
      DOUBLE PRECISION C(*) , Ayb(*) , Rcond , Berr_out(*) ,            &
...
!     .. Local Scalars ..
      ...
      DOUBLE PRECISION yk , dyk , ymin , normy , normx , normdx ,       &
     &                 dxrat , dzrat , prevnormdx , prev_dz_z ,         &
     &                 dxratmax , dzratmax , dx_x , dz_z , final_dx_x , &
     &                 final_dz_z , eps , hugeval , incr_thresh
      LOGICAL incr_prec
      COMPLEX*16 zdum
!     ..
!     .. Parameters ..
...
!
            ...
            IF ( y_prec_state==BASE_RESIDUAL ) THEN
               CALL ZGBMV(trans,m,N,Kl,Ku,(-1.0D+0,0.0D+0),Ab,Ldab,     &
     &                    Y(1,j),1,(1.0D+0,0.0D+0),Res,1)
            ELSEIF ( y_prec_state==EXTRA_RESIDUAL ) THEN
               CALL BLAS_ZGBMV_X(Trans_type,N,N,Kl,Ku,(-1.0D+0,0.0D+0), &
     &                           Ab,Ldab,Y(1,j),1,(1.0D+0,0.0D+0),Res,1,&
     &                           Prec_type)
            ELSE
               CALL BLAS_ZGBMV2_X(Trans_type,N,N,Kl,Ku,(-1.0D+0,0.0D+0),&
     &                            Ab,Ldab,Y(1,j),Y_tail,1,              &
...
!
         CALL ZCOPY(N,B(1,j),1,Res,1)
         CALL ZGBMV(trans,N,N,Kl,Ku,(-1.0D+0,0.0D+0),Ab,Ldab,Y(1,j),1,  &
     &              (1.0D+0,0.0D+0),Res,1)
...
!
         CALL ZLA_GBAMV(Trans_type,N,N,Kl,Ku,1.0D+0,Ab,Ldab,Y(1,j),1,   &
     &                  1.0D+0,Ayb,1)
