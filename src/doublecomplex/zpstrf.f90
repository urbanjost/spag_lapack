!*==zpstrf.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
                     ...
                     IF ( pvt<N )                                       &
     &                    CALL ZSWAP(N-pvt,A(j,pvt+1),Lda,A(pvt,pvt+1), &
     &                    Lda)
                     DO i = j + 1 , pvt - 1
                        ztemp = DCONJG(A(j,i))
...
!
                  IF ( j<N ) THEN
                     ...
                     CALL ZGEMV('Trans',j-k,N-j,-CONE,A(k,j+1),Lda,     &
     &                          A(k,j),1,CONE,A(j,j+1),Lda)
                     CALL ZLACGV(j-1,A(1,j),1)
...
!
               IF ( k+jb<=N ) CALL ZHERK('Upper','Conj Trans',N-j+1,jb, &
     &              -ONE,A(k,j),Lda,ONE,A(j,j),Lda)
!
            ENDDO
!
         ELSE
...
!
                     ...
                     IF ( pvt<N )                                       &
     &                    CALL ZSWAP(N-pvt,A(pvt+1,j),1,A(pvt+1,pvt),1)
                     DO i = j + 1 , pvt - 1
                        ztemp = DCONJG(A(i,j))
...
!
                  IF ( j<N ) THEN
                     ...
                     CALL ZGEMV('No Trans',N-j,j-k,-CONE,A(j+1,k),Lda,  &
     &                          A(j,k),Lda,CONE,A(j+1,j),1)
                     CALL ZLACGV(j-1,A(j,1),Lda)
...
!              Update trailing matrix, J already incremented
!
               IF ( k+jb<=N ) CALL ZHERK('Lower','No Trans',N-j+1,jb,   &
