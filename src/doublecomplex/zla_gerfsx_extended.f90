!*==zla_gerfsx_extended.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
     ...
     &                    (1.0D+0,0.0D+0),Res,1)
            ELSEIF ( y_prec_state==EXTRA_RESIDUAL ) THEN
               CALL BLAS_ZGEMV_X(Trans_type,N,N,(-1.0D+0,0.0D+0),A,Lda, &
     &                           Y(1,j),1,(1.0D+0,0.0D+0),Res,1,        &
     &                           Prec_type)
            ELSE
               CALL BLAS_ZGEMV2_X(Trans_type,N,N,(-1.0D+0,0.0D+0),A,Lda,&
     &                            Y(1,j),Y_tail,1,(1.0D+0,0.0D+0),Res,1,&
     &                            Prec_type)
            ENDIF
...
!
         CALL ZLA_GEAMV(Trans_type,N,N,1.0D+0,A,Lda,Y(1,j),1,1.0D+0,Ayb,&
     &                  1)
