!*==zsytrf_rk.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
      ...
      IF ( nb>1 .AND. nb<N ) THEN
         ...
         IF ( Lwork<iws ) THEN
            ...
            nbmin = MAX(2,ILAENV(2,'ZSYTRF_RK',Uplo,N,-1,-1,-1))
         ENDIF
      ELSE
         iws = 1
      ENDIF
...
!
               CALL ZLASYF_RK(Uplo,k,nb,kb,A,Lda,E,Ipiv,Work,ldwork,    &
     &                        iinfo)
            ELSE
!
...
!
            IF ( k<N ) THEN
               DO i = k , (k-kb+1) , -1
                  ip = ABS(Ipiv(i))
                  IF ( ip/=i ) CALL ZSWAP(N-k,A(i,k+1),Lda,A(ip,k+1),   &
     &                 Lda)
               ENDDO
            ENDIF
!
...
!
               CALL ZLASYF_RK(Uplo,N-k+1,nb,kb,A(k,k),Lda,E(k),Ipiv(k), &
     &                        Work,ldwork,iinfo)
...
!
               CALL ZSYTF2_RK(Uplo,N-k+1,A(k,k),Lda,E(k),Ipiv(k),iinfo)
               kb = N - k + 1
!
            ENDIF
!
...
!
            IF ( k>1 ) THEN
               DO i = k , (k+kb-1) , 1
                  ip = ABS(Ipiv(i))
                  IF ( ip/=i ) CALL ZSWAP(k-1,A(i,1),Lda,A(ip,1),Lda)
               ENDDO
            ENDIF
!
