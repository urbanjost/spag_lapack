!*==zbdsqr.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Local Scalars ..
     ...
     &        nm12 , nm13 , oldll , oldm
      DOUBLE PRECISION abse , abss , cosl , cosr , cs , eps , f , g ,   &
     &                 h , mu , oldcs , oldsn , r , shift , sigmn ,     &
...
!
         IF ( Nru>0 ) CALL ZLASR('R','V','F',Nru,N,Rwork(1),Rwork(N),U, &
     &                           Ldu)
         IF ( Ncc>0 ) CALL ZLASR('L','V','F',N,Ncc,Rwork(1),Rwork(N),C, &
     &                           Ldc)
      ENDIF
!
...
!
         IF ( Ncvt>0 ) CALL ZDROT(Ncvt,Vt(m-1,1),Ldvt,Vt(m,1),Ldvt,cosr,&
     &                            sinr)
         IF ( Nru>0 ) CALL ZDROT(Nru,U(1,m-1),1,U(1,m),1,cosl,sinl)
         IF ( Ncc>0 ) CALL ZDROT(Ncc,C(m-1,1),Ldc,C(m,1),Ldc,cosl,sinl)
         m = m - 2
         GOTO 100
      ENDIF
!
...
!
            IF ( Ncvt>0 ) CALL ZLASR('L','V','F',m-ll+1,Ncvt,Rwork(1),  &
     &                               Rwork(N),Vt(ll,1),Ldvt)
            IF ( Nru>0 ) CALL ZLASR('R','V','F',Nru,m-ll+1,Rwork(nm12+1)&
     &                              ,Rwork(nm13+1),U(1,ll),Ldu)
            IF ( Ncc>0 ) CALL ZLASR('L','V','F',m-ll+1,Ncc,Rwork(nm12+1)&
...
!           Update singular vectors
!
            IF ( Ncvt>0 ) CALL ZLASR('L','V','B',m-ll+1,Ncvt,           &
     &                               Rwork(nm12+1),Rwork(nm13+1),       &
     &                               Vt(ll,1),Ldvt)
            IF ( Nru>0 ) CALL ZLASR('R','V','B',Nru,m-ll+1,Rwork(1),    &
     &                              Rwork(N),U(1,ll),Ldu)
            IF ( Ncc>0 ) CALL ZLASR('L','V','B',m-ll+1,Ncc,Rwork(1),    &
     &                              Rwork(N),C(ll,1),Ldc)
!
!           Test convergence
!
...
!           Update singular vectors if desired
!
         IF ( Ncvt>0 ) CALL ZLASR('L','V','B',m-ll+1,Ncvt,Rwork(nm12+1),&
...
!
            D(isub) = D(N+1-i)
            D(N+1-i) = smin
            IF ( Ncvt>0 ) CALL ZSWAP(Ncvt,Vt(isub,1),Ldvt,Vt(N+1-i,1),  &
     &                               Ldvt)
            IF ( Nru>0 ) CALL ZSWAP(Nru,U(1,isub),1,U(1,N+1-i),1)
            IF ( Ncc>0 ) CALL ZSWAP(Ncc,C(isub,1),Ldc,C(N+1-i,1),Ldc)
         ENDIF
      ENDDO
!
!     End of ZBDSQR
!
99999 END SUBROUTINE ZBDSQR
