!*==zla_herfsx_extended.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Array Arguments ..
      INTEGER Ipiv(*)
      COMPLEX*16 A(Lda,*) , Af(Ldaf,*) , B(Ldb,*) , Y(Ldy,*) , Res(*) , &
     &           Dy(*) , Y_tail(*)
      DOUBLE PRECISION C(*) , Ayb(*) , Rcond , Berr_out(*) ,            &
...
!
!     .. Local Scalars ..
      INTEGER uplo2 , cnt , i , j , x_state , z_state , y_prec_state
...
!     .. External Subroutines ..
     ...
     &         BLAS_ZHEMV2_X , ZLA_HEAMV , ZLA_WWADDW , ZLA_LIN_BERR
...
!
     ...
     &                    DCMPLX(1.0D+0),Res,1)
            ELSEIF ( y_prec_state==EXTRA_RESIDUAL ) THEN
               CALL BLAS_ZHEMV_X(uplo2,N,DCMPLX(-1.0D+0),A,Lda,Y(1,j),1,&
     &                           DCMPLX(1.0D+0),Res,1,Prec_type)
            ELSE
               CALL BLAS_ZHEMV2_X(uplo2,N,DCMPLX(-1.0D+0),A,Lda,Y(1,j), &
...
!
         CALL ZCOPY(N,B(1,j),1,Res,1)
         CALL ZHEMV(Uplo,N,DCMPLX(-1.0D+0),A,Lda,Y(1,j),1,DCMPLX(1.0D+0)&
     &              ,Res,1)
