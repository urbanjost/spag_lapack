!*==ztgevc.f90  processed by SPAG 7.51RB at 20:09 on  3 Mar 2022
!> \brief \b ZTGEVC
!
!  =========== DOCUMENTATION ===========
!
...
!     ..
!     .. Local Scalars ..
      LOGICAL compl , compr , ilall , ilback , ilbbad , ilcomp , lsa ,  &
     &        lsb
      INTEGER i , ibeg , ieig , iend , ihwmny , im , iside , isrc , j , &
     &        je , jr
...
!
         DO je = 1 , N
            ...
            IF ( ilcomp ) THEN
               ieig = ieig + 1
!
               IF ( ABS1(S(je,je))<=safmin .AND. ABS(DBLE(P(je,je)))    &
     &              <=safmin ) THEN
!
...
!
               IF ( ilback ) THEN
                  CALL ZGEMV('N',N,N+1-je,CONE,Vl(1,je),Ldvl,Work(je),1,&
     &                       CZERO,Work(N+1),1)
                  isrc = 2
                  ibeg = 1
               ELSE
...
!
               IF ( ilback ) THEN
                  CALL ZGEMV('N',N,je,CONE,Vr,Ldvr,Work,1,CZERO,        &
     &                       Work(N+1),1)
                  isrc = 2
                  iend = N
               ELSE
