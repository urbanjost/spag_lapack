!*==zggglm.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b ZGGGLM
!
!  =========== DOCUMENTATION ===========
!
...
!     unitary.
!
      CALL ZGGQRF(N,M,P,A,Lda,Work,B,Ldb,Work(M+1),Work(M+np+1),        &
     &            Lwork-M-np,Info)
      lopt = Work(M+np+1)
!
...
!     Update d1 = d1 - T12*y2
!
      CALL ZGEMV('No transpose',M,N-M,-CONE,B(1,M+P-N+1),Ldb,Y(M+P-N+1),&
     &           1,CONE,D,1)
!
...
!
      CALL ZUNMRQ('Left','Conjugate transpose',P,1,np,B(MAX(1,N-P+1),1),&
