!*==zlarfb_gett.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b ZLARFB_GETT
!
!  =========== DOCUMENTATION ===========
!
...
!
!
         IF ( lnotident ) CALL ZTRMM('L','L','C','U',K,N-K,CONE,A,Lda,  &
     &                               Work,Ldwork)
!
!        col2_(3) Compute W2: = W2 + (V2**H) * B2 = W2 + (B1**H) * B2
!        V2 stored in B1.
!
         IF ( M>0 ) CALL ZGEMM('C','N',K,N-K,M,CONE,B,Ldb,B(1,K+1),Ldb, &
     &                         CONE,Work,Ldwork)
!
...
!        V2 stored in B1.
!
         IF ( M>0 ) CALL ZGEMM('N','N',M,N-K,K,-CONE,B,Ldb,Work,Ldwork, &
     &                         CONE,B(1,K+1),Ldb)
!
!
...
!           V1 stored in A1 (diagonal ones are not stored).
!
         IF ( lnotident ) CALL ZTRMM('L','L','N','U',K,N-K,CONE,A,Lda,  &
     &                               Work,Ldwork)
!
...
!
      IF ( lnotident ) CALL ZTRMM('L','L','C','U',K,K,CONE,A,Lda,Work,  &
     &                            Ldwork)
!
...
!
      IF ( M>0 ) CALL ZTRMM('R','U','N','N',M,K,-CONE,Work,Ldwork,B,Ldb)
!
      IF ( lnotident ) THEN
!
