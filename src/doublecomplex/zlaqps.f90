!*==zlaqps.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Parameters ..
      ...
      PARAMETER (ZERO=0.0D+0,ONE=1.0D+0,CZERO=(0.0D+0,0.0D+0),          &
     &           CONE=(1.0D+0,0.0D+0))
!     ..
!     .. Local Scalars ..
...
!
            IF ( k>1 ) THEN
               DO j = 1 , k - 1
                  F(k,j) = DCONJG(F(k,j))
               ENDDO
               CALL ZGEMV('No transpose',M-rk+1,k-1,-CONE,A(rk,1),Lda,  &
...
!
            IF ( k<N ) CALL ZGEMV('Conjugate transpose',M-rk+1,N-k,     &
...
!
               CALL ZGEMV('No transpose',N,k-1,CONE,F(1,1),Ldf,Auxv(1), &
     &                    1,CONE,F(1,k),1)
            ENDIF
!
...
!
            IF ( k<N ) CALL ZGEMM('No transpose','Conjugate transpose', &
