!*==zunbdb4.f90  processed by SPAG 7.51RB at 20:09 on  3 Mar 2022
...
!     .. Array Arguments ..
      DOUBLE PRECISION Phi(*) , Theta(*)
      COMPLEX*16 Phantom(*) , Taup1(*) , Taup2(*) , Tauq1(*) , Work(*) ,&
     &           X11(Ldx11,*) , X21(Ldx21,*)
!     ..
!
!  ====================================================================
...
!
         IF ( i==1 ) THEN
            DO j = 1 , M
               Phantom(j) = ZERO
            ENDDO
            CALL ZUNBDB5(P,M-P,Q,Phantom(1),1,Phantom(P+1),1,X11,Ldx11, &
     &                   X21,Ldx21,Work(iorbdb5),lorbdb5,childinfo)
            ...
            Phantom(1) = ONE
            Phantom(P+1) = ONE
            CALL ZLARF('L',P,Q,Phantom(1),1,DCONJG(Taup1(1)),X11,Ldx11, &
     &                 Work(ilarf))
            CALL ZLARF('L',M-P,Q,Phantom(P+1),1,DCONJG(Taup2(1)),X21,   &
     &                 Ldx21,Work(ilarf))
         ELSE
            CALL ZUNBDB5(P-i+1,M-P-i+1,Q-i+1,X11(i,i-1),1,X21(i,i-1),1, &
     ...
     &                   lorbdb5,childinfo)
            ...
            X11(i,i-1) = ONE
            X21(i,i-1) = ONE
            CALL ZLARF('L',P-i+1,Q-i+1,X11(i,i-1),1,DCONJG(Taup1(i)),   &
     &                 X11(i,i),Ldx11,Work(ilarf))
            CALL ZLARF('L',M-P-i+1,Q-i+1,X21(i,i-1),1,DCONJG(Taup2(i)), &
...
!
         ...
         c = DBLE(X21(i,i))
         X21(i,i) = ONE
         CALL ZLARF('R',P-i,Q-i+1,X21(i,i),Ldx21,Tauq1(i),X11(i+1,i),   &
     &              Ldx11,Work(ilarf))
         CALL ZLARF('R',M-P-i,Q-i+1,X21(i,i),Ldx21,Tauq1(i),X21(i+1,i), &
     &              Ldx21,Work(ilarf))
         CALL ZLACGV(Q-i+1,X21(i,i),Ldx21)
         IF ( i<M-Q ) THEN
            s = SQRT(DZNRM2(P-i,X11(i+1,i),1)                           &
     &          **2+DZNRM2(M-P-i,X21(i+1,i),1)**2)
            Phi(i) = ATAN2(s,c)
         ENDIF
!
      ENDDO
...
!
     ...
     &              Ldx11,Work(ilarf))
         CALL ZLARF('R',Q-P,Q-i+1,X11(i,i),Ldx11,Tauq1(i),X21(M-Q+1,i), &
     &              Ldx21,Work(ilarf))
         CALL ZLACGV(Q-i+1,X11(i,i),Ldx11)
      ENDDO
!
