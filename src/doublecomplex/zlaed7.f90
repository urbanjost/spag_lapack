!*==zlaed7.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
!     .. Local Scalars ..
      INTEGER coltyp , curr , i , idlmda , indx , indxc , indxp , iq ,  &
     &        iw , iz , k , n1 , n2 , ptr
!     ..
...
!
      CALL ZLAED8(k,N,Qsiz,Q,Ldq,D,Rho,Cutpnt,Rwork(iz),Rwork(idlmda),  &
...
!
      IF ( k/=0 ) THEN
         CALL DLAED9(k,1,k,N,D,Rwork(iq),k,Rho,Rwork(idlmda),Rwork(iw), &
     &               Qstore(Qptr(curr)),k,Info)
         CALL ZLACRM(Qsiz,k,Work,Qsiz,Qstore(Qptr(curr)),k,Q,Ldq,       &
     &               Rwork(iq))
