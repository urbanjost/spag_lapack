!*==zcgesv.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
      CALL CGETRS('No transpose',N,Nrhs,Swork(ptsa),N,Ipiv,Swork(ptsx), &
     &            N,Info)
!
...
!
         CALL ZLACPY('All',N,Nrhs,B,Ldb,Work,N)
!
         CALL ZGEMM('No Transpose','No Transpose',N,Nrhs,N,NEGONE,A,Lda,&
     &              X,Ldx,ONE,Work,N)
!
!        Check whether the NRHS normwise backward errors satisfy the
...
!
 300  CALL ZGETRF(N,N,A,Lda,Ipiv,Info)
!
      IF ( Info/=0 ) RETURN
!
