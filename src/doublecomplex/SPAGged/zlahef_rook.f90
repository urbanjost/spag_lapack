!*==zlahef_rook.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      REAL(R8KIND) :: absakk , alpha , colmax , dtemp , r1 , rowmax ,   &
     &                sfmin , t
      REAL(R8KIND) :: CABS1
...
!
                     IF ( k<N ) CALL ZSWAP(N-k,A(k,k+1),Lda,A(p,k+1),   &
     &                    Lda)
...
!                 (used to handle NaN and Inf)
!
                     IF ( ABS(DBLE(W(imax,k+1)))>=alpha*rowmax ) THEN
!
!                    interchange rows and columns K and IMAX,
!                    use 1-by-1 pivot block
!
...
!
                     DO j = k + 2 , N
                        A(j,k) = t*((d11*W(j,k)-W(j,k+1))/DCONJG(d21))
...
!     End of ZLAHEF_ROOK
!
99999 END SUBROUTINE ZLAHEF_ROOK
