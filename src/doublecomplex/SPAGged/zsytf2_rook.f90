!*==zsytf2_rook.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      ...
      REAL(R8KIND) :: CABS1
      COMPLEX(CX16KIND) :: d11 , d12 , d21 , d22 , t , wk , wkm1 ,      &
     &                     wkp1 , z
      LOGICAL :: done , upper
...
!
                  ...
                  IF ( p<(k-1) ) CALL ZSWAP(k-p-1,A(p+1,k),1,A(p,p+1),  &
     &                 Lda)
                  t = A(k,k)
                  A(k,k) = A(p,p)
                  A(p,p) = t
...
!
                  ...
                  IF ( p>(k+1) ) CALL ZSWAP(p-k-1,A(k+1,k),1,A(p,k+1),  &
     &                 Lda)
                  t = A(k,k)
                  A(k,k) = A(p,p)
                  A(p,p) = t
...
!
                        d11 = CONE/A(k,k)
                        CALL ZSYR(Uplo,N-k,-d11,A(k+1,k),1,A(k+1,k+1),  &
     &                            Lda)
!
!                    Store L(k) in column k
!
