!*==zhetrf_rook.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
               CALL ZLAHEF_ROOK(Uplo,k,nb,kb,A,Lda,Ipiv,Work,ldwork,    &
     &                          iinfo)
            ELSE
!
...
!
               CALL ZLAHEF_ROOK(Uplo,N-k+1,nb,kb,A(k,k),Lda,Ipiv(k),    &
     &                          Work,ldwork,iinfo)
            ELSE
!
!           Use unblocked code to factorize columns k:n of A
!
               CALL ZHETF2_ROOK(Uplo,N-k+1,A(k,k),Lda,Ipiv(k),iinfo)
               kb = N - k + 1
            ENDIF
!
!        Set INFO on the first occurrence of a zero pivot
