!*==zhptrf.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b ZHPTRF
!
!  =========== DOCUMENTATION ===========
!
...
!
      REAL(R8KIND) :: absakk , alpha , colmax , d , d11 , d22 , r1 ,    &
     &                rowmax , tt
      REAL(R8KIND) :: CABS1
      COMPLEX(CX16KIND) :: d12 , d21 , t , wk , wkm1 , wkp1 , zdum
...
!
                     kp = k
                  ELSEIF ( ABS(DBLE(Ap(kpc+imax-1)))>=alpha*rowmax )    &
     &                     THEN
!
...
!
                  DO j = k - 2 , 1 , -1
                     wkm1 = d*(d11*Ap(j+(k-2)*(k-1)/2)-DCONJG(d12)      &
     &                      *Ap(j+(k-1)*k/2))
     ...
     &                     - Ap(i+(k-2)*(k-1)/2)*DCONJG(wkm1)
                     ...
                     Ap(j+(k-1)*k/2) = wk
                     Ap(j+(k-2)*(k-1)/2) = wkm1
                     Ap(j+(j-1)*j/2) = DCMPLX(DBLE(Ap(j+(j-1)*j/2)),    &
     &                                 0.0D+0)
                  ENDDO
!
!
...
!
                     kp = k
                  ELSEIF ( ABS(DBLE(Ap(kpc)))>=alpha*rowmax ) THEN
!
!                 interchange rows and columns K and IMAX, use 1-by-1
!                 pivot block
!
...
!
                     r1 = ONE/DBLE(Ap(kc))
                     CALL ZHPR(Uplo,N-k,-r1,Ap(kc+1),1,Ap(kc+N-k+1))
!
...
!
     ...
     &                    -d21*Ap(j+k*(2*N-k-1)/2))
                     wkp1 = d*(d22*Ap(j+k*(2*N-k-1)/2)-DCONJG(d21)      &
     &                      *Ap(j+(k-1)*(2*N-k)/2))
                     DO i = j , N
                        Ap(i+(j-1)*(2*N-j)/2) = Ap(i+(j-1)*(2*N-j)/2)   &
     &                     - Ap(i+(k-1)*(2*N-k)/2)*DCONJG(wk)           &
     &                     - Ap(i+k*(2*N-k-1)/2)*DCONJG(wkp1)
                     ENDDO
                     Ap(j+(k-1)*(2*N-k)/2) = wk
                     Ap(j+k*(2*N-k-1)/2) = wkp1
                     Ap(j+(j-1)*(2*N-j)/2)                              &
     &                  = DCMPLX(DBLE(Ap(j+(j-1)*(2*N-j)/2)),0.0D+0)
                  ENDDO
               ENDIF
            ENDIF
!
