!*==zggesx.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      IF ( Info==0 ) THEN
         IF ( N>0 ) THEN
            ...
            IF ( ilvsl ) maxwrk = MAX(maxwrk,N*(1+ILAENV(1,'ZUNGQR',' ',&
     &                            N,1,N,-1)))
            lwrk = maxwrk
...
!                            otherwise, need 1 )
!
         CALL ZTGSEN(ijob,ilvsl,ilvsr,Bwork,N,A,Lda,B,Ldb,Alpha,Beta,   &
