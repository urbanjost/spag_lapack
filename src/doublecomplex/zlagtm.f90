!*==zlagtm.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
            DO j = 1 , Nrhs
               ...
               ELSE
                  B(1,j) = B(1,j) + DCONJG(D(1))*X(1,j) + DCONJG(Dl(1)) &
     &                     *X(2,j)
                  B(N,j) = B(N,j) + DCONJG(Du(N-1))*X(N-1,j)            &
     &                     + DCONJG(D(N))*X(N,j)
                  DO i = 2 , N - 1
                     B(i,j) = B(i,j) + DCONJG(Du(i-1))*X(i-1,j)         &
     &                        + DCONJG(D(i))*X(i,j) + DCONJG(Dl(i))     &
     &                        *X(i+1,j)
                  ENDDO
               ENDIF
            ENDDO
