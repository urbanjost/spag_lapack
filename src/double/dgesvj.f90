!*==dgesvj.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b DGESVJ
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Local Scalars ..
     ...
     &                 big , bigtheta , cs , ctol , epsln , large ,     &
     &                 mxaapq , mxsinj , rootbig , rooteps , rootsfmin ,&
...
!
      IF ( N==1 ) THEN
         IF ( lsvec ) CALL DLASCL('G',0,0,Sva(1),skl,M,1,A(1,1),Lda,    &
     &                            ierr)
         Work(1) = ONE/skl
         IF ( Sva(1)>=sfmin ) THEN
            Work(2) = ONE
...
!
     ...
     &                  tol,1,Work(N+1),Lwork-N,ierr)
!
            CALL DGSVJ0(Jobv,M,n4,A,Lda,Work,Sva,mvl,V,Ldv,epsln,sfmin, &
     &                  tol,1,Work(N+1),Lwork-N,ierr)
!
            CALL DGSVJ1(Jobv,M,n2,n4,A,Lda,Work,Sva,mvl,V,Ldv,epsln,    &
...
!
!
            CALL DGSVJ0(Jobv,n4,n4,A,Lda,Work,Sva,mvl,V,Ldv,epsln,sfmin,&
     &                  tol,2,Work(N+1),Lwork-N,ierr)
!
            CALL DGSVJ0(Jobv,n2,n4,A(1,n4+1),Lda,Work(n4+1),Sva(n4+1),  &
     &                  mvl,V(n4*q+1,n4+1),Ldv,epsln,sfmin,tol,1,       &
     &                  Work(N+1),Lwork-N,ierr)
!
            CALL DGSVJ1(Jobv,n2,n2,n4,A,Lda,Work,Sva,mvl,V,Ldv,epsln,   &
     &                  sfmin,tol,1,Work(N+1),Lwork-N,ierr)
!
            CALL DGSVJ0(Jobv,n2+n4,n4,A(1,n2+1),Lda,Work(n2+1),Sva(n2+1)&
...
!
     ...
     &                                  *Work(p)*Work(q)/aaqq)/aapp
                              ELSE
                                 ...
                                 CALL DLASCL('G',0,0,aapp,Work(p),M,1,  &
     &                              Work(N+1),Lda,ierr)
     ...
     &                              Work(N+1),Lda,ierr)
                                 aapq = DDOT(M,Work(N+1),1,A(1,p),1)    &
     &                                  *Work(p)/aapp
                              ENDIF
                           ENDIF
!
...
!              .. have to use modified Gram-Schmidt like transformation
                                 CALL DCOPY(M,A(1,p),1,Work(N+1),1)
                                 CALL DLASCL('G',0,0,aapp,ONE,M,1,      &
     &                              Work(N+1),Lda,ierr)
                                 CALL DLASCL('G',0,0,aaqq,ONE,M,1,A(1,q)&
     &                              ,Lda,ierr)
                                 temp1 = -aapq*Work(p)/Work(q)
                                 CALL DAXPY(M,temp1,Work(N+1),1,A(1,q), &
     &                              1)
...
!
     ...
     &                              )
                              ...
                              ELSE
                                 CALL DCOPY(M,A(1,q),1,Work(N+1),1)
                                 CALL DLASCL('G',0,0,aaqq,ONE,M,1,      &
     &                              Work(N+1),Lda,ierr)
                                 CALL DLASCL('G',0,0,aapp,ONE,M,1,A(1,p)&
     &                              ,Lda,ierr)
                                 temp1 = -aapq*Work(q)/Work(p)
                                 CALL DAXPY(M,temp1,Work(N+1),1,A(1,p), &
     &                              1)
...
!
     ...
     &        swband = i
!
         IF ( (i>swband+1) .AND. (mxaapq<DSQRT(DBLE(N))*tol) .AND.      &
