!*==dggev3.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Local Scalars ..
     ...
     &        iright , irows , itau , iwrk , jc , jr , lwkopt
...
!
      IF ( Info==0 ) THEN
         ...
         ENDIF
         IF ( ilv ) THEN
            CALL DGGHD3(Jobvl,Jobvr,N,1,N,A,Lda,B,Ldb,Vl,Ldvl,Vr,Ldvr,  &
     &                  Work,-1,ierr)
            lwkopt = MAX(lwkopt,3*N+INT(Work(1)))
            CALL DHGEQZ('S',Jobvl,Jobvr,N,1,N,A,Lda,B,Ldb,Alphar,Alphai,&
     &                  Beta,Vl,Ldvl,Vr,Ldvr,Work,-1,ierr)
            lwkopt = MAX(lwkopt,2*N+INT(Work(1)))
         ELSE
            CALL DGGHD3('N','N',N,1,N,A,Lda,B,Ldb,Vl,Ldvl,Vr,Ldvr,Work, &
     &                  -1,ierr)
            lwkopt = MAX(lwkopt,3*N+INT(Work(1)))
            CALL DHGEQZ('E',Jobvl,Jobvr,N,1,N,A,Lda,B,Ldb,Alphar,Alphai,&
     &                  Beta,Vl,Ldvl,Vr,Ldvr,Work,-1,ierr)
            lwkopt = MAX(lwkopt,2*N+INT(Work(1)))
         ENDIF
...
!
         CALL DGGHD3(Jobvl,Jobvr,N,ilo,ihi,A,Lda,B,Ldb,Vl,Ldvl,Vr,Ldvr, &
     &               Work(iwrk),Lwork+1-iwrk,ierr)
      ELSE
         CALL DGGHD3('N','N',irows,1,irows,A(ilo,ilo),Lda,B(ilo,ilo),   &
