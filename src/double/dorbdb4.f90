!*==dorbdb4.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
         IF ( i==1 ) THEN
            DO j = 1 , M
               Phantom(j) = ZERO
            ENDDO
            CALL DORBDB5(P,M-P,Q,Phantom(1),1,Phantom(P+1),1,X11,Ldx11, &
     ...
     &                 Work(ilarf))
            CALL DLARF('L',M-P,Q,Phantom(P+1),1,Taup2(1),X21,Ldx21,     &
     &                 Work(ilarf))
         ELSE
            CALL DORBDB5(P-i+1,M-P-i+1,Q-i+1,X11(i,i-1),1,X21(i,i-1),1, &
     ...
     &                   lorbdb5,childinfo)
            ...
            X11(i,i-1) = ONE
            X21(i,i-1) = ONE
            CALL DLARF('L',P-i+1,Q-i+1,X11(i,i-1),1,Taup1(i),X11(i,i),  &
     &                 Ldx11,Work(ilarf))
            CALL DLARF('L',M-P-i+1,Q-i+1,X21(i,i-1),1,Taup2(i),X21(i,i),&
     &                 Ldx21,Work(ilarf))
         ENDIF
!
         ...
         c = X21(i,i)
         X21(i,i) = ONE
         CALL DLARF('R',P-i,Q-i+1,X21(i,i),Ldx21,Tauq1(i),X11(i+1,i),   &
     &              Ldx11,Work(ilarf))
         CALL DLARF('R',M-P-i,Q-i+1,X21(i,i),Ldx21,Tauq1(i),X21(i+1,i), &
     &              Ldx21,Work(ilarf))
         IF ( i<M-Q ) THEN
            s = SQRT(DNRM2(P-i,X11(i+1,i),1)                            &
     &          **2+DNRM2(M-P-i,X21(i+1,i),1)**2)
            Phi(i) = ATAN2(s,c)
         ENDIF
!
      ENDDO
...
!
     ...
     &              Ldx11,Work(ilarf))
         CALL DLARF('R',Q-P,Q-i+1,X11(i,i),Ldx11,Tauq1(i),X21(M-Q+1,i), &
     &              Ldx21,Work(ilarf))
      ENDDO
!
