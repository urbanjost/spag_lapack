!*==dlatbs.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Local Scalars ..
      ...
      DOUBLE PRECISION bignum , grow , rec , smlnum , sumj , tjj ,      &
     &                 tjjs , tmax , tscal , uscal , xbnd , xj , xmax
!     ..
!     .. External Functions ..
      LOGICAL LSAME
      INTEGER IDAMAX
...
!
                     jlen = MIN(Kd,j-1)
                     CALL DAXPY(jlen,-X(j)*tscal,Ab(Kd+1-jlen,j),1,     &
     &                          X(j-jlen),1)
                     i = IDAMAX(j-1,X,1)
                     xmax = ABS(X(i))
                  ENDIF
...
!
                  jlen = MIN(Kd,N-j)
                  IF ( jlen>0 ) CALL DAXPY(jlen,-X(j)*tscal,Ab(2,j),1,  &
     &                 X(j+1),1)
