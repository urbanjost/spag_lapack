!*==dsytrd_sy2sb.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL XERBLA , DSYR2K , DSYMM , DGEMM , DCOPY , DLARFT ,       &
     &         DGELQF , DGEQRF , DLASET
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MIN , MAX
!     ..
...
!
            CALL DGELQF(Kd,pn,A(i,i+Kd),Lda,Tau(i),Work(s2pos),ls2,     &
     &                  iinfo)
!
...
!            Form the matrix T
!
            CALL DLARFT('Forward','Rowwise',pn,pk,A(i,i+Kd),Lda,Tau(i), &
     &                  Work(tpos),ldt)
!
!            Compute W:
!
            CALL DGEMM('Conjugate','No transpose',pk,pn,pk,ONE,         &
     &                 Work(tpos),ldt,A(i,i+Kd),Lda,ZERO,Work(s2pos),   &
     &                 lds2)
!
            CALL DSYMM('Right',Uplo,pk,pn,ONE,A(i+Kd,i+Kd),Lda,         &
     &                 Work(s2pos),lds2,ZERO,Work(wpos),ldw)
!
            CALL DGEMM('No transpose','Conjugate',pk,pk,pn,ONE,         &
     &                 Work(wpos),ldw,Work(s2pos),lds2,ZERO,Work(s1pos),&
     &                 lds1)
!
            CALL DGEMM('No transpose','No transpose',pk,pn,pk,-HALF,    &
...
!
            CALL DSYR2K(Uplo,'Conjugate',pn,pk,-ONE,A(i,i+Kd),Lda,      &
...
!
            CALL DGEQRF(pn,Kd,A(i+Kd,i),Lda,Tau(i),Work(s2pos),ls2,     &
     &                  iinfo)
!
!            Copy the upper portion of A into AB
!
...
!            Form the matrix T
!
            CALL DLARFT('Forward','Columnwise',pn,pk,A(i+Kd,i),Lda,     &
     &                  Tau(i),Work(tpos),ldt)
!
!            Compute W:
!
            CALL DGEMM('No transpose','No transpose',pn,pk,pk,ONE,      &
     &                 A(i+Kd,i),Lda,Work(tpos),ldt,ZERO,Work(s2pos),   &
     &                 lds2)
!
            CALL DSYMM('Left',Uplo,pn,pk,ONE,A(i+Kd,i+Kd),Lda,          &
     &                 Work(s2pos),lds2,ZERO,Work(wpos),ldw)
!
            CALL DGEMM('Conjugate','No transpose',pk,pk,pn,ONE,         &
     &                 Work(s2pos),lds2,Work(wpos),ldw,ZERO,Work(s1pos),&
     &                 lds1)
!
            CALL DGEMM('No transpose','No transpose',pn,pk,pk,-HALF,    &
...
!            an update of the form:  A := A - V*W' - W*V'
!
            CALL DSYR2K(Uplo,'No transpose',pn,pk,-ONE,A(i+Kd,i),Lda,   &
     &                  Work(wpos),ldw,RONE,A(i+Kd,i+Kd),Lda)
!            ==================================================================
