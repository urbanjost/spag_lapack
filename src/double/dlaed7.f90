!*==dlaed7.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     ..
!     .. Array Arguments ..
      INTEGER Givcol(2,*) , Givptr(*) , Indxq(*) , Iwork(*) , Perm(*) , &
     &        Prmptr(*) , Qptr(*)
...
!     ..
!     .. Local Scalars ..
      INTEGER coltyp , curr , i , idlmda , indx , indxc , indxp , iq2 , &
     &        is , iw , iz , k , ldq2 , n1 , n2 , ptr
!     ..
...
!
      ...
      ELSEIF ( N<0 ) THEN
         Info = -2
      ELSEIF ( Icompq==1 .AND. Qsiz<N ) THEN
         Info = -3
...
!
     ...
     &            Givnum,Qstore,Qptr,Work(iz),Work(iz+N),Info)
!
...
!
      CALL DLAED8(Icompq,k,N,Qsiz,D,Q,Ldq,Indxq,Rho,Cutpnt,Work(iz),    &
     &            Work(idlmda),Work(iq2),ldq2,Work(iw),                 &
     &            Perm(Prmptr(curr)),Givptr(curr+1),                    &
     &            Givcol(1,Givptr(curr)),Givnum(1,Givptr(curr)),        &
     &            Iwork(indxp),Iwork(indx),Info)
...
!
      IF ( k/=0 ) THEN
         CALL DLAED9(k,1,k,N,D,Work(is),k,Rho,Work(idlmda),Work(iw),    &
     &               Qstore(Qptr(curr)),k,Info)
         IF ( Info==0 ) THEN
            IF ( Icompq==1 ) CALL DGEMM('N','N',Qsiz,k,k,ONE,Work(iq2), &
     &                                  ldq2,Qstore(Qptr(curr)),k,ZERO, &
     &                                  Q,Ldq)
