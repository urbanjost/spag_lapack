!*==dgetsls.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b DGETSLS
!
!  Definition:
!  ===========
...
!
     ...
     &           tszo , wsizem , wsizeo
      LOGICAL :: lquery , tran
      REAL(R8KIND) , DIMENSION(5) :: tq
      REAL(R8KIND) , DIMENSION(1) :: workq
!
...
!
         IF ( M>=N ) THEN
            ...
            tszo = INT(tq(1))
            lwo = INT(workq(1))
            CALL DGEMQR('L',Trans,M,Nrhs,N,A,Lda,tq,tszo,B,Ldb,workq,-1,&
     &                  info2)
            lwo = MAX(lwo,INT(workq(1)))
            CALL DGEQR(M,N,A,Lda,tq,-2,workq,-2,info2)
            tszm = INT(tq(1))
            lwm = INT(workq(1))
            CALL DGEMQR('L',Trans,M,Nrhs,N,A,Lda,tq,tszm,B,Ldb,workq,-1,&
     &                  info2)
         ...
         ELSE
            ...
            tszo = INT(tq(1))
            lwo = INT(workq(1))
            CALL DGEMLQ('L',Trans,N,Nrhs,M,A,Lda,tq,tszo,B,Ldb,workq,-1,&
     &                  info2)
            lwo = MAX(lwo,INT(workq(1)))
            CALL DGELQ(M,N,A,Lda,tq,-2,workq,-2,info2)
            tszm = INT(tq(1))
            lwm = INT(workq(1))
            CALL DGEMLQ('L',Trans,N,Nrhs,M,A,Lda,tq,tszm,B,Ldb,workq,-1,&
     &                  info2)
            lwm = MAX(lwm,INT(workq(1)))
            wsizeo = tszo + lwo
            wsizem = tszm + lwm
...
!
            CALL DGEMQR('L','T',M,Nrhs,N,A,Lda,Work(lw2+1),lw1,B,Ldb,   &
     &                  Work(1),lw2,Info)
!
...
!
            CALL DTRTRS('U','T','N',N,Nrhs,A,Lda,B,Ldb,Info)
!
            IF ( Info>0 ) RETURN
!
!           B(N+1:M,1:NRHS) = ZERO
...
!
            CALL DGEMQR('L','N',M,Nrhs,N,A,Lda,Work(lw2+1),lw1,B,Ldb,   &
     &                  Work(1),lw2,Info)
!
            scllen = M
!
...
!
            CALL DTRTRS('L','N','N',M,Nrhs,A,Lda,B,Ldb,Info)
!
            IF ( Info>0 ) RETURN
!
!           B(M+1:N,1:NRHS) = 0
...
!
            CALL DGEMLQ('L','T',N,Nrhs,M,A,Lda,Work(lw2+1),lw1,B,Ldb,   &
     &                  Work(1),lw2,Info)
!
...
!
            CALL DGEMLQ('L','N',N,Nrhs,M,A,Lda,Work(lw2+1),lw1,B,Ldb,   &
     &                  Work(1),lw2,Info)
!
!           workspace at least NRHS, optimally NRHS*NB
!
