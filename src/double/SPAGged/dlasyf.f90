!*==dlasyf.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      REAL(R8KIND) :: absakk , alpha , colmax , d11 , d21 , d22 , r1 ,  &
     &                rowmax , t
...
!
                     A(kp,kp) = A(kk,kk)
                     CALL DCOPY(kk-1-kp,A(kp+1,kk),1,A(kp,kp+1),Lda)
                     IF ( kp>1 ) CALL DCOPY(kp-1,A(1,kk),1,A(1,kp),1)
!
...
!
                     IF ( k<N ) CALL DSWAP(N-k,A(kk,k+1),Lda,A(kp,k+1), &
     &                    Lda)
...
!
            ...
            CALL DGEMV('No transpose',N-k+1,k-1,-ONE,A(k,1),Lda,W(k,1), &
     &                 Ldw,ONE,W(k,k),1)
!
            kstep = 1
!
...
!
                  A(kp,kp) = A(kk,kk)
                  CALL DCOPY(kp-kk-1,A(kk+1,kk),1,A(kp,kk+1),Lda)
                  IF ( kp<N ) CALL DCOPY(N-kp,A(kp+1,kk),1,A(kp+1,kp),1)
!
