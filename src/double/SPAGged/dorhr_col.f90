!*==dorhr_col.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b DORHR_COL
!
!  =========== DOCUMENTATION ===========
!
...
!     (1-2) Solve for V2.
!
      IF ( M>N ) CALL DTRSM('R','U','N','N',M-N,N,ONE,A,Lda,A(N+1,1),   &
     &                      Lda)
!
...
!
         DO j = jb , jb + jnb - 1
            IF ( D(j)==ONE ) CALL DSCAL(j-jbtemp1,-ONE,T(1,j),1)
         ENDDO
!
...
!
         CALL DTRSM('R','L','T','U',jnb,jnb,ONE,A(jb,jb),Lda,T(1,jb),   &
     &              Ldt)
!
      ENDDO
!
