!*==dtrevc.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b DTREVC
!
!  =========== DOCUMENTATION ===========
!
...
!
                           CALL DLALN2(.FALSE.,1,1,smin,ONE,T(j,j),Ldt, &
...
!
                           CALL DLALN2(.FALSE.,2,1,smin,ONE,T(j-1,j-1), &
...
!
                  ...
                  ELSE
                     IF ( ki>1 )                                        &
     &                    CALL DGEMV('N',N,ki-1,ONE,Vr,Ldvr,Work(1+N),1,&
     &                    Work(ki+N),Vr(1,ki),1)
!
...
!                    1-by-1 diagonal block
!
                           CALL DLALN2(.FALSE.,1,2,smin,ONE,T(j,j),Ldt, &
...
!                    2-by-2 diagonal block
!
                           CALL DLALN2(.FALSE.,2,2,smin,ONE,T(j-1,j-1), &
...
!
     ...
     &                        - DDOT(j-ki-1,T(ki+1,j+1),1,Work(ki+1+N), &
     &                        1)
!
!                    Solve
...
!
                           CALL DLALN2(.TRUE.,2,1,smin,ONE,T(j,j),Ldt,  &
     &                                 ONE,ONE,Work(j+N),N,wr,ZERO,x,2, &
     &                                 scale,xnorm,ierr)
!
!                    Scale if necessary
...
!
                  ELSE
!
                     IF ( ki<N ) CALL DGEMV('N',N,N-ki,ONE,Vl(1,ki+1),  &
...
!
                           Work(j+1+N) = Work(j+1+N)                    &
     &                        - DDOT(j-ki-2,T(ki+2,j+1),1,Work(ki+2+N), &
     &                        1)
!
                           Work(j+1+n2) = Work(j+1+n2)                  &
     &                        - DDOT(j-ki-2,T(ki+2,j+1),1,Work(ki+2+n2),&
     &                        1)
!
...
!                      ([T(j+1,j) T(j+1,j+1)]               )
!
                           CALL DLALN2(.TRUE.,2,2,smin,ONE,T(j,j),Ldt,  &
     &                                 ONE,ONE,Work(j+N),N,wr,-wi,x,2,  &
     &                                 scale,xnorm,ierr)
!
!                    Scale if necessary
