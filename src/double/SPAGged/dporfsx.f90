!*==dporfsx.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b DPORFSX
!
!  =========== DOCUMENTATION ===========
!
...
!
      IF ( ref_type/=0 ) THEN
         ...
         CALL DLA_PORFSX_EXTENDED(prec_type,Uplo,N,Nrhs,A,Lda,Af,Ldaf,  &
...
!
         IF ( rcequ ) THEN
            rcond_tmp = DLA_PORCOND(Uplo,N,A,Lda,Af,Ldaf,-1,S,Info,Work,&
     &                  Iwork)
         ELSE
            rcond_tmp = DLA_PORCOND(Uplo,N,A,Lda,Af,Ldaf,0,S,Info,Work, &
     &                  Iwork)
         ENDIF
         DO j = 1 , Nrhs
!
...
!
         ...
         DO j = 1 , Nrhs
            IF ( Err_bnds_comp(j,LA_LINRX_ERR_I)<cwise_wrong ) THEN
               rcond_tmp = DLA_PORCOND(Uplo,N,A,Lda,Af,Ldaf,1,X(1,j),   &
     &                     Info,Work,Iwork)
            ELSE
               rcond_tmp = 0.0D+0
            ENDIF
