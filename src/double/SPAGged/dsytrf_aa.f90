!*==dsytrf_aa.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b DSYTRF_AA
!
!  =========== DOCUMENTATION ===========
!
...
!        Panel factorization
!
            CALL DLASYF_AA(Uplo,2-k1,N-j,jb,A(MAX(1,j),j+1),Lda,        &
...
!
                  alpha = A(j,j+1)
                  A(j,j+1) = ONE
                  CALL DCOPY(N-j,A(j-1,j+1),Lda,Work((j+1-j1+1)+jb*N),1)
...
!
                     j3 = j2
                     DO mj = nj - 1 , 1 , -1
                        CALL DGEMV('No transpose',mj,jb+1,-ONE,         &
...
!
                     CALL DGEMM('Transpose','Transpose',nj,N-j3+1,jb+1, &
...
!        Panel factorization
!
            CALL DLASYF_AA(Uplo,2-k1,N-j,jb,A(j+1,MAX(1,j)),Lda,        &
     &                     Ipiv(j+1),Work,N,Work(N*nb+1))
!
!        Adjust IPIV and apply it back (J-th step picks (J+1)-th pivot)
!
...
!
                  alpha = A(j+1,j)
                  A(j+1,j) = ONE
                  CALL DCOPY(N-j,A(j+1,j-1),1,Work((j+1-j1+1)+jb*N),1)
                  CALL DSCAL(N-j,alpha,Work((j+1-j1+1)+jb*N),1)
!
!              K1 identifies if the previous column of the panel has been
!               explicitly stored, e.g., K1=1 and K2= 0 for the first panel,
...
!
                     CALL DGEMM('No transpose','Transpose',N-j3+1,nj,   &
