!*==dgebd2.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
            IF ( i<N ) CALL DLARF('Left',M-i+1,N-i,A(i,i),1,Tauq(i),    &
     &                            A(i,i+1),Lda,Work)
            A(i,i) = D(i)
!
            IF ( i<N ) THEN
...
!
               CALL DLARF('Left',M-i,N-i,A(i+1,i),1,Tauq(i),A(i+1,i+1), &
     &                    Lda,Work)
               A(i+1,i) = E(i)
            ELSE
               Tauq(i) = ZERO
