!*==dtpcon.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b DTPCON
!
!  =========== DOCUMENTATION ===========
!
...
!              Multiply by inv(A).
!
                  CALL DLATPS(Uplo,'No transpose',Diag,normin,N,Ap,Work,&
     &                        scale,Work(2*N+1),Info)
               ELSE
!
!              Multiply by inv(A**T).
!
                  CALL DLATPS(Uplo,'Transpose',Diag,normin,N,Ap,Work,   &
     &                        scale,Work(2*N+1),Info)
               ENDIF
               normin = 'Y'
!
