!*==dggglm.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b DGGGLM
!
!  =========== DOCUMENTATION ===========
!
...
!     ..
!     .. Array Arguments ..
      DOUBLE PRECISION A(Lda,*) , B(Ldb,*) , D(*) , Work(*) , X(*) ,    &
     &                 Y(*)
!     ..
!
!  ===================================================================
...
!     .. Local Scalars ..
      LOGICAL lquery
      INTEGER i , lopt , lwkmin , lwkopt , nb , nb1 , nb2 , nb3 , nb4 , &
     &        np
!     ..
...
!     orthogonal.
!
      CALL DGGQRF(N,M,P,A,Lda,Work,B,Ldb,Work(M+1),Work(M+np+1),        &
     &            Lwork-M-np,Info)
      lopt = Work(M+np+1)
!
...
!                                               ( d2 ) N-M
!
      CALL DORMQR('Left','Transpose',N,1,M,A,Lda,Work,D,MAX(1,N),       &
...
!     Update d1 = d1 - T12*y2
!
      CALL DGEMV('No transpose',M,N-M,-ONE,B(1,M+P-N+1),Ldb,Y(M+P-N+1), &
     &           1,ONE,D,1)
!
...
!
      CALL DORMRQ('Left','Transpose',P,1,np,B(MAX(1,N-P+1),1),Ldb,      &
