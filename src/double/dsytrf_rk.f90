!*==dsytrf_rk.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
               CALL DLASYF_RK(Uplo,k,nb,kb,A,Lda,E,Ipiv,Work,ldwork,    &
     &                        iinfo)
            ELSE
!
...
!
            IF ( k<N ) THEN
               DO i = k , (k-kb+1) , -1
                  ip = ABS(Ipiv(i))
                  IF ( ip/=i ) CALL DSWAP(N-k,A(i,k+1),Lda,A(ip,k+1),   &
     &                 Lda)
               ENDDO
            ENDIF
!
...
!
               CALL DLASYF_RK(Uplo,N-k+1,nb,kb,A(k,k),Lda,E(k),Ipiv(k), &
     &                        Work,ldwork,iinfo)
...
!
            IF ( k>1 ) THEN
               DO i = k , (k+kb-1) , 1
                  ip = ABS(Ipiv(i))
                  IF ( ip/=i ) CALL DSWAP(k-1,A(i,1),Lda,A(ip,1),Lda)
               ENDDO
            ENDIF
!
