!*==zdrgev3.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b ZDRGEV3
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZDRGEV3(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A, &
...
!
               ...
               CALL ZGGEV3('V','V',n,S,Lda,T,Lda,Alpha,Beta,Q,Ldq,Z,Ldq,&
     &                     Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'ZGGEV31' , ierr , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
     ...
     &              'ZGGEV31' , Result(2) , n , jtype , ioldsd
!
...
!
     ...
     &              'Right' , 'ZGGEV31' , Result(4) , n , jtype , ioldsd
!
!           Do test (5)
!
               CALL ZLACPY(' ',n,n,A,Lda,S,Lda)
               CALL ZLACPY(' ',n,n,B,Lda,T,Lda)
               CALL ZGGEV3('N','N',n,S,Lda,T,Lda,Alpha1,Beta1,Q,Ldq,Z,  &
     &                     Ldq,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'ZGGEV32' , ierr , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL ZLACPY(' ',n,n,A,Lda,S,Lda)
               CALL ZLACPY(' ',n,n,B,Lda,T,Lda)
               CALL ZGGEV3('V','N',n,S,Lda,T,Lda,Alpha1,Beta1,Qe,Ldqe,Z,&
     &                     Ldq,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'ZGGEV33' , ierr , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL ZLACPY(' ',n,n,A,Lda,S,Lda)
               CALL ZLACPY(' ',n,n,B,Lda,T,Lda)
               CALL ZGGEV3('N','V',n,S,Lda,T,Lda,Alpha1,Beta1,Q,Ldq,Qe, &
     &                     Ldqe,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'ZGGEV34' , ierr , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
      RETURN
!
99001 FORMAT (' ZDRGEV3: ',A,' returned INFO=',I6,'.',/3X,'N=',I6,      &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99002 FORMAT (' ZDRGEV3: ',A,' Eigenvectors from ',A,                   &
...
99003 FORMAT (/1X,A3,' -- Complex Generalized eigenvalue problem ',     &
     &        'driver')
!
99004 FORMAT (' Matrix types (see ZDRGEV3 for details): ')
!
