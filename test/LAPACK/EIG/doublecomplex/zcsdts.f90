!*==zcsdts.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b ZCSDTS
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZCSDTS(M,P,Q,X,Xf,Ldx,U1,Ldu1,U2,Ldu2,V1t,Ldv1t,V2t,   &
...
!
      ...
      CALL ZHERK('Upper','Conjugate transpose',M,M,-REALONE,X,Ldx,      &
     &           REALONE,Work,Ldx)
      IF ( M>0 ) THEN
...
!     Compute the CSD
!
      CALL ZUNCSD('Y','Y','Y','Y','N','D',M,P,Q,Xf(1,1),Ldx,Xf(1,Q+1),  &
...
!
      CALL ZLACPY('Full',M,M,X,Ldx,Xf,Ldx)
!
      CALL ZGEMM('No transpose','Conjugate transpose',P,Q,Q,ONE,Xf,Ldx, &
     &           V1t,Ldv1t,ZERO,Work,Ldx)
!
      CALL ZGEMM('Conjugate transpose','No transpose',P,Q,P,ONE,U1,Ldu1,&
...
!
     ...
     &      - DCMPLX(COS(Theta(i)),0.0D0)
      ENDDO
!
      CALL ZGEMM('No transpose','Conjugate transpose',P,M-Q,M-Q,ONE,    &
     &           Xf(1,Q+1),Ldx,V2t,Ldv2t,ZERO,Work,Ldx)
!
      CALL ZGEMM('Conjugate transpose','No transpose',P,M-Q,P,ONE,U1,   &
...
!
      CALL ZGEMM('Conjugate transpose','No transpose',M-P,Q,M-P,ONE,U2, &
...
!
     ...
     &      - DCMPLX(SIN(Theta(r-i+1)),0.0D0)
      ENDDO
!
      CALL ZGEMM('No transpose','Conjugate transpose',M-P,M-Q,M-Q,ONE,  &
     &           Xf(P+1,Q+1),Ldx,V2t,Ldv2t,ZERO,Work,Ldx)
!
      CALL ZGEMM('Conjugate transpose','No transpose',M-P,M-Q,M-P,ONE,  &
...
!
      ...
      CALL ZHERK('Upper','Conjugate transpose',P,P,-REALONE,U1,Ldu1,    &
     &           REALONE,Work,Ldu1)
!
...
!
      ...
      CALL ZHERK('Upper','Conjugate transpose',M-P,M-P,-REALONE,U2,Ldu2,&
     &           REALONE,Work,Ldu2)
!
...
!
      ...
      CALL ZHERK('Upper','No transpose',Q,Q,-REALONE,V1t,Ldv1t,REALONE, &
     &           Work,Ldv1t)
!
...
!
      ...
      CALL ZHERK('Upper','No transpose',M-Q,M-Q,-REALONE,V2t,Ldv2t,     &
     &           REALONE,Work,Ldv2t)
!
...
!
      ...
      CALL ZHERK('Upper','Conjugate transpose',Q,M,-REALONE,X,Ldx,      &
     &           REALONE,Work,Ldx)
      IF ( M>0 ) THEN
...
!     Compute the CSD
!
      CALL ZUNCSD2BY1('Y','Y','Y',M,P,Q,Xf(1,1),Ldx,Xf(P+1,1),Ldx,Theta,&
     &                U1,Ldu1,U2,Ldu2,V1t,Ldv1t,Work,Lwork,Rwork,       &
     &                17*(r+2),Iwork,info)
!
!     Compute [X11;X21] := diag(U1,U2)'*[X11;X21]*V1 - [D11;D21]
!
      CALL ZGEMM('No transpose','Conjugate transpose',P,Q,Q,ONE,X,Ldx,  &
     &           V1t,Ldv1t,ZERO,Work,Ldx)
!
      CALL ZGEMM('Conjugate transpose','No transpose',P,Q,P,ONE,U1,Ldu1,&
     &           Work,Ldx,ZERO,X,Ldx)
