!*==ddrves.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b DDRVES
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DDRVES(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
...
!     .. Array Arguments ..
      LOGICAL Bwork(*) , Dotype(*)
      INTEGER Iseed(4) , Iwork(*) , Nn(*)
      DOUBLE PRECISION A(Lda,*) , H(Lda,*) , Ht(Lda,*) , Result(13) ,   &
     &                 Vs(Ldvs,*) , Wi(*) , Wit(*) , Work(*) , Wr(*) ,  &
     &                 Wrt(*)
!     ..
!
!  =====================================================================
...
!
                     ...
                     CALL DGEES('V',sort,DSLECT,n,H,Lda,sdim,Wr,Wi,Vs,  &
     &                          Ldvs,Work,nnwork,Bwork,iinfo)
                     IF ( iinfo/=0 .AND. iinfo/=n+2 ) THEN
                        Result(1+rsub) = ulpinv
                        WRITE (Nounit,FMT=99008) 'DGEES1' , iinfo , n , &
     &                         jtype , ioldsd
                        Info = ABS(iinfo)
                        EXIT
                     ENDIF
...
!
                     lwork = MAX(1,2*n*n)
                     CALL DHST01(n,1,n,A,Lda,H,Lda,Vs,Ldvs,Work,lwork,  &
     &                           res)
                     Result(2+rsub) = res(1)
                     Result(3+rsub) = res(2)
!
...
!
                     ...
                     CALL DGEES('N',sort,DSLECT,n,Ht,Lda,sdim,Wrt,Wit,  &
     &                          Vs,Ldvs,Work,nnwork,Bwork,iinfo)
                     IF ( iinfo/=0 .AND. iinfo/=n+2 ) THEN
                        Result(5+rsub) = ulpinv
                        WRITE (Nounit,FMT=99008) 'DGEES2' , iinfo , n , &
     &                         jtype , ioldsd
                        Info = ABS(iinfo)
                        EXIT
                     ENDIF
...
!
     ...
     &                          DSLECT(Wr(i),-Wi(i)) ) knteig = knteig +&
     &                          1
                           IF ( i<n ) THEN
                              IF ( (DSLECT(Wr(i+1),Wi(i+1)) .OR. DSLECT(&
     &                             Wr(i+1),-Wi(i+1))) .AND.             &
     &                             (.NOT.(DSLECT(Wr(i),Wi(i)) .OR.      &
     &                             DSLECT(Wr(i),-Wi(i)))) .AND.         &
...
99007 FORMAT (' N=',I5,', IWK=',I2,', seed=',4(I4,','),' type ',I2,     &
     &        ', test(',I2,')=',G10.3)
99008 FORMAT (' DDRVES: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
!
!     End of DDRVES
!
      END SUBROUTINE DDRVES
