!*==ddrvst2stg.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b DDRVST2STG
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DDRVST2STG(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,&
...
!
     ...
     &                              iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSYEV_2STAGE(N,'//uplo// &
     &                      ')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  CALL DSYEVX_2STAGE('N','A',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m2,Wa2,Z,Ldu,Work,Lwork,    &
     &                               Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSYEVX_2STAGE(N,A,'//    &
...
!
                  ...
                  CALL DLACPY(' ',n,n,V,Ldu,A,Lda)
                  SRNamt = 'DSYEVX_2STAGE'
                  CALL DSYEVX_2STAGE('N','I',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Work,Lwork,    &
     &                               Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSYEVX_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  CALL DLACPY(' ',n,n,V,Ldu,A,Lda)
                  SRNamt = 'DSYEVX_2STAGE'
                  CALL DSYEVX_2STAGE('N','V',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Work,Lwork,    &
     &                               Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSYEVX_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  SRNamt = 'DSBEV_2STAGE'
                  CALL DSBEV_2STAGE('N',uplo,n,kd,V,Ldu,D3,Z,Ldu,Work,  &
     &                              Lwork,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSBEV_2STAGE(N,'//uplo// &
     &                      ')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  CALL DSBEVX_2STAGE('N','A',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m3,Wa3,Z,Ldu,Work, &
     &                               Lwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSBEVX_2STAGE(N,A,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  SRNamt = 'DSBEVX_2STAGE'
                  CALL DSBEVX_2STAGE('N','I',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m3,Wa3,Z,Ldu,Work, &
     &                               Lwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSBEVX_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  SRNamt = 'DSBEVX_2STAGE'
                  CALL DSBEVX_2STAGE('N','V',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m3,Wa3,Z,Ldu,Work, &
     &                               Lwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSBEVX_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  CALL DSYEVD_2STAGE('N',uplo,n,A,Ldu,D3,Work,Lwork,    &
     &                               Iwork,liwedc,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSYEVD_2STAGE(N,'//uplo//&
     &                      ')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  CALL DSBEVD_2STAGE('N',uplo,n,kd,V,Ldu,D3,Z,Ldu,Work, &
     &                               Lwork,Iwork,liwedc,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSBEVD_2STAGE(N,'//uplo//&
     &                      ')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  CALL DSYEVR_2STAGE('N','A',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m2,Wa2,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Iwork(2*n+1),Liwork-2*n,     &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSYEVR_2STAGE(N,A,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  CALL DLACPY(' ',n,n,V,Ldu,A,Lda)
                  SRNamt = 'DSYEVR_2STAGE'
                  CALL DSYEVR_2STAGE('N','I',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Iwork(2*n+1),Liwork-2*n,     &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSYEVR_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  CALL DLACPY(' ',n,n,V,Ldu,A,Lda)
                  SRNamt = 'DSYEVR_2STAGE'
                  CALL DSYEVR_2STAGE('N','V',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Iwork(2*n+1),Liwork-2*n,     &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'DSYEVR_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
99001 FORMAT (' DDRVST2STG: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,   &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
!
!     End of DDRVST2STG
!
      END SUBROUTINE DDRVST2STG
