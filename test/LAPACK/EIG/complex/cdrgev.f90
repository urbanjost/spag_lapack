!*==cdrgev.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b CDRGEV
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CDRGEV(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
...
!
               ...
               CALL CGGEV('V','V',n,S,Lda,T,Lda,Alpha,Beta,Q,Ldq,Z,Ldq, &
     &                    Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'CGGEV1' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
     ...
     &              'CGGEV1' , Result(2) , n , jtype , ioldsd
!
...
!
     ...
     &              'Right' , 'CGGEV1' , Result(4) , n , jtype , ioldsd
!
!           Do test (5)
!
               CALL CLACPY(' ',n,n,A,Lda,S,Lda)
               CALL CLACPY(' ',n,n,B,Lda,T,Lda)
               CALL CGGEV('N','N',n,S,Lda,T,Lda,Alpha1,Beta1,Q,Ldq,Z,   &
     &                    Ldq,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'CGGEV2' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL CLACPY(' ',n,n,A,Lda,S,Lda)
               CALL CLACPY(' ',n,n,B,Lda,T,Lda)
               CALL CGGEV('V','N',n,S,Lda,T,Lda,Alpha1,Beta1,Qe,Ldqe,Z, &
     &                    Ldq,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'CGGEV3' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL CLACPY(' ',n,n,A,Lda,S,Lda)
               CALL CLACPY(' ',n,n,B,Lda,T,Lda)
               CALL CGGEV('N','V',n,S,Lda,T,Lda,Alpha1,Beta1,Q,Ldq,Qe,  &
     &                    Ldqe,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'CGGEV4' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
      RETURN
!
99001 FORMAT (' CDRGEV: ',A,' returned INFO=',I6,'.',/3X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99002 FORMAT (' CDRGEV: ',A,' Eigenvectors from ',A,' incorrectly ',    &
...
99003 FORMAT (/1X,A3,' -- Complex Generalized eigenvalue problem ',     &
     &        'driver')
!
99004 FORMAT (' Matrix types (see CDRGEV for details): ')
!
