!*==cdrgsx.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b CDRGSX
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CDRGSX(Nsize,Ncmax,Thresh,Nin,Nout,A,Lda,B,Ai,Bi,Z,Q,  &
...
!
         ...
         maxwrk = MAX(maxwrk,                                           &
     &            Nsize*(1+ILAENV(1,'CUNGQR',' ',Nsize,1,Nsize,-1)))
!
!        workspace for cgesvd
!
         bdspac = 3*Nsize*Nsize/2
         maxwrk = MAX(maxwrk,                                           &
     &            Nsize*Nsize*(1+ILAENV(1,'CGEBRD',' ',Nsize*Nsize/2,   &
     &            Nsize*Nsize/2,-1,-1)))
...
!     Do tests (1) to (4)
!
            CALL CGET51(1,MPLusn,A,Lda,Ai,Lda,Q,Lda,Z,Lda,Work,Rwork,   &
     &                  result(1))
            CALL CGET51(1,MPLusn,B,Lda,Bi,Lda,Q,Lda,Z,Lda,Work,Rwork,   &
     &                  result(2))
            CALL CGET51(3,MPLusn,B,Lda,Bi,Lda,Q,Lda,Q,Lda,Work,Rwork,   &
     &                  result(3))
            CALL CGET51(3,MPLusn,B,Lda,Bi,Lda,Z,Lda,Z,Lda,Work,Rwork,   &
     &                  result(4))
!
...
!
     ...
     &                           Lda)
!
                     CALL CLATM5(prtype,M,N,Ai,Lda,Ai(M+1,M+1),Lda,     &
...
!
                     ...
                     CALL CLACPY('Full',MPLusn,MPLusn,Bi,Lda,B,Lda)
!
                     CALL CGGESX('V','V','S',CLCTSX,sense,MPLusn,Ai,Lda,&
...
!
                     IF ( linfo/=0 .AND. linfo/=MPLusn+2 ) THEN
                        result(1) = ulpinv
                        WRITE (Nout,FMT=99001) 'CGGESX' , linfo ,       &
     &                         MPLusn , prtype
                        Info = linfo
                        CYCLE
                     ENDIF
...
!
                        CALL CLAKF2(mm,MPLusn-mm,Ai,Lda,Ai(mm+1,mm+1),  &
     &                              Bi,Bi(mm+1,mm+1),C,Ldc)
!
                        CALL CGESVD('N','N',mn2,mn2,C,Ldc,S,Work,1,     &
...
!     Summary
!
 100  CALL ALASVM('CGX',Nout,nerrs,ntestt,0)
!
      Work(1) = maxwrk
!
      RETURN
!
99001 FORMAT (' CDRGSX: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,')')
!
99002 FORMAT (' CDRGSX: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', Input Example #',I2,')')
!
99003 FORMAT (' CDRGSX: S not in Schur form at eigenvalue ',I6,'.',/9X, &
     &        'N=',I6,', JTYPE=',I6,')')
!
...
99008 FORMAT (' Matrix order=',I2,', type=',I2,', a=',E10.3,            &
