!*==cdrvbd.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b CDRVBD
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CDRVBD(Nsizes,Mm,Nn,Ntypes,Dotype,Iseed,Thresh,A,Lda,U,&
...
!     .. Array Arguments ..
      ...
      INTEGER Iseed(4) , Iwork(*) , Mm(*) , Nn(*)
      REAL E(*) , Rwork(*) , S(*) , Ssav(*)
      COMPLEX A(Lda,*) , Asav(Lda,*) , U(Ldu,*) , Usav(Ldu,*) ,         &
...
!     .. External Subroutines ..
     ...
     &         CGESVDQ , CGESVJ , CGEJSV , CGESVDX , CLACPY , CLASET ,  &
     &         CLATMS , CUNT01 , CUNT03
!     ..
!     .. Intrinsic Functions ..
...
!
                  ...
                  SRNamt = 'CGESVD'
                  CALL CGESVD('A','A',m,n,A,Lda,Ssav,Usav,Ldu,Vtsav,    &
...
!              Do tests 1--4
!
                  CALL CBDT01(m,n,0,Asav,Lda,Usav,Ldu,Ssav,E,Vtsav,Ldvt,&
     &                        Work,Rwork,result(1))
                  IF ( m/=0 .AND. n/=0 ) THEN
                     CALL CUNT01('Columns',mnmin,m,Usav,Ldu,Work,Lwork, &
     &                           Rwork,result(2))
                     CALL CUNT01('Rows',mnmin,n,Vtsav,Ldvt,Work,Lwork,  &
     &                           Rwork,result(3))
                  ENDIF
                  result(4) = 0
                  DO i = 1 , mnmin - 1
...
!
     ...
     &                       .AND. ijvt==1)) ) THEN
                           ...
                           SRNamt = 'CGESVD'
                           CALL CGESVD(jobu,jobvt,m,n,A,Lda,S,U,Ldu,Vt, &
     &                                 Ldvt,Work,lswork,Rwork,iinfo)
!
!                    Compare U
!
     ...
     &                              iinfo)
                              ELSEIF ( iju==3 ) THEN
                                 CALL CUNT03('C',m,m,m,mnmin,Usav,Ldu,U,&
...
!
     ...
     &                              iinfo)
                              ELSEIF ( ijvt==3 ) THEN
                                 CALL CUNT03('R',n,n,n,mnmin,Vtsav,Ldvt,&
...
!
                  CALL CLACPY('F',m,n,Asav,Lda,A,Lda)
                  SRNamt = 'CGESDD'
                  CALL CGESDD('A',m,n,A,Lda,Ssav,Usav,Ldu,Vtsav,Ldvt,   &
...
!
                  ...
                  DO ijq = 0 , 2
                     ...
                     CALL CLACPY('F',m,n,Asav,Lda,A,Lda)
                     SRNamt = 'CGESDD'
                     CALL CGESDD(jobq,m,n,A,Lda,S,U,Ldu,Vt,Ldvt,Work,   &
...
!
     ...
     &                           U,Ldu,Work,Lwork,Rwork,dif,iinfo)
                           ENDIF
                        ELSEIF ( ijq==2 ) THEN
                           CALL CUNT03('C',m,mnmin,m,mnmin,Usav,Ldu,U,  &
     &                                 Ldu,Work,Lwork,Rwork,dif,iinfo)
                        ENDIF
                     ENDIF
...
!
     ...
     &                           Ldvt,A,Lda,Work,Lwork,Rwork,dif,iinfo)
                           ENDIF
                        ELSEIF ( ijq==2 ) THEN
                           CALL CUNT03('R',n,mnmin,n,mnmin,Vtsav,Ldvt,  &
...
!
                     lrwork = MAX(2,m,5*n)
                     liwork = MAX(n,1)
                     CALL CGESVDQ('H','N','N','A','A',m,n,A,Lda,Ssav,   &
     &                            Usav,Ldu,Vtsav,Ldvt,numrank,Iwork,    &
     &                            liwork,Work,Lwork,Rwork,lrwork,iinfo)
!
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99005) 'CGESVDQ' , iinfo , m ,&
...
!                 Do tests 36--39
!
                     CALL CBDT01(m,n,0,Asav,Lda,Usav,Ldu,Ssav,E,Vtsav,  &
     &                           Ldvt,Work,Rwork,result(36))
                     IF ( m/=0 .AND. n/=0 ) THEN
                        CALL CUNT01('Columns',m,m,Usav,Ldu,Work,Lwork,  &
     &                              Rwork,result(37))
                        CALL CUNT01('Rows',n,n,Vtsav,Ldvt,Work,Lwork,   &
     &                              Rwork,result(38))
                     ENDIF
                     result(39) = ZERO
                     DO i = 1 , mnmin - 1
...
!
                     ...
                     SRNamt = 'CGESVJ'
                     CALL CGESVJ('G','U','V',m,n,Usav,Lda,Ssav,0,A,Ldvt,&
...
!
                     ...
                     SRNamt = 'CGEJSV'
                     CALL CGEJSV('G','U','V','R','N','N',m,n,Vtsav,Lda, &
...
!
                  CALL CLACPY('F',m,n,Asav,Lda,A,Lda)
                  SRNamt = 'CGESVDX'
                  CALL CGESVDX('V','V','A',m,n,A,Lda,vl,vu,il,iu,ns,    &
...
!
     ...
     &                       .AND. ijvt==1)) ) THEN
                           ...
                           CALL CLACPY('F',m,n,Asav,Lda,A,Lda)
                           SRNamt = 'CGESVDX'
                           CALL CGESVDX(jobu,jobvt,'A',m,n,A,Lda,vl,vu, &
...
!
                  ...
                  CALL CLACPY('F',m,n,Asav,Lda,A,Lda)
                  SRNamt = 'CGESVDX'
                  CALL CGESVDX('V','V','I',m,n,A,Lda,vl,vu,il,iu,nsi,S, &
...
!
                  ...
                  result(31) = ZERO
                  result(32) = ZERO
                  CALL CBDT05(m,n,Asav,Lda,S,nsi,U,Ldu,Vt,Ldvt,Work,    &
     &                        result(30))
                  IF ( m/=0 .AND. n/=0 ) THEN
                     CALL CUNT01('Columns',m,nsi,U,Ldu,Work,Lwork,Rwork,&
     &                           result(31))
                     CALL CUNT01('Rows',nsi,n,Vt,Ldvt,Work,Lwork,Rwork, &
     &                           result(32))
                  ENDIF
!
!              Do tests 11--13
!
     ...
     &                       (Ssav(ns)-Ssav(1)))
                  ...
                  CALL CLACPY('F',m,n,Asav,Lda,A,Lda)
                  SRNamt = 'CGESVDX'
                  CALL CGESVDX('V','V','V',m,n,A,Lda,vl,vu,il,iu,nsv,S, &
     &                         U,Ldu,Vt,Ldvt,Work,Lwork,Rwork,Iwork,    &
     &                         iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99005) 'GESVDX' , iinfo , m , n ,&
...
!
                  ...
                  result(34) = ZERO
                  result(35) = ZERO
                  CALL CBDT05(m,n,Asav,Lda,S,nsv,U,Ldu,Vt,Ldvt,Work,    &
     &                        result(33))
                  IF ( m/=0 .AND. n/=0 ) THEN
                     CALL CUNT01('Columns',m,nsv,U,Ldu,Work,Lwork,Rwork,&
     &                           result(34))
                     CALL CUNT01('Rows',nsv,n,Vt,Ldvt,Work,Lwork,Rwork, &
     &                           result(35))
                  ENDIF
!
...
99002 FORMAT (' Tests performed with Test Threshold = ',F8.2,           &
     ...
     &        ' decreasing order, else 1/ulp',/' CGESJV: ',             &
     &        //'19 = | A - U diag(S) VT | / ( |A| max(M,N) ulp )',     &
     &        /'20 = | I - U**T U | / ( M ulp ) ',                      &
     &        /'21 = | I - VT VT**T | / ( N ulp ) ',                    &
...
99003 FORMAT (' M=',I5,', N=',I5,', type ',I1,', IWS=',I1,', seed=',    &
     &        4(I4,','),' test(',I2,')=',G11.4)
99004 FORMAT (' CDRVBD: ',A,' returned INFO=',I6,'.',/9X,'M=',I6,', N=',&
     &        I6,', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
99005 FORMAT (' CDRVBD: ',A,' returned INFO=',I6,'.',/9X,'M=',I6,', N=',&
