!*==scsdts.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b SCSDTS
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SCSDTS(M,P,Q,X,Xf,Ldx,U1,Ldu1,U2,Ldu2,V1t,Ldv1t,V2t,   &
...
!
      ...
      CALL SSYRK('Upper','Conjugate transpose',M,M,-ONE,X,Ldx,ONE,Work, &
     &           Ldx)
      IF ( M>0 ) THEN
...
!     Compute the CSD
!
      CALL SORCSD('Y','Y','Y','Y','N','D',M,P,Q,Xf(1,1),Ldx,Xf(1,Q+1),  &
...
!
      CALL SLACPY('Full',M,M,X,Ldx,Xf,Ldx)
!
      CALL SGEMM('No transpose','Conjugate transpose',P,Q,Q,ONE,Xf,Ldx, &
     &           V1t,Ldv1t,ZERO,Work,Ldx)
!
      CALL SGEMM('Conjugate transpose','No transpose',P,Q,P,ONE,U1,Ldu1,&
...
!
     ...
     &      - COS(Theta(i))
      ENDDO
!
      CALL SGEMM('No transpose','Conjugate transpose',P,M-Q,M-Q,ONE,    &
     &           Xf(1,Q+1),Ldx,V2t,Ldv2t,ZERO,Work,Ldx)
!
      CALL SGEMM('Conjugate transpose','No transpose',P,M-Q,P,ONE,U1,   &
...
!
      CALL SGEMM('Conjugate transpose','No transpose',M-P,Q,M-P,ONE,U2, &
...
!
     ...
     &      - SIN(Theta(r-i+1))
      ENDDO
!
      CALL SGEMM('No transpose','Conjugate transpose',M-P,M-Q,M-Q,ONE,  &
     &           Xf(P+1,Q+1),Ldx,V2t,Ldv2t,ZERO,Work,Ldx)
!
      CALL SGEMM('Conjugate transpose','No transpose',M-P,M-Q,M-P,ONE,  &
...
!
      ...
      CALL SSYRK('Upper','Conjugate transpose',P,P,-ONE,U1,Ldu1,ONE,    &
     &           Work,Ldu1)
!
...
!
      ...
      CALL SSYRK('Upper','Conjugate transpose',M-P,M-P,-ONE,U2,Ldu2,ONE,&
     &           Work,Ldu2)
!
...
!
      ...
      CALL SSYRK('Upper','No transpose',Q,Q,-ONE,V1t,Ldv1t,ONE,Work,    &
     &           Ldv1t)
!
...
!
      ...
      CALL SSYRK('Upper','No transpose',M-Q,M-Q,-ONE,V2t,Ldv2t,ONE,Work,&
     &           Ldv2t)
!
...
!
      ...
      CALL SSYRK('Upper','Conjugate transpose',Q,M,-ONE,X,Ldx,ONE,Work, &
     &           Ldx)
      IF ( M>0 ) THEN
...
!     Compute the CSD
!
      CALL SORCSD2BY1('Y','Y','Y',M,P,Q,Xf(1,1),Ldx,Xf(P+1,1),Ldx,Theta,&
     &                U1,Ldu1,U2,Ldu2,V1t,Ldv1t,Work,Lwork,Iwork,info)
!
!     Compute [X11;X21] := diag(U1,U2)'*[X11;X21]*V1 - [D11;D21]
!
      CALL SGEMM('No transpose','Conjugate transpose',P,Q,Q,ONE,X,Ldx,  &
     &           V1t,Ldv1t,ZERO,Work,Ldx)
!
      CALL SGEMM('Conjugate transpose','No transpose',P,Q,P,ONE,U1,Ldu1,&
     &           Work,Ldx,ZERO,X,Ldx)
