!*==schkhs.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b SCHKHS
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SCHKHS(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
...
!     .. Array Arguments ..
      LOGICAL Dotype(*) , Select(*)
      INTEGER Iseed(4) , Iwork(*) , Nn(*)
      REAL A(Lda,*) , Evectl(Ldu,*) , Evectr(Ldu,*) , Evectx(Ldu,*) ,   &
     &     Evecty(Ldu,*) , H(Lda,*) , Result(14) , T1(Lda,*) , T2(Lda,*)&
     &     , Tau(*) , U(Ldu,*) , Uu(Ldu,*) , Uz(Ldu,*) , Wi1(*) , Wi2(*)&
     &     , Wi3(*) , Work(*) , Wr1(*) , Wr2(*) , Wr3(*) , Z(Ldu,*)
!     ..
!
!  =====================================================================
!
...
!
               IF ( iinfo/=0 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'SGEHRD' , iinfo , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(iinfo)
                  GOTO 10
               ENDIF
!
               ...
               ntest = 2
!
               CALL SHST01(n,ilo,ihi,A,Lda,H,Lda,U,Ldu,Work,Nwork,      &
     &                     Result(1))
!
...
!
               ...
               Result(3) = ulpinv
!
               CALL SHSEQR('E','N',n,ilo,ihi,T2,Lda,Wr3,Wi3,Uz,Ldu,Work,&
     &                     Nwork,iinfo)
               IF ( iinfo/=0 ) THEN
                  WRITE (Nounit,FMT=99001) 'SHSEQR(E)' , iinfo , n ,    &
     &                   jtype , ioldsd
                  IF ( iinfo<=n+2 ) THEN
                     Info = ABS(iinfo)
                     GOTO 10
...
!
               CALL SLACPY(' ',n,n,H,Lda,T2,Lda)
!
               CALL SHSEQR('S','N',n,ilo,ihi,T2,Lda,Wr2,Wi2,Uz,Ldu,Work,&
     &                     Nwork,iinfo)
               IF ( iinfo/=0 .AND. iinfo<=n+2 ) THEN
                  WRITE (Nounit,FMT=99001) 'SHSEQR(S)' , iinfo , n ,    &
     &                   jtype , ioldsd
                  Info = ABS(iinfo)
                  GOTO 10
               ENDIF
...
!
               CALL SHSEQR('S','V',n,ilo,ihi,T1,Lda,Wr1,Wi1,Uz,Ldu,Work,&
     &                     Nwork,iinfo)
               IF ( iinfo/=0 .AND. iinfo<=n+2 ) THEN
                  WRITE (Nounit,FMT=99001) 'SHSEQR(V)' , iinfo , n ,    &
     &                   jtype , ioldsd
                  Info = ABS(iinfo)
                  GOTO 10
               ENDIF
...
!                and 4: | I - Z Z' | / ( n ulp )
!
               CALL SHST01(n,ilo,ihi,H,Lda,T1,Lda,Z,Ldu,Work,Nwork,     &
     &                     Result(3))
!
!           Do Tests 5: | A - UZ T (UZ)' | / ( |A| n ulp )
!                and 6: | I - UZ (UZ)' | / ( n ulp )
!
               CALL SHST01(n,ilo,ihi,A,Lda,T1,Lda,Uz,Ldu,Work,Nwork,    &
     &                     Result(5))
!
...
!
               ...
               DO
                  ...
                  IF ( j<=0 ) THEN
!
                     CALL STREVC('Right','All',Select,n,T1,Lda,dumma,   &
     &                           Ldu,Evectr,Ldu,n,in,Work,iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'STREVC(R,A)' , iinfo ,&
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        EXIT
                     ENDIF
...
!
     ...
     &                           Ldu,Evectl,Ldu,n,in,Work,iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'STREVC(R,S)' , iinfo ,&
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        EXIT
                     ENDIF
!
 ...
 2                   IF ( .NOT.match ) WRITE (Nounit,FMT=99003)         &
     &                    'Right' , 'STREVC' , n , jtype , ioldsd
!
!           Compute the Left eigenvector Matrix:
!
                     ntest = 10
                     Result(10) = ulpinv
                     CALL STREVC('Left','All',Select,n,T1,Lda,Evectl,   &
     &                           Ldu,dumma,Ldu,n,in,Work,iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'STREVC(L,A)' , iinfo ,&
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        EXIT
                     ENDIF
...
!
     ...
     &                           Ldu,dumma,Ldu,n,in,Work,iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'STREVC(L,S)' , iinfo ,&
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        EXIT
                     ENDIF
...
!
     ...
     &                           Work,Iwork,Iwork,iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'SHSEIN(R)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) EXIT
                     ELSE
...
!
                     ...
                     ENDDO
!
                     CALL SHSEIN('Left','Qr','Ninitv',Select,n,H,Lda,   &
     &                           Wr3,Wi3,Evecty,Ldu,dumma,Ldu,n1,in,    &
     &                           Work,Iwork,Iwork,iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'SHSEIN(L)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) EXIT
                     ELSE
...
!
     ...
     &                           Ldu,Tau,Evectx,Ldu,Work,Nwork,iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'SORMHR(R)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) EXIT
                     ELSE
...
!
     ...
     &                           Ldu,Tau,Evecty,Ldu,Work,Nwork,iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'SORMHR(L)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                        ENDIF
...
!
 10            ntestt = ntestt + ntest
               CALL SLAFTS('SHS',n,n,jtype,ntest,Result,ioldsd,Thresh,  &
     &                     Nounit,nerrs)
!
            ENDDO
         ENDIF
...
!
      RETURN
!
99001 FORMAT (' SCHKHS: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
99002 FORMAT (' SCHKHS: ',A,' Eigenvectors from ',A,' incorrectly ',    &
     &        'normalized.',/' Bits of error=',0P,G10.3,',',9X,'N=',I6, &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
99003 FORMAT (' SCHKHS: Selected ',A,' Eigenvectors from ',A,           &
