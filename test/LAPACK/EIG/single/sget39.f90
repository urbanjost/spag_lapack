!*==sget39.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b SGET39
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Local Scalars ..
     ...
     &        ndim
      REAL bignum , domin , dumm , eps , norm , normtb , resid , scale ,&
     &     smlnum , w , xnorm
!     ..
!     .. External Functions ..
      INTEGER ISAMAX
...
!     .. Local Arrays ..
      ...
      REAL b(LDT) , d(LDT2) , dum(1) , t(LDT,LDT) , vm1(5) , vm2(5) ,   &
     &     vm3(5) , vm4(5) , vm5(3) , work(LDT) , x(LDT2) , y(LDT2)
!     ..
!     .. Data statements ..
      DATA idim/4 , 5*5/
...
!
                        ...
                        DO i = 1 , n
                           DO j = 1 , n
                              t(i,j) = REAL(ival(i,j,ndim))*vm1(ivm1)
...
!
                        ...
                        Knt = Knt + 1
                        CALL SLAQTR(.FALSE.,.TRUE.,n,t,LDT,dum,dumm,    &
     &                              scale,x,work,info)
...
!
                        ...
                        CALL SGEMV('No transpose',n,n,ONE,t,LDT,x,1,    &
     &                             -scale,y,1)
                        xnorm = SASUM(n,x,1)
                        resid = SASUM(n,y,1)
...
!
                        CALL SCOPY(n,d,1,x,1)
                        Knt = Knt + 1
                        CALL SLAQTR(.TRUE.,.TRUE.,n,t,LDT,dum,dumm,     &
     &                              scale,x,work,info)
                        IF ( info/=0 ) Ninfo = Ninfo + 1
!
!                       || T*x - scale*d || /
...
!
                        CALL SCOPY(n,d,1,y,1)
                        CALL SGEMV('Transpose',n,n,ONE,t,LDT,x,1,-scale,&
     &                             y,1)
                        xnorm = SASUM(n,x,1)
                        resid = SASUM(n,y,1)
                        domin = MAX(smlnum,(smlnum/eps)*norm,(norm*eps) &
...
!
                        ...
                        Knt = Knt + 1
                        CALL SLAQTR(.FALSE.,.FALSE.,n,t,LDT,b,w,scale,x,&
     &                              work,info)
                        IF ( info/=0 ) Ninfo = Ninfo + 1
!
!                       ||(T+i*B)*(x1+i*x2) - scale*(d1+i*d2)|| /
...
!
                        ...
                        ENDDO
                        CALL SGEMV('No transpose',n,n,ONE,t,LDT,x(1+n), &
     &                             1,ONE,y(1+n),1)
!
                        resid = SASUM(2*n,y,1)
...
!
                        CALL SCOPY(2*n,d,1,x,1)
                        Knt = Knt + 1
                        CALL SLAQTR(.TRUE.,.FALSE.,n,t,LDT,b,w,scale,x, &
     &                              work,info)
                        IF ( info/=0 ) Ninfo = Ninfo + 1
!
!                       ||(T+i*B)*(x1+i*x2) - scale*(d1+i*d2)|| /
...
!
                        ...
                        ENDDO
                        CALL SGEMV('Transpose',n,n,ONE,t,LDT,x,1,ONE,y, &
     &                             1)
!
                        ...
                        ENDDO
                        CALL SGEMV('Transpose',n,n,ONE,t,LDT,x(1+n),1,  &
     &                             -ONE,y(1+n),1)
!
                        resid = SASUM(2*n,y,1)
                        domin = MAX(smlnum,(smlnum/eps)*normtb,         &
