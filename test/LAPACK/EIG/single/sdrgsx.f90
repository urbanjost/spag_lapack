!*==sdrgsx.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b SDRGSX
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SDRGSX(Nsize,Ncmax,Thresh,Nin,Nout,A,Lda,B,Ai,Bi,Z,Q,  &
...
!     .. Array Arguments ..
     ...
     &     Beta(*) , Bi(Lda,*) , C(Ldc,*) , Q(Lda,*) , S(*) , Work(*) , &
     &     Z(Lda,*)
!     ..
!
!  =====================================================================
...
!     .. Local Scalars ..
     ...
     &        prtype , qba , qbb
      REAL abnrm , bignum , diftru , pltru , smlnum , temp1 , temp2 ,   &
     &     thrsh2 , ulp , ulpinv , weight
!     ..
!     .. Local Arrays ..
...
!
         maxwrk = 9*(Nsize+1) + Nsize*ILAENV(1,'SGEQRF',' ',Nsize,1,    &
     &            Nsize,0)
         maxwrk = MAX(maxwrk,9*(Nsize+1)                                &
     &            +Nsize*ILAENV(1,'SORGQR',' ',Nsize,1,Nsize,-1))
!
!        workspace for sgesvd
!
         bdspac = 5*Nsize*Nsize/2
         maxwrk = MAX(maxwrk,3*Nsize*Nsize/2+                           &
     &            Nsize*Nsize*ILAENV(1,'SGEBRD',' ',Nsize*Nsize/2,      &
     &            Nsize*Nsize/2,-1,-1))
...
!     Do tests (1) to (4)
!
            CALL SGET51(1,MPLusn,A,Lda,Ai,Lda,Q,Lda,Z,Lda,Work,result(1)&
     &                  )
            CALL SGET51(1,MPLusn,B,Lda,Bi,Lda,Q,Lda,Z,Lda,Work,result(2)&
     &                  )
            CALL SGET51(3,MPLusn,B,Lda,Bi,Lda,Q,Lda,Q,Lda,Work,result(3)&
     &                  )
            CALL SGET51(3,MPLusn,B,Lda,Bi,Lda,Z,Lda,Z,Lda,Work,result(4)&
     &                  )
!
...
!
     ...
     &                    /MAX(smlnum,ABS(Beta(j)),ABS(Bi(j,j))))/ulp
               ...
               ELSE
                  ...
                  ENDIF
                  IF ( .NOT.ilabad ) THEN
                     CALL SGET53(Ai(i1,i1),Lda,Bi(i1,i1),Lda,Beta(j),   &
...
!
                     ...
                     K = 0
!
                     CALL SLASET('Full',MPLusn,MPLusn,ZERO,ZERO,Ai,Lda)
                     CALL SLASET('Full',MPLusn,MPLusn,ZERO,ZERO,Bi,Lda)
!
                     CALL SLATM5(prtype,M,N,Ai,Lda,Ai(M+1,M+1),Lda,     &
...
!
                     ...
                     CALL SLACPY('Full',MPLusn,MPLusn,Bi,Lda,B,Lda)
!
                     CALL SGGESX('V','V','S',SLCTSX,sense,MPLusn,Ai,Lda,&
...
!
                     IF ( linfo/=0 .AND. linfo/=MPLusn+2 ) THEN
                        result(1) = ulpinv
                        WRITE (Nout,FMT=99001) 'SGGESX' , linfo ,       &
     &                         MPLusn , prtype
                        Info = linfo
                        CYCLE
                     ENDIF
...
!
                        CALL SLAKF2(mm,MPLusn-mm,Ai,Lda,Ai(mm+1,mm+1),  &
     &                              Bi,Bi(mm+1,mm+1),C,Ldc)
!
                        CALL SGESVD('N','N',mn2,mn2,C,Ldc,S,Work,1,     &
...
!     Summary
!
 100  CALL ALASVM('SGX',Nout,nerrs,ntestt,0)
!
      Work(1) = maxwrk
!
      RETURN
!
99001 FORMAT (' SDRGSX: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,')')
!
99002 FORMAT (' SDRGSX: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', Input Example #',I2,')')
!
99003 FORMAT (' SDRGSX: SGET53 returned INFO=',I1,' for eigenvalue ',I6,&
     &        '.',/9X,'N=',I6,', JTYPE=',I6,')')
!
99004 FORMAT (' SDRGSX: S not in Schur form at eigenvalue ',I6,'.',/9X, &
     &        'N=',I6,', JTYPE=',I6,')')
!
...
99009 FORMAT (' Matrix order=',I2,', type=',I2,', a=',E10.3,            &
     &        ', order(A_11)=',I2,', result ',I2,' is ',0P,F8.2)
99010 FORMAT (' Matrix order=',I2,', type=',I2,', a=',E10.3,            &
