!*==sget34.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b SGET34
!
!  =========== DOCUMENTATION ===========
!
...
!     ..
!     .. Local Arrays ..
      REAL q(4,4) , result(2) , t(4,4) , t1(4,4) , val(9) , vm(2) ,     &
     &     work(LWORK)
!     ..
!     .. External Functions ..
      REAL SLAMCH
         ...
         DO iam = 1 , 2
            DO ib = 1 , 9
               DO ic = 1 , 9
                  ...
                  CALL SLAEXC(.TRUE.,2,t,4,q,4,1,1,1,work,info)
                  IF ( info/=0 ) Ninfo(info) = Ninfo(info) + 1
                  CALL SHST01(2,1,2,t1,4,t,4,q,4,work,LWORK,result)
...
!
     ...
     &                            ABS(t(3,2)),ABS(t(3,3)))
                           ...
                           CALL SCOPY(16,val(1),0,q,1)
                           CALL SCOPY(4,val(3),0,q,5)
                           CALL SLAEXC(.TRUE.,3,t,4,q,4,1,1,2,work,info)
                           IF ( info/=0 ) Ninfo(info) = Ninfo(info) + 1
                           CALL SHST01(3,1,3,t1,4,t,4,q,4,work,LWORK,   &
     &                                 result)
                           res = result(1) + result(2)
                           IF ( info==0 ) THEN
...
!
     ...
     &                            ABS(t(3,2)),ABS(t(3,3)))
                           ...
                           CALL SCOPY(16,val(1),0,q,1)
                           CALL SCOPY(4,val(3),0,q,5)
                           CALL SLAEXC(.TRUE.,3,t,4,q,4,1,2,1,work,info)
                           IF ( info/=0 ) Ninfo(info) = Ninfo(info) + 1
                           CALL SHST01(3,1,3,t1,4,t,4,q,4,work,LWORK,   &
     &                                 result)
                           res = result(1) + result(2)
...
!
     ...
     &                                 *ABS(val(icm))
                                    ...
                                    CALL SCOPY(16,val(1),0,q,1)
                                    CALL SCOPY(4,val(3),0,q,5)
                                    CALL SLAEXC(.TRUE.,4,t,4,q,4,1,2,2, &
     &                                 work,info)
                                    IF ( info/=0 ) Ninfo(info)          &
     &                                 = Ninfo(info) + 1
                                    CALL SHST01(4,1,4,t1,4,t,4,q,4,work,&
     &                                 LWORK,result)
                                    res = result(1) + result(2)
                                    IF ( info==0 ) THEN
