!*==ssyt21.f90  processed by SPAG 7.51RB at 17:37 on  4 Mar 2022
!> \brief \b SSYT21
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SSYT21(Itype,Uplo,N,Kband,A,Lda,D,E,U,Ldu,V,Ldv,Tau,   &
     &                  Work,Result)
      IMPLICIT NONE
!*--SSYT21211
!
...
!     ..
!     .. Array Arguments ..
      REAL A(Lda,*) , D(*) , E(*) , Result(2) , Tau(*) , U(Ldu,*) ,     &
     &     V(Ldv,*) , Work(*)
!     ..
!
!  =====================================================================
...
!
               vsave = V(j+1,j)
               V(j+1,j) = ONE
               CALL SLARFY('L',N-j,V(j+1,j),1,Tau(j),Work((N+1)*j+1),N, &
     &                     Work(N**2+1))
               V(j+1,j) = vsave
               Work((N+1)*(j-1)+1) = D(j)
            ENDDO
...
!
         ...
         IF ( lower ) THEN
            CALL SORM2R('R','T',N,N-1,N-1,V(2,1),Ldv,Tau,Work(N+1),N,   &
     &                  Work(N**2+1),iinfo)
         ELSE
            CALL SORM2L('R','T',N,N-1,N-1,V(1,2),Ldv,Tau,Work,N,        &
     &                  Work(N**2+1),iinfo)
         ENDIF
         IF ( iinfo/=0 ) THEN
            Result(1) = TEN/ulp
