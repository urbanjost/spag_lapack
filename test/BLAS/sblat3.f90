!*==sblat3.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!     .. Local Arrays ..
      REAL aa(NMAX*NMAX) , ab(NMAX,2*NMAX) , alf(NALMAX) , as(NMAX*NMAX)&
     &     , bb(NMAX*NMAX) , bet(NBEMAX) , bs(NMAX*NMAX) , c(NMAX,NMAX) &
...
99005 FORMAT (' TESTS OF THE REAL             LEVEL 3 BLAS',//' THE F', &
...
99010 FORMAT (' SUBPROGRAM NAME ',A6,' NOT RECOGNIZED',/' ******* T',   &
     &        'ESTS ABANDONED *******')
99011 FORMAT (' ERROR IN SMMCH -  IN-LINE DOT PRODUCTS ARE BEING EVALU',&
...
!
     ...
     &                                lda , ldb
                                 IF ( Rewi ) REWIND Ntra
                                 CALL STRSM(side,uplo,transa,diag,m,n,  &
     &                              alpha,Aa,lda,Bb,ldb)
                              ENDIF
!
!                          Check if error-exit was taken incorrectly.
...
!
                                    IF ( left ) THEN
                                       CALL SMMCH(transa,'N',m,n,m,ONE, &
     &                                    A,Nmax,C,Nmax,ZERO,B,Nmax,Ct, &
     &                                    G,Bb,ldb,Eps,err,Fatal,Nout,  &
     &                                    .FALSE.)
                                    ELSE
                                       CALL SMMCH('N',transa,m,n,n,ONE, &
...
!     .. Scalar Arguments ..
      ...
      CHARACTER*6 Sname
!     .. Array Arguments ..
      REAL Aa(Nmax*Nmax) , Ab(2*Nmax*Nmax) , Alf(Nalf) , As(Nmax*Nmax) ,&
...
!
                     IF ( tran ) THEN
                        CALL SMAKE('GE',' ',' ',ma,na,Ab,2*Nmax,Aa,lda, &
     &                             reset,ZERO)
                     ELSE
...
!
                     ...
                     lbb = laa
                     IF ( tran ) THEN
                        CALL SMAKE('GE',' ',' ',ma,na,Ab(k+1),2*Nmax,Bb,&
     &                             ldb,reset,ZERO)
                     ELSE
                        CALL SMAKE('GE',' ',' ',ma,na,Ab(k*Nmax+1),Nmax,&
     &                             Bb,ldb,reset,ZERO)
                     ENDIF
!
                     DO icu = 1 , 2
...
!*==smmch.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
