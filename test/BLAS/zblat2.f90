!*==zblat2.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
!> \brief \b ZBLAT2
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Local Arrays ..
     ...
     &           as(NMAX*NMAX) , bet(NBEMAX) , x(NMAX) , xs(NMAX*INCMAX)&
...
!     .. External Subroutines ..
      EXTERNAL ZCHK1 , ZCHK2 , ZCHK3 , ZCHK4 , ZCHK5 , ZCHK6 , ZCHKE ,  &
     &         ZMVCH
!     .. Intrinsic Functions ..
...
!     .. Data statements ..
      DATA snames/'ZGEMV ' , 'ZGBMV ' , 'ZHEMV ' , 'ZHBMV ' , 'ZHPMV ' ,&
     &     'ZTRMV ' , 'ZTBMV ' , 'ZTPMV ' , 'ZTRSV ' , 'ZTBSV ' ,       &
     &     'ZTPSV ' , 'ZGERC ' , 'ZGERU ' , 'ZHER  ' , 'ZHPR  ' ,       &
     &     'ZHER2 ' , 'ZHPR2 '/
...
!     the result computed by ZMVCH.
      trans = 'N'
      CALL ZMVCH(trans,n,n,ONE,a,NMAX,x,1,ZERO,y,1,yt,g,yy,eps,err,     &
     &           fatal,nout,.TRUE.)
      same = LZE(yy,yt,n)
...
!           Test ZHER2, 16, and ZHPR2, 17.
               CALL ZCHK6(snames(isnum),eps,thresh,nout,ntra,trace,rewi,&
     &                    fatal,nidim,idim,nalf,alf,ninc,inc,NMAX,      &
     &                    INCMAX,a,aa,as,x,xx,xs,y,yy,ys,yt,g,z)
            ELSE
...
99006 FORMAT (' ABSOLUTE VALUE OF INCX OR INCY IS 0 OR GREATER THAN ',  &
     &        I2)
99007 FORMAT (' TESTS OF THE COMPLEX*16       LEVEL 2 BLAS',//' THE F', &
...
99014 FORMAT (' SUBPROGRAM NAME ',A6,' NOT RECOGNIZED',/' ******* T',   &
     &        'ESTS ABANDONED *******')
99015 FORMAT (' ERROR IN ZMVCH -  IN-LINE DOT PRODUCTS ARE BEING EVALU',&
...
!
      END PROGRAM ZBLAT2
!*==zchk1.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!     .. Array Arguments ..
     ...
     &           As(Nmax*Nmax) , Bet(Nbet) , X(Nmax) , Xs(Nmax*Incmax) ,&
...
!
                  transl = ZERO
                  CALL ZMAKE(Sname(2:3),' ',' ',m,n,A,Nmax,Aa,lda,kl,ku,&
     &                       reset,transl)
!
                  DO ic = 1 , 3
                     trans = ich(ic:ic)
...
!
                        transl = HALF
                        CALL ZMAKE('GE',' ',' ',1,nl,X,1,Xx,ABS(incx),0,&
     &                             nl-1,reset,transl)
                        IF ( nl>1 ) THEN
                           X(nl/2) = ZERO
...
!
     ...
     &                                 alpha , lda , incx , beta , incy
                                    IF ( Rewi ) REWIND Ntra
                                    CALL ZGEMV(trans,m,n,alpha,Aa,lda,  &
     ...
     &                                 incy
                                    IF ( Rewi ) REWIND Ntra
                                    CALL ZGBMV(trans,m,n,kl,ku,alpha,Aa,&
...
!                                Check the result.
!
                                 CALL ZMVCH(trans,m,n,alpha,A,Nmax,X,   &
...
!*==zchk2.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!
                  transl = ZERO
                  CALL ZMAKE(Sname(2:3),uplo,' ',n,n,A,Nmax,Aa,lda,k,k, &
     &                       reset,transl)
!
                  DO ix = 1 , Ninc
                     incx = Inc(ix)
...
!
                     transl = HALF
                     CALL ZMAKE('GE',' ',' ',1,n,X,1,Xx,ABS(incx),0,n-1,&
     &                          reset,transl)
                     IF ( n>1 ) THEN
                        X(n/2) = ZERO
...
!
     ...
     &                                lda , incx , beta , incy
                                 IF ( Rewi ) REWIND Ntra
                                 CALL ZHEMV(uplo,n,alpha,Aa,lda,Xx,incx,&
     &                              beta,Yy,incy)
                              ELSEIF ( banded ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99006)    &
     &                                nc , Sname , uplo , n , k ,       &
     &                                alpha , lda , incx , beta , incy
                                 IF ( Rewi ) REWIND Ntra
                                 CALL ZHBMV(uplo,n,k,alpha,Aa,lda,Xx,   &
     &                              incx,beta,Yy,incy)
                              ELSEIF ( packed ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99005)    &
     &                                nc , Sname , uplo , n , alpha ,   &
...
!                             Check the result.
!
                              CALL ZMVCH('N',n,n,alpha,A,Nmax,X,incx,   &
...
!*==zchk3.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!     .. Scalar Arguments ..
      ...
      CHARACTER*6 Sname
!     .. Array Arguments ..
      COMPLEX*16 A(Nmax,Nmax) , Aa(Nmax*Nmax) , As(Nmax*Nmax) , X(Nmax) &
     &           , Xs(Nmax*Incmax) , Xt(Nmax) , Xx(Nmax*Incmax) ,       &
     &           Z(Nmax)
      DOUBLE PRECISION G(Nmax)
      INTEGER Idim(Nidim) , Inc(Ninc) , Kb(Nkb)
!     .. Local Scalars ..
...
!     .. External Functions ..
      ...
      EXTERNAL LZE , LZERES
!     .. External Subroutines ..
      EXTERNAL ZMAKE , ZMVCH , ZTBMV , ZTBSV , ZTPMV , ZTPSV , ZTRMV ,  &
     &         ZTRSV
!     .. Intrinsic Functions ..
      INTRINSIC ABS , MAX
!     .. Scalars in Common ..
...
!
                        transl = ZERO
                        CALL ZMAKE(Sname(2:3),uplo,diag,n,n,A,Nmax,Aa,  &
...
!
     ...
     &                                n , lda , incx
                                 IF ( Rewi ) REWIND Ntra
                                 CALL ZTRMV(uplo,trans,diag,n,Aa,lda,Xx,&
     &                              incx)
                              ELSEIF ( banded ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99006)    &
     &                                nc , Sname , uplo , trans , diag ,&
     &                                n , k , lda , incx
                                 IF ( Rewi ) REWIND Ntra
                                 CALL ZTBMV(uplo,trans,diag,n,k,Aa,lda, &
     &                              Xx,incx)
                              ELSEIF ( packed ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99005)    &
     &                                nc , Sname , uplo , trans , diag ,&
     ...
     &                                n , lda , incx
                                 IF ( Rewi ) REWIND Ntra
                                 CALL ZTRSV(uplo,trans,diag,n,Aa,lda,Xx,&
     &                              incx)
                              ELSEIF ( banded ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99006)    &
     &                                nc , Sname , uplo , trans , diag ,&
     &                                n , k , lda , incx
                                 IF ( Rewi ) REWIND Ntra
                                 CALL ZTBSV(uplo,trans,diag,n,k,Aa,lda, &
     &                              Xx,incx)
                              ELSEIF ( packed ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99005)    &
     &                                nc , Sname , uplo , trans , diag ,&
...
!                             Check the result.
!
                              CALL ZMVCH(trans,n,n,ONE,A,Nmax,X,incx,   &
...
!
                              ...
                              ENDDO
                              CALL ZMVCH(trans,n,n,ONE,A,Nmax,Z,incx,   &
...
!*==zchk4.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!
                  transl = HALF
                  CALL ZMAKE('GE',' ',' ',1,m,X,1,Xx,ABS(incx),0,m-1,   &
     &                       reset,transl)
                  IF ( m>1 ) THEN
                     X(m/2) = ZERO
...
!
                     transl = ZERO
                     CALL ZMAKE('GE',' ',' ',1,n,Y,1,Yy,ABS(incy),0,n-1,&
     &                          reset,transl)
                     IF ( n>1 ) THEN
                        Y(n/2) = ZERO
...
!
                        transl = ZERO
                        CALL ZMAKE(Sname(2:3),' ',' ',m,n,A,Nmax,Aa,lda,&
...
!
     ...
     &                       m , n , alpha , incx , incy , lda
                        IF ( conj ) THEN
                           IF ( Rewi ) REWIND Ntra
                           CALL ZGERC(m,n,alpha,Xx,incx,Yy,incy,Aa,lda)
                        ELSE
                           IF ( Rewi ) REWIND Ntra
                           CALL ZGERU(m,n,alpha,Xx,incx,Yy,incy,Aa,lda)
                        ENDIF
!
!                    Check if error-exit was taken incorrectly.
!
...
!
                        ...
                        DO j = 1 , n
                           ...
                           CALL ZMVCH('N',m,1,alpha,Z,Nmax,w,1,ONE,     &
...
!*==zchk5.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!
                     transl = ZERO
                     CALL ZMAKE(Sname(2:3),uplo,' ',n,n,A,Nmax,Aa,lda,  &
...
!
                        ...
                        DO j = 1 , n
                           ...
                           ELSE
                              ...
                              lj = n - j + 1
                           ENDIF
                           CALL ZMVCH('N',lj,1,alpha,Z(jj),lj,w,1,ONE,  &
...
!*==zchk6.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!
     ...
     &                          lda
                           IF ( Rewi ) REWIND Ntra
                           CALL ZHER2(uplo,n,alpha,Xx,incx,Yy,incy,Aa,  &
     &                                lda)
                        ELSEIF ( packed ) THEN
                           IF ( Trace ) WRITE (Ntra,FMT=99006) nc ,     &
     &                          Sname , uplo , n , alpha , incx , incy
                           IF ( Rewi ) REWIND Ntra
                           CALL ZHPR2(uplo,n,alpha,Xx,incx,Yy,incy,Aa)
                        ENDIF
!
!                    Check if error-exit was taken incorrectly.
!
...
!
                        ...
                        IF ( null ) THEN
                           isame(8) = LZE(As,Aa,laa)
                        ELSE
                           isame(8) = LZERES(Sname(2:3),uplo,n,n,As,Aa, &
     &                                lda)
                        ENDIF
...
!
                           ...
                           DO j = 1 , n
                              ...
                              ELSE
                                 ...
                                 lj = n - j + 1
                              ENDIF
                              CALL ZMVCH('N',lj,2,ONE,Z(jj,1),Nmax,w,1, &
...
!     End of ZCHK6.
!
      END SUBROUTINE ZCHK6
!*==zchke.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL CHKXER , ZGBMV , ZGEMV , ZGERC , ZGERU , ZHBMV , ZHEMV , &
     &         ZHER , ZHER2 , ZHPMV , ZHPR , ZHPR2 , ZTBMV , ZTBSV ,    &
     &         ZTPMV , ZTPSV , ZTRMV , ZTRSV
!     .. Common blocks ..
      COMMON /INFOC / INFot , NOUtc , OK , LERr
!     .. Executable Statements ..
...
!*==zmake.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
      SUBROUTINE ZMAKE(Type,Uplo,Diag,M,N,A,Nmax,Aa,Lda,Kl,Ku,Reset,    &
     &                 Transl)
      IMPLICIT NONE
!*--ZMAKE2622
!
...
!*==zmvch.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
      SUBROUTINE ZMVCH(Trans,M,N,Alpha,A,Nmax,X,Incx,Beta,Y,Incy,Yt,G,  &
...
!     End of ZMVCH.
!
      END SUBROUTINE ZMVCH
!*==lze.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!*==lzeres.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
...
!*==zbeg.f90  processed by SPAG 7.51RB at 17:52 on  4 Mar 2022
