!*==ctgsja.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b CTGSJA
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      INTEGER i , j , kcycle
      REAL a1 , a3 , b1 , b3 , csq , csu , csv , error , gamma , rwk ,  &
     &     ssmin
...
!
               ...
               ENDIF
!
               CALL CLAGS2(upper,a1,a2,a3,b1,b2,b3,csu,snu,csv,snv,csq, &
     &                     snq)
!
!              Update (K+I)-th and (K+J)-th rows of matrix A: U**H *A
!
               IF ( K+j<=M ) CALL CROT(L,A(K+j,N-L+1),Lda,A(K+i,N-L+1), &
     &                                 Lda,csu,CONJG(snu))
!
...
!
               IF ( wantu .AND. K+j<=M )                                &
     &              CALL CROT(M,U(1,K+j),1,U(1,K+i),1,csu,snu)
!
               IF ( wantv ) CALL CROT(P,V(1,j),1,V(1,i),1,csv,snv)
!
               IF ( wantq ) CALL CROT(N,Q(1,N-L+j),1,Q(1,N-L+i),1,csq,  &
     &                                snq)
!
            ENDDO
         ENDDO
...
!
            ...
            ELSE
               CALL CSSCAL(L-i+1,ONE/Beta(K+i),B(i,N-L+i),Ldb)
