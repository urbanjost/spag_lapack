!*==dsytrf_aa_2stage.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL XERBLA , DCOPY , DLACPY , DLASET , DGBTRF , DGEMM ,      &
     &         DGETRF , DSYGST , DSWAP , DTRSM
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MIN , MAX
!     ..
...
!                 H(I,J) = T(I,I)*U(I,J) + T(I,I+1)*U(I+1,J)
                  ...
                  ELSE
                     jb = 2*nb
                  ENDIF
                  CALL DGEMM('NoTranspose','NoTranspose',nb,kb,jb,ONE,  &
...
!
     ...
     &                  Tb(td+1+(j*nb)*ldtb),ldtb-1)
            IF ( j>1 ) THEN
!              T(J,J) = U(1:J,J)'*H(1:J)
               CALL DGEMM('Transpose','NoTranspose',kb,kb,(j-1)*nb,-ONE,&
     &                    A(1,j*nb+1),Lda,Work(nb+1),N,ONE,             &
     &                    Tb(td+1+(j*nb)*ldtb),ldtb-1)
!              T(J,J) += U(J,J)'*T(J,J-1)*U(J-1,J)
               CALL DGEMM('Transpose','NoTranspose',kb,nb,kb,ONE,       &
     &                    A((j-1)*nb+1,j*nb+1),Lda,                     &
     &                    Tb(td+nb+1+((j-1)*nb)*ldtb),ldtb-1,ZERO,      &
     &                    Work(1),N)
               CALL DGEMM('NoTranspose','NoTranspose',kb,kb,nb,-ONE,    &
     &                    Work(1),N,A((j-2)*nb+1,j*nb+1),Lda,ONE,       &
     &                    Tb(td+1+(j*nb)*ldtb),ldtb-1)
            ENDIF
            IF ( j>0 ) CALL DSYGST(1,'Upper',kb,Tb(td+1+(j*nb)*ldtb),   &
...
!
                  IF ( j==1 ) THEN
                     CALL DGEMM('NoTranspose','NoTranspose',kb,kb,kb,   &
     &                          ONE,Tb(td+1+(j*nb)*ldtb),ldtb-1,        &
     &                          A((j-1)*nb+1,j*nb+1),Lda,ZERO,          &
     &                          Work(j*nb+1),N)
                  ELSE
                     CALL DGEMM('NoTranspose','NoTranspose',kb,kb,nb+kb,&
...
!
                  CALL DGEMM('Transpose','NoTranspose',nb,N-(j+1)*nb,   &
...
!
               DO k = 1 , nb
                  CALL DCOPY(N-(j+1)*nb,A(j*nb+k,(j+1)*nb+1),Lda,       &
     &                       Work(1+(k-1)*N),1)
               ENDDO
!
!              Factorize panel
!
               CALL DGETRF(N-(j+1)*nb,nb,Work,N,Ipiv((j+1)*nb+1),iinfo)
...
!
               DO k = 1 , nb
                  CALL DCOPY(N-(j+1)*nb,Work(1+(k-1)*N),1,              &
...
!
     ...
     &                     Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
               CALL DLACPY('Upper',kb,nb,Work,N,Tb(td+nb+1+(j*nb)*ldtb),&
     &                     ldtb-1)
               IF ( j>0 ) CALL DTRSM('R','U','N','U',kb,nb,ONE,         &
     &                               A((j-1)*nb+1,j*nb+1),Lda,          &
     &                               Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
!
...
!
     ...
     &                  = Tb(td+nb+i-k+1+(j*nb+k-1)*ldtb)
                  ENDDO
               ENDDO
               CALL DLASET('Lower',kb,nb,ZERO,ONE,A(j*nb+1,(j+1)*nb+1), &
     &                     Lda)
!
...
!                    > Apply pivots to previous columns of L
                     CALL DSWAP(k-1,A((j+1)*nb+1,i1),1,A((j+1)*nb+1,i2),&
     &                          1)
!                    > Swap A(I1+1:M, I1) with A(I2, I1+1:M)
                     IF ( i2>(i1+1) )                                   &
     &                    CALL DSWAP(i2-i1-1,A(i1,i1+1),Lda,A(i1+1,i2), &
     &                    1)
...
!                    > Swap A(I1, I1) with A(I2, I2)
                     ...
                     A(i2,i2) = piv
!                    > Apply pivots to previous columns of L
                     IF ( j>0 ) CALL DSWAP(j*nb,A(1,i1),1,A(1,i2),1)
                  ENDIF
               ENDDO
            ENDIF
         ENDDO
...
!                  H(I,J) = T(I,I)*L(J,I)' + T(I+1,I)'*L(J,I+1)'
                  ...
                  ELSE
                     jb = 2*nb
                  ENDIF
                  CALL DGEMM('NoTranspose','Transpose',nb,kb,jb,ONE,    &
     &                       Tb(td+1+(i*nb)*ldtb),ldtb-1,               &
...
!
     ...
     &                  Tb(td+1+(j*nb)*ldtb),ldtb-1)
            IF ( j>1 ) THEN
!              T(J,J) = L(J,1:J)*H(1:J)
               CALL DGEMM('NoTranspose','NoTranspose',kb,kb,(j-1)*nb,   &
     &                    -ONE,A(j*nb+1,1),Lda,Work(nb+1),N,ONE,        &
     &                    Tb(td+1+(j*nb)*ldtb),ldtb-1)
!              T(J,J) += L(J,J)*T(J,J-1)*L(J,J-1)'
               CALL DGEMM('NoTranspose','NoTranspose',kb,nb,kb,ONE,     &
     &                    A(j*nb+1,(j-1)*nb+1),Lda,                     &
     &                    Tb(td+nb+1+((j-1)*nb)*ldtb),ldtb-1,ZERO,      &
     &                    Work(1),N)
               CALL DGEMM('NoTranspose','Transpose',kb,kb,nb,-ONE,      &
     &                    Work(1),N,A(j*nb+1,(j-2)*nb+1),Lda,ONE,       &
     &                    Tb(td+1+(j*nb)*ldtb),ldtb-1)
            ENDIF
            IF ( j>0 ) CALL DSYGST(1,'Lower',kb,Tb(td+1+(j*nb)*ldtb),   &
...
!
                  IF ( j==1 ) THEN
                     CALL DGEMM('NoTranspose','Transpose',kb,kb,kb,ONE, &
     &                          Tb(td+1+(j*nb)*ldtb),ldtb-1,            &
     &                          A(j*nb+1,(j-1)*nb+1),Lda,ZERO,          &
     &                          Work(j*nb+1),N)
                  ELSE
                     CALL DGEMM('NoTranspose','Transpose',kb,kb,nb+kb,  &
     &                          ONE,Tb(td+nb+1+((j-1)*nb)*ldtb),ldtb-1, &
...
!                 Update with the previous column
!
                  CALL DGEMM('NoTranspose','NoTranspose',N-(j+1)*nb,nb, &
...
!              Factorize panel
!
               CALL DGETRF(N-(j+1)*nb,nb,A((j+1)*nb+1,j*nb+1),Lda,      &
     &                     Ipiv((j+1)*nb+1),iinfo)
!               IF (IINFO.NE.0 .AND. INFO.EQ.0) THEN
!                  INFO = IINFO+(J+1)*NB
!               END IF
...
!
               ...
               CALL DLASET('Full',kb,nb,ZERO,ZERO,                      &
     &                     Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
               CALL DLACPY('Upper',kb,nb,A((j+1)*nb+1,j*nb+1),Lda,      &
     &                     Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
               IF ( j>0 ) CALL DTRSM('R','L','T','U',kb,nb,ONE,         &
     &                               A(j*nb+1,(j-1)*nb+1),Lda,          &
     &                               Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
!
...
!
     ...
     &                  = Tb(td+nb+i-k+1+(j*nb+k-1)*ldtb)
                  ENDDO
               ENDDO
               CALL DLASET('Upper',kb,nb,ZERO,ONE,A((j+1)*nb+1,j*nb+1), &
     &                     Lda)
!
!              Apply pivots to trailing submatrix of A
!
...
!                    > Swap A(I1+1:M, I1) with A(I2, I1+1:M)
                     IF ( i2>(i1+1) )                                   &
     &                    CALL DSWAP(i2-i1-1,A(i1+1,i1),1,A(i2,i1+1),   &
     &                    Lda)
!                    > Swap A(I2+1:M, I1) with A(I2+1:M, I2)
                     IF ( i2<N )                                        &
...
!                    > Swap A(I1, I1) with A(I2, I2)
                     ...
                     A(i2,i2) = piv
!                    > Apply pivots to previous columns of L
                     IF ( j>0 ) CALL DSWAP(j*nb,A(i1,1),Lda,A(i2,1),Lda)
                  ENDIF
               ENDDO
!
!              Apply pivots to previous columns of L
