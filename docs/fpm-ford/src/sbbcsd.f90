!*==sbbcsd.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b SBBCSD
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Local Scalars ..
     ...
     &        iv1tcs , iv1tsn , iv2tcs , iv2tsn , j , lworkmin ,        &
     &        lworkopt , maxit , mini
...
!
         IF ( mu<=nu ) THEN
            CALL SLARTGS(B11d(imin),B11e(imin),mu,Work(iv1tcs+imin-1),  &
     &                   Work(iv1tsn+imin-1))
         ELSE
            CALL SLARTGS(B21d(imin),B21e(imin),nu,Work(iv1tcs+imin-1),  &
     &                   Work(iv1tsn+imin-1))
         ENDIF
!
...
!
     ...
     &                   Work(iu1cs+imin-1),r)
         ELSEIF ( mu<=nu ) THEN
            CALL SLARTGS(B11e(imin),B11d(imin+1),mu,Work(iu1cs+imin-1), &
     &                   Work(iu1sn+imin-1))
         ELSE
            CALL SLARTGS(B12d(imin),B12e(imin),nu,Work(iu1cs+imin-1),   &
     &                   Work(iu1sn+imin-1))
         ENDIF
     ...
     &                   Work(iu2cs+imin-1),r)
         ELSEIF ( nu<mu ) THEN
            CALL SLARTGS(B21e(imin),B21d(imin+1),nu,Work(iu2cs+imin-1), &
     &                   Work(iu2sn+imin-1))
         ELSE
            CALL SLARTGS(B22d(imin),B22e(imin),mu,Work(iu2cs+imin-1),   &
     &                   Work(iu2sn+imin-1))
         ENDIF
...
!
            IF ( .NOT.restart11 .AND. .NOT.restart21 ) THEN
               CALL SLARTGP(x2,x1,Work(iv1tsn+i-1),Work(iv1tcs+i-1),r)
     ...
     &                      Work(iv1tsn+i-1))
            ...
            IF ( .NOT.restart12 .AND. .NOT.restart22 ) THEN
               CALL SLARTGP(y2,y1,Work(iv2tsn+i-1-1),Work(iv2tcs+i-1-1),&
     &                      r)
...
!
            IF ( .NOT.restart11 .AND. .NOT.restart12 ) THEN
               CALL SLARTGP(x2,x1,Work(iu1sn+i-1),Work(iu1cs+i-1),r)
     ...
     &                      Work(iu1sn+i-1))
            ...
            IF ( .NOT.restart21 .AND. .NOT.restart22 ) THEN
               CALL SLARTGP(y2,y1,Work(iu2sn+i-1),Work(iu2cs+i-1),r)
...
!
         IF ( wantu1 ) THEN
            IF ( colmajor ) THEN
               CALL SLASR('R','V','F',P,imax-imin+1,Work(iu1cs+imin-1), &
     &                    Work(iu1sn+imin-1),U1(1,imin),Ldu1)
            ELSE
               CALL SLASR('L','V','F',imax-imin+1,P,Work(iu1cs+imin-1), &
     &                    Work(iu1sn+imin-1),U1(imin,1),Ldu1)
            ENDIF
         ENDIF
         IF ( wantu2 ) THEN
            IF ( colmajor ) THEN
               CALL SLASR('R','V','F',M-P,imax-imin+1,Work(iu2cs+imin-1)&
     &                    ,Work(iu2sn+imin-1),U2(1,imin),Ldu2)
            ELSE
               CALL SLASR('L','V','F',imax-imin+1,M-P,Work(iu2cs+imin-1)&
     &                    ,Work(iu2sn+imin-1),U2(imin,1),Ldu2)
            ENDIF
         ENDIF
         IF ( wantv1t ) THEN
            IF ( colmajor ) THEN
               CALL SLASR('L','V','F',imax-imin+1,Q,Work(iv1tcs+imin-1),&
     &                    Work(iv1tsn+imin-1),V1t(imin,1),Ldv1t)
            ELSE
               CALL SLASR('R','V','F',Q,imax-imin+1,Work(iv1tcs+imin-1),&
...
!
         IF ( mini/=i ) THEN
            ...
            IF ( colmajor ) THEN
               ...
               IF ( wantv1t ) CALL SSWAP(Q,V1t(i,1),Ldv1t,V1t(mini,1),  &
     &              Ldv1t)
               IF ( wantv2t ) CALL SSWAP(M-Q,V2t(i,1),Ldv2t,V2t(mini,1),&
     &              Ldv2t)
            ELSE
               IF ( wantu1 ) CALL SSWAP(P,U1(i,1),Ldu1,U1(mini,1),Ldu1)
               IF ( wantu2 ) CALL SSWAP(M-P,U2(i,1),Ldu2,U2(mini,1),    &
     &                                  Ldu2)
