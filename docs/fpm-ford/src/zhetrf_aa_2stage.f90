!*==zhetrf_aa_2stage.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL XERBLA , ZCOPY , ZLACGV , ZLACPY , ZLASET , ZGBTRF ,     &
     &         ZGEMM , ZGETRF , ZHEGST , ZSWAP , ZTRSM
!     ..
!     .. Intrinsic Functions ..
...
!                  H(I,J) = T(I,I)*U(I,J) + T(I+1,I)*U(I+1,J)
                  ...
                  ELSE
                     jb = 2*nb
                  ENDIF
                  CALL ZGEMM('NoTranspose','NoTranspose',nb,kb,jb,ONE,  &
...
!
     ...
     &                  Tb(td+1+(j*nb)*ldtb),ldtb-1)
            IF ( j>1 ) THEN
!              T(J,J) = U(1:J,J)'*H(1:J)
               CALL ZGEMM('Conjugate transpose','NoTranspose',kb,kb,    &
     &                    (j-1)*nb,-ONE,A(1,j*nb+1),Lda,Work(nb+1),N,   &
     &                    ONE,Tb(td+1+(j*nb)*ldtb),ldtb-1)
!              T(J,J) += U(J,J)'*T(J,J-1)*U(J-1,J)
               CALL ZGEMM('Conjugate transpose','NoTranspose',kb,nb,kb, &
     &                    ONE,A((j-1)*nb+1,j*nb+1),Lda,                 &
     &                    Tb(td+nb+1+((j-1)*nb)*ldtb),ldtb-1,ZERO,      &
     &                    Work(1),N)
               CALL ZGEMM('NoTranspose','NoTranspose',kb,kb,nb,-ONE,    &
     &                    Work(1),N,A((j-2)*nb+1,j*nb+1),Lda,ONE,       &
     &                    Tb(td+1+(j*nb)*ldtb),ldtb-1)
            ENDIF
            IF ( j>0 ) CALL ZHEGST(1,'Upper',kb,Tb(td+1+(j*nb)*ldtb),   &
...
!
                  CALL ZGEMM('Conjugate transpose','NoTranspose',nb,    &
...
!                  Copy only L-factor
!
                  CALL ZCOPY(N-k-(j+1)*nb,Work(k+1+(k-1)*N),1,          &
...
!
     ...
     &                     ldtb-1)
               IF ( j>0 ) CALL ZTRSM('R','U','N','U',kb,nb,ONE,         &
...
!
     ...
     &                  = DCONJG(Tb(td+nb+i-k+1+(j*nb+k-1)*ldtb))
                  ENDDO
               ENDDO
               CALL ZLASET('Lower',kb,nb,ZERO,ONE,A(j*nb+1,(j+1)*nb+1), &
     &                     Lda)
!
...
!                  H(I,J) = T(I,I)*L(J,I)' + T(I+1,I)'*L(J,I+1)'
                  ...
                  ELSE
                     jb = 2*nb
                  ENDIF
                  CALL ZGEMM('NoTranspose','Conjugate transpose',nb,kb, &
...
!              T(J,J) += L(J,J)*T(J,J-1)*L(J,J-1)'
               CALL ZGEMM('NoTranspose','NoTranspose',kb,nb,kb,ONE,     &
     &                    A(j*nb+1,(j-1)*nb+1),Lda,                     &
     &                    Tb(td+nb+1+((j-1)*nb)*ldtb),ldtb-1,ZERO,      &
     &                    Work(1),N)
               CALL ZGEMM('NoTranspose','Conjugate transpose',kb,kb,nb, &
     &                    -ONE,Work(1),N,A(j*nb+1,(j-2)*nb+1),Lda,ONE,  &
     &                    Tb(td+1+(j*nb)*ldtb),ldtb-1)
            ENDIF
            IF ( j>0 ) CALL ZHEGST(1,'Lower',kb,Tb(td+1+(j*nb)*ldtb),   &
...
!
                  IF ( j==1 ) THEN
                     CALL ZGEMM('NoTranspose','Conjugate transpose',kb, &
...
!
     ...
     &                     Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
               IF ( j>0 ) CALL ZTRSM('R','L','C','U',kb,nb,ONE,         &
     &                               A(j*nb+1,(j-1)*nb+1),Lda,          &
     &                               Tb(td+nb+1+(j*nb)*ldtb),ldtb-1)
!
...
!
     ...
     &                  = DCONJG(Tb(td+nb+i-k+1+(j*nb+k-1)*ldtb))
                  ENDDO
               ENDDO
               CALL ZLASET('Upper',kb,nb,ZERO,ONE,A((j+1)*nb+1,j*nb+1), &
     &                     Lda)
!
!              Apply pivots to trailing submatrix of A
!
