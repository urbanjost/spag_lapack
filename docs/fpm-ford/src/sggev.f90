!*==sggev.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Local Scalars ..
     ...
     &        iright , irows , itau , iwrk , jc , jr , maxwrk , minwrk
...
!
      IF ( Info==0 ) THEN
         minwrk = MAX(1,8*N)
         maxwrk = MAX(1,N*(7+ILAENV(1,'SGEQRF',' ',N,1,N,0)))
         maxwrk = MAX(maxwrk,N*(7+ILAENV(1,'SORMQR',' ',N,1,N,0)))
         IF ( ilvl ) maxwrk = MAX(maxwrk,N*(7+ILAENV(1,'SORGQR',' ',N,1,&
     &                        N,-1)))
         Work(1) = maxwrk
!
