!*==zgebd2.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
            IF ( i<N ) CALL ZLARF('Left',M-i+1,N-i,A(i,i),1,            &
...
!
               CALL ZLARF('Right',M-i,N-i,A(i,i+1),Lda,Taup(i),         &
     &                    A(i+1,i+1),Lda,Work)
               CALL ZLACGV(N-i,A(i,i+1),Lda)
               A(i,i+1) = E(i)
            ELSE
...
!
            IF ( i<M ) CALL ZLARF('Right',M-i,N-i+1,A(i,i),Lda,Taup(i), &
     &                            A(i+1,i),Lda,Work)
            CALL ZLACGV(N-i+1,A(i,i),Lda)
            A(i,i) = D(i)
!
...
!
               CALL ZLARF('Left',M-i,N-i,A(i+1,i),1,DCONJG(Tauq(i)),    &
     &                    A(i+1,i+1),Lda,Work)
               A(i+1,i) = E(i)
            ELSE
               Tauq(i) = ZERO
