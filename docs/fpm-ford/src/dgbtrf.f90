!*==dgbtrf.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b DGBTRF
!
!  =========== DOCUMENTATION ===========
!
...
!     .. External Subroutines ..
      EXTERNAL DCOPY , DGBTF2 , DGEMM , DGER , DLASWP , DSCAL , DSWAP , &
     &         DTRSM , XERBLA
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MAX , MIN
...
!
                  jm = MIN(ju,j+jb-1)
                  IF ( jm>jj ) CALL DGER(km,jm-jj,-ONE,Ab(kv+2,jj),1,   &
...
!
               nw = MIN(jj-j+1,i3)
               IF ( nw>0 ) CALL DCOPY(nw,Ab(kv+Kl+1-jj+j,jj),1,         &
     &                                work31(1,jj-j+1),1)
            ENDDO
            IF ( j+jb<=N ) THEN
!
...
!                    Update A22
!
                  IF ( i2>0 ) CALL DGEMM('No transpose','No transpose', &
...
!                    Update A32
!
                  IF ( i3>0 ) CALL DGEMM('No transpose','No transpose', &
...
!
               nw = MIN(i3,jj-j+1)
               IF ( nw>0 ) CALL DCOPY(nw,work31(1,jj-j+1),1,Ab(kv+Kl+1- &
     &                                jj+j,jj),1)
            ENDDO
         ENDDO
      ENDIF
