!*==zbbcsd.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b ZBBCSD
!
!  =========== DOCUMENTATION ===========
!
...
!     ..
!     .. Array Arguments ..
      DOUBLE PRECISION B11d(*) , B11e(*) , B12d(*) , B12e(*) , B21d(*) ,&
     &                 B21e(*) , B22d(*) , B22e(*) , Phi(*) , Theta(*) ,&
     &                 Rwork(*)
...
!     .. Local Scalars ..
     ...
     &        lrworkopt , maxit , mini
      DOUBLE PRECISION b11bulge , b12bulge , b21bulge , b22bulge ,      &
...
!
         IF ( mu<=nu ) THEN
            CALL DLARTGS(B11d(imin),B11e(imin),mu,Rwork(iv1tcs+imin-1), &
     &                   Rwork(iv1tsn+imin-1))
         ELSE
            CALL DLARTGS(B21d(imin),B21e(imin),nu,Rwork(iv1tcs+imin-1), &
     &                   Rwork(iv1tsn+imin-1))
         ENDIF
!
...
!
     ...
     &                   Rwork(iu1cs+imin-1),r)
         ELSEIF ( mu<=nu ) THEN
            CALL DLARTGS(B11e(imin),B11d(imin+1),mu,Rwork(iu1cs+imin-1),&
     &                   Rwork(iu1sn+imin-1))
         ELSE
            CALL DLARTGS(B12d(imin),B12e(imin),nu,Rwork(iu1cs+imin-1),  &
     &                   Rwork(iu1sn+imin-1))
         ENDIF
     ...
     &                   Rwork(iu2cs+imin-1),r)
         ELSEIF ( nu<mu ) THEN
            CALL DLARTGS(B21e(imin),B21d(imin+1),nu,Rwork(iu2cs+imin-1),&
     &                   Rwork(iu2sn+imin-1))
         ELSE
            CALL DLARTGS(B22d(imin),B22e(imin),mu,Rwork(iu2cs+imin-1),  &
     &                   Rwork(iu2sn+imin-1))
         ENDIF
...
!
     ...
     &                      Rwork(iv2tcs+i-1-1),r)
            ELSEIF ( nu<mu ) THEN
               CALL DLARTGS(B12e(i-1),B12d(i),nu,Rwork(iv2tcs+i-1-1),   &
     &                      Rwork(iv2tsn+i-1-1))
            ELSE
               CALL DLARTGS(B22e(i-1),B22d(i),mu,Rwork(iv2tcs+i-1-1),   &
     &                      Rwork(iv2tsn+i-1-1))
            ENDIF
!
...
!
     ...
     &                   Rwork(iv2tcs+imax-1-1),r)
         ELSEIF ( .NOT.restart12 .AND. restart22 ) THEN
            CALL DLARTGP(b12bulge,B12d(imax-1),Rwork(iv2tsn+imax-1-1),  &
     &                   Rwork(iv2tcs+imax-1-1),r)
         ELSEIF ( restart12 .AND. .NOT.restart22 ) THEN
            CALL DLARTGP(b22bulge,B22d(imax-1),Rwork(iv2tsn+imax-1-1),  &
     &                   Rwork(iv2tcs+imax-1-1),r)
         ELSEIF ( nu<mu ) THEN
...
!
         IF ( wantu1 ) THEN
            IF ( colmajor ) THEN
               CALL ZLASR('R','V','F',P,imax-imin+1,Rwork(iu1cs+imin-1),&
     &                    Rwork(iu1sn+imin-1),U1(1,imin),Ldu1)
            ELSE
               CALL ZLASR('L','V','F',imax-imin+1,P,Rwork(iu1cs+imin-1),&
     ...
     &                    U2(imin,1),Ldu2)
         ...
         IF ( wantv1t ) THEN
            IF ( colmajor ) THEN
               CALL ZLASR('L','V','F',imax-imin+1,Q,Rwork(iv1tcs+imin-1)&
     &                    ,Rwork(iv1tsn+imin-1),V1t(imin,1),Ldv1t)
            ELSE
               CALL ZLASR('R','V','F',Q,imax-imin+1,Rwork(iv1tcs+imin-1)&
...
!
         IF ( mini/=i ) THEN
            ...
            Theta(i) = thetamin
            IF ( colmajor ) THEN
               IF ( wantu1 ) CALL ZSWAP(P,U1(1,i),1,U1(1,mini),1)
               IF ( wantu2 ) CALL ZSWAP(M-P,U2(1,i),1,U2(1,mini),1)
               IF ( wantv1t ) CALL ZSWAP(Q,V1t(i,1),Ldv1t,V1t(mini,1),  &
     &              Ldv1t)
               IF ( wantv2t ) CALL ZSWAP(M-Q,V2t(i,1),Ldv2t,V2t(mini,1),&
     &              Ldv2t)
            ELSE
               IF ( wantu1 ) CALL ZSWAP(P,U1(i,1),Ldu1,U1(mini,1),Ldu1)
               IF ( wantu2 ) CALL ZSWAP(M-P,U2(i,1),Ldu2,U2(mini,1),    &
     &                                  Ldu2)
               IF ( wantv1t ) CALL ZSWAP(Q,V1t(1,i),1,V1t(1,mini),1)
               IF ( wantv2t ) CALL ZSWAP(M-Q,V2t(1,i),1,V2t(1,mini),1)
            ENDIF
         ENDIF
!
      ENDDO
