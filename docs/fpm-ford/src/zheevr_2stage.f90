!*==zheevr_2stage.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Local Scalars ..
     ...
     &        liwmin , llwork , llrwork , llwrkn , lrwmin , lwmin ,     &
     &        nsplit , lhtrd , lwtrd , kd , ib , indhous
...
!
      IF ( N==1 ) THEN
         ...
         IF ( alleig .OR. indeig ) THEN
            M = 1
            W(1) = DBLE(A(1,1))
         ELSEIF ( Vl<DBLE(A(1,1)) .AND. Vu>=DBLE(A(1,1)) ) THEN
            M = 1
            W(1) = DBLE(A(1,1))
         ENDIF
         IF ( wantz ) THEN
...
!
      CALL ZHETRD_2STAGE(Jobz,Uplo,N,A,Lda,Rwork(indrd),Rwork(indre),   &
...
!
            ...
            ELSE
               tryrac = .FALSE.
            ENDIF
            CALL ZSTEMR(Jobz,'A',N,Rwork(indrdd),Rwork(indree),Vl,Vu,Il,&
...
!
      ...
      CALL DSTEBZ(Range,order,N,vll,vuu,Il,Iu,abstll,Rwork(indrd),      &
     &            Rwork(indre),M,nsplit,W,Iwork(indibl),Iwork(indisp),  &
     &            Rwork(indrwk),Iwork(indiwo),Info)
!
      IF ( wantz ) THEN
         CALL ZSTEIN(N,Rwork(indrd),Rwork(indre),M,W,Iwork(indibl),     &
