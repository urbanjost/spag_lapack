!*==slarfb_gett.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b SLARFB_GETT
!
!  =========== DOCUMENTATION ===========
!
...
!
!
         IF ( lnotident ) CALL STRMM('L','L','T','U',K,N-K,ONE,A,Lda,   &
     &                               Work,Ldwork)
!
!        col2_(3) Compute W2: = W2 + (V2**T) * B2 = W2 + (B1**T) * B2
!        V2 stored in B1.
!
         IF ( M>0 ) CALL SGEMM('T','N',K,N-K,M,ONE,B,Ldb,B(1,K+1),Ldb,  &
     &                         ONE,Work,Ldwork)
!
...
!        V2 stored in B1.
!
         IF ( M>0 ) CALL SGEMM('N','N',M,N-K,K,-ONE,B,Ldb,Work,Ldwork,  &
     &                         ONE,B(1,K+1),Ldb)
!
!
...
!           V1 stored in A1 (diagonal ones are not stored).
!
         IF ( lnotident ) CALL STRMM('L','L','N','U',K,N-K,ONE,A,Lda,   &
     &                               Work,Ldwork)
!
...
!
      IF ( lnotident ) CALL STRMM('L','L','T','U',K,K,ONE,A,Lda,Work,   &
     &                            Ldwork)
!
...
!
      IF ( M>0 ) CALL STRMM('R','U','N','N',M,K,-ONE,Work,Ldwork,B,Ldb)
!
      IF ( lnotident ) THEN
!
