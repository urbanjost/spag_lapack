!*==csysvxx.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL CSYEQUB , CSYTRF , CSYTRS , CLACPY , CLAQSY , XERBLA ,   &
     &         CLASCL2 , CSYRFSX
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MAX , MIN
...
!
            IF ( N>0 ) Rpvgrw = CLA_SYRPVGRW(Uplo,N,Info,A,Lda,Af,Ldaf, &
     &                          Ipiv,Rwork)
            RETURN
         ENDIF
      ENDIF
...
!
      IF ( N>0 ) Rpvgrw = CLA_SYRPVGRW(Uplo,N,Info,A,Lda,Af,Ldaf,Ipiv,  &
     &                    Rwork)
!
...
!
      CALL CSYRFSX(Uplo,Equed,N,Nrhs,A,Lda,Af,Ldaf,Ipiv,S,B,Ldb,X,Ldx,  &
