!*==zlarzb.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
         IF ( L>0 ) CALL ZGEMM('Transpose','Conjugate transpose',N,K,L, &
...
!
         IF ( L>0 ) CALL ZGEMM('Transpose','Transpose',L,N,K,-ONE,V,Ldv,&
...
!
         IF ( L>0 ) CALL ZGEMM('No transpose','Transpose',M,K,L,ONE,    &
...
!
         ...
         ENDDO
         CALL ZTRMM('Right','Lower',Trans,'Non-unit',M,K,ONE,T,Ldt,Work,&
     &              Ldwork)
         DO j = 1 , K
            CALL ZLACGV(K-j+1,T(j,j),1)
         ENDDO
...
!
         ...
         ENDDO
         IF ( L>0 ) CALL ZGEMM('No transpose','No transpose',M,L,K,-ONE,&
