!*==strtri.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b STRTRI
!
!  =========== DOCUMENTATION ===========
!
...
!
            CALL STRMM('Left','Upper','No transpose',Diag,j-1,jb,ONE,A, &
     &                 Lda,A(1,j),Lda)
            CALL STRSM('Right','Upper','No transpose',Diag,j-1,jb,-ONE, &
     &                 A(j,j),Lda,A(1,j),Lda)
!
...
!
               CALL STRMM('Left','Lower','No transpose',Diag,N-j-jb+1,  &
     &                    jb,ONE,A(j+jb,j+jb),Lda,A(j+jb,j),Lda)
               CALL STRSM('Right','Lower','No transpose',Diag,N-j-jb+1, &
