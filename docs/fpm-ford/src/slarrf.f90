!*==slarrf.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     ..
!     .. Array Arguments ..
      REAL D(*) , Dplus(*) , L(*) , Ld(*) , Lplus(*) , W(*) , Wgap(*) , &
     &     Werr(*) , Work(*)
!     ..
!
!  =====================================================================
...
!     .. Parameters ..
      ...
      PARAMETER (ONE=1.0E0,TWO=2.0E0,QUART=0.25E0,MAXGROWTH1=8.E0,      &
     &           MAXGROWTH2=8.E0)
!     ..
!     .. Local Scalars ..
      ...
      REAL avgap , bestshift , clwdth , eps , fact , fail , fail2 ,     &
     &     growthbound , ldelta , ldmax , lsigma , max1 , max2 ,        &
     &     mingap , oldp , prod , rdelta , rdmax , rrr1 , rrr2 ,        &
     &     rsigma , s , smlgrowth , tmp , znm2
!     ..
!     .. External Functions ..
      LOGICAL SISNAN
      REAL SLAMCH
...
!        both MAX1 and MAX2 are NaN
      IF ( sawnan1 .AND. sawnan2 ) GOTO 200
      IF ( .NOT.sawnan1 ) THEN
         indx = 1
...
!     Moreover, we use the refined RRR test only for isolated clusters.
      IF ( (clwdth<mingap/REAL(128)) .AND. (MIN(max1,max2)<fail2) .AND. &
     &     (.NOT.sawnan1) .AND. (.NOT.sawnan2) ) THEN
         dorrr1 = .TRUE.
      ELSE
         dorrr1 = .FALSE.
      ENDIF
      ...
      IF ( tryrrr1 .AND. dorrr1 ) THEN
         IF ( indx==1 ) THEN
            ...
            DO i = N - 1 , 1 , -1
               IF ( prod<=eps ) THEN
                  prod = ((Dplus(i+1)*Work(N+i+1))/(Dplus(i)*Work(N+i)))&
     &                   *oldp
               ELSE
