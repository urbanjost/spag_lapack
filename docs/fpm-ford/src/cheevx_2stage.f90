!*==cheevx_2stage.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. Local Scalars ..
     ...
     &        jj , llwork , nsplit , lwmin , lhtrd , lwtrd , kd , ib ,  &
     &        indhous
...
!
      IF ( N==1 ) THEN
         IF ( alleig .OR. indeig ) THEN
            ...
            W(1) = REAL(A(1,1))
         ELSEIF ( valeig ) THEN
            IF ( Vl<REAL(A(1,1)) .AND. Vu>=REAL(A(1,1)) ) THEN
               M = 1
               W(1) = REAL(A(1,1))
            ENDIF
         ENDIF
...
!
      CALL CHETRD_2STAGE(Jobz,Uplo,N,A,Lda,Rwork(indd),Rwork(inde),     &
...
!
      ...
      IF ( (alleig .OR. test) .AND. (Abstol<=ZERO) ) THEN
         ...
         ELSE
            ...
            CALL CUNGTR(Uplo,N,Z,Ldz,Work(indtau),Work(indwrk),llwork,  &
     &                  iinfo)
            CALL SCOPY(N-1,Rwork(inde),1,Rwork(indee),1)
