!*==ssytrf_rook.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b SSYTRF_ROOK
!
!  =========== DOCUMENTATION ===========
!
...
!
               CALL SLASYF_ROOK(Uplo,k,nb,kb,A,Lda,Ipiv,Work,ldwork,    &
     &                          iinfo)
            ELSE
!
...
!
               CALL SLASYF_ROOK(Uplo,N-k+1,nb,kb,A(k,k),Lda,Ipiv(k),    &
     &                          Work,ldwork,iinfo)
            ELSE
!
!           Use unblocked code to factorize columns k:n of A
!
               CALL SSYTF2_ROOK(Uplo,N-k+1,A(k,k),Lda,Ipiv(k),iinfo)
               kb = N - k + 1
            ENDIF
!
!        Set INFO on the first occurrence of a zero pivot
