!*==dlahr2.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
      EXTERNAL DAXPY , DCOPY , DGEMM , DGEMV , DLACPY , DLARFG , DSCAL ,&
     &         DTRMM , DTRMV
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC MIN
...
!
     ...
     &              ZERO,T(1,i),1)
         CALL DGEMV('NO TRANSPOSE',N-K,i-1,-ONE,Y(K+1,1),Ldy,T(1,i),1,  &
     &              ONE,Y(K+1,i),1)
...
!
      ...
      CALL DTRMM('RIGHT','Lower','NO TRANSPOSE','UNIT',K,Nb,ONE,A(K+1,1)&
     &           ,Lda,Y,Ldy)
      IF ( N>K+Nb ) CALL DGEMM('NO TRANSPOSE','NO TRANSPOSE',K,Nb,      &
     &                         N-K-Nb,ONE,A(1,2+Nb),Lda,A(K+1+Nb,1),Lda,&
     &                         ONE,Y,Ldy)
      CALL DTRMM('RIGHT','Upper','NO TRANSPOSE','NON-UNIT',K,Nb,ONE,T,  &
     &           Ldt,Y,Ldy)
!
!
!     End of DLAHR2
