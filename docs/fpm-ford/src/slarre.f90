!*==slarre.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
!     .. Parameters ..
      REAL FAC , FOUR , FOURTH , FUDGE , HALF , HNDRD , MAXGROWTH ,     &
     &     ONE , PERT , TWO , ZERO
      PARAMETER (ZERO=0.0E0,ONE=1.0E0,TWO=2.0E0,FOUR=4.0E0,             &
     &           HNDRD=100.0E0,PERT=4.0E0,HALF=ONE/TWO,FOURTH=ONE/FOUR, &
     &           FAC=HALF,MAXGROWTH=64.0E0,FUDGE=2.0E0)
...
!     .. Local Scalars ..
     ...
     &        in , indl , indu , irange , j , jblk , mb , mm , wbegin , &
     &        wend
      REAL avgap , bsrtol , clwdth , dmax , dpivot , eabs , emax ,      &
     &     eold , eps , gl , gu , isleft , isrght , rtl , rtol , s1 ,   &
     &     s2 , safmin , sgndef , sigma , spdiam , tau , tmp , tmp1
...
!        SLARRD needs a WORK of size 4*N, IWORK of size 3*N
         CALL SLARRD(Range,'B',N,Vl,Vu,Il,Iu,Gers,bsrtol,D,E,E2,Pivmin, &
...
!           Find approximations to the extremal eigenvalues of the block
            CALL SLARRK(in,1,gl,gu,D(ibegin),E2(ibegin),Pivmin,rtl,tmp, &
     &                  tmp1,iinfo)
            IF ( iinfo/=0 ) THEN
               Info = -1
               RETURN
            ...
            CALL SLARRK(in,in,gl,gu,D(ibegin),E2(ibegin),Pivmin,rtl,tmp,&
     &                  tmp1,iinfo)
            IF ( iinfo/=0 ) THEN
               Info = -1
               RETURN
...
!        Compute the negcount at the 1/4 and 3/4 points
         IF ( mb>1 ) CALL SLARRC('T',in,s1,s2,D(ibegin),E(ibegin),      &
...
!           use bisection to find EV from INDL to INDU
            CALL SLARRB(in,D(ibegin),Work(ibegin),indl,indu,Rtol1,Rtol2,&
