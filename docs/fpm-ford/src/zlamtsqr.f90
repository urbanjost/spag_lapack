!*==zlamtsqr.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
!> \brief \b ZLAMTSQR
!
!  Definition:
!  ===========
...
!
         ...
         IF ( kk>0 ) THEN
            ii = M - kk + 1
            CALL ZTPMQRT('L','N',kk,N,K,0,Nb,A(ii,1),Lda,T(1,ctr*K+1),  &
...
!
            ctr = ctr - 1
            CALL ZTPMQRT('L','N',Mb-K,N,K,0,Nb,A(i,1),Lda,T(1,ctr*K+1), &
...
!
         CALL ZGEMQRT('L','N',Mb,N,K,Nb,A(1,1),Lda,T,Ldt,C(1,1),Ldc,    &
     &                Work,Info)
!
...
!
         ...
         ii = M - kk + 1
         ctr = 1
         CALL ZGEMQRT('L','C',Mb,N,K,Nb,A(1,1),Lda,T,Ldt,C(1,1),Ldc,    &
     &                Work,Info)
!
...
!         Multiply Q to the current block of C (I:I+MB,1:N)
!
            CALL ZTPMQRT('L','C',Mb-K,N,K,0,Nb,A(i,1),Lda,T(1,ctr*K+1), &
     &                   Ldt,C(1,1),Ldc,C(i,1),Ldc,Work,Info)
            ctr = ctr + 1
!
         ENDDO
...
!
!
         IF ( ii<=M ) CALL ZTPMQRT('L','C',kk,N,K,0,Nb,A(ii,1),Lda,     &
...
!
         ...
         IF ( kk>0 ) THEN
            ii = N - kk + 1
            CALL ZTPMQRT('R','C',M,kk,K,0,Nb,A(ii,1),Lda,T(1,ctr*K+1),  &
...
!
            ctr = ctr - 1
            CALL ZTPMQRT('R','C',M,Mb-K,K,0,Nb,A(i,1),Lda,T(1,ctr*K+1), &
...
!
         CALL ZGEMQRT('R','C',M,Mb,K,Nb,A(1,1),Lda,T,Ldt,C(1,1),Ldc,    &
     &                Work,Info)
!
...
!
         ...
         ii = N - kk + 1
         ctr = 1
         CALL ZGEMQRT('R','N',M,Mb,K,Nb,A(1,1),Lda,T,Ldt,C(1,1),Ldc,    &
     &                Work,Info)
!
         DO i = Mb + 1 , ii - Mb + K , (Mb-K)
!
!         Multiply Q to the current block of C (1:M,I:I+MB)
!
            CALL ZTPMQRT('R','N',M,Mb-K,K,0,Nb,A(i,1),Lda,T(1,ctr*K+1), &
     &                   Ldt,C(1,1),Ldc,C(1,i),Ldc,Work,Info)
            ctr = ctr + 1
!
         ENDDO
...
!
!
         IF ( ii<=N ) CALL ZTPMQRT('R','N',M,kk,K,0,Nb,A(ii,1),Lda,     &
