!*==dtplqt2.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
            DO j = 1 , M - i
               T(M,j) = (A(i+j,i))
            ENDDO
            CALL DGEMV('N',M-i,p,ONE,B(i+1,1),Ldb,B(i,1),Ldb,ONE,T(M,1),&
     &                 Ldt)
!
...
!
         CALL DGEMV('N',i-1-p,L,alpha,B(mp,np),Ldb,B(i,np),Ldb,ZERO,    &
     &              T(i,mp),Ldt)
!
!        B1
!
