!*==dorbdb3.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
      IF ( M<0 ) THEN
         Info = -1
      ELSEIF ( 2*P<M .OR. P>M ) THEN
         Info = -2
      ELSEIF ( Q<M-P .OR. M-Q<M-P ) THEN
         Info = -3
...
!
      DO i = 1 , M - P
!
         IF ( i>1 ) CALL DROT(Q-i+1,X11(i-1,i),Ldx11,X21(i,i),Ldx11,c,s)
!
         CALL DLARFGP(Q-i+1,X21(i,i),X21(i,i+1),Ldx21,Tauq1(i))
         s = X21(i,i)
         X21(i,i) = ONE
         CALL DLARF('R',P-i+1,Q-i+1,X21(i,i),Ldx21,Tauq1(i),X11(i,i),   &
     &              Ldx11,Work(ilarf))
         CALL DLARF('R',M-P-i,Q-i+1,X21(i,i),Ldx21,Tauq1(i),X21(i+1,i), &
     &              Ldx21,Work(ilarf))
         c = SQRT(DNRM2(P-i+1,X11(i,i),1)**2+DNRM2(M-P-i,X21(i+1,i),1)  &
     &       **2)
         Theta(i) = ATAN2(s,c)
!
         CALL DORBDB5(P-i+1,M-P-i,Q-i,X11(i,i),1,X21(i+1,i),1,X11(i,i+1)&
     &                ,Ldx11,X21(i+1,i+1),Ldx21,Work(iorbdb5),lorbdb5,  &
     &                childinfo)
         ...
         IF ( i<M-P ) THEN
            ...
            s = SIN(Phi(i))
            X21(i+1,i) = ONE
            CALL DLARF('L',M-P-i,Q-i,X21(i+1,i),1,Taup2(i),X21(i+1,i+1),&
     &                 Ldx21,Work(ilarf))
         ENDIF
         X11(i,i) = ONE
         CALL DLARF('L',P-i+1,Q-i,X11(i,i),1,Taup1(i),X11(i,i+1),Ldx11, &
     &              Work(ilarf))
!
      ENDDO
!
