!*==cla_porfsx_extended.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!     .. External Subroutines ..
     ...
     &         BLAS_CHEMV2_X , CLA_HEAMV , CLA_WWADDW , CLA_LIN_BERR ,  &
     &         SLAMCH
      REAL SLAMCH
!     ..
!     .. Intrinsic Functions ..
...
!
     ...
     &                    Res,1)
            ELSEIF ( y_prec_state==EXTRA_RESIDUAL ) THEN
               CALL BLAS_CHEMV_X(uplo2,N,CMPLX(-1.0),A,Lda,Y(1,j),1,    &
     &                           CMPLX(1.0),Res,1,Prec_type)
            ELSE
               CALL BLAS_CHEMV2_X(uplo2,N,CMPLX(-1.0),A,Lda,Y(1,j),     &
