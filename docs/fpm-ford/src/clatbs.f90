!*==clatbs.f90  processed by SPAG 7.51RB at 20:08 on  3 Mar 2022
...
!
                     jlen = MIN(Kd,j-1)
                     CALL CAXPY(jlen,-X(j)*tscal,Ab(Kd+1-jlen,j),1,     &
     &                          X(j-jlen),1)
                     i = ICAMAX(j-1,X,1)
                     xmax = CABS1(X(i))
                  ENDIF
...
!
                  jlen = MIN(Kd,N-j)
                  IF ( jlen>0 ) CALL CAXPY(jlen,-X(j)*tscal,Ab(2,j),1,  &
     &                 X(j+1),1)
...
!
                  ...
                  ELSE
                     jlen = MIN(Kd,N-j)
                     IF ( jlen>1 ) csumj = CDOTU(jlen,Ab(2,j),1,X(j+1), &
     &                    1)
                  ENDIF
!
...
!
                  ...
                  ELSE
                     jlen = MIN(Kd,N-j)
                     IF ( jlen>1 ) csumj = CDOTC(jlen,Ab(2,j),1,X(j+1), &
     &                    1)
                  ENDIF
!
!                 Otherwise, use in-line code for the dot product.
!
               ELSEIF ( upper ) THEN
                  jlen = MIN(Kd,j-1)
                  DO i = 1 , jlen
                     csumj = csumj + (CONJG(Ab(Kd+i-jlen,j))*uscal)     &
     &                       *X(j-jlen-1+i)
                  ENDDO
               ELSE
                  jlen = MIN(Kd,N-j)
