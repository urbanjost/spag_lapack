!*==dsyevx_2stage.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
     ...
     &           indibl , indisp , indiwo , indtau , indwkn , indwrk ,  &
     &           iscale , itmp1 , j , jj , kd , lhtrd , llwork ,        &
     &           llwrkn , lwmin , lwtrd , nsplit
      CHARACTER :: order
!
...
!
      ...
      IF ( (alleig .OR. test) .AND. (Abstol<=ZERO) ) THEN
         ...
         ELSE
            ...
            CALL DORGTR(Uplo,N,Z,Ldz,Work(indtau),Work(indwrk),llwork,  &
     &                  iinfo)
            CALL DCOPY(N-1,Work(inde),1,Work(indee),1)
