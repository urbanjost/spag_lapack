!*==dgghd3.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b DGGHD3
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      REAL(R8KIND) :: c , c1 , c2 , s , s1 , s2 , temp , temp1 , temp2 ,&
     &                temp3
...
!
               IF ( jj>0 ) THEN
                  DO i = jj , 1 , -1
                     CALL DROT(Ihi-top,A(top+1,j+i+1),1,A(top+1,j+i),1, &
     &                         A(j+1+i,j),-B(j+1+i,j))
                  ENDDO
               ENDIF
!
...
!
                  jrow = Ihi - nblst + 1
                  CALL DGEMV('Transpose',nblst,len,ONE,Work,nblst,      &
     &                       A(jrow,j+1),1,ZERO,Work(pw),1)
                  ...
                  DO i = jrow , jrow + nblst - len - 1
                     ...
                     ppw = ppw + 1
                  ENDDO
                  CALL DTRMV('Lower','Transpose','Non-unit',nblst-len,  &
...
!
     ...
     &                          1)
                     CALL DGEMV('Transpose',nnb,len,ONE,Work(ppwo),     &
...
!
            cola = N - jcol - nnb + 1
            j = Ihi - nblst + 1
            CALL DGEMM('Transpose','No Transpose',nblst,cola,nblst,ONE, &
     &                 Work,nblst,A(j,jcol+nnb),Lda,ZERO,Work(pw),nblst)
            CALL DLACPY('All',nblst,cola,Work(pw),nblst,A(j,jcol+nnb),  &
     &                  Lda)
            ppwo = nblst*nblst + 1
            j0 = j - nnb
            DO j = j0 , jcol + 1 , -nnb
...
!
                  CALL DORM22('Left','Transpose',2*nnb,cola,nnb,nnb,    &
...
!
                  CALL DGEMM('Transpose','No Transpose',2*nnb,cola,     &
...
!
            IF ( wantq ) THEN
               ...
               ELSE
                  ...
                  nh = N
               ENDIF
               CALL DGEMM('No Transpose','No Transpose',nh,nblst,nblst, &
...
!
                     CALL DORM22('Right','No Transpose',nh,2*nnb,nnb,   &
...
!                    Ignore the structure of U.
!
                     CALL DGEMM('No Transpose','No Transpose',nh,2*nnb, &
     &                          2*nnb,ONE,Q(topq,j),Ldq,Work(ppwo),     &
     &                          2*nnb,ZERO,Work(pw),nh)
                     CALL DLACPY('All',nh,2*nnb,Work(pw),nh,Q(topq,j),  &
     &                           Ldq)
                  ENDIF
                  ppwo = ppwo + 4*nnb*nnb
               ENDDO
...
!
            IF ( top>0 ) THEN
               j = Ihi - nblst + 1
               CALL DGEMM('No Transpose','No Transpose',top,nblst,nblst,&
...
!                    Exploit the structure of U.
!
                     CALL DORM22('Right','No Transpose',top,2*nnb,nnb,  &
...
!                    Ignore the structure of U.
!
                     CALL DGEMM('No Transpose','No Transpose',top,2*nnb,&
     &                          2*nnb,ONE,A(1,j),Lda,Work(ppwo),2*nnb,  &
     &                          ZERO,Work(pw),top)
                     CALL DLACPY('All',top,2*nnb,Work(pw),top,A(1,j),   &
     &                           Lda)
                  ENDIF
                  ppwo = ppwo + 4*nnb*nnb
               ENDDO
...
!
     ...
     &                          ZERO,Work(pw),top)
                     CALL DLACPY('All',top,2*nnb,Work(pw),top,B(1,j),   &
     &                           Ldb)
                  ENDIF
                  ppwo = ppwo + 4*nnb*nnb
               ENDDO
...
!
     ...
     &                          2*nnb,ZERO,Work(pw),nh)
                     CALL DLACPY('All',nh,2*nnb,Work(pw),nh,Z(topq,j),  &
     &                           Ldz)
                  ENDIF
                  ppwo = ppwo + 4*nnb*nnb
               ENDDO
...
!
      ...
      ENDIF
!
      IF ( jcol<Ihi ) CALL DGGHRD(compq2,compz2,N,jcol,Ihi,A,Lda,B,Ldb, &
     &                            Q,Ldq,Z,Ldz,ierr)
      Work(1) = DBLE(lwkopt)
!
!
