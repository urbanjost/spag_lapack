!*==dlasyf_aa.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b DLASYF_AA
!
!  =========== DOCUMENTATION ===========
!
...
!           first column
!
            IF ( k>2 ) CALL DGEMV('No transpose',mj,j-k1,-ONE,H(j,k1),  &
...
!
                  IF ( i2<M ) CALL DSWAP(M-i2,A(J1+i1-1,i2+1),Lda,      &
     &                 A(J1+i2-1,i2+1),Lda)
!
...
!
                  IF ( i1>(k1-1) ) CALL DSWAP(i1-k1+1,A(1,i1),1,A(1,i2),&
     &                 1)
               ELSE
                  Ipiv(j+1) = j + 1
               ENDIF
...
!
               IF ( j<Nb ) CALL DCOPY(M-j,A(k+1,j+1),Lda,H(j+1,j+1),1)
!
...
!
                  IF ( i2<M )                                           &
     &                 CALL DSWAP(M-i2,A(i2+1,J1+i1-1),1,A(i2+1,J1+i2-1)&
     &                 ,1)
!
!              Swap A(I1, I1) with A(I2, I2)
!
...
!
               IF ( j<Nb ) CALL DCOPY(M-j,A(j+1,k+1),1,H(j+1,j+1),1)
!
!           Compute L(J+2, J+1) = WORK( 3:M ) / T(J, J+1),
!            where A(J, J+1) = T(J, J+1) and A(J+2:M, J) = L(J+2:M, J+1)
!
