!*==dpstf2.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
               ...
               IF ( pvt<N ) CALL DSWAP(N-pvt,A(j,pvt+1),Lda,A(pvt,pvt+1)&
     &                                 ,Lda)
...
!
            IF ( j<N ) THEN
               CALL DGEMV('Trans',j-1,N-j,-ONE,A(1,j+1),Lda,A(1,j),1,   &
     &                    ONE,A(j,j+1),Lda)
...
!
               ...
               IF ( pvt<N ) CALL DSWAP(N-pvt,A(pvt+1,j),1,A(pvt+1,pvt), &
     &                                 1)
...
!
            IF ( j<N ) THEN
               CALL DGEMV('No Trans',N-j,j-1,-ONE,A(j+1,1),Lda,A(j,1),  &
     &                    Lda,ONE,A(j+1,j),1)
