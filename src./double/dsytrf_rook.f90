!*==dsytrf_rook.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b DSYTRF_ROOK
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      IF ( nb>1 .AND. nb<N ) THEN
         ...
         IF ( Lwork<iws ) THEN
            ...
            nbmin = MAX(2,ILAENV(2,'DSYTRF_ROOK',Uplo,N,-1,-1,-1))
         ENDIF
      ELSE
         iws = 1
      ENDIF
...
!
               CALL DLASYF_ROOK(Uplo,k,nb,kb,A,Lda,Ipiv,Work,ldwork,    &
     &                          iinfo)
            ELSE
!
...
!
               CALL DLASYF_ROOK(Uplo,N-k+1,nb,kb,A(k,k),Lda,Ipiv(k),    &
     &                          Work,ldwork,iinfo)
            ELSE
!
!           Use unblocked code to factorize columns k:n of A
!
               CALL DSYTF2_ROOK(Uplo,N-k+1,A(k,k),Lda,Ipiv(k),iinfo)
               kb = N - k + 1
            ENDIF
!
!        Set INFO on the first occurrence of a zero pivot
