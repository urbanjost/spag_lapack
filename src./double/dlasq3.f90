!*==dlasq3.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      REAL(R8KIND) :: eps , s , t , temp , tol , tol2
...
!
               IF ( Dmin<=ZERO .OR. N0<n0in ) THEN
                  IF ( CBIAS*Z(4*I0+Pp-3)<Z(4*N0+Pp-3) ) THEN
                     ...
                     Z(4*N0+Pp-1) = MIN(Z(4*N0+Pp-1),Z(4*I0+Pp-1),Z(4*I0&
     &                              +Pp+3))
...
!        Success.
!
                  IF ( Dmin>=ZERO .AND. Dmin1>=ZERO ) GOTO 300
!
...
!
                        ...
                        Ttype = Ttype - 12
                     ENDIF
                  ELSEIF ( DISNAN(Dmin) ) THEN
!
!        NaN.
!
                     IF ( Tau==ZERO ) GOTO 200
                     Tau = ZERO
                  ELSE
!
...
!     Risk of underflow.
!
 200  CALL DLASQ6(I0,N0,Z,Pp,Dmin,Dmin1,Dmin2,Dn,Dn1,Dn2)
      Ndiv = Ndiv + (N0-I0+2)
      Iter = Iter + 1
      Tau = ZERO
!
 300  IF ( Tau<Sigma ) THEN
         Desig = Desig + Tau
         t = Sigma + Desig
         Desig = Desig - (t-Sigma)
      ELSE
