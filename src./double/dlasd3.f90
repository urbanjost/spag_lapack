!*==dlasd3.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      ...
      ENDIF
      IF ( Ctot(1)>0 ) THEN
         CALL DGEMM('N','N',Nl,K,Ctot(1),ONE,U2(1,2),Ldu2,Q(2,1),Ldq,   &
     &              ZERO,U(1,1),Ldu)
         IF ( Ctot(3)>0 ) THEN
            ktemp = 2 + Ctot(1) + Ctot(2)
            CALL DGEMM('N','N',Nl,K,Ctot(3),ONE,U2(1,ktemp),Ldu2,       &
     &                 Q(ktemp,1),Ldq,ONE,U(1,1),Ldu)
         ENDIF
      ELSEIF ( Ctot(3)>0 ) THEN
         ktemp = 2 + Ctot(1) + Ctot(2)
         CALL DGEMM('N','N',Nl,K,Ctot(3),ONE,U2(1,ktemp),Ldu2,Q(ktemp,1)&
     &              ,Ldq,ZERO,U(1,1),Ldu)
      ELSE
      ...
      ktemp = 2 + Ctot(1)
      ctemp = Ctot(2) + Ctot(3)
      CALL DGEMM('N','N',Nr,K,ctemp,ONE,U2(nlp2,ktemp),Ldu2,Q(ktemp,1), &
...
!
      ...
      ENDIF
      ktemp = 1 + Ctot(1)
      CALL DGEMM('N','N',K,nlp1,ktemp,ONE,Q(1,1),Ldq,Vt2(1,1),Ldvt2,    &
     &           ZERO,Vt(1,1),Ldvt)
      ktemp = 2 + Ctot(1) + Ctot(2)
      IF ( ktemp<=Ldvt2 ) CALL DGEMM('N','N',K,nlp1,Ctot(3),ONE,        &
...
!
      ...
      CALL DGEMM('N','N',K,nrp1,ctemp,ONE,Q(1,ktemp),Ldq,Vt2(ktemp,nlp2)&
