...
!> \brief \b DGETRF VARIANT: iterative version of Sivan Toledo's recursive LU algorithm
!
!  =========== DOCUMENTATION ===========
!
...
!        Factor the current column
         IF ( A(j,j)/=ZERO .AND. .NOT.DISNAN(A(j,j)) ) THEN
...
!        Solve for U block.
         CALL DTRSM('Left','Lower','No transpose','Unit',kahead,kcols,  &
     &              ONE,A(kstart,kstart),Lda,A(kstart,j+1),Lda)
!        Schur complement.
         CALL DGEMM('No transpose','No transpose',M-j,kcols,kahead,     &
...
!     If short and wide, handle the rest of the columns.
      IF ( M<N ) THEN
         ...
         CALL DTRSM('Left','Lower','No transpose','Unit',M,N-M,ONE,A,   &
