!*==claqr5.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
     ...
     &        tst1 , tst2 , ulp
      INTEGER :: i2 , i4 , incol , j , jbot , jcol , jlen , jrow ,      &
     &           jtop , k , k1 , kdu , kms , krcol , m , m22 , mbot ,   &
     &           mtop , nbmps , ndcol , ns , nu
      COMPLEX , DIMENSION(3) :: vt
!
...
!
               k = krcol + 2*(m22-1)
               IF ( k==Ktop-1 ) THEN
                  CALL CLAQR1(2,H(k+1,k+1),Ldh,S(2*m22-1),S(2*m22),     &
     &                        V(1,m22))
                  beta = V(1,m22)
...
!
               refsum = CONJG(V(1,m))                                   &
     &                  *(H(k+1,k+1)+CONJG(V(2,m))*H(k+2,k+1)           &
...
!
            DO jcol = MIN(ndcol,Kbot) + 1 , jbot , Nh
               ...
               CALL CGEMM('C','N',nu,jlen,nu,ONE,U(k1,k1),Ldu,          &
...
!
            DO jrow = jtop , MAX(Ktop,incol) - 1 , Nv
               ...
               CALL CGEMM('N','N',jlen,nu,nu,ONE,H(jrow,incol+k1),Ldh,  &
...
!
            IF ( Wantz ) THEN
               DO jrow = Iloz , Ihiz , Nv
                  ...
                  CALL CGEMM('N','N',jlen,nu,nu,ONE,Z(jrow,incol+k1),   &
