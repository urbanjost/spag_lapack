!*==clagtm.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
            DO j = 1 , Nrhs
               ...
               ELSE
                  B(1,j) = B(1,j) + CONJG(D(1))*X(1,j) + CONJG(Dl(1))   &
     &                     *X(2,j)
                  B(N,j) = B(N,j) + CONJG(Du(N-1))*X(N-1,j)             &
     &                     + CONJG(D(N))*X(N,j)
                  DO i = 2 , N - 1
                     B(i,j) = B(i,j) + CONJG(Du(i-1))*X(i-1,j)          &
     &                        + CONJG(D(i))*X(i,j) + CONJG(Dl(i))       &
     &                        *X(i+1,j)
                  ENDDO
               ENDIF
            ENDDO
