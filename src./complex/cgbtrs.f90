!*==cgbtrs.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CGBTRS
!
!  =========== DOCUMENTATION ===========
!
...
!
         IF ( lnoti ) THEN
            DO j = 1 , N - 1
               ...
               CALL CGERU(lm,Nrhs,-ONE,Ab(kd+1,j),1,B(j,1),Ldb,B(j+1,1),&
     &                    Ldb)
            ENDDO
         ENDIF
!
...
!
            CALL CTBSV('Upper','No transpose','Non-unit',N,Kl+Ku,Ab,    &
     &                 Ldab,B(1,i),1)
         ENDDO
!
...
!
            CALL CTBSV('Upper','Transpose','Non-unit',N,Kl+Ku,Ab,Ldab,  &
     &                 B(1,i),1)
         ENDDO
!
...
!
         IF ( lnoti ) THEN
            DO j = N - 1 , 1 , -1
               lm = MIN(Kl,N-j)
               CALL CGEMV('Transpose',lm,Nrhs,-ONE,B(j+1,1),Ldb,        &
...
!
            CALL CTBSV('Upper','Conjugate transpose','Non-unit',N,Kl+Ku,&
     &                 Ab,Ldab,B(1,i),1)
         ENDDO
!
...
!
         IF ( lnoti ) THEN
            DO j = N - 1 , 1 , -1
               ...
               CALL CGEMV('Conjugate transpose',lm,Nrhs,-ONE,B(j+1,1),  &
