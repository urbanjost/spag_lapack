!*==cporfsx.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CPORFSX
!
!  =========== DOCUMENTATION ===========
!
...
!
      IF ( ref_type/=0 ) THEN
         ...
         CALL CLA_PORFSX_EXTENDED(prec_type,Uplo,N,Nrhs,A,Lda,Af,Ldaf,  &
     &                            rcequ,S,B,Ldb,X,Ldx,Berr,n_norms,     &
     &                            Err_bnds_norm,Err_bnds_comp,Work,     &
     &                            Rwork,Work(N+1),                      &
     &                            TRANSFER(Rwork(1:2*N),(/(ZERO,ZERO)/),&
     &                            N),Rcond,ithresh,rthresh,             &
...
!
         IF ( rcequ ) THEN
            rcond_tmp = CLA_PORCOND_C(Uplo,N,A,Lda,Af,Ldaf,S,.TRUE.,    &
     &                  Info,Work,Rwork)
         ELSE
            rcond_tmp = CLA_PORCOND_C(Uplo,N,A,Lda,Af,Ldaf,S,.FALSE.,   &
     &                  Info,Work,Rwork)
         ENDIF
         DO j = 1 , Nrhs
!
...
!
         ...
         DO j = 1 , Nrhs
            IF ( Err_bnds_comp(j,LA_LINRX_ERR_I)<cwise_wrong ) THEN
               rcond_tmp = CLA_PORCOND_X(Uplo,N,A,Lda,Af,Ldaf,X(1,j),   &
     &                     Info,Work,Rwork)
            ELSE
               rcond_tmp = 0.0
            ENDIF
