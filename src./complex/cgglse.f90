!*==cgglse.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!     unitary.
!
      CALL CGGRQF(P,M,N,B,Ldb,Work,A,Lda,Work(P+1),Work(P+mn+1),        &
     &            Lwork-P-mn,Info)
      lopt = Work(P+mn+1)
!
...
!                       ( c2 ) M+P-N
!
      CALL CUNMQR('Left','Conjugate Transpose',M,1,mn,A,Lda,Work(P+1),C,&
...
!
      IF ( P>0 ) THEN
         CALL CTRTRS('Upper','No transpose','Non-unit',P,1,B(1,N-P+1),  &
     &               Ldb,D,P,Info)
!
         IF ( Info>0 ) THEN
            Info = 1
...
!        Update c1
!
         CALL CGEMV('No transpose',N-P,P,-CONE,A(1,N-P+1),Lda,D,1,CONE, &
     &              C,1)
      ENDIF
!
...
!
      IF ( N>P ) THEN
         CALL CTRTRS('Upper','No transpose','Non-unit',N-P,1,A,Lda,C,   &
     &               N-P,Info)
!
         IF ( Info>0 ) THEN
            Info = 2
...
!
      IF ( M<N ) THEN
         nr = M + P - N
         IF ( nr>0 ) CALL CGEMV('No transpose',nr,N-M,-CONE,A(N-P+1,M+1)&
     &                          ,Lda,D(nr+1),1,CONE,C(N-P+1),1)
      ELSE
         nr = P
      ENDIF
      IF ( nr>0 ) THEN
         CALL CTRMV('Upper','No transpose','Non unit',nr,A(N-P+1,N-P+1),&
     &              Lda,D,1)
...
!
      CALL CUNMRQ('Left','Conjugate Transpose',N,1,P,B,Ldb,Work(1),X,N, &
