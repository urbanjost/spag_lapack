!*==chetrf_aa.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CHETRF_AA
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      ELSEIF ( Lda<MAX(1,N) ) THEN
         Info = -4
      ELSEIF ( Lwork<(2*N) .AND. .NOT.lquery ) THEN
         Info = -7
      ENDIF
!
      IF ( Info==0 ) THEN
...
!        Panel factorization
!
            CALL CLAHEF_AA(Uplo,2-k1,N-j,jb,A(MAX(1,j),j+1),Lda,        &
...
!
                  alpha = CONJG(A(j,j+1))
                  A(j,j+1) = ONE
                  CALL CCOPY(N-j,A(j-1,j+1),Lda,Work((j+1-j1+1)+jb*N),1)
...
!
                     j3 = j2
                     DO mj = nj - 1 , 1 , -1
                        CALL CGEMM('Conjugate transpose','Transpose',1, &
     &                             mj,jb+1,-ONE,A(j1-k2,j3),Lda,        &
     &                             Work((j3-j1+1)+k1*N),N,ONE,A(j3,j3), &
     &                             Lda)
                        j3 = j3 + 1
                     ENDDO
!
...
!
                     CALL CGEMM('Conjugate transpose','Transpose',nj,   &
     &                          N-j3+1,jb+1,-ONE,A(j1-k2,j2),Lda,       &
     &                          Work((j3-j1+1)+k1*N),N,ONE,A(j2,j3),Lda)
                  ENDDO
!
!              Recover T( J, J+1 )
!
...
!        Panel factorization
!
            CALL CLAHEF_AA(Uplo,2-k1,N-j,jb,A(j+1,MAX(1,j)),Lda,        &
     &                     Ipiv(j+1),Work,N,Work(N*nb+1))
!
!        Adjust IPIV and apply it back (J-th step picks (J+1)-th pivot)
!
            DO j2 = j + 2 , MIN(N,j+jb+1)
               Ipiv(j2) = Ipiv(j2) + j
               IF ( (j2/=Ipiv(j2)) .AND. ((j1-k1)>2) )                  &
     &              CALL CSWAP(j1-k1-2,A(j2,1),Lda,A(Ipiv(j2),1),Lda)
            ENDDO
            j = j + jb
!
!        Trailing submatrix update, where
...
!
                  alpha = CONJG(A(j+1,j))
                  A(j+1,j) = ONE
                  CALL CCOPY(N-j,A(j+1,j-1),1,Work((j+1-j1+1)+jb*N),1)
                  CALL CSCAL(N-j,alpha,Work((j+1-j1+1)+jb*N),1)
!
!              K1 identifies if the previous column of the panel has been
!               explicitly stored, e.g., K1=0 and K2=1 for the first panel,
...
!
     ...
     &                             mj,1,jb+1,-ONE,Work((j3-j1+1)+k1*N), &
     &                             N,A(j3,j1-k2),Lda,ONE,A(j3,j3),Lda)
                        j3 = j3 + 1
                     ENDDO
!
...
!
                     CALL CGEMM('No transpose','Conjugate transpose',   &
     &                          N-j3+1,nj,jb+1,-ONE,Work((j3-j1+1)+k1*N)&
     &                          ,N,A(j2,j1-k2),Lda,ONE,A(j3,j2),Lda)
                  ENDDO
!
!              Recover T( J+1, J )
!
