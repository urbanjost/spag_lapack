!*==csytri2x.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CSYTRI2X
!
!  =========== DOCUMENTATION ===========
!
...
!
      COMPLEX :: ak , akkp1 , akp1 , d , t , u01_ip1_j , u01_i_j ,      &
     &           u11_ip1_j , u11_i_j
...
!       U11**T*invD1*U11->U11
!
            CALL CTRMM('L','U','T','U',nnb,nnb,ONE,A(cut+1,cut+1),Lda,  &
     &                 Work(u11+1,1),N+Nb+1)
!
            DO i = 1 , nnb
               DO j = i , nnb
...
!
            CALL CGEMM('T','N',nnb,nnb,cut,ONE,A(1,cut+1),Lda,Work,     &
...
!
         ...
         DO WHILE ( i<=N )
            ...
            ELSE
               ip = -Ipiv(i)
               i = i + 1
               IF ( (i-1)<ip ) CALL CSYSWAPR(Uplo,N,A,Lda,i-1,ip)
               IF ( (i-1)>ip ) CALL CSYSWAPR(Uplo,N,A,Lda,ip,i-1)
            ENDIF
            i = i + 1
         ENDDO
      ELSE
...
!       L11**T*invD1*L11->L11
!
            CALL CTRMM('L',Uplo,'T','U',nnb,nnb,ONE,A(cut+1,cut+1),Lda, &
     &                 Work(u11+1,1),N+Nb+1)
!
            DO i = 1 , nnb
               DO j = 1 , i
...
!
               CALL CGEMM('T','N',nnb,nnb,N-nnb-cut,ONE,                &
...
!        L01 =  L22**T*invD2*L21
!
               CALL CTRMM('L',Uplo,'T','U',N-nnb-cut,nnb,ONE,           &
