!*==chgeqz.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b CHGEQZ
!
!  =========== DOCUMENTATION ===========
!
...
!
      COMPLEX :: abi12 , abi22 , ad11 , ad12 , ad21 , ad22 , ctemp ,    &
     &           ctemp2 , ctemp3 , eshift , s , shift , signbc , u12 ,  &
     &           x , y
      REAL :: ABS1
      REAL :: absb , anorm , ascale , atol , bnorm , bscale , btol , c ,&
     &        safmin , temp , temp2 , tempr , ulp
...
!
     ...
     &                            T(jch+1,jch+1),Ldt,c,s)
                        IF ( ilq ) CALL CROT(N,Q(1,jch),1,Q(1,jch+1),1, &
     &                       c,CONJG(s))
...
!
     ...
     &                            T(ifrstm,jch-1),1,c,s)
                        IF ( ilz ) CALL CROT(N,Z(1,jch),1,Z(1,jch-1),1, &
     &                       c,s)
                     ENDDO
                  ENDIF
                  GOTO 20
...
!
 20         ctemp = H(ilast,ilast)
            ...
            H(ilast,ilast-1) = CZERO
            CALL CROT(ilast-ifrstm,H(ifrstm,ilast),1,H(ifrstm,ilast-1), &
     &                1,c,s)
            CALL CROT(ilast-ifrstm,T(ifrstm,ilast),1,T(ifrstm,ilast-1), &
     &                1,c,s)
            IF ( ilz ) CALL CROT(N,Z(1,ilast),1,Z(1,ilast-1),1,c,s)
!
...
!
               ...
               IF ( ctemp/=ZERO ) THEN
                  ...
                  IF ( temp2>ZERO ) THEN
                     IF ( REAL(x/temp2)*REAL(y)+AIMAG(x/temp2)*AIMAG(y) &
     &                    <ZERO ) y = -y
                  ENDIF
