!*==dgelsx.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      REAL(R8KIND) :: anrm , bignum , bnrm , c1 , c2 , s1 , s2 , smax , &
...
!
         IF ( Rank<mn ) THEN
            i = Rank + 1
            CALL DLAIC1(IMIN,Rank,Work(ismin),smin,A(1,i),A(i,i),sminpr,&
     &                  s1,c1)
            CALL DLAIC1(IMAX,Rank,Work(ismax),smax,A(1,i),A(i,i),smaxpr,&
     &                  s2,c2)
!
...
!     [R11,R12] = [ T11, 0 ] * Y
!
         IF ( Rank<N ) CALL DTZRQF(Rank,N,A,Lda,Work(mn+1),Info)
!
!     Details of Householder rotations stored in WORK(MN+1:2*MN)
!
!     B(1:M,1:NRHS) := Q**T * B(1:M,1:NRHS)
!
         CALL DORM2R('Left','Transpose',M,Nrhs,mn,A,Lda,Work(1),B,Ldb,  &
     &               Work(2*mn+1),Info)
!
!     workspace NRHS
!
...
!
         CALL DTRSM('Left','Upper','No transpose','Non-unit',Rank,Nrhs, &
     &              ONE,A,Lda,B,Ldb)
!
         DO i = Rank + 1 , N
            DO j = 1 , Nrhs
