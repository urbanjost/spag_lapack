!*==zporfsx.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b ZPORFSX
!
!  =========== DOCUMENTATION ===========
!
...
!
      IF ( ref_type/=0 ) THEN
         ...
         CALL ZLA_PORFSX_EXTENDED(prec_type,Uplo,N,Nrhs,A,Lda,Af,Ldaf,  &
...
!
         IF ( rcequ ) THEN
            rcond_tmp = ZLA_PORCOND_C(Uplo,N,A,Lda,Af,Ldaf,S,.TRUE.,    &
     &                  Info,Work,Rwork)
         ELSE
            rcond_tmp = ZLA_PORCOND_C(Uplo,N,A,Lda,Af,Ldaf,S,.FALSE.,   &
     &                  Info,Work,Rwork)
         ENDIF
         DO j = 1 , Nrhs
!
...
!
         ...
         DO j = 1 , Nrhs
            IF ( Err_bnds_comp(j,LA_LINRX_ERR_I)<cwise_wrong ) THEN
               rcond_tmp = ZLA_PORCOND_X(Uplo,N,A,Lda,Af,Ldaf,X(1,j),   &
     &                     Info,Work,Rwork)
            ELSE
               rcond_tmp = 0.0D+0
            ENDIF
