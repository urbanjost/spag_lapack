!*==zgbtrf.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b ZGBTRF
!
!  =========== DOCUMENTATION ===========
!
...
!
     ...
     &           jp , ju , k2 , km , kv , nb , nw
      ...
      COMPLEX(CX16KIND) , DIMENSION(LDWORK,NBMAX) :: work13 , work31
!
...
!
                  jm = MIN(ju,j+jb-1)
                  IF ( jm>jj ) CALL ZGERU(km,jm-jj,-ONE,Ab(kv+2,jj),1,  &
...
!
               nw = MIN(jj-j+1,i3)
               IF ( nw>0 ) CALL ZCOPY(nw,Ab(kv+Kl+1-jj+j,jj),1,         &
     &                                work31(1,jj-j+1),1)
            ENDDO
            IF ( j+jb<=N ) THEN
!
...
!              A32.
!
               CALL ZLASWP(j2,Ab(kv+1-jb,j+jb),Ldab-1,1,jb,Ipiv(j),1)
!
...
!                    Update A22
!
                  IF ( i2>0 ) CALL ZGEMM('No transpose','No transpose', &
...
!                    Update A32
!
                  IF ( i3>0 ) CALL ZGEMM('No transpose','No transpose', &
...
!
               nw = MIN(i3,jj-j+1)
               IF ( nw>0 ) CALL ZCOPY(nw,work31(1,jj-j+1),1,Ab(kv+Kl+1- &
     &                                jj+j,jj),1)
            ENDDO
         ENDDO
      ENDIF
