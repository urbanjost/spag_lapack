!*==zunhr_col.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b ZUNHR_COL
!
!  =========== DOCUMENTATION ===========
!
...
!     (1-2) Solve for V2.
!
      IF ( M>N ) CALL ZTRSM('R','U','N','N',M-N,N,CONE,A,Lda,A(N+1,1),  &
     &                      Lda)
!
...
!
         DO j = jb , jb + jnb - 1
            IF ( D(j)==CONE ) CALL ZSCAL(j-jbtemp1,-CONE,T(1,j),1)
         ENDDO
!
...
!
         CALL ZTRSM('R','L','C','U',jnb,jnb,CONE,A(jb,jb),Lda,T(1,jb),  &
     &              Ldt)
!
      ENDDO
!
