!*==zlauum.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
         DO i = 1 , N , nb
            ib = MIN(nb,N-i+1)
            CALL ZTRMM('Right','Upper','Conjugate transpose','Non-unit',&
     &                 i-1,ib,CONE,A(i,i),Lda,A(1,i),Lda)
            CALL ZLAUU2('Upper',ib,A(i,i),Lda,Info)
            IF ( i+ib<=N ) THEN
               CALL ZGEMM('No transpose','Conjugate transpose',i-1,ib,  &
...
!
         DO i = 1 , N , nb
            ib = MIN(nb,N-i+1)
            CALL ZTRMM('Left','Lower','Conjugate transpose','Non-unit', &
     &                 ib,i-1,CONE,A(i,i),Lda,A(i,1),Lda)
            CALL ZLAUU2('Lower',ib,A(i,i),Lda,Info)
            IF ( i+ib<=N ) THEN
               CALL ZGEMM('Conjugate transpose','No transpose',ib,i-1,  &
     &                    N-i-ib+1,CONE,A(i+ib,i),Lda,A(i+ib,1),Lda,    &
     &                    CONE,A(i,1),Lda)
               CALL ZHERK('Lower','Conjugate transpose',ib,N-i-ib+1,ONE,&
