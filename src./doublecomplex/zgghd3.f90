!*==zgghd3.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b ZGGHD3
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      REAL(R8KIND) :: c
      COMPLEX(CX16KIND) :: c1 , c2 , ctemp , s , s1 , s2 , temp ,       &
     &                     temp1 , temp2 , temp3
...
!
               IF ( jj>0 ) THEN
                  DO i = jj , 1 , -1
                     c = DBLE(A(j+1+i,j))
                     CALL ZROT(Ihi-top,A(top+1,j+i+1),1,A(top+1,j+i),1, &
     &                         c,-DCONJG(B(j+1+i,j)))
                  ENDDO
               ENDIF
!
...
!
                  jrow = Ihi - nblst + 1
                  CALL ZGEMV('Conjugate',nblst,len,CONE,Work,nblst,     &
     &                       A(jrow,j+1),1,CZERO,Work(pw),1)
                  ...
                  DO i = jrow , jrow + nblst - len - 1
                     ...
                     ppw = ppw + 1
                  ENDDO
                  CALL ZTRMV('Lower','Conjugate','Non-unit',nblst-len,  &
...
!
     ...
     &                          1)
                     CALL ZGEMV('Conjugate',nnb,len,CONE,Work(ppwo),    &
...
!
            cola = N - jcol - nnb + 1
            j = Ihi - nblst + 1
            CALL ZGEMM('Conjugate','No Transpose',nblst,cola,nblst,CONE,&
     &                 Work,nblst,A(j,jcol+nnb),Lda,CZERO,Work(pw),     &
     &                 nblst)
            CALL ZLACPY('All',nblst,cola,Work(pw),nblst,A(j,jcol+nnb),  &
     &                  Lda)
            ppwo = nblst*nblst + 1
            j0 = j - nnb
            DO j = j0 , jcol + 1 , -nnb
...
!
                  CALL ZUNM22('Left','Conjugate',2*nnb,cola,nnb,nnb,    &
...
!
                  CALL ZGEMM('Conjugate','No Transpose',2*nnb,cola,     &
...
!
            IF ( wantq ) THEN
               ...
               ELSE
                  ...
                  nh = N
               ENDIF
               CALL ZGEMM('No Transpose','No Transpose',nh,nblst,nblst, &
...
!
                     CALL ZUNM22('Right','No Transpose',nh,2*nnb,nnb,   &
...
!                    Ignore the structure of U.
!
                     CALL ZGEMM('No Transpose','No Transpose',nh,2*nnb, &
     &                          2*nnb,CONE,Q(topq,j),Ldq,Work(ppwo),    &
     &                          2*nnb,CZERO,Work(pw),nh)
                     CALL ZLACPY('All',nh,2*nnb,Work(pw),nh,Q(topq,j),  &
     &                           Ldq)
                  ENDIF
                  ppwo = ppwo + 4*nnb*nnb
               ENDDO
...
!
               ...
               pw = nblst*nblst + 1
               DO i = 1 , n2nb
                  CALL ZLASET('All',2*nnb,2*nnb,CZERO,CONE,Work(pw),    &
     &                        2*nnb)
                  pw = pw + 4*nnb*nnb
               ENDDO
!
...
!
                  ...
                  DO jrow = j0 , j + 2 , -nnb
                     ...
                     DO i = jrow + nnb - 1 , jrow , -1
                        ...
                        DO jj = ppw , ppw + len - 1
                           temp = Work(jj+2*nnb)
                           Work(jj+2*nnb) = ctemp*temp - DCONJG(s)      &
     &                        *Work(jj)
                           Work(jj) = s*temp + ctemp*Work(jj)
                        ENDDO
                        len = len + 1
...
!
            IF ( top>0 ) THEN
               j = Ihi - nblst + 1
               CALL ZGEMM('No Transpose','No Transpose',top,nblst,nblst,&
...
!                    Exploit the structure of U.
!
                     CALL ZUNM22('Right','No Transpose',top,2*nnb,nnb,  &
...
!                    Ignore the structure of U.
!
                     CALL ZGEMM('No Transpose','No Transpose',top,2*nnb,&
     &                          2*nnb,CONE,A(1,j),Lda,Work(ppwo),2*nnb, &
     &                          CZERO,Work(pw),top)
                     CALL ZLACPY('All',top,2*nnb,Work(pw),top,A(1,j),   &
     &                           Lda)
                  ENDIF
                  ppwo = ppwo + 4*nnb*nnb
               ENDDO
...
!
     ...
     &                          CZERO,Work(pw),top)
                     CALL ZLACPY('All',top,2*nnb,Work(pw),top,B(1,j),   &
     &                           Ldb)
                  ENDIF
                  ppwo = ppwo + 4*nnb*nnb
               ENDDO
...
!
     ...
     &                          2*nnb,CZERO,Work(pw),nh)
                     CALL ZLACPY('All',nh,2*nnb,Work(pw),nh,Z(topq,j),  &
     &                           Ldz)
                  ENDIF
                  ppwo = ppwo + 4*nnb*nnb
               ENDDO
...
!
      ...
      ENDIF
!
      IF ( jcol<Ihi ) CALL ZGGHRD(compq2,compz2,N,jcol,Ihi,A,Lda,B,Ldb, &
     &                            Q,Ldq,Z,Ldz,ierr)
      Work(1) = DCMPLX(lwkopt)
!
!
