!*==zgelsy.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      REAL(R8KIND) :: anrm , bignum , bnrm , smax , smaxpr , smin ,     &
     &                sminpr , smlnum , wsize
      COMPLEX(CX16KIND) :: c1 , c2 , s1 , s2
...
!     [R11,R12] = [ T11, 0 ] * Y
!
         IF ( Rank<N ) CALL ZTZRZF(Rank,N,A,Lda,Work(mn+1),Work(2*mn+1),&
     &                             Lwork-2*mn,Info)
!
...
!
         IF ( Rank<N ) CALL ZUNMRZ('Left','Conjugate transpose',N,Nrhs, &
