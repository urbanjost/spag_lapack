!*==ztgsy2.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      ...
      LOGICAL :: notran
      COMPLEX(CX16KIND) , DIMENSION(LDZ) :: rhs
      REAL(R8KIND) :: scaloc
      COMPLEX(CX16KIND) , DIMENSION(LDZ,LDZ) :: z
!
...
!
               ...
               ENDIF
               IF ( j<N ) THEN
                  CALL ZAXPY(N-j,rhs(2),B(j,j+1),Ldb,C(i,j+1),Ldc)
                  CALL ZAXPY(N-j,rhs(2),E(j,j+1),Lde,F(i,j+1),Ldf)
               ENDIF
!
            ENDDO
         ENDDO
...
!
               DO k = 1 , j - 1
                  F(i,k) = F(i,k) + rhs(1)*DCONJG(B(k,j)) + rhs(2)      &
     &                     *DCONJG(E(k,j))
               ENDDO
               DO k = i + 1 , M
