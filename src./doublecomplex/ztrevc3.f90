!*==ztrevc3.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b ZTREVC3
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      REAL(R8KIND) :: CABS1
      COMPLEX(CX16KIND) :: cdum
      INTEGER :: i , ii , is , iv , j , k , ki , maxwrk , nb
      REAL(R8KIND) :: ovfl , remax , scale , smin , smlnum , ulp , unfl
!
...
!              version 1: back-transform each vector with GEMV, Q*x.
               IF ( ki>1 ) CALL ZGEMV('N',N,ki-1,CONE,Vr,Ldvr,          &
...
!              or if this was last vector, do the GEMM
               IF ( (iv==1) .OR. (ki==1) ) THEN
                  CALL ZGEMM('N','N',N,nb-iv+1,ki+nb-iv,CONE,Vr,Ldvr,   &
...
!                 normalize vectors
                  ...
                  ENDDO
                  CALL ZLACPY('F',N,nb-iv+1,Work(1+(nb+iv)*N),N,Vr(1,ki)&
     &                        ,Ldvr)
                  iv = nb
               ELSE
                  iv = iv - 1
...
!              ------------------------------
!              version 1: back-transform each vector with GEMV, Q*x.
               IF ( ki<N ) CALL ZGEMV('N',N,N-ki,CONE,Vl(1,ki+1),Ldvl,  &
...
!              or if this was last vector, do the GEMM
               IF ( (iv==nb) .OR. (ki==N) ) THEN
                  CALL ZGEMM('N','N',N,iv,N-ki+iv,CONE,Vl(1,ki-iv+1),   &
...
!                 normalize vectors
                  DO k = 1 , iv
                     ...
                     CALL ZDSCAL(N,remax,Work(1+(nb+k)*N),1)
                  ENDDO
                  CALL ZLACPY('F',N,iv,Work(1+(nb+1)*N),N,Vl(1,ki-iv+1),&
     &                        Ldvl)
                  iv = 1
               ELSE
                  iv = iv + 1
