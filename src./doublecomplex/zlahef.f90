!*==zlahef.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      REAL(R8KIND) :: absakk , alpha , colmax , r1 , rowmax , t
      REAL(R8KIND) :: CABS1
      COMPLEX(CX16KIND) :: d11 , d21 , d22 , z
...
!
                     ...
                     CALL ZCOPY(k-imax,A(imax,imax+1),Lda,W(imax+1,kw-1)&
     &                          ,1)
                     CALL ZLACGV(k-imax,W(imax+1,kw-1),1)
                     IF ( k<N ) THEN
                        CALL ZGEMV('No transpose',k,N-k,-CONE,A(1,k+1), &
     &                             Lda,W(imax,kw+1),Ldw,CONE,W(1,kw-1), &
     &                             1)
...
!
!              Case(3)
                     ELSEIF ( ABS(DBLE(W(imax,kw-1)))>=alpha*rowmax )   &
     &                        THEN
!
...
!
                     IF ( k<N ) CALL ZSWAP(N-k,A(kk,k+1),Lda,A(kp,k+1), &
     &                    Lda)
...
!
                  ...
                  IF ( imax<N ) CALL ZCOPY(N-imax,A(imax+1,imax),1,     &
     &                 W(imax+1,k+1),1)
...
!
!              Case(3)
                  ELSEIF ( ABS(DBLE(W(imax,k+1)))>=alpha*rowmax ) THEN
!
!                 interchange rows and columns K and IMAX, use 1-by-1
!                 pivot block
!
