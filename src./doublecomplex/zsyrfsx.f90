!*==zsyrfsx.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b ZSYRFSX
!
!  =========== DOCUMENTATION ===========
!
...
!
      IF ( ref_type/=0 ) THEN
         ...
         CALL ZLA_SYRFSX_EXTENDED(prec_type,Uplo,N,Nrhs,A,Lda,Af,Ldaf,  &
...
!
         IF ( rcequ ) THEN
            rcond_tmp = ZLA_SYRCOND_C(Uplo,N,A,Lda,Af,Ldaf,Ipiv,S,      &
...
!
         ...
         DO j = 1 , Nrhs
            IF ( Err_bnds_comp(j,LA_LINRX_ERR_I)<cwise_wrong ) THEN
               rcond_tmp = ZLA_SYRCOND_X(Uplo,N,A,Lda,Af,Ldaf,Ipiv,     &
