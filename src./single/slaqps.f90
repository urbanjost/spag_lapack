!*==slaqps.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
            IF ( k>1 ) CALL SGEMV('No transpose',M-rk+1,k-1,-ONE,A(rk,1)&
...
!
            IF ( k<N ) CALL SGEMV('Transpose',M-rk+1,N-k,Tau(k),        &
...
!
               CALL SGEMV('No transpose',N,k-1,ONE,F(1,1),Ldf,Auxv(1),1,&
     &                    ONE,F(1,k),1)
            ENDIF
!
...
!
            IF ( k<N ) CALL SGEMV('No transpose',N-k,k,-ONE,F(k+1,1),   &
