!*==slarfb.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!              W := W * V1
!
               CALL STRMM('Right','Lower','No transpose','Unit',N,K,ONE,&
     &                    V,Ldv,Work,Ldwork)
!
!                 W := W + C2**T * V2
!
               IF ( M>K ) CALL SGEMM('Transpose','No transpose',N,K,M-K,&
     &                               ONE,C(K+1,1),Ldc,V(K+1,1),Ldv,ONE, &
     &                               Work,Ldwork)
!
!              W := W * T**T  or  W * T
!
               CALL STRMM('Right','Upper',transt,'Non-unit',N,K,ONE,T,  &
     &                    Ldt,Work,Ldwork)
!
!              C := C - V * W**T
!
...
!                 C2 := C2 - V2 * W**T
!
               IF ( M>K ) CALL SGEMM('No transpose','Transpose',M-K,N,K,&
     &                               -ONE,V(K+1,1),Ldv,Work,Ldwork,ONE, &
     &                               C(K+1,1),Ldc)
!
!              W := W * V1**T
!
               CALL STRMM('Right','Lower','Transpose','Unit',N,K,ONE,V, &
     &                    Ldv,Work,Ldwork)
!
!              C1 := C1 - W**T
!
...
!              W := W * V1
!
               CALL STRMM('Right','Lower','No transpose','Unit',M,K,ONE,&
     &                    V,Ldv,Work,Ldwork)
!
!                 W := W + C2 * V2
!
               IF ( N>K ) CALL SGEMM('No transpose','No transpose',M,K, &
     &                               N-K,ONE,C(1,K+1),Ldc,V(K+1,1),Ldv, &
     &                               ONE,Work,Ldwork)
!
!              W := W * T  or  W * T**T
!
               CALL STRMM('Right','Upper',Trans,'Non-unit',M,K,ONE,T,   &
     &                    Ldt,Work,Ldwork)
!
!              C := C - W * V**T
!
...
!                 C2 := C2 - W * V2**T
!
               IF ( N>K ) CALL SGEMM('No transpose','Transpose',M,N-K,K,&
     &                               -ONE,Work,Ldwork,V(K+1,1),Ldv,ONE, &
     &                               C(1,K+1),Ldc)
!
!              W := W * V1**T
!
               CALL STRMM('Right','Lower','Transpose','Unit',M,K,ONE,V, &
     &                    Ldv,Work,Ldwork)
!
!              C1 := C1 - W
!
...
!              W := W * V2
!
            CALL STRMM('Right','Upper','No transpose','Unit',N,K,ONE,   &
...
!              W := W * V2
!
            CALL STRMM('Right','Upper','No transpose','Unit',M,K,ONE,   &
     &                 V(N-K+1,1),Ldv,Work,Ldwork)
!
!                 W := W + C1 * V1
!
            IF ( N>K ) CALL SGEMM('No transpose','No transpose',M,K,N-K,&
     &                            ONE,C,Ldc,V,Ldv,ONE,Work,Ldwork)
!
!              W := W * T  or  W * T**T
!
            CALL STRMM('Right','Lower',Trans,'Non-unit',M,K,ONE,T,Ldt,  &
     &                 Work,Ldwork)
!
!              C := C - W * V**T
!
...
!              W := W * V1**T
!
               CALL STRMM('Right','Upper','Transpose','Unit',N,K,ONE,V, &
     &                    Ldv,Work,Ldwork)
!
!                 W := W + C2**T * V2**T
!
               IF ( M>K ) CALL SGEMM('Transpose','Transpose',N,K,M-K,   &
...
!                 C2 := C2 - V2**T * W**T
!
               IF ( M>K ) CALL SGEMM('Transpose','Transpose',M-K,N,K,   &
...
!              W := W * V1**T
!
               CALL STRMM('Right','Upper','Transpose','Unit',M,K,ONE,V, &
     &                    Ldv,Work,Ldwork)
!
!                 W := W + C2 * V2**T
!
               IF ( N>K ) CALL SGEMM('No transpose','Transpose',M,K,N-K,&
...
!                 C2 := C2 - W * V2
!
               IF ( N>K ) CALL SGEMM('No transpose','No transpose',M,   &
...
!                 W := W + C1**T * V1**T
!
            IF ( M>K ) CALL SGEMM('Transpose','Transpose',N,K,M-K,ONE,C,&
...
!                 C1 := C1 - V1**T * W**T
!
            IF ( M>K ) CALL SGEMM('Transpose','Transpose',M-K,N,K,-ONE, &
...
!                 C1 := C1 - W * V1
!
            IF ( N>K ) CALL SGEMM('No transpose','No transpose',M,N-K,K,&
     &                            -ONE,Work,Ldwork,V,Ldv,ONE,C,Ldc)
!
!              W := W * V2
!
