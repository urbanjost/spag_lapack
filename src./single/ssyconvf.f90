!*==ssyconvf.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b SSYCONVF
!
!  =========== DOCUMENTATION ===========
!
...
!
                  ip = Ipiv(i)
                  IF ( i<N ) THEN
                     IF ( ip/=i ) CALL SSWAP(N-i,A(i,i+1),Lda,A(ip,i+1),&
     &                    Lda)
                  ENDIF
!
               ELSE
...
!
                  ip = Ipiv(i)
                  IF ( i<N ) THEN
                     IF ( ip/=i ) CALL SSWAP(N-i,A(ip,i+1),Lda,A(i,i+1),&
     &                    Lda)
                  ENDIF
!
               ELSE
...
!
               ip = Ipiv(i)
               IF ( i>1 ) THEN
                  IF ( ip/=i ) CALL SSWAP(i-1,A(i,1),Lda,A(ip,1),Lda)
               ENDIF
!
            ELSE
!
...
!
               ip = -Ipiv(i)
               IF ( i>1 ) THEN
                  IF ( ip/=(i+1) ) CALL SSWAP(i-1,A(i+1,1),Lda,A(ip,1), &
     &                 Lda)
               ENDIF
!
!                 Convert IPIV
...
!
               ip = Ipiv(i)
               IF ( i>1 ) THEN
                  IF ( ip/=i ) CALL SSWAP(i-1,A(ip,1),Lda,A(i,1),Lda)
               ENDIF
!
            ELSE
!
...
!
               ...
               ip = -Ipiv(i)
               IF ( i>1 ) THEN
                  IF ( ip/=(i+1) ) CALL SSWAP(i-1,A(ip,1),Lda,A(i+1,1), &
     &                 Lda)
               ENDIF
!
!                 Convert IPIV
