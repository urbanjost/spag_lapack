!*==sgejsv.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b SGEJSV
!
!  =========== DOCUMENTATION ===========
!
...
!
     ...
     &           l2aber , l2kill , l2pert , l2rank , l2tran , lsvec ,   &
...
!              .. V is available as workspace
               ...
               ENDDO
               CALL SPOCON('U',N,V,Ldv,ONE,temp1,Work(N+1),             &
     &                     Iwork(2*N+M+1),ierr)
            ELSEIF ( lsvec ) THEN
!              .. U is available as workspace
               ...
               ENDDO
               CALL SPOCON('U',N,U,Ldu,ONE,temp1,Work(N+1),             &
     &                     Iwork(2*N+M+1),ierr)
            ELSE
...
!           .. the columns of R are scaled to have unit Euclidean lengths.
               CALL SPOCON('U',N,Work(N+1),N,ONE,temp1,Work(N+N*N+1),   &
     &                     Iwork(2*N+M+1),ierr)
            ENDIF
...
!
         CALL SGESVJ('L','NoU','NoV',nr,nr,A,Lda,Sva,N,V,Ldv,Work,Lwork,&
     &               Info)
!
         scalem = Work(1)
         numrank = NINT(Work(2))
...
!
            CALL SGESVJ('L','U','N',N,nr,V,Ldv,Sva,nr,A,Lda,Work,Lwork, &
     &                  Info)
            scalem = Work(1)
            numrank = NINT(Work(2))
...
!
            ...
            CALL SLASET('Upper',nr-1,nr-1,ZERO,ZERO,V(1,2),Ldv)
            CALL SGEQRF(nr,nr,V,Ldv,Work(N+1),Work(2*N+1),Lwork-2*N,    &
     &                  ierr)
            DO p = 1 , nr
            ...
            CALL SLASET('Upper',nr-1,nr-1,ZERO,ZERO,V(1,2),Ldv)
!
            CALL SGESVJ('Lower','U','N',nr,nr,V,Ldv,Sva,nr,U,Ldu,       &
     &                  Work(N+1),Lwork-N,Info)
            ...
            ENDIF
!
            CALL SORMLQ('Left','Transpose',N,N,nr,A,Lda,Work,V,Ldv,     &
...
!
         IF ( transp ) CALL SLACPY('All',N,N,V,Ldv,U,Ldu)
!
...
!
         ...
         CALL SLASET('Upper',nr-1,nr-1,ZERO,ZERO,U(1,2),Ldu)
!
         CALL SGESVJ('Lower','U','N',nr,nr,U,Ldu,Sva,nr,A,Lda,Work(N+1),&
     &               Lwork-N,Info)
         scalem = Work(N+1)
         numrank = NINT(Work(N+2))
!
         ...
         ENDIF
!
         CALL SORMQR('Left','No Tr',M,n1,N,A,Lda,Work,U,Ldu,Work(N+1),  &
     &               Lwork-N,ierr)
!
...
!
         ...
         ENDDO
!
         IF ( transp ) CALL SLACPY('All',N,N,U,Ldu,V,Ldv)
!
      ELSE
!
!        .. Full SVD ..
...
!
            ...
            CALL SGESVJ('L','U','V',nr,nr,U,Ldu,Sva,N,V,Ldv,            &
     &                  Work(2*N+N*nr+1),Lwork-2*N-N*nr,Info)
            ...
            CALL SORMQR('L','N',N,N,nr,Work(2*N+1),N,Work(N+1),V,Ldv,   &
...
!              R1^t = Q2 * R2
               CALL SGEQRF(N,nr,V,Ldv,Work(N+1),Work(2*N+1),Lwork-2*N,  &
     &                     ierr)
!
               IF ( l2pert ) THEN
                  xsc = SQRT(small)/epsln
...
!              R1^t * P2 = Q2 * R2
               DO p = 1 , nr
                  Iwork(N+p) = 0
               ENDDO
               CALL SGEQP3(N,nr,V,Ldv,Iwork(N+1),Work(N+1),Work(2*N+1), &
     &                     Lwork-2*N,ierr)
...
!
            IF ( condr1<cond_ok ) THEN
!
               CALL SGESVJ('L','U','N',nr,nr,V,Ldv,Sva,nr,U,Ldu,        &
...
!                 factorization is then built in explicitly.
                  CALL STRSM('L','U','T','N',nr,nr,ONE,Work(2*N+1),N,V, &
     &                       Ldv)
                  IF ( nr<N ) THEN
                     CALL SLASET('A',N-nr,nr,ZERO,ZERO,V(nr+1,1),Ldv)
                     CALL SLASET('A',nr,N-nr,ZERO,ZERO,V(1,nr+1),Ldv)
     ...
     &                           Ldv)
                  ENDIF
                  CALL SORMQR('L','N',N,N,nr,Work(2*N+1),N,Work(N+1),V, &
...
!              accumulation of Jacobi rotations.
               CALL SGESVJ('L','U','V',nr,nr,V,Ldv,Sva,nr,U,Ldu,        &
     &                     Work(2*N+N*nr+nr+1),Lwork-2*N-N*nr-nr,Info)
               scalem = Work(2*N+N*nr+nr+1)
               numrank = NINT(Work(2*N+N*nr+nr+2))
               IF ( nr<N ) THEN
...
!
            CALL SORMQR('Left','No_Tr',M,n1,N,A,Lda,Work,U,Ldu,Work(N+1)&
     &                  ,Lwork-N,ierr)
...
!
            ...
            ENDIF
!
            CALL SGESVJ('Upper','U','N',N,N,Work(N+1),N,Sva,N,U,Ldu,    &
...
!
            ...
            ENDDO
!
            CALL STRSM('Left','Upper','NoTrans','No UD',N,N,ONE,A,Lda,  &
     &                 Work(N+1),N)
            DO p = 1 , N
