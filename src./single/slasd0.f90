!*==slasd0.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
         ...
         nrf = ic + 1
         sqrei = 1
         CALL SLASDQ('U',sqrei,nl,nlp1,nl,ncc,D(nlf),E(nlf),Vt(nlf,nlf),&
     &               Ldvt,U(nlf,nlf),Ldu,U(nlf,nlf),Ldu,Work,Info)
         ...
         ENDIF
         nrp1 = nr + sqrei
         CALL SLASDQ('U',sqrei,nr,nrp1,nr,ncc,D(nrf),E(nrf),Vt(nrf,nrf),&
...
!
         ...
         DO i = lf , ll
            ...
            alpha = D(ic)
            beta = E(ic)
            CALL SLASD1(nl,nr,sqrei,D(nlf),alpha,beta,U(nlf,nlf),Ldu,   &
