!*==slaqr0.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
         lwkopt = 1
         IF ( Lwork/=-1 ) CALL SLAHQR(Wantt,Wantz,N,Ilo,Ihi,H,Ldh,Wr,Wi,&
...
!
         CALL SLAQR3(Wantt,Wantz,N,Ilo,Ihi,nwr+1,H,Ldh,Iloz,Ihiz,Z,Ldz, &
...
!           ==== Aggressive early deflation ====
!
            CALL SLAQR3(Wantt,Wantz,N,ktop,kbot,nw,H,Ldh,Iloz,Ihiz,Z,   &
...
!
                  IF ( kbot-ks+1<=ns/2 ) THEN
                     ...
                     IF ( ns>nmin ) THEN
                        CALL SLAQR4(.FALSE.,.FALSE.,ns,1,ns,H(kt,1),Ldh,&
     &                              Wr(ks),Wi(ks),1,1,zdum,1,Work,Lwork,&
     &                              inf)
                     ELSE
                        CALL SLAHQR(.FALSE.,.FALSE.,ns,1,ns,H(kt,1),Ldh,&
