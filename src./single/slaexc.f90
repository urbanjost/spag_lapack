!*==slaexc.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
...
!
      REAL :: cs , dnorm , eps , scale , smlnum , sn , t11 , t22 , t33 ,&
     &        tau , tau1 , tau2 , temp , thresh , wi1 , wi2 , wr1 ,     &
     &        wr2 , xnorm
      REAL , DIMENSION(LDD,4) :: d
      INTEGER :: ierr , j2 , j3 , j4 , k , nd
      REAL , DIMENSION(3) :: u , u1 , u2
      REAL , DIMENSION(LDX,2) :: x
!
...
!
         IF ( j3<=N ) CALL SROT(N-J1-1,T(J1,j3),Ldt,T(j2,j3),Ldt,cs,sn)
...
!
         CALL SLASY2(.FALSE.,.FALSE.,-1,N1,N2,d,LDD,d(N1+1,N1+1),LDD,   &
...
!           Accumulate transformation in the matrix Q.
!
               IF ( Wantq ) CALL SLARFX('R',N,3,u,tau,Q(1,J1),Ldq,Work)
            ENDIF
         ELSEIF ( k==3 ) THEN
!
!
...
!
            CALL SLANV2(T(J1,J1),T(J1,j2),T(j2,J1),T(j2,j2),wr1,wi1,wr2,&
     &                  wi2,cs,sn)
...
!
            j3 = J1 + N2
            j4 = j3 + 1
            CALL SLANV2(T(j3,j3),T(j3,j4),T(j4,j3),T(j4,j4),wr1,wi1,wr2,&
     &                  wi2,cs,sn)
