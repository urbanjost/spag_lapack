!*==ssbgst.f90  processed by SPAG 7.51RB at 21:36 on  3 Mar 2022
!> \brief \b SSBGST
!
!  =========== DOCUMENTATION ===========
!
...
!
               ...
               IF ( kbt>0 ) CALL SGER(N-m,kbt,-ONE,X(m+1,i),1,Bb(kb1-kbt&
     &                                ,i),1,X(m+1,i-kbt),Ldx)
            ENDIF
!
...
!
                  CALL SLARTG(Ab(k+1,i-k+Ka),ra1,Work(N+i-k+Ka-m),      &
     &                        Work(i-k+Ka-m),ra)
!
...
!
            IF ( nrt>0 ) CALL SLARGV(nrt,Ab(1,j2t),inca,Work(j2t-m),ka1,&
     &                               Work(N+j2t-m),ka1)
            IF ( nr>0 ) THEN
!
...
!
               DO l = 1 , Ka - 1
                  CALL SLARTV(nr,Ab(ka1-l,j2),inca,Ab(Ka-l,j2+1),inca,  &
...
!              blocks
!
               CALL SLAR2V(nr,Ab(ka1,j2),Ab(ka1,j2+1),Ab(Ka,j2+1),inca, &
     &                     Work(N+j2-m),Work(j2-m),ka1)
!
            ENDIF
!
...
!
               DO j = j2 , j1 , ka1
                  CALL SROT(N-m,X(m+1,j),1,X(m+1,j+1),1,Work(N+j-m),    &
     &                      Work(j-m))
               ENDDO
            ENDIF
         ENDDO
...
!
            DO l = Kb - k , 1 , -1
               nrt = (N-j2+Ka+l)/ka1
               IF ( nrt>0 ) CALL SLARTV(nrt,Ab(l,j2-l+1),inca,Ab(l+1,j2-&
...
!
               DO j = j2 , j1 , ka1
                  CALL SROT(N-m,X(m+1,j),1,X(m+1,j+1),1,Work(N+j),      &
     &                      Work(j))
               ENDDO
            ENDIF
         ENDDO
...
!
               CALL SSCAL(N-m,ONE/bii,X(m+1,i),1)
               IF ( kbt>0 ) CALL SGER(N-m,kbt,-ONE,X(m+1,i),1,Bb(kbt+1,i&
...
!           have been created outside the band
!
            IF ( nrt>0 ) CALL SLARGV(nrt,Ab(ka1,j2t-Ka),inca,Work(j2t-m)&
...
!
               DO l = 1 , Ka - 1
                  CALL SLARTV(nr,Ab(l+1,j2-l),inca,Ab(l+2,j2-l),inca,   &
     &                        Work(N+j2-m),Work(j2-m),ka1)
               ENDDO
!
!              apply rotations in 1st set from both sides to diagonal
!              blocks
!
               CALL SLAR2V(nr,Ab(1,j2),Ab(1,j2+1),Ab(2,j2),inca,        &
     &                     Work(N+j2-m),Work(j2-m),ka1)
!
            ENDIF
!
...
!              which have been created outside the band
!
               CALL SLARGV(nr,Ab(ka1,j2-Ka),inca,Work(j2),ka1,Work(N+j2)&
     &                     ,ka1)
!
...
!           have been created outside the band
!
               IF ( nrt>0 ) CALL SLARGV(nrt,Ab(1,j1+Ka),inca,Work(j1),  &
     &                                  ka1,Work(N+j1),ka1)
               IF ( nr>0 ) THEN
!
!              apply rotations in 1st set from the left
!
                  DO l = 1 , Ka - 1
                     CALL SLARTV(nr,Ab(ka1-l,j1+l),inca,Ab(Ka-l,j1+l),  &
...
!              blocks
!
                  CALL SLAR2V(nr,Ab(ka1,j1),Ab(ka1,j1-1),Ab(Ka,j1),inca,&
...
!
               DO l = Ka - 1 , Kb - k + 1 , -1
                  ...
                  j1t = j2 - (nrt-1)*ka1
                  IF ( nrt>0 )                                          &
     &                 CALL SLARTV(nrt,Ab(l,j1t),inca,Ab(l+1,j1t-1),    &
...
!
               DO l = Kb - k , 1 , -1
                  ...
                  j1t = j2 - (nrt-1)*ka1
                  IF ( nrt>0 )                                          &
     &                 CALL SLARTV(nrt,Ab(l,j1t+Ka),inca,Ab(l+1,j1t+Ka- &
...
!              which have been created outside the band
!
                  CALL SLARGV(nr,Ab(1,j1+Ka),inca,Work(m-Kb+j1),ka1,    &
     &                        Work(N+m-Kb+j1),ka1)
!
!              apply rotations in 2nd set from the left
!
...
!
                  DO j = j1 , j2 , ka1
                     CALL SROT(nx,X(1,j),1,X(1,j-1),1,Work(N+m-Kb+j),   &
     &                         Work(m-Kb+j))
                  ENDDO
               ENDIF
            ENDDO
...
!                 generate rotation to annihilate a(i,i+k-ka-1)
!
                     CALL SLARTG(Ab(ka1-k,i+k-Ka),ra1,Work(N+i+k-Ka),   &
     &                           Work(i+k-Ka),ra)
!
!                 create nonzero element a(i+k,i+k-ka-1) outside the
!                 band and store it in WORK(m-kb+i+k)
...
!           have been created outside the band
!
               IF ( nrt>0 ) CALL SLARGV(nrt,Ab(ka1,j1),inca,Work(j1),   &
     &                                  ka1,Work(N+j1),ka1)
               IF ( nr>0 ) THEN
!
!              apply rotations in 1st set from the right
!
                  DO l = 1 , Ka - 1
                     CALL SLARTV(nr,Ab(l+1,j1),inca,Ab(l+2,j1-1),inca,  &
     &                           Work(N+j1),Work(j1),ka1)
                  ENDDO
!
!              apply rotations in 1st set from both sides to diagonal
!              blocks
!
                  CALL SLAR2V(nr,Ab(1,j1),Ab(1,j1-1),Ab(2,j1-1),inca,   &
     &                        Work(N+j1),Work(j1),ka1)
!
               ENDIF
!
...
!
               DO l = Ka - 1 , Kb - k + 1 , -1
                  nrt = (j2+l-1)/ka1
                  j1t = j2 - (nrt-1)*ka1
                  IF ( nrt>0 ) CALL SLARTV(nrt,Ab(ka1-l+1,j1t-ka1+l),   &
...
!              which have been created outside the band
!
                  CALL SLARGV(nr,Ab(ka1,j1),inca,Work(m-Kb+j1),ka1,     &
     &                        Work(N+m-Kb+j1),ka1)
!
!              apply rotations in 2nd set from the right
!
