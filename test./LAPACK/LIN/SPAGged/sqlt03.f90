!*==sqlt03.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SQLT03
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      IF ( K>0 .AND. M>K ) CALL SLACPY('Full',M-K,K,Af(1,N-K+1),Lda,    &
     &                                 Q(1,M-K+1),Lda)
...
!
      SRNamt = 'SORGQL'
      CALL SORGQL(M,M,K,Q,Lda,Tau(minmn-K+1),Work,Lwork,info)
!
      DO iside = 1 , 2
         IF ( iside==1 ) THEN
            side = 'L'
...
!
            SRNamt = 'SORMQL'
            IF ( K>0 ) CALL SORMQL(side,trans,mc,nc,K,Af(1,N-K+1),Lda,  &
