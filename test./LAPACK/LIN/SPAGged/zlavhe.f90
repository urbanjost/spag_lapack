!*==zlavhe.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZLAVHE
!
!  =========== DOCUMENTATION ===========
!
...
!
                     CALL ZGERU(k-1,Nrhs,ONE,A(1,k),1,B(k,1),Ldb,B(1,1),&
     &                          Ldb)
!
...
!
                     CALL ZGERU(k-1,Nrhs,ONE,A(1,k),1,B(k,1),Ldb,B(1,1),&
     &                          Ldb)
                     CALL ZGERU(k-1,Nrhs,ONE,A(1,k+1),1,B(k+1,1),Ldb,   &
     &                          B(1,1),Ldb)
!
!                 Interchange if P(K) != I.
!
...
!                 Apply the transformation.
!
                     CALL ZGERU(N-k,Nrhs,ONE,A(k+1,k),1,B(k,1),Ldb,     &
     &                          B(k+1,1),Ldb)
!
...
!
                     CALL ZGERU(N-k,Nrhs,ONE,A(k+1,k),1,B(k,1),Ldb,     &
     &                          B(k+1,1),Ldb)
                     CALL ZGERU(N-k,Nrhs,ONE,A(k+1,k-1),1,B(k-1,1),Ldb, &
     &                          B(k+1,1),Ldb)
!
!                 Interchange if a permutation was applied at the
!                 K-th step of the factorization.
...
!
                  ...
                  CALL ZGEMV('Conjugate',k-1,Nrhs,ONE,B,Ldb,A(1,k),1,   &
     &                       ONE,B(k,1),Ldb)
                  CALL ZLACGV(Nrhs,B(k,1),Ldb)
               ENDIF
               IF ( nounit ) CALL ZSCAL(Nrhs,A(k,k),B(k,1),Ldb)
...
!
                  CALL ZLACGV(Nrhs,B(k,1),Ldb)
                  CALL ZGEMV('Conjugate',k-2,Nrhs,ONE,B,Ldb,A(1,k),1,   &
     &                       ONE,B(k,1),Ldb)
                  CALL ZLACGV(Nrhs,B(k,1),Ldb)
!
                  CALL ZLACGV(Nrhs,B(k-1,1),Ldb)
                  CALL ZGEMV('Conjugate',k-2,Nrhs,ONE,B,Ldb,A(1,k-1),1, &
     &                       ONE,B(k-1,1),Ldb)
                  CALL ZLACGV(Nrhs,B(k-1,1),Ldb)
               ENDIF
!
