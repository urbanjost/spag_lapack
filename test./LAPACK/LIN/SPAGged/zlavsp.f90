!*==zlavsp.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZLAVSP
!
!  =========== DOCUMENTATION ===========
!
...
!
                  CALL ZGEMV('Transpose',k-1,Nrhs,ONE,B,Ldb,A(kc),1,ONE,&
     &                       B(k,1),Ldb)
               ENDIF
               IF ( nounit ) CALL ZSCAL(Nrhs,A(kc+k-1),B(k,1),Ldb)
               k = k - 1
...
!                 Apply the transformations.
!
                  CALL ZGEMV('Transpose',k-2,Nrhs,ONE,B,Ldb,A(kc),1,ONE,&
     &                       B(k,1),Ldb)
!
                  CALL ZGEMV('Transpose',k-2,Nrhs,ONE,B,Ldb,A(kcnext),1,&
     &                       ONE,B(k-1,1),Ldb)
               ENDIF
!
...
!                 Apply the transformation
!
                  CALL ZGEMV('Transpose',N-k-1,Nrhs,ONE,B(k+2,1),Ldb,   &
