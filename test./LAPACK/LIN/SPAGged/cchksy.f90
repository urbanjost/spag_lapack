!*==cchksy.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CCHKSY
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CCHKSY(Dotype,Nn,Nval,Nnb,Nbval,Nns,Nsval,Thresh,      &
...
!
                           IF ( info/=0 )                               &
     &                          CALL ALAERH(path,'CSYTRI2',info,0,uplo, &
...
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'CSYTRS',info,0,    &
...
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'CSYTRS2',info,0,   &
     &                             uplo,n,n,-1,-1,nrhs,imat,nfail,nerrs,&
     &                             Nout)
!
                              CALL CLACPY('Full',n,nrhs,B,lda,Work,lda)
...
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'CSYRFS',info,0,    &
     &                             uplo,n,n,-1,-1,nrhs,imat,nfail,nerrs,&
     &                             Nout)
!
                              CALL CGET04(n,nrhs,X,lda,Xact,lda,rcondc, &
...
!
 2                         anorm = CLANSY('1',uplo,n,A,lda,Rwork)
                           SRNamt = 'CSYCON'
                           CALL CSYCON(uplo,n,Afac,lda,Iwork,anorm,     &
     &                                 rcond,Work,info)
!
!                 Check error code from CSYCON and handle error.
!
                           IF ( info/=0 )                               &
     &                          CALL ALAERH(path,'CSYCON',info,0,uplo,n,&
