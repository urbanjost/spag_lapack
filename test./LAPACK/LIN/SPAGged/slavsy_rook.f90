!*==slavsy_rook.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SLAVSY_ROOK
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SLAVSY_ROOK(Uplo,Trans,Diag,N,Nrhs,A,Lda,Ipiv,B,Ldb,   &
     &                       Info)
      IMPLICIT NONE
!*--SLAVSY_ROOK161
!
...
!
                     CALL SGER(k-1,Nrhs,ONE,A(1,k),1,B(k,1),Ldb,B(1,1), &
     &                         Ldb)
!
...
!
                     CALL SGER(k-1,Nrhs,ONE,A(1,k),1,B(k,1),Ldb,B(1,1), &
     &                         Ldb)
                     CALL SGER(k-1,Nrhs,ONE,A(1,k+1),1,B(k+1,1),Ldb,    &
     &                         B(1,1),Ldb)
!
...
!                 Apply the transformation.
!
                     CALL SGER(N-k,Nrhs,ONE,A(k+1,k),1,B(k,1),Ldb,      &
     &                         B(k+1,1),Ldb)
!
!                 Interchange if a permutation was applied at the
!                 K-th step of the factorization.
...
!
                     CALL SGER(N-k,Nrhs,ONE,A(k+1,k),1,B(k,1),Ldb,      &
     &                         B(k+1,1),Ldb)
                     CALL SGER(N-k,Nrhs,ONE,A(k+1,k-1),1,B(k-1,1),Ldb,  &
     &                         B(k+1,1),Ldb)
!
!                 Interchange if a permutation was applied at the
!                 K-th step of the factorization.
...
!                 Apply the transformations
!
                  CALL SGEMV('Transpose',k-2,Nrhs,ONE,B,Ldb,A(1,k),1,   &
     &                       ONE,B(k,1),Ldb)
                  CALL SGEMV('Transpose',k-2,Nrhs,ONE,B,Ldb,A(1,k-1),1, &
     &                       ONE,B(k-1,1),Ldb)
               ENDIF
!
