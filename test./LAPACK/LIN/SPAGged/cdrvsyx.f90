!*==cdrvsy.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CDRVSYX
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CDRVSY(Dotype,Nn,Nval,Nrhs,Thresh,Tsterr,Nmax,A,Afac,  &
...
!
                           ...
                           CALL CSYTRF(uplo,n,Afac,lda,Iwork,Work,lwork,&
     &                                 info)
!
...
!
                           SRNamt = 'CSYSV '
                           CALL CSYSV(uplo,n,Nrhs,Afac,lda,Iwork,X,lda, &
     &                                Work,lwork,info)
!
...
!
                           IF ( info/=k ) THEN
                              CALL ALAERH(path,'CSYSV ',info,k,uplo,n,n,&
...
!
                        DO k = k1 , 6
                           IF ( result(k)>=Thresh ) THEN
                              IF ( nfail==0 .AND. nerrs==0 )            &
     &                             CALL ALADHD(Nout,path)
                              WRITE (Nout,FMT=99002) 'CSYSVX' , fact ,  &
     &                               uplo , n , imat , k , result(k)
                              nfail = nfail + 1
                           ENDIF
                        ENDDO
...
!
                        ...
                        n_err_bnds = 3
                        equed = 'N'
                        CALL CSYSVXX(fact,uplo,n,Nrhs,A,lda,Afac,lda,   &
...
!
                        IF ( info/=k .AND. info<=n ) THEN
                           CALL ALAERH(path,'CSYSVXX',info,k,fact//uplo,&
     &                                 n,n,-1,-1,Nrhs,imat,nfail,nerrs, &
     &                                 Nout)
                           CYCLE
                        ENDIF
...
!
                        DO k = k1 , 6
                           IF ( result(k)>=Thresh ) THEN
                              IF ( nfail==0 .AND. nerrs==0 )            &
     &                             CALL ALADHD(Nout,path)
                              WRITE (Nout,FMT=99002) 'CSYSVXX' , fact , &
     &                               uplo , n , imat , k , result(k)
                              nfail = nfail + 1
                           ENDIF
                        ENDDO
...
!     Test Error Bounds from CSYSVXX
