!*==zlavsy_rook.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZLAVSY_ROOK
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZLAVSY_ROOK(Uplo,Trans,Diag,N,Nrhs,A,Lda,Ipiv,B,Ldb,   &
     &                       Info)
      IMPLICIT NONE
!*--ZLAVSY_ROOK159
!
...
!                 Apply the transformation
!
                     CALL ZGEMV('Transpose',k-1,Nrhs,CONE,B,Ldb,A(1,k), &
     &                          1,CONE,B(k,1),Ldb)
                  ENDIF
                  IF ( nounit ) CALL ZSCAL(Nrhs,A(k,k),B(k,1),Ldb)
                  k = k - 1
...
!                 Apply the transformations
!
                     CALL ZGEMV('Transpose',k-2,Nrhs,CONE,B,Ldb,A(1,k), &
     &                          1,CONE,B(k,1),Ldb)
                     CALL ZGEMV('Transpose',k-2,Nrhs,CONE,B,Ldb,A(1,k-1)&
     &                          ,1,CONE,B(k-1,1),Ldb)
                  ENDIF
!
...
!                 Apply the transformation
!
                     CALL ZGEMV('Transpose',N-k-1,Nrhs,CONE,B(k+2,1),   &
