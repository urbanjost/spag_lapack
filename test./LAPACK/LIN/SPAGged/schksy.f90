!*==schksy.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SCHKSY
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SCHKSY(Dotype,Nn,Nval,Nnb,Nbval,Nns,Nsval,Thresh,      &
...
!     .. External Subroutines ..
     ...
     &         SSYCON , SSYCONV , SSYRFS , SSYT01 , SSYTRF , SSYTRI2 ,  &
...
!
                           IF ( info/=0 )                               &
     &                          CALL ALAERH(path,'SSYTRI2',info,-1,uplo,&
...
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'SSYTRS',info,0,    &
...
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'SSYTRS2',info,0,   &
     &                             uplo,n,n,-1,-1,nrhs,imat,nfail,nerrs,&
     &                             Nout)
!
                              CALL SLACPY('Full',n,nrhs,B,lda,Work,lda)
...
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'SSYRFS',info,0,    &
     &                             uplo,n,n,-1,-1,nrhs,imat,nfail,nerrs,&
     &                             Nout)
!
                              CALL SGET04(n,nrhs,X,lda,Xact,lda,rcondc, &
...
!
 2                         anorm = SLANSY('1',uplo,n,A,lda,Rwork)
                           SRNamt = 'SSYCON'
                           CALL SSYCON(uplo,n,Afac,lda,Iwork,anorm,     &
     &                                 rcond,Work,Iwork(n+1),info)
!
!                 Check error code from SSYCON and handle error.
!
                           IF ( info/=0 )                               &
     &                          CALL ALAERH(path,'SSYCON',info,0,uplo,n,&
