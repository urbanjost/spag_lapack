!*==ddrvsy.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DDRVSYX
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DDRVSY(Dotype,Nn,Nval,Nrhs,Thresh,Tsterr,Nmax,A,Afac,  &
...
!
                           ...
                           CALL DSYTRF(uplo,n,Afac,lda,Iwork,Work,lwork,&
     &                                 info)
!
...
!
                           SRNamt = 'DSYSV '
                           CALL DSYSV(uplo,n,Nrhs,Afac,lda,Iwork,X,lda, &
     &                                Work,lwork,info)
!
...
!
                           IF ( info/=k ) THEN
                              CALL ALAERH(path,'DSYSV ',info,k,uplo,n,n,&
...
!
                        DO k = k1 , 6
                           IF ( result(k)>=Thresh ) THEN
                              IF ( nfail==0 .AND. nerrs==0 )            &
     &                             CALL ALADHD(Nout,path)
                              WRITE (Nout,FMT=99002) 'DSYSVX' , fact ,  &
     &                               uplo , n , imat , k , result(k)
                              nfail = nfail + 1
                           ENDIF
                        ENDDO
...
!
                        ...
                        n_err_bnds = 3
                        equed = 'N'
                        CALL DSYSVXX(fact,uplo,n,Nrhs,A,lda,Afac,lda,   &
...
!
                        IF ( info/=k .AND. info<=n ) THEN
                           CALL ALAERH(path,'DSYSVXX',info,k,fact//uplo,&
     &                                 n,n,-1,-1,Nrhs,imat,nfail,nerrs, &
     &                                 Nout)
                           CYCLE
                        ENDIF
...
!
                        DO k = k1 , 6
                           IF ( result(k)>=Thresh ) THEN
                              IF ( nfail==0 .AND. nerrs==0 )            &
     &                             CALL ALADHD(Nout,path)
                              WRITE (Nout,FMT=99002) 'DSYSVXX' , fact , &
     &                               uplo , n , imat , k , result(k)
                              nfail = nfail + 1
                           ENDIF
                        ENDDO
