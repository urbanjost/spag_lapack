!*==zdrvgt.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZDRVGT
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZDRVGT(Dotype,Nn,Nval,Nrhs,Thresh,Tsterr,A,Af,B,X,Xact,&
...
!
                     ...
                     DO i = 1 , n
                        ...
                        ENDDO
                        X(i) = ONE
                        CALL ZGTTRS('Conjugate transpose',n,1,Af,Af(m+1)&
     &                              ,Af(n+m+1),Af(n+2*m+1),Iwork,X,lda, &
     &                              info)
                        ainvnm = MAX(ainvnm,DZASUM(n,X,1))
                     ENDDO
...
!
                        SRNamt = 'ZGTSV '
                        CALL ZGTSV(n,Nrhs,Af,Af(m+1),Af(n+m+1),X,lda,   &
     &                             info)
!
!                    Check error code from ZGTSV .
!
                        IF ( info/=izero )                              &
     &                       CALL ALAERH(path,'ZGTSV ',info,izero,' ',n,&
...
!
                        ...
                        ENDDO
                     ENDIF
                     CALL ZLASET('Full',n,Nrhs,DCMPLX(ZERO),DCMPLX(ZERO)&
     &                           ,X,lda)
!
...
!
                     SRNamt = 'ZGTSVX'
                     CALL ZGTSVX(fact,trans,n,Nrhs,A,A(m+1),A(n+m+1),Af,&
...
!
                     DO k = k1 , nt
                        IF ( result(k)>=Thresh ) THEN
                           IF ( nfail==0 .AND. nerrs==0 )               &
     &                          CALL ALADHD(Nout,path)
                           WRITE (Nout,FMT=99002) 'ZGTSVX' , fact ,     &
...
!
                     ...
                     IF ( result(6)>=Thresh ) THEN
                        IF ( nfail==0 .AND. nerrs==0 )                  &
     &                       CALL ALADHD(Nout,path)
                        WRITE (Nout,FMT=99002) 'ZGTSVX' , fact , trans ,&
     &                         n , imat , k , result(k)
                        nfail = nfail + 1
                     ENDIF
                     nrun = nrun + nt - k1 + 2
