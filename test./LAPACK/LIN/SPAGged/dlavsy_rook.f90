!*==dlavsy_rook.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DLAVSY_ROOK
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DLAVSY_ROOK(Uplo,Trans,Diag,N,Nrhs,A,Lda,Ipiv,B,Ldb,   &
     &                       Info)
      IMPLICIT NONE
!*--DLAVSY_ROOK161
!
...
!
                  IF ( nounit ) CALL DSCAL(Nrhs,A(k,k),B(k,1),Ldb)
!
...
!
                     CALL DGER(k-1,Nrhs,ONE,A(1,k),1,B(k,1),Ldb,B(1,1), &
     &                         Ldb)
!
!                 Interchange if P(K) .ne. I.
!
                     kp = Ipiv(k)
                     IF ( kp/=k ) CALL DSWAP(Nrhs,B(k,1),Ldb,B(kp,1),   &
     &                    Ldb)
                  ENDIF
                  k = k + 1
               ELSE
...
!
                     CALL DGER(k-1,Nrhs,ONE,A(1,k),1,B(k,1),Ldb,B(1,1), &
     &                         Ldb)
                     CALL DGER(k-1,Nrhs,ONE,A(1,k+1),1,B(k+1,1),Ldb,    &
     &                         B(1,1),Ldb)
!
...
!                 Apply the transformation.
!
                     CALL DGER(N-k,Nrhs,ONE,A(k+1,k),1,B(k,1),Ldb,      &
     &                         B(k+1,1),Ldb)
!
!                 Interchange if a permutation was applied at the
!                 K-th step of the factorization.
...
!
                     CALL DGER(N-k,Nrhs,ONE,A(k+1,k),1,B(k,1),Ldb,      &
     &                         B(k+1,1),Ldb)
                     CALL DGER(N-k,Nrhs,ONE,A(k+1,k-1),1,B(k-1,1),Ldb,  &
     &                         B(k+1,1),Ldb)
!
!                 Interchange if a permutation was applied at the
!                 K-th step of the factorization.
...
!
                  kp = ABS(Ipiv(k-1))
                  IF ( kp/=k-1 ) CALL DSWAP(Nrhs,B(k-1,1),Ldb,B(kp,1),  &
     &                 Ldb)
!
!                 Apply the transformations
!
                  CALL DGEMV('Transpose',k-2,Nrhs,ONE,B,Ldb,A(1,k),1,   &
     &                       ONE,B(k,1),Ldb)
                  CALL DGEMV('Transpose',k-2,Nrhs,ONE,B,Ldb,A(1,k-1),1, &
     &                       ONE,B(k-1,1),Ldb)
               ENDIF
!
...
!                 Apply the transformation
!
                  CALL DGEMV('Transpose',N-k-1,Nrhs,ONE,B(k+2,1),Ldb,   &
     &                       A(k+2,k+1),1,ONE,B(k+1,1),Ldb)
                  CALL DGEMV('Transpose',N-k-1,Nrhs,ONE,B(k+2,1),Ldb,   &
     &                       A(k+2,k),1,ONE,B(k,1),Ldb)
               ENDIF
!
!              Multiply by the diagonal block if non-unit.
!
