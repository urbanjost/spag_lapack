!*==zchkgt.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZCHKGT
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZCHKGT(Dotype,Nn,Nval,Nns,Nsval,Thresh,Tsterr,A,Af,B,X,&
...
!
               ...
               SRNamt = 'ZGTTRF'
               CALL ZGTTRF(n,Af,Af(m+1),Af(n+m+1),Af(n+2*m+1),Iwork,    &
     &                     info)
!
!           Check error code from ZGTTRF.
!
               IF ( info/=izero ) CALL ALAERH(path,'ZGTTRF',info,izero, &
     &              ' ',n,n,1,1,-1,imat,nfail,nerrs,Nout)
               trfcon = info/=0
!
               CALL ZGTT01(n,A,A(m+1),A(n+m+1),Af,Af(m+1),Af(n+m+1),    &
...
!
                     ...
                     DO i = 1 , n
                        ...
                        ENDDO
                        X(i) = ONE
                        CALL ZGTTRS(trans,n,1,Af,Af(m+1),Af(n+m+1),     &
...
!
                  SRNamt = 'ZGTCON'
                  CALL ZGTCON(norm,n,Af,Af(m+1),Af(n+m+1),Af(n+2*m+1),  &
     &                        Iwork,anorm,rcond,Work,info)
!
!              Check error code from ZGTCON.
!
                  IF ( info/=0 ) CALL ALAERH(path,'ZGTCON',info,0,norm, &
     &                 n,n,-1,-1,-1,imat,nfail,nerrs,Nout)
!
...
!
                        CALL ZLAGTM(trans,n,nrhs,ONE,A,A(m+1),A(n+m+1), &
     &                              Xact,lda,ZERO,B,lda)
!
!+    TEST 2
...
!
                        ...
                        SRNamt = 'ZGTTRS'
                        CALL ZGTTRS(trans,n,nrhs,Af,Af(m+1),Af(n+m+1),  &
     &                              Af(n+2*m+1),Iwork,X,lda,info)
!
...
!
                        ...
                        CALL ZGTT02(trans,n,nrhs,A,A(m+1),A(n+m+1),X,   &
...
!
                        SRNamt = 'ZGTRFS'
                        CALL ZGTRFS(trans,n,nrhs,A,A(m+1),A(n+m+1),Af,  &
...
!
                        CALL ZGET04(n,nrhs,X,lda,Xact,lda,rcondc,       &
     &                              result(4))
                        CALL ZGTT05(trans,n,nrhs,A,A(m+1),A(n+m+1),B,   &
