!*==cchkge.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CCHKGE
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CCHKGE(Dotype,Nm,Mval,Nn,Nval,Nnb,Nbval,Nns,Nsval,     &
...
!
                     IF ( info/=0 ) THEN
                        CALL ALAERH(path,'CLATMS',info,0,' ',m,n,-1,-1, &
...
!
                           IF ( info/=0 )                               &
     &                          CALL ALAERH(path,'CGETRI',info,0,' ',n, &
     &                          n,-1,-1,nb,imat,nfail,nerrs,Nout)
!
...
!                    of A.
!
                           CALL CGET03(n,A,lda,Ainv,lda,Work,lda,Rwork, &
     &                                 rcondo,result(2))
                           anormo = CLANGE('O',m,n,A,lda,Rwork)
!
...
!
     ...
     &                                 lda)
                                    SRNamt = 'CGETRS'
                                    CALL CGETRS(trans,n,nrhs,Afac,lda,  &
     &                                 Iwork,X,lda,info)
!
!                       Check error code from CGETRS.
!
                                    IF ( info/=0 )                      &
     &                                  CALL ALAERH(path,'CGETRS',info, &
     &                                 0,trans,n,n,-1,-1,nrhs,imat,     &
     &                                 nfail,nerrs,Nout)
!
                                    CALL CLACPY('Full',n,nrhs,B,lda,    &
     &                                 Work,lda)
                                    CALL CGET02(trans,n,n,nrhs,A,lda,X, &
...
!
                                    SRNamt = 'CGERFS'
                                    CALL CGERFS(trans,n,nrhs,A,lda,Afac,&
...
!
                                    IF ( info/=0 )                      &
     &                                  CALL ALAERH(path,'CGERFS',info, &
     &                                 0,trans,n,n,-1,-1,nrhs,imat,     &
     &                                 nfail,nerrs,Nout)
!
                                    CALL CGET04(n,nrhs,X,lda,Xact,lda,  &
     &                                 rcondc,result(5))
                                    CALL CGET07(trans,n,nrhs,A,lda,B,   &
...
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'CGECON',info,0,    &
