!*==sdrvpo.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SDRVPOX
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SDRVPO(Dotype,Nn,Nval,Nrhs,Thresh,Tsterr,Nmax,A,Afac,  &
...
!
                           SRNamt = 'SPOSVX'
                           CALL SPOSVX(fact,uplo,n,Nrhs,A,lda,Afac,lda, &
...
!                       refinement.
!
                              CALL SPOT05(uplo,n,Nrhs,Asav,lda,B,lda,X, &
...
!
     ...
     &                                CALL ALADHD(Nout,path)
                                 IF ( prefac ) THEN
                                    WRITE (Nout,FMT=99003) 'SPOSVX' ,   &
     &                                 fact , uplo , n , equed , imat , &
     &                                 k , result(k)
                                 ELSE
                                    WRITE (Nout,FMT=99002) 'SPOSVX' ,   &
     &                                 fact , uplo , n , imat , k ,     &
     &                                 result(k)
                                 ENDIF
                                 nfail = nfail + 1
...
!
                           SRNamt = 'SPOSVXX'
                           n_err_bnds = 3
                           CALL SPOSVXX(fact,uplo,n,Nrhs,A,lda,Afac,lda,&
