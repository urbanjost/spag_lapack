!*==cchkrq.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CCHKRQ
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CCHKRQ(Dotype,Nm,Mval,Nn,Nval,Nnb,Nbval,Nxval,Nrhs,    &
...
!
      IF ( Tsterr ) CALL CERRRQ(path,Nout)
      INFot = 0
      CALL XLAENV(2,2)
!
      lda = Nmax
...
!                       Test CGERQF
!
                           CALL CRQT01(m,n,A,Af,Aq,Ar,lda,Tau,Work,     &
...
!                       returned by CRQT01
!
                           CALL CRQT02(m,n,k,A,Af,Aq,Ar,lda,Tau,Work,   &
     &                                 lwork,Rwork,result(1))
                        ENDIF
                        IF ( m>=k ) THEN
!
...
!                       by CRQT01
!
                           CALL CRQT03(m,n,k,Af,Ac,Ar,Aq,lda,Tau,Work,  &
...
!
                              CALL CLACPY('Full',m,Nrhs,B,lda,X(n-m+1), &
     &                           lda)
                              SRNamt = 'CGERQS'
                              CALL CGERQS(m,n,Nrhs,Af,lda,Tau,X,lda,    &
     &                           Work,lwork,info)
!
