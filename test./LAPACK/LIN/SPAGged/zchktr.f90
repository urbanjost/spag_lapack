!*==zchktr.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZCHKTR
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZCHKTR(Dotype,Nn,Nval,Nnb,Nbval,Nns,Nsval,Thresh,      &
...
!
                  SRNamt = 'ZLATTR'
                  CALL ZLATTR(imat,uplo,'No transpose',diag,iseed,n,A,  &
...
!
                     IF ( info/=0 )                                     &
     &                    CALL ALAERH(path,'ZTRTRI',info,0,uplo//diag,n,&
...
!
                     SRNamt = 'ZLATTR'
                     CALL ZLATTR(imat,uplo,trans,diag,iseed,n,A,lda,X,  &
     &                           Work,Rwork,info)
!
!+    TEST 8
...
!
                     ...
                     CALL ZLATRS(uplo,trans,diag,'N',n,A,lda,B,scale,   &
     &                           Rwork,info)
!
!                 Check error code from ZLATRS.
!
                     IF ( info/=0 )                                     &
     &                    CALL ALAERH(path,'ZLATRS',info,0,uplo//trans//&
     &                    diag//'N',n,n,-1,-1,-1,imat,nfail,nerrs,Nout)
!
                     CALL ZTRT03(uplo,trans,diag,n,1,A,lda,scale,Rwork, &
...
!
                     ...
                     CALL ZLATRS(uplo,trans,diag,'Y',n,A,lda,B(n+1),    &
     &                           scale,Rwork,info)
!
!                 Check error code from ZLATRS.
!
...
!
                     IF ( result(8)>=Thresh ) THEN
                        IF ( nfail==0 .AND. nerrs==0 )                  &
     &                       CALL ALAHD(Nout,path)
                        WRITE (Nout,FMT=99004) 'ZLATRS' , uplo , trans ,&
