!*==crqt03.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CRQT03
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      IF ( K>0 .AND. N>K ) CALL CLACPY('Full',K,N-K,Af(M-K+1,1),Lda,    &
     &                                 Q(N-K+1,1),Lda)
...
!
      SRNamt = 'CUNGRQ'
      CALL CUNGRQ(N,N,K,Q,Lda,Tau(minmn-K+1),Work,Lwork,info)
!
      DO iside = 1 , 2
         IF ( iside==1 ) THEN
            side = 'L'
...
!
            SRNamt = 'CUNMRQ'
            IF ( K>0 ) CALL CUNMRQ(side,trans,mc,nc,K,Af(M-K+1,1),Lda,  &
