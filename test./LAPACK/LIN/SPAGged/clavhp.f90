!*==clavhp.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CLAVHP
!
!  =========== DOCUMENTATION ===========
!
...
!
                     CALL CGERU(k-1,Nrhs,ONE,A(kc),1,B(k,1),Ldb,B(1,1), &
     &                          Ldb)
!
...
!
                     CALL CGERU(k-1,Nrhs,ONE,A(kc),1,B(k,1),Ldb,B(1,1), &
     &                          Ldb)
                     CALL CGERU(k-1,Nrhs,ONE,A(kcnext),1,B(k+1,1),Ldb,  &
     &                          B(1,1),Ldb)
!
!                 Interchange if P(K) != I.
!
...
!                 Apply the transformation.
!
                     CALL CGERU(N-k,Nrhs,ONE,A(kc+1),1,B(k,1),Ldb,      &
     &                          B(k+1,1),Ldb)
!
...
!
                     CALL CGERU(N-k,Nrhs,ONE,A(kc+1),1,B(k,1),Ldb,      &
     &                          B(k+1,1),Ldb)
                     CALL CGERU(N-k,Nrhs,ONE,A(kcnext+2),1,B(k-1,1),Ldb,&
     &                          B(k+1,1),Ldb)
!
!                 Interchange if a permutation was applied at the
!                 K-th step of the factorization.
...
!
                  ...
                  CALL CGEMV('Conjugate',k-1,Nrhs,ONE,B,Ldb,A(kc),1,ONE,&
     &                       B(k,1),Ldb)
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
               ENDIF
               IF ( nounit ) CALL CSCAL(Nrhs,A(kc+k-1),B(k,1),Ldb)
...
!
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
                  CALL CGEMV('Conjugate',k-2,Nrhs,ONE,B,Ldb,A(kc),1,ONE,&
     &                       B(k,1),Ldb)
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
!
                  CALL CLACGV(Nrhs,B(k-1,1),Ldb)
                  CALL CGEMV('Conjugate',k-2,Nrhs,ONE,B,Ldb,A(kcnext),1,&
     &                       ONE,B(k-1,1),Ldb)
                  CALL CLACGV(Nrhs,B(k-1,1),Ldb)
               ENDIF
!
