!*==dchkge.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DCHKGE
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DCHKGE(Dotype,Nm,Mval,Nn,Nval,Nnb,Nbval,Nns,Nsval,     &
...
!
                     IF ( info/=0 ) THEN
                        CALL ALAERH(path,'DLATMS',info,0,' ',m,n,-1,-1, &
...
!
                           IF ( info/=0 )                               &
     &                          CALL ALAERH(path,'DGETRI',info,0,' ',n, &
     &                          n,-1,-1,nb,imat,nfail,nerrs,Nout)
!
...
!                    of A.
!
                           CALL DGET03(n,A,lda,Ainv,lda,Work,lda,Rwork, &
     &                                 rcondo,result(2))
                           anormo = DLANGE('O',m,n,A,lda,Rwork)
!
...
!
     ...
     &                                 lda)
                                    SRNamt = 'DGETRS'
                                    CALL DGETRS(trans,n,nrhs,Afac,lda,  &
     &                                 Iwork,X,lda,info)
!
!                       Check error code from DGETRS.
!
                                    IF ( info/=0 )                      &
     &                                  CALL ALAERH(path,'DGETRS',info, &
     &                                 0,trans,n,n,-1,-1,nrhs,imat,     &
     &                                 nfail,nerrs,Nout)
!
                                    CALL DLACPY('Full',n,nrhs,B,lda,    &
     &                                 Work,lda)
                                    CALL DGET02(trans,n,n,nrhs,A,lda,X, &
...
!
                                    SRNamt = 'DGERFS'
                                    CALL DGERFS(trans,n,nrhs,A,lda,Afac,&
...
!
                                    IF ( info/=0 )                      &
     &                                  CALL ALAERH(path,'DGERFS',info, &
     &                                 0,trans,n,n,-1,-1,nrhs,imat,     &
     &                                 nfail,nerrs,Nout)
!
                                    CALL DGET04(n,nrhs,X,lda,Xact,lda,  &
     &                                 rcondc,result(5))
                                    CALL DGET07(trans,n,nrhs,A,lda,B,   &
...
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'DGECON',info,0,    &
