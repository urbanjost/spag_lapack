!*==zchkpo.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZCHKPO
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZCHKPO(Dotype,Nn,Nval,Nnb,Nbval,Nns,Nsval,Thresh,      &
...
!
                           IF ( info/=0 )                               &
     &                          CALL ALAERH(path,'ZPOTRI',info,0,uplo,n,&
...
!
                                 SRNamt = 'ZLARHS'
                                 CALL ZLARHS(path,xtype,uplo,' ',n,n,kl,&
...
!
                                 IF ( info/=0 )                         &
     &                                 CALL ALAERH(path,'ZPOTRS',info,0,&
...
!
                                 SRNamt = 'ZPORFS'
                                 CALL ZPORFS(uplo,n,nrhs,A,lda,Afac,lda,&
...
!
                                 IF ( info/=0 )                         &
     &                                 CALL ALAERH(path,'ZPORFS',info,0,&
     &                                uplo,n,n,-1,-1,nrhs,imat,nfail,   &
     &                                nerrs,Nout)
!
                                 CALL ZGET04(n,nrhs,X,lda,Xact,lda,     &
     &                              rcondc,result(5))
                                 CALL ZPOT05(uplo,n,nrhs,A,lda,B,lda,X, &
...
!
                              ...
                              SRNamt = 'ZPOCON'
                              CALL ZPOCON(uplo,n,Afac,lda,anorm,rcond,  &
     &                           Work,Rwork,info)
!
!                 Check error code from ZPOCON.
!
                              IF ( info/=0 )                            &
     &                             CALL ALAERH(path,'ZPOCON',info,0,    &
