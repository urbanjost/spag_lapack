!*==clavhe_rook.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CLAVHE_ROOK
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CLAVHE_ROOK(Uplo,Trans,Diag,N,Nrhs,A,Lda,Ipiv,B,Ldb,   &
     &                       Info)
      IMPLICIT NONE
!*--CLAVHE_ROOK160
!
...
!
                  ...
                  CALL CGEMV('Conjugate',k-1,Nrhs,CONE,B,Ldb,A(1,k),1,  &
     &                       CONE,B(k,1),Ldb)
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
               ENDIF
               IF ( nounit ) CALL CSCAL(Nrhs,A(k,k),B(k,1),Ldb)
...
!
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
                  CALL CGEMV('Conjugate',k-2,Nrhs,CONE,B,Ldb,A(1,k),1,  &
     &                       CONE,B(k,1),Ldb)
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
!
                  CALL CLACGV(Nrhs,B(k-1,1),Ldb)
                  CALL CGEMV('Conjugate',k-2,Nrhs,CONE,B,Ldb,A(1,k-1),1,&
     &                       CONE,B(k-1,1),Ldb)
                  CALL CLACGV(Nrhs,B(k-1,1),Ldb)
               ENDIF
!
...
!
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
                  CALL CGEMV('Conjugate',N-k,Nrhs,CONE,B(k+1,1),Ldb,    &
...
!
                  CALL CLACGV(Nrhs,B(k+1,1),Ldb)
                  CALL CGEMV('Conjugate',N-k-1,Nrhs,CONE,B(k+2,1),Ldb,  &
