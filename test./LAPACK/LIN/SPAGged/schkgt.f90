!*==schkgt.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SCHKGT
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SCHKGT(Dotype,Nn,Nval,Nns,Nsval,Thresh,Tsterr,A,Af,B,X,&
...
!
               ...
               SRNamt = 'SGTTRF'
               CALL SGTTRF(n,Af,Af(m+1),Af(n+m+1),Af(n+2*m+1),Iwork,    &
     &                     info)
!
!           Check error code from SGTTRF.
!
               IF ( info/=izero ) CALL ALAERH(path,'SGTTRF',info,izero, &
     &              ' ',n,n,1,1,-1,imat,nfail,nerrs,Nout)
               trfcon = info/=0
!
               CALL SGTT01(n,A,A(m+1),A(n+m+1),Af,Af(m+1),Af(n+m+1),    &
...
!
                     ...
                     DO i = 1 , n
                        ...
                        ENDDO
                        X(i) = ONE
                        CALL SGTTRS(trans,n,1,Af,Af(m+1),Af(n+m+1),     &
...
!
                  SRNamt = 'SGTCON'
                  CALL SGTCON(norm,n,Af,Af(m+1),Af(n+m+1),Af(n+2*m+1),  &
     &                        Iwork,anorm,rcond,Work,Iwork(n+1),info)
!
!              Check error code from SGTCON.
!
                  IF ( info/=0 ) CALL ALAERH(path,'SGTCON',info,0,norm, &
     &                 n,n,-1,-1,-1,imat,nfail,nerrs,Nout)
!
...
!
                        ...
                        SRNamt = 'SGTTRS'
                        CALL SGTTRS(trans,n,nrhs,Af,Af(m+1),Af(n+m+1),  &
     &                              Af(n+2*m+1),Iwork,X,lda,info)
!
...
!
                        SRNamt = 'SGTRFS'
                        CALL SGTRFS(trans,n,nrhs,A,A(m+1),A(n+m+1),Af,  &
