!*==claptm.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CLAPTM
!
!  =========== DOCUMENTATION ===========
!
...
!
            DO j = 1 , Nrhs
               ...
               ELSE
                  ...
                  B(N,j) = B(N,j) + CONJG(E(N-1))*X(N-1,j) + D(N)*X(N,j)
                  DO i = 2 , N - 1
                     B(i,j) = B(i,j) + CONJG(E(i-1))*X(i-1,j) + D(i)    &
     &                        *X(i,j) + E(i)*X(i+1,j)
                  ENDDO
               ENDIF
            ENDDO
...
!
            DO j = 1 , Nrhs
               IF ( N==1 ) THEN
                  B(1,j) = B(1,j) + D(1)*X(1,j)
               ELSE
                  B(1,j) = B(1,j) + D(1)*X(1,j) + CONJG(E(1))*X(2,j)
