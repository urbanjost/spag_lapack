!*==dlattr.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DLATTR
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Local Scalars ..
     ...
     &                 smlnum , star1 , texp , tleft , tscal , ulp ,    &
     &                 unfl , x , y , z
!     ..
!     .. External Functions ..
      LOGICAL LSAME
...
!
               IF ( N>j+1 ) CALL DROT(N-j-1,A(j,j+2),Lda,A(j+1,j+2),Lda,&
     &                                c,s)
!
...
!              Multiply by [ c -s;  s  c] on the right.
!
               IF ( N>j+1 ) CALL DROT(N-j-1,A(j+2,j+1),1,A(j+2,j),1,c,  &
     &                                -s)
!
!              Multiply by [-c  s; -s -c] on the left.
!
