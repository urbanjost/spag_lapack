!*==schktp.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SCHKTP
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SCHKTP(Dotype,Nn,Nval,Nns,Nsval,Thresh,Tsterr,Nmax,Ap, &
...
!
                  SRNamt = 'SLATTP'
                  CALL SLATTP(imat,uplo,'No transpose',diag,iseed,n,Ap, &
     &                        X,Work,info)
!
...
!
                  IF ( info/=0 )                                        &
     &                 CALL ALAERH(path,'STPTRI',info,0,uplo//diag,n,n, &
...
!
                        SRNamt = 'SLARHS'
                        CALL SLARHS(path,xtype,uplo,trans,n,n,0,idiag,  &
...
!
                        SRNamt = 'STPTRS'
                        CALL STPTRS(uplo,trans,diag,n,nrhs,Ap,X,lda,    &
     &                              info)
!
!                 Check error code from STPTRS.
!
                        IF ( info/=0 )                                  &
     &                       CALL ALAERH(path,'STPTRS',info,0,uplo//    &
     &                       trans//diag,n,n,-1,-1,-1,imat,nfail,nerrs, &
     &                       Nout)
!
                        CALL STPT02(uplo,trans,diag,n,nrhs,Ap,X,lda,B,  &
     &                              lda,Work,result(2))
!
!+    TEST 3
...
!
                        SRNamt = 'STPRFS'
                        CALL STPRFS(uplo,trans,diag,n,nrhs,Ap,B,lda,X,  &
...
!
                        IF ( info/=0 )                                  &
     &                       CALL ALAERH(path,'STPRFS',info,0,uplo//    &
     &                       trans//diag,n,n,-1,-1,nrhs,imat,nfail,     &
     &                       nerrs,Nout)
!
                        CALL SGET04(n,nrhs,X,lda,Xact,lda,rcondc,       &
     &                              result(4))
                        CALL STPT05(uplo,trans,diag,n,nrhs,Ap,B,lda,X,  &
...
!
                     SRNamt = 'STPCON'
                     CALL STPCON(norm,uplo,diag,n,Ap,rcond,Work,Iwork,  &
     &                           info)
!
!                 Check error code from STPCON.
!
                     IF ( info/=0 )                                     &
     &                    CALL ALAERH(path,'STPCON',info,0,norm//uplo// &
...
!
                     IF ( result(7)>=Thresh ) THEN
                        IF ( nfail==0 .AND. nerrs==0 )                  &
     &                       CALL ALAHD(Nout,path)
                        WRITE (Nout,FMT=99003) 'STPCON' , norm , uplo , &
...
!
                     SRNamt = 'SLATTP'
                     CALL SLATTP(imat,uplo,trans,diag,iseed,n,Ap,X,Work,&
     &                           info)
!
!+    TEST 8
...
!
                     ...
                     CALL SLATPS(uplo,trans,diag,'N',n,Ap,B,scale,Rwork,&
     &                           info)
!
!                 Check error code from SLATPS.
!
                     IF ( info/=0 )                                     &
     &                    CALL ALAERH(path,'SLATPS',info,0,uplo//trans//&
     &                    diag//'N',n,n,-1,-1,-1,imat,nfail,nerrs,Nout)
!
                     CALL STPT03(uplo,trans,diag,n,1,Ap,scale,Rwork,ONE,&
...
!
                     ...
                     CALL SLATPS(uplo,trans,diag,'Y',n,Ap,B(n+1),scale, &
     &                           Rwork,info)
!
!                 Check error code from SLATPS.
!
...
!
                     IF ( result(8)>=Thresh ) THEN
                        IF ( nfail==0 .AND. nerrs==0 )                  &
     &                       CALL ALAHD(Nout,path)
                        WRITE (Nout,FMT=99004) 'SLATPS' , uplo , trans ,&
