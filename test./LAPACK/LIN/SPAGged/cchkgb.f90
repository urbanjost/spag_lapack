!*==cchkgb.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CCHKGB
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CCHKGB(Dotype,Nm,Mval,Nn,Nval,Nnb,Nbval,Nns,Nsval,     &
...
!     .. Local Scalars ..
     ...
     &        nimat , nkl , nku , nrhs , nrun
      REAL ainvnm , anorm , anormi , anormo , cndnum , rcond , rcondc , &
     &     rcondi , rcondo
!     ..
!     .. Local Arrays ..
...
!
                                 anormo = CLANGB('O',n,kl,ku,A,lda,     &
     &                              Rwork)
                                 anormi = CLANGB('I',n,kl,ku,A,lda,     &
     &                              Rwork)
!
                                 IF ( info==0 ) THEN
!
...
!
     ...
     &                                 CMPLX(ONE),Work,ldb)
                                    SRNamt = 'CGBTRS'
                                    CALL CGBTRS('No transpose',n,kl,ku, &
...
!
                                         SRNamt = 'CLARHS'
                                         CALL CLARHS(path,xtype,' ',    &
     &                                      trans,n,n,kl,ku,nrhs,A,lda, &
     &                                      Xact,ldb,B,ldb,iseed,info)
                                         xtype = 'C'
                                         CALL CLACPY('Full',n,nrhs,B,   &
     &                                      ldb,X,ldb)
!
                                         SRNamt = 'CGBTRS'
                                         CALL CGBTRS(trans,n,kl,ku,nrhs,&
...
!
                                         IF ( info/=0 )                 &
     &                                      CALL ALAERH(path,'CGBTRS',  &
     &                                      info,0,trans,n,n,kl,ku,-1,  &
     &                                      imat,nfail,nerrs,Nout)
!
                                         CALL CLACPY('Full',n,nrhs,B,   &
     &                                      ldb,Work,ldb)
                                         CALL CGBT02(trans,m,n,kl,ku,   &
...
!
                                         SRNamt = 'CGBRFS'
                                         CALL CGBRFS(trans,n,kl,ku,nrhs,&
...
!
                                         IF ( info/=0 )                 &
     &                                      CALL ALAERH(path,'CGBRFS',  &
     &                                      info,0,trans,n,n,kl,ku,nrhs,&
     &                                      imat,nfail,nerrs,Nout)
!
                                         CALL CGET04(n,nrhs,X,ldb,Xact, &
     &                                      ldb,rcondc,result(4))
                                         CALL CGBT05(trans,n,kl,ku,nrhs,&
...
!
99001 FORMAT (' *** In CCHKGB, LA=',I5,' is too small for M=',I5,', N=',&
     &        I5,', KL=',I4,', KU=',I4,/' ==> Increase LA to at least ',&
     &        I5)
99002 FORMAT (' *** In CCHKGB, LAFAC=',I5,' is too small for M=',I5,    &
     &        ', N=',I5,', KL=',I4,', KU=',I4,                          &
