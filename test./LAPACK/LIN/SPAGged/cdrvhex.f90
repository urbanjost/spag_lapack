!*==cdrvhe.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CDRVHEX
!
!  =========== DOCUMENTATION ===========
!
...
!
                        ...
                        n_err_bnds = 3
                        equed = 'N'
                        CALL CHESVXX(fact,uplo,n,Nrhs,A,lda,Afac,lda,   &
...
!
                        IF ( info/=k .AND. info<=n ) THEN
                           CALL ALAERH(path,'CHESVXX',info,k,fact//uplo,&
     &                                 n,n,-1,-1,Nrhs,imat,nfail,nerrs, &
     &                                 Nout)
                           CYCLE
                        ENDIF
...
!
                        DO k = k1 , 6
                           IF ( result(k)>=Thresh ) THEN
                              IF ( nfail==0 .AND. nerrs==0 )            &
     &                             CALL ALADHD(Nout,path)
                              WRITE (Nout,FMT=99002) 'CHESVXX' , fact , &
     &                               uplo , n , imat , k , result(k)
                              nfail = nfail + 1
                           ENDIF
                        ENDDO
