!*==dchkpb.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DCHKPB
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DCHKPB(Dotype,Nn,Nval,Nnb,Nbval,Nns,Nsval,Thresh,      &
...
!
     ...
     &                           A(ioff-izero+i1),1)
                              iw = iw + izero - i1
                              CALL DCOPY(i2-izero+1,Work(iw),1,A(ioff), &
     &                           MAX(ldab-1,1))
                           ELSE
                              ioff = (i1-1)*ldab + 1
                              CALL DCOPY(izero-i1,Work(iw),1,           &
...
!
     ...
     &                           MAX(ldab-1,1),Work(iw),1)
                              ioff = (izero-1)*ldab + 1
                              iw = iw + izero - i1
                              CALL DSWAP(i2-izero+1,A(ioff),1,Work(iw), &
     &                           1)
                           ENDIF
                        ENDIF
!
...
!
                                 CALL DLASET('Full',n,n,ZERO,ONE,Ainv,  &
     &                              lda)
                                 SRNamt = 'DPBTRS'
                                 CALL DPBTRS(uplo,n,kd,n,Afac,ldab,Ainv,&
     &                              lda,info)
!
!                    Compute RCONDC = 1/(norm(A) * norm(inv(A))).
!
                                 anorm = DLANSB('1',uplo,n,kd,A,ldab,   &
     &                              Rwork)
...
!
                                    SRNamt = 'DLARHS'
                                    CALL DLARHS(path,xtype,uplo,' ',n,n,&
...
!
                                    SRNamt = 'DPBTRS'
                                    CALL DPBTRS(uplo,n,kd,nrhs,Afac,    &
     &                                 ldab,X,lda,info)
!
!                    Check error code from DPBTRS.
!
                                    IF ( info/=0 )                      &
     &                                  CALL ALAERH(path,'DPBTRS',info, &
...
!
                                    SRNamt = 'DPBRFS'
                                    CALL DPBRFS(uplo,n,kd,nrhs,A,ldab,  &
...
!
                                    IF ( info/=0 )                      &
     &                                  CALL ALAERH(path,'DPBRFS',info, &
     &                                 0,uplo,n,n,kd,kd,nrhs,imat,nfail,&
     &                                 nerrs,Nout)
!
                                    CALL DGET04(n,nrhs,X,lda,Xact,lda,  &
     &                                 rcondc,result(4))
                                    CALL DPBT05(uplo,n,kd,nrhs,A,ldab,B,&
...
!
                                 SRNamt = 'DPBCON'
                                 CALL DPBCON(uplo,n,kd,Afac,ldab,anorm, &
     &                              rcond,Work,Iwork,info)
!
!                    Check error code from DPBCON.
!
                                 IF ( info/=0 )                         &
     &                                 CALL ALAERH(path,'DPBCON',info,0,&
