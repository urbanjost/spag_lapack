!*==schkrq.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SCHKRQ
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SCHKRQ(Dotype,Nm,Mval,Nn,Nval,Nnb,Nbval,Nxval,Nrhs,    &
...
!
      IF ( Tsterr ) CALL SERRRQ(path,Nout)
      INFot = 0
      CALL XLAENV(2,2)
!
      lda = Nmax
...
!                       Test SGERQF
!
                           CALL SRQT01(m,n,A,Af,Aq,Ar,lda,Tau,Work,     &
...
!                       returned by SRQT01
!
                           CALL SRQT02(m,n,k,A,Af,Aq,Ar,lda,Tau,Work,   &
     &                                 lwork,Rwork,result(1))
                        ENDIF
                        IF ( m>=k ) THEN
!
...
!                       by SRQT01
!
                           CALL SRQT03(m,n,k,Af,Ac,Ar,Aq,lda,Tau,Work,  &
...
!
                              CALL SLACPY('Full',m,Nrhs,B,lda,X(n-m+1), &
     &                           lda)
                              SRNamt = 'SGERQS'
                              CALL SGERQS(m,n,Nrhs,Af,lda,Tau,X,lda,    &
     &                           Work,lwork,info)
!
