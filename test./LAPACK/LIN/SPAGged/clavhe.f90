!*==clavhe.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CLAVHE
!
!  =========== DOCUMENTATION ===========
!
...
!
                     CALL CGERU(k-1,Nrhs,ONE,A(1,k),1,B(k,1),Ldb,B(1,1),&
     &                          Ldb)
!
...
!
                     CALL CGERU(k-1,Nrhs,ONE,A(1,k),1,B(k,1),Ldb,B(1,1),&
     &                          Ldb)
                     CALL CGERU(k-1,Nrhs,ONE,A(1,k+1),1,B(k+1,1),Ldb,   &
     &                          B(1,1),Ldb)
!
!                 Interchange if P(K) != I.
!
...
!                 Apply the transformation.
!
                     CALL CGERU(N-k,Nrhs,ONE,A(k+1,k),1,B(k,1),Ldb,     &
     &                          B(k+1,1),Ldb)
!
...
!
                     CALL CGERU(N-k,Nrhs,ONE,A(k+1,k),1,B(k,1),Ldb,     &
     &                          B(k+1,1),Ldb)
                     CALL CGERU(N-k,Nrhs,ONE,A(k+1,k-1),1,B(k-1,1),Ldb, &
     &                          B(k+1,1),Ldb)
!
!                 Interchange if a permutation was applied at the
!                 K-th step of the factorization.
...
!
                  ...
                  CALL CGEMV('Conjugate',k-1,Nrhs,ONE,B,Ldb,A(1,k),1,   &
     &                       ONE,B(k,1),Ldb)
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
               ENDIF
               IF ( nounit ) CALL CSCAL(Nrhs,A(k,k),B(k,1),Ldb)
...
!
                  kp = ABS(Ipiv(k))
                  IF ( kp/=k-1 ) CALL CSWAP(Nrhs,B(k-1,1),Ldb,B(kp,1),  &
     &                 Ldb)
!
!                 Apply the transformations
!                    y = y - B' conjg(x),
...
!
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
                  CALL CGEMV('Conjugate',k-2,Nrhs,ONE,B,Ldb,A(1,k),1,   &
     &                       ONE,B(k,1),Ldb)
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
!
                  CALL CLACGV(Nrhs,B(k-1,1),Ldb)
                  CALL CGEMV('Conjugate',k-2,Nrhs,ONE,B,Ldb,A(1,k-1),1, &
     &                       ONE,B(k-1,1),Ldb)
                  CALL CLACGV(Nrhs,B(k-1,1),Ldb)
               ENDIF
!
...
!
                  CALL CLACGV(Nrhs,B(k,1),Ldb)
                  CALL CGEMV('Conjugate',N-k,Nrhs,ONE,B(k+1,1),Ldb,     &
...
!
                  CALL CLACGV(Nrhs,B(k+1,1),Ldb)
                  CALL CGEMV('Conjugate',N-k-1,Nrhs,ONE,B(k+2,1),Ldb,   &
