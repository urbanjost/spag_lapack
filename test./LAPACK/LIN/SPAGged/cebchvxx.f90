!*==cebchvxx.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CEBCHVXX
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Local Arrays ..
      REAL tstrat(NTESTS) , rinv(NMAX) , params(NPARAMS) , s(NMAX) ,    &
     &     r(NMAX) , c(NMAX) , rwork(3*NMAX) , diff(NMAX,NMAX) ,        &
...
!     .. External Subroutines ..
      EXTERNAL CLAHILB , CGESVXX , CSYSVXX , CPOSVXX , CGBSVXX ,        &
     &         CLACPY , LSAMEN
      LOGICAL LSAMEN
...
!        right hand side, all scaled by the LCM(1,..,2N-1).
         CALL CLAHILB(n,n,a,lda,invhilb,lda,b,lda,work,info,Path)
...
99008                FORMAT (3X,I2,': Raw componentwise error estimate')
     ...
     &                            tstrat(i)
99009             FORMAT (' C',A2,'SVXX: N =',I2,', RHS = ',I2,         &
...
99010    FORMAT (' C',A2,'SVXX: ',I6,' out of ',I6,                     &
     &           ' tests failed to pass the threshold')
      ELSE
         WRITE (*,99011) c2
99011    FORMAT (' C',A2,'SVXX passed the tests of error bounds')
      ENDIF
 
99012 FORMAT (' C',A2,'SVXX: N =',I2,', INFO = ',I3,', ORCOND = ',G12.5,&
     &        ', real RCOND = ',G12.5)
