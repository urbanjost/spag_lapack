!*==zchksy_aa.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZCHKSY_AA
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZCHKSY_AA(Dotype,Nn,Nval,Nnb,Nbval,Nns,Nsval,Thresh,   &
...
!
                        SRNamt = 'ZSYTRF_AA'
                        lwork = MAX(1,n*nb+n)
                        CALL ZSYTRF_AA(uplo,n,Afac,lda,Iwork,Ainv,lwork,&
     &                                 info)
!
...
!
                        IF ( info/=k )                                  &
     &                       CALL ALAERH(path,'ZSYTRF_AA',info,k,uplo,n,&
     &                       n,-1,-1,nb,imat,nfail,nerrs,Nout)
!
!+    TEST 1
!                 Reconstruct matrix from factors and compute residual.
!
                        CALL ZSYT01_AA(uplo,n,A,lda,Afac,lda,Iwork,Ainv,&
     &                                 lda,Rwork,result(1))
                        nt = 1
!
!
...
!
                           SRNamt = 'ZSYTRS_AA'
                           lwork = MAX(1,3*n-2)
                           CALL ZSYTRS_AA(uplo,n,nrhs,Afac,lda,Iwork,X, &
     &                        lda,Work,lwork,info)
!
...
!
     ...
     &                                 nrhs , imat , k , result(k)
                              ...
                              ENDDO
                           ELSEIF ( izero==0 ) THEN
                              CALL ALAERH(path,'ZSYTRS_AA',info,0,uplo, &
