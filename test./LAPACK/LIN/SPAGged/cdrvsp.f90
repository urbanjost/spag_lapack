!*==cdrvsp.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CDRVSP
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CDRVSP(Dotype,Nn,Nval,Nrhs,Thresh,Tsterr,Nmax,A,Afac,  &
...
!     .. External Subroutines ..
     ...
     &         CLACPY , CLARHS , CLASET , CLATB4 , CLATMS , CLATSP ,    &
     &         CPPT05 , CSPSV , CSPSVX , CSPT01 , CSPT02 , CSPTRF ,     &
     &         CSPTRI , XLAENV
!     ..
!     .. Scalars in Common ..
      LOGICAL LERr , OK
...
!
                           IF ( info/=k ) THEN
                              CALL ALAERH(path,'CSPSV ',info,k,uplo,n,n,&
...
!                    residual.
!
                           CALL CSPT01(uplo,n,A,Afac,Iwork,Ainv,lda,    &
     &                                 Rwork,result(1))
!
...
!
                        SRNamt = 'CSPSVX'
                        CALL CSPSVX(fact,uplo,n,Nrhs,A,Afac,Iwork,B,lda,&
...
!
                        IF ( info/=k ) THEN
                           CALL ALAERH(path,'CSPSVX',info,k,fact//uplo, &
...
!
                        DO k = k1 , 6
                           IF ( result(k)>=Thresh ) THEN
                              IF ( nfail==0 .AND. nerrs==0 )            &
     &                             CALL ALADHD(Nout,path)
                              WRITE (Nout,FMT=99002) 'CSPSVX' , fact ,  &
     &                               uplo , n , imat , k , result(k)
                              nfail = nfail + 1
                           ENDIF
                        ENDDO
