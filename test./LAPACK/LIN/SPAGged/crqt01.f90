!*==crqt01.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CRQT01
!
!  =========== DOCUMENTATION ===========
!
...
!
      ...
      IF ( M<=N ) THEN
         IF ( M>0 ) CALL CLACPY('Upper',M,M,Af(1,N-M+1),Lda,R(1,N-M+1), &
     &                          Lda)
      ELSE
         IF ( M>N .AND. N>0 ) CALL CLACPY('Full',M-N,N,Af,Lda,R,Lda)
         IF ( N>0 ) CALL CLACPY('Upper',N,N,Af(M-N+1,1),Lda,R(M-N+1,1), &
     &                          Lda)
      ENDIF
!
!     Compute R - A*Q'
