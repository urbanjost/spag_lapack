!*==zget23.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZGET23
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZGET23(Comp,Isrt,Balanc,Jtype,Thresh,Iseed,Nounit,N,A, &
...
!
      ...
      CALL ZGEEVX(Balanc,'V','V',sense,N,H,Lda,W,Vl,Ldvl,Vr,Ldvr,ilo,   &
     &            ihi,Scale,abnrm,Rconde,Rcondv,Work,Lwork,Rwork,iinfo)
      IF ( iinfo/=0 ) THEN
         Result(1) = ulpinv
         IF ( Jtype/=22 ) THEN
            WRITE (Nounit,FMT=99002) 'ZGEEVX1' , iinfo , N , Jtype ,    &
     &                               Balanc , Iseed
         ELSE
            WRITE (Nounit,FMT=99001) 'ZGEEVX1' , iinfo , N , Iseed(1)
         ENDIF
         Info = ABS(iinfo)
         RETURN
      ENDIF
...
!
      DO j = 1 , N
         ...
         DO jj = 1 , N
            ...
            IF ( DIMAG(Vr(jj,j))==ZERO .AND. ABS(DBLE(Vr(jj,j)))>vrmx ) &
...
!
      DO j = 1 , N
         ...
         DO jj = 1 , N
            vtst = ABS(Vl(jj,j))
            IF ( vtst>vmx ) vmx = vtst
            IF ( DIMAG(Vl(jj,j))==ZERO .AND. ABS(DBLE(Vl(jj,j)))>vrmx ) &
...
!
         CALL ZLACPY('F',N,N,A,Lda,H,Lda)
         CALL ZGEEVX(Balanc,'N','N',sense,N,H,Lda,W1,cdum,1,cdum,1,ilo1,&
     &               ihi1,Scale1,abnrm1,Rcnde1,Rcndv1,Work,Lwork,Rwork, &
     &               iinfo)
         IF ( iinfo/=0 ) THEN
            Result(1) = ulpinv
            IF ( Jtype/=22 ) THEN
               WRITE (Nounit,FMT=99002) 'ZGEEVX2' , iinfo , N , Jtype , &
     &                                  Balanc , Iseed
            ELSE
               WRITE (Nounit,FMT=99001) 'ZGEEVX2' , iinfo , N , Iseed(1)
            ENDIF
            Info = ABS(iinfo)
            CYCLE
         ENDIF
...
!
         CALL ZLACPY('F',N,N,A,Lda,H,Lda)
         CALL ZGEEVX(Balanc,'N','V',sense,N,H,Lda,W1,cdum,1,Lre,Ldlre,  &
     &               ilo1,ihi1,Scale1,abnrm1,Rcnde1,Rcndv1,Work,Lwork,  &
     &               Rwork,iinfo)
         IF ( iinfo/=0 ) THEN
            Result(1) = ulpinv
            IF ( Jtype/=22 ) THEN
               WRITE (Nounit,FMT=99002) 'ZGEEVX3' , iinfo , N , Jtype , &
     &                                  Balanc , Iseed
            ELSE
               WRITE (Nounit,FMT=99001) 'ZGEEVX3' , iinfo , N , Iseed(1)
            ENDIF
            Info = ABS(iinfo)
            CYCLE
         ENDIF
...
!
         CALL ZLACPY('F',N,N,A,Lda,H,Lda)
         CALL ZGEEVX(Balanc,'V','N',sense,N,H,Lda,W1,Lre,Ldlre,cdum,1,  &
     &               ilo1,ihi1,Scale1,abnrm1,Rcnde1,Rcndv1,Work,Lwork,  &
     &               Rwork,iinfo)
         IF ( iinfo/=0 ) THEN
            Result(1) = ulpinv
            IF ( Jtype/=22 ) THEN
               WRITE (Nounit,FMT=99002) 'ZGEEVX4' , iinfo , N , Jtype , &
     &                                  Balanc , Iseed
            ELSE
               WRITE (Nounit,FMT=99001) 'ZGEEVX4' , iinfo , N , Iseed(1)
            ENDIF
            Info = ABS(iinfo)
            CYCLE
         ENDIF
...
!
      IF ( Comp ) THEN
         CALL ZLACPY('F',N,N,A,Lda,H,Lda)
         CALL ZGEEVX('N','V','V','B',N,H,Lda,W,Vl,Ldvl,Vr,Ldvr,ilo,ihi, &
     &               Scale,abnrm,Rconde,Rcondv,Work,Lwork,Rwork,iinfo)
         IF ( iinfo/=0 ) THEN
            Result(1) = ulpinv
            WRITE (Nounit,FMT=99001) 'ZGEEVX5' , iinfo , N , Iseed(1)
            Info = ABS(iinfo)
            GOTO 99999
         ENDIF
!
...
!
      ENDIF
!
99001 FORMAT (' ZGET23: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', INPUT EXAMPLE NUMBER = ',I4)
99002 FORMAT (' ZGET23: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
