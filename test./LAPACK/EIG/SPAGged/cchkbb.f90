!*==cchkbb.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CCHKBB
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CCHKBB(Nsizes,Mval,Nval,Nwdths,Kk,Ntypes,Dotype,Nrhs,  &
...
!     .. Array Arguments ..
      ...
      INTEGER Iseed(4) , Kk(*) , Mval(*) , Nval(*)
      REAL Bd(*) , Be(*) , Result(*) , Rwork(*)
      COMPLEX A(Lda,*) , Ab(Ldab,*) , C(Ldc,*) , Cc(Ldc,*) , P(Ldp,*) , &
     &        Q(Ldq,*) , Work(*)
!     ..
!
!  =====================================================================
...
!
                     CALL CGBBRD('B',m,n,Nrhs,kl,ku,Ab,Ldab,Bd,Be,Q,Ldq,&
     &                           P,Ldp,Cc,Ldc,Work,Rwork,iinfo)
!
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'CGBBRD' , iinfo , n , &
     &                         jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                           RETURN
...
!
                     CALL CBDT01(m,n,-1,A,Lda,Q,Ldq,Bd,Be,P,Ldp,Work,   &
     &                           Rwork,Result(1))
                     CALL CUNT01('Columns',m,m,Q,Ldq,Work,Lwork,Rwork,  &
     &                           Result(2))
                     CALL CUNT01('Rows',n,n,P,Ldp,Work,Lwork,Rwork,     &
     &                           Result(3))
                     CALL CBDT02(m,Nrhs,C,Ldc,Cc,Ldc,Q,Ldq,Work,Rwork,  &
     &                           Result(4))
!
...
!
      ...
      RETURN
!
99001 FORMAT (' CCHKBB: ',A,' returned INFO=',I5,'.',/9X,'M=',I5,' N=', &
