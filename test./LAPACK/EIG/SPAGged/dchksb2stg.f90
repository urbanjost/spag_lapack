!*==dchksb2stg.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DCHKSBSTG
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DCHKSB2STG(Nsizes,Nn,Nwdths,Kk,Ntypes,Dotype,Iseed,    &
...
!     .. Array Arguments ..
      LOGICAL Dotype(*)
      INTEGER Iseed(4) , Kk(*) , Nn(*)
      DOUBLE PRECISION A(Lda,*) , Result(*) , Sd(*) , Se(*) , D1(*) ,   &
     &                 D2(*) , D3(*) , U(Ldu,*) , Work(*)
!     ..
!
!  =====================================================================
...
!     .. Local Scalars ..
     ...
     &                 temp1 , temp2 , temp3 , temp4 , ulp , ulpinv ,   &
     &                 unfl
!     ..
!     .. Local Arrays ..
...
!
                           CALL DLATMS(n,n,'S',Iseed,'S',Work,imode,    &
...
!
                           CALL DLATMS(n,n,'S',Iseed,'P',Work,imode,    &
     &                                 cond,anorm,k,k,'Q',A,Lda,        &
     &                                 Work(n+1),iinfo)
!
...
!
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'DSBTRD(U)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                           RETURN
...
!              Do tests 1 and 2
!
                     CALL DSBT21('Upper',n,k,1,A,Lda,Sd,Se,U,Ldu,Work,  &
     &                           Result(1))
!
...
!
                     CALL DSTEQR('N',n,D1,Work,Work(n+1),Ldu,Work(n+1), &
     &                           iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'DSTEQR(N)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                           RETURN
...
!
                     ...
                     lh = MAX(1,4*n)
                     lw = Lwork - lh
                     CALL DSYTRD_SB2ST('N','N',"U",n,k,U,Ldu,Sd,Se,Work,&
...
!
                     ...
                     IF ( n>0 ) CALL DCOPY(n-1,Se,1,Work,1)
!
                     CALL DSTEQR('N',n,D2,Work,Work(n+1),Ldu,Work(n+1), &
     &                           iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'DSTEQR(N)' , iinfo ,  &
     &                         n , jtype , ioldsd
...
!
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'DSBTRD(L)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                           RETURN
...
!              Do tests 3 and 4
!
                     CALL DSBT21('Lower',n,k,1,A,Lda,Sd,Se,U,Ldu,Work,  &
     &                           Result(3))
!
...
!
                     ...
                     lh = MAX(1,4*n)
                     lw = Lwork - lh
                     CALL DSYTRD_SB2ST('N','N',"L",n,k,U,Ldu,Sd,Se,Work,&
     &                                 lh,Work(lh+1),lw,iinfo)
!
...
!
                     ...
                     IF ( n>0 ) CALL DCOPY(n-1,Se,1,Work,1)
!
                     CALL DSTEQR('N',n,D3,Work,Work(n+1),Ldu,Work(n+1), &
     &                           iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'DSTEQR(N)' , iinfo ,  &
     &                         n , jtype , ioldsd
...
!
      ...
      RETURN
!
99001 FORMAT (' DCHKSBSTG: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,    &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99002 FORMAT (/1X,A3,                                                   &
     &        ' -- Real Symmetric Banded Tridiagonal Reduction Routines'&
     &        )
99003 FORMAT (' Matrix types (see DCHKSBSTG for details): ')
!
