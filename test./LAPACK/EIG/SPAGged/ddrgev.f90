!*==ddrgev.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DDRGEV
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DDRGEV(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
...
!     .. Array Arguments ..
      LOGICAL Dotype(*)
      INTEGER Iseed(4) , Nn(*)
      DOUBLE PRECISION A(Lda,*) , Alphai(*) , Alphar(*) , Alphi1(*) ,   &
     &                 Alphr1(*) , B(Lda,*) , Beta(*) , Beta1(*) ,      &
     &                 Q(Ldq,*) , Qe(Ldqe,*) , Result(*) , S(Lda,*) ,   &
     &                 T(Lda,*) , Work(*) , Z(Ldq,*)
!     ..
!
!  =====================================================================
...
!
               ...
               CALL DGGEV('V','V',n,S,Lda,T,Lda,Alphar,Alphai,Beta,Q,   &
     &                    Ldq,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'DGGEV1' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL DGET52(.TRUE.,n,A,Lda,B,Lda,Q,Ldq,Alphar,Alphai,    &
     &                     Beta,Work,Result(1))
               IF ( Result(2)>Thresh ) WRITE (Nounit,FMT=99002) 'Left' ,&
     &              'DGGEV1' , Result(2) , n , jtype , ioldsd
!
!           Do the tests (3) and (4)
!
               CALL DGET52(.FALSE.,n,A,Lda,B,Lda,Z,Ldq,Alphar,Alphai,   &
     &                     Beta,Work,Result(3))
               IF ( Result(4)>Thresh ) WRITE (Nounit,FMT=99002)         &
     &              'Right' , 'DGGEV1' , Result(4) , n , jtype , ioldsd
!
!           Do the test (5)
!
               CALL DLACPY(' ',n,n,A,Lda,S,Lda)
               CALL DLACPY(' ',n,n,B,Lda,T,Lda)
               CALL DGGEV('N','N',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Q,  &
     &                    Ldq,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'DGGEV2' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
!
     ...
     &                 .OR. Beta(j)/=Beta1(j) ) Result(5) = ulpinv
               ENDDO
!
...
!
               CALL DLACPY(' ',n,n,A,Lda,S,Lda)
               CALL DLACPY(' ',n,n,B,Lda,T,Lda)
               CALL DGGEV('V','N',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Qe, &
     &                    Ldqe,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'DGGEV3' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL DLACPY(' ',n,n,A,Lda,S,Lda)
               CALL DLACPY(' ',n,n,B,Lda,T,Lda)
               CALL DGGEV('N','V',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Q,  &
     &                    Ldq,Qe,Ldqe,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'DGGEV4' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
      RETURN
!
99001 FORMAT (' DDRGEV: ',A,' returned INFO=',I6,'.',/3X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',4(I4,','),I5,')')
!
99002 FORMAT (' DDRGEV: ',A,' Eigenvectors from ',A,' incorrectly ',    &
     &        'normalized.',/' Bits of error=',0P,G10.3,',',3X,'N=',I4, &
     &        ', JTYPE=',I3,', ISEED=(',4(I4,','),I5,')')
!
99003 FORMAT (/1X,A3,' -- Real Generalized eigenvalue problem driver')
!
99004 FORMAT (' Matrix types (see DDRGEV for details): ')
!
