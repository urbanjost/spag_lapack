!*==ddrges.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DDRGES
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DDRGES(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
...
!
                  ...
                  CALL DGGES('V','V',sort,DLCTES,n,S,Lda,T,Lda,sdim,    &
     &                       Alphar,Alphai,Beta,Q,Ldq,Z,Ldq,Work,Lwork, &
     &                       Bwork,iinfo)
                  IF ( iinfo/=0 .AND. iinfo/=n+2 ) THEN
                     Result(1+rsub+isort) = ulpinv
                     WRITE (Nounit,FMT=99001) 'DGGES' , iinfo , n ,     &
     &                      jtype , ioldsd
                     Info = ABS(iinfo)
                     EXIT
                  ENDIF
...
!
      RETURN
!
99001 FORMAT (' DDRGES: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',4(I4,','),I5,')')
!
99002 FORMAT (' DDRGES: DGET53 returned INFO=',I1,' for eigenvalue ',I6,&
     &        '.',/9X,'N=',I6,', JTYPE=',I6,', ISEED=(',4(I4,','),I5,   &
     &        ')')
!
99003 FORMAT (' DDRGES: S not in Schur form at eigenvalue ',I6,'.',/9X, &
     &        'N=',I6,', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99004 FORMAT (/1X,A3,' -- Real Generalized Schur form driver')
!
99005 FORMAT (' Matrix types (see DDRGES for details): ')
!
