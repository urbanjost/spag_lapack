!*==dspt21.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DSPT21
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DSPT21(Itype,Uplo,N,Kband,Ap,D,E,U,Ldu,Vp,Tau,Work,    &
     &                  Result)
      IMPLICIT NONE
!*--DSPT21225
!
...
!
               IF ( Tau(j)/=ZERO ) THEN
                  vsave = Vp(jp+j+1)
                  Vp(jp+j+1) = ONE
                  CALL DSPMV('L',N-j,ONE,Work(jp1+j+1),Vp(jp+j+1),1,    &
     &                       ZERO,Work(lap+1),1)
                  temp = -HALF*Tau(j)                                   &
     &                   *DDOT(N-j,Work(lap+1),1,Vp(jp+j+1),1)
                  CALL DAXPY(N-j,temp,Vp(jp+j+1),1,Work(lap+1),1)
                  CALL DSPR2('L',N-j,-Tau(j),Vp(jp+j+1),1,Work(lap+1),1,&
     &                       Work(jp1+j+1))
                  Vp(jp+j+1) = vsave
               ENDIF
               Work(jp+j) = D(j)
...
!
               IF ( Tau(j)/=ZERO ) THEN
                  vsave = Vp(jp1+j)
                  Vp(jp1+j) = ONE
                  CALL DSPMV('U',j,ONE,Work,Vp(jp1+1),1,ZERO,Work(lap+1)&
     &                       ,1)
                  temp = -HALF*Tau(j)*DDOT(j,Work(lap+1),1,Vp(jp1+1),1)
                  CALL DAXPY(j,temp,Vp(jp1+1),1,Work(lap+1),1)
                  CALL DSPR2('U',j,-Tau(j),Vp(jp1+1),1,Work(lap+1),1,   &
     &                       Work)
                  Vp(jp1+j) = vsave
               ENDIF
               Work(jp1+j+1) = D(j+1)
