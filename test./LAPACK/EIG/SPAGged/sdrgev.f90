!*==sdrgev.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SDRGEV
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SDRGEV(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
...
!
               ...
               CALL SGGEV('V','V',n,S,Lda,T,Lda,Alphar,Alphai,Beta,Q,   &
     &                    Ldq,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'SGGEV1' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
     ...
     &              'SGGEV1' , Result(2) , n , jtype , ioldsd
!
...
!
     ...
     &              'Right' , 'SGGEV1' , Result(4) , n , jtype , ioldsd
!
!           Do the test (5)
!
               CALL SLACPY(' ',n,n,A,Lda,S,Lda)
               CALL SLACPY(' ',n,n,B,Lda,T,Lda)
               CALL SGGEV('N','N',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Q,  &
     &                    Ldq,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'SGGEV2' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL SLACPY(' ',n,n,A,Lda,S,Lda)
               CALL SLACPY(' ',n,n,B,Lda,T,Lda)
               CALL SGGEV('V','N',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Qe, &
     &                    Ldqe,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'SGGEV3' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL SLACPY(' ',n,n,A,Lda,S,Lda)
               CALL SLACPY(' ',n,n,B,Lda,T,Lda)
               CALL SGGEV('N','V',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Q,  &
     &                    Ldq,Qe,Ldqe,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'SGGEV4' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
      RETURN
!
99001 FORMAT (' SDRGEV: ',A,' returned INFO=',I6,'.',/3X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',4(I4,','),I5,')')
!
99002 FORMAT (' SDRGEV: ',A,' Eigenvectors from ',A,' incorrectly ',    &
...
!
99004 FORMAT (' Matrix types (see SDRGEV for details): ')
!
