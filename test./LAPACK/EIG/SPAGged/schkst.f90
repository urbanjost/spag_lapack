!*==schkst.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SCHKST
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SCHKST(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
     &                  Lda,Ap,Sd,Se,D1,D2,D3,D4,D5,Wa1,Wa2,Wa3,Wr,U,   &
     &                  Ldu,V,Vp,Tau,Z,Work,Lwork,Iwork,Liwork,Result,  &
     &                  Info)
      IMPLICIT NONE
!*--SCHKST595
!
...
!     .. Array Arguments ..
      LOGICAL Dotype(*)
      INTEGER Iseed(4) , Iwork(*) , Nn(*)
      REAL A(Lda,*) , Ap(*) , D1(*) , D2(*) , D3(*) , D4(*) , D5(*) ,   &
     &     Result(*) , Sd(*) , Se(*) , Tau(*) , U(Ldu,*) , V(Ldu,*) ,   &
     &     Vp(*) , Wa1(*) , Wa2(*) , Wa3(*) , Work(*) , Wr(*) , Z(Ldu,*)
!     ..
!
!  =====================================================================
!
...
!
                  CALL SLATMS(n,n,'S',Iseed,'S',Work,imode,cond,anorm,0,&
...
!
                  CALL SLATMS(n,n,'S',Iseed,'S',Work,imode,cond,anorm,n,&
...
!
                  CALL SLATMR(n,n,'S',Iseed,'S',Work,6,ONE,ONE,'T','N', &
...
!
                  CALL SLATMS(n,n,'S',Iseed,'P',Work,imode,cond,anorm,n,&
     &                        n,'N',A,Lda,Work(n+1),iinfo)
!
...
!
                  CALL SLATMS(n,n,'S',Iseed,'P',Work,imode,cond,anorm,1,&
...
!
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSYTRD(U)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SORGTR(U)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!           Do tests 1 and 2
!
            CALL SSYT21(2,'Upper',n,1,A,Lda,Sd,Se,U,Ldu,V,Ldu,Tau,Work, &
     &                  Result(1))
            CALL SSYT21(3,'Upper',n,1,A,Lda,Sd,Se,U,Ldu,V,Ldu,Tau,Work, &
     &                  Result(2))
!
!           Call SSYTRD and SORGTR to compute S and U from
...
!
            ntest = 3
            CALL SSYTRD('L',n,V,Ldu,Sd,Se,Tau,Work,Lwork,iinfo)
!
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSYTRD(L)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SORGTR(L)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
            ...
            ENDIF
!
            CALL SSYT21(2,'Lower',n,1,A,Lda,Sd,Se,U,Ldu,V,Ldu,Tau,Work, &
     &                  Result(3))
            CALL SSYT21(3,'Lower',n,1,A,Lda,Sd,Se,U,Ldu,V,Ldu,Tau,Work, &
     &                  Result(4))
!
...
!
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSPTRD(U)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SOPGTR(U)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!           Do tests 5 and 6
!
            CALL SSPT21(2,'Upper',n,1,Ap,Sd,Se,U,Ldu,Vp,Tau,Work,       &
     &                  Result(5))
            CALL SSPT21(3,'Upper',n,1,Ap,Sd,Se,U,Ldu,Vp,Tau,Work,       &
     &                  Result(6))
!
...
!
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSPTRD(L)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SOPGTR(L)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
            ...
            ENDIF
!
            CALL SSPT21(2,'Lower',n,1,Ap,Sd,Se,U,Ldu,Vp,Tau,Work,       &
     &                  Result(7))
            CALL SSPT21(3,'Lower',n,1,Ap,Sd,Se,U,Ldu,Vp,Tau,Work,       &
     &                  Result(8))
!
...
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSTEQR(V)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSTEQR(N)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
               ...
               IF ( iinfo/=0 ) THEN
                  WRITE (Nounit,FMT=99001) 'SPTEQR(V)' , iinfo , n ,    &
     &                   jtype , ioldsd
                  Info = ABS(iinfo)
                  IF ( iinfo<0 ) THEN
                     RETURN
...
!
               ...
               IF ( iinfo/=0 ) THEN
                  WRITE (Nounit,FMT=99001) 'SPTEQR(N)' , iinfo , n ,    &
     &                   jtype , ioldsd
                  Info = ABS(iinfo)
                  IF ( iinfo<0 ) THEN
                     RETURN
...
!
     ...
     &                                  jtype , ioldsd
            ...
            ENDIF
!
            CALL SSTEIN(n,Sd,Se,m,Wa1,Iwork(1),Iwork(n+1),Z,Ldu,Work,   &
     &                  Iwork(2*n+1),Iwork(3*n+1),iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSTEIN' , iinfo , n , jtype ,  &
     &                                  ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            ntest = 22
            CALL SSTEDC('I',n,D1,Work,Z,Ldu,Work(n+1),lwedc-n,Iwork,    &
     &                  liwedc,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSTEDC(I)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            ntest = 24
            CALL SSTEDC('V',n,D1,Work,Z,Ldu,Work(n+1),lwedc-n,Iwork,    &
     &                  liwedc,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSTEDC(V)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            ntest = 26
            CALL SSTEDC('N',n,D2,Work,Z,Ldu,Work(n+1),lwedc-n,Iwork,    &
     &                  liwedc,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'SSTEDC(N)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               IF ( iinfo<0 ) THEN
                  RETURN
...
!
            IF ( ILAENV(10,'SSTEMR','VA',1,0,0,0)==1 .AND.              &
     &           ILAENV(11,'SSTEMR','VA',1,0,0,0)==1 ) THEN
!
...
!
               ...
               IF ( jtype==21 .AND. SREL ) THEN
                  ntest = 27
                  abstol = unfl + unfl
                  CALL SSTEMR('V','A',n,Sd,Se,vl,vu,il,iu,m,Wr,Z,Ldu,n, &
     &                        Iwork(1),tryrac,Work,Lwork,Iwork(2*n+1),  &
     &                        Lwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'SSTEMR(V,A,rel)' ,       &
...
!
                  IF ( SRANGE ) THEN
                     ntest = 28
                     abstol = unfl + unfl
                     CALL SSTEMR('V','I',n,Sd,Se,vl,vu,il,iu,m,Wr,Z,Ldu,&
     &                           n,Iwork(1),tryrac,Work,Lwork,          &
     &                           Iwork(2*n+1),Lwork-2*n,iinfo)
!
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'SSTEMR(V,I,rel)' ,    &
     &                         iinfo , n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                           RETURN
...
!
               IF ( SRANGE ) THEN
                  ...
                  IF ( iu<il ) THEN
                     ...
                     il = itemp
                  ENDIF
                  CALL SSTEMR('V','I',n,D5,Work,vl,vu,il,iu,m,D1,Z,Ldu, &
     &                        n,Iwork(1),tryrac,Work(n+1),Lwork-n,      &
     &                        Iwork(2*n+1),Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'SSTEMR(V,I)' , iinfo ,   &
     &                      n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = 31
                  CALL SSTEMR('N','I',n,D5,Work,vl,vu,il,iu,m,D2,Z,Ldu, &
     &                        n,Iwork(1),tryrac,Work(n+1),Lwork-n,      &
     &                        Iwork(2*n+1),Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'SSTEMR(N,I)' , iinfo ,   &
     &                      n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
     ...
     &                       TWO*rtunfl)
                  ...
                  ENDIF
!
                  CALL SSTEMR('V','V',n,D5,Work,vl,vu,il,iu,m,D1,Z,Ldu, &
     &                        n,Iwork(1),tryrac,Work(n+1),Lwork-n,      &
     &                        Iwork(2*n+1),Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'SSTEMR(V,V)' , iinfo ,   &
     &                      n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = 34
                  CALL SSTEMR('N','V',n,D5,Work,vl,vu,il,iu,m,D2,Z,Ldu, &
     &                        n,Iwork(1),tryrac,Work(n+1),Lwork-n,      &
     &                        Iwork(2*n+1),Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'SSTEMR(N,V)' , iinfo ,   &
     &                      n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
               ntest = 35
!
               CALL SSTEMR('V','A',n,D5,Work,vl,vu,il,iu,m,D1,Z,Ldu,n,  &
     &                     Iwork(1),tryrac,Work(n+1),Lwork-n,           &
     &                     Iwork(2*n+1),Liwork-2*n,iinfo)
               IF ( iinfo/=0 ) THEN
                  WRITE (Nounit,FMT=99001) 'SSTEMR(V,A)' , iinfo , n ,  &
     &                   jtype , ioldsd
                  Info = ABS(iinfo)
                  IF ( iinfo<0 ) THEN
                     RETURN
...
!
               ntest = 37
               CALL SSTEMR('N','A',n,D5,Work,vl,vu,il,iu,m,D2,Z,Ldu,n,  &
     &                     Iwork(1),tryrac,Work(n+1),Lwork-n,           &
     &                     Iwork(2*n+1),Liwork-2*n,iinfo)
               IF ( iinfo/=0 ) THEN
                  WRITE (Nounit,FMT=99001) 'SSTEMR(N,A)' , iinfo , n ,  &
     &                   jtype , ioldsd
                  Info = ABS(iinfo)
                  IF ( iinfo<0 ) THEN
                     RETURN
...
!
      ...
      RETURN
!
99001 FORMAT (' SCHKST: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99002 FORMAT (/1X,A3,' -- Real Symmetric eigenvalue problem')
99003 FORMAT (' Matrix types (see SCHKST for details): ')
!
...
99007 FORMAT (' N=',I5,', seed=',4(I4,','),' type ',I2,', test(',I2,    &
     &        ')=',G10.3)
!
99008 FORMAT (/'Test performed:  see SCHKST for details.',/)
!     End of SCHKST
!
      END SUBROUTINE SCHKST
