!*==chpt21.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CHPT21
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CHPT21(Itype,Uplo,N,Kband,Ap,D,E,U,Ldu,Vp,Tau,Work,    &
     &                  Rwork,Result)
      IMPLICIT NONE
!*--CHPT21232
!
...
!     .. External Subroutines ..
      EXTERNAL CAXPY , CCOPY , CGEMM , CHPMV , CHPR , CHPR2 , CLACPY ,  &
     &         CLASET , CUPMTR
!     ..
!     .. Intrinsic Functions ..
...
!
               IF ( Tau(j)/=CZERO ) THEN
                  vsave = Vp(jp+j+1)
                  Vp(jp+j+1) = CONE
                  CALL CHPMV('L',N-j,CONE,Work(jp1+j+1),Vp(jp+j+1),1,   &
     &                       CZERO,Work(lap+1),1)
                  temp = -HALF*Tau(j)                                   &
     &                   *CDOTC(N-j,Work(lap+1),1,Vp(jp+j+1),1)
                  CALL CAXPY(N-j,temp,Vp(jp+j+1),1,Work(lap+1),1)
                  CALL CHPR2('L',N-j,-Tau(j),Vp(jp+j+1),1,Work(lap+1),1,&
     &                       Work(jp1+j+1))
!
                  Vp(jp+j+1) = vsave
               ENDIF
...
!
     ...
     &                       Work(lap+1),1)
                  temp = -HALF*Tau(j)*CDOTC(j,Work(lap+1),1,Vp(jp1+1),1)
                  CALL CAXPY(j,temp,Vp(jp1+1),1,Work(lap+1),1)
                  CALL CHPR2('U',j,-Tau(j),Vp(jp1+1),1,Work(lap+1),1,   &
     &                       Work)
                  Vp(jp1+j) = vsave
               ENDIF
               Work(jp1+j+1) = D(j+1)
