!*==zdrvst2stg.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZDRVST2STG
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZDRVST2STG(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,&
     &                      A,Lda,D1,D2,D3,Wa1,Wa2,Wa3,U,Ldu,V,Tau,Z,   &
     &                      Work,Lwork,Rwork,Lrwork,Iwork,Liwork,Result,&
     &                      Info)
      IMPLICIT NONE
!*--ZDRVST2STG342
!
...
!
                  ntest = ntest + 2
                  CALL ZHEEVD_2STAGE('N',uplo,n,A,Ldu,D3,Work,Lwork,    &
     &                               Rwork,lrwedc,Iwork,liwedc,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'ZHEEVD_2STAGE(N,'//uplo//&
     &                      ')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL ZHEEVX_2STAGE('N','A',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m2,Wa2,Z,Ldu,Work,Lwork,    &
     &                               Rwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'ZHEEVX_2STAGE(N,A,'//    &
...
!
                  ntest = ntest + 2
!
                  CALL ZHEEVX_2STAGE('N','I',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Work,Lwork,    &
     &                               Rwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'ZHEEVX_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
!
                  CALL ZHEEVX_2STAGE('N','V',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Work,Lwork,    &
     &                               Rwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'ZHEEVX_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL ZHBEVD_2STAGE('N',uplo,n,kd,V,Ldu,D3,Z,Ldu,Work, &
     &                               Lwork,Rwork,lrwedc,Iwork,liwedc,   &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'ZHBEVD_2STAGE(N,'//uplo//&
     &                      ')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  ENDIF
!
                  CALL ZHBEVX_2STAGE('N','A',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m2,Wa2,Z,Ldu,Work, &
     &                               Lwork,Rwork,Iwork,Iwork(5*n+1),    &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'ZHBEVX_2STAGE(N,A,'//    &
     &                      uplo//')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
                     ELSE
...
!
                  ...
                  ELSE
                     ...
                     ENDDO
                  ENDIF
                  CALL ZHBEVX_2STAGE('N','I',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m3,Wa3,Z,Ldu,Work, &
     &                               Lwork,Rwork,Iwork,Iwork(5*n+1),    &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'ZHBEVX_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  ELSE
                     ...
                     ENDDO
                  ENDIF
                  CALL ZHBEVX_2STAGE('N','V',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m3,Wa3,Z,Ldu,Work, &
     &                               Lwork,Rwork,Iwork,Iwork(5*n+1),    &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'ZHBEVX_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL ZHEEV_2STAGE('N',uplo,n,A,Ldu,D3,Work,Lwork,     &
     &                              Rwork,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'ZHEEV_2STAGE(N,'//uplo// &
     &                      ')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL ZHBEV_2STAGE('N',uplo,n,kd,V,Ldu,D3,Z,Ldu,Work,  &
     &                              Lwork,Rwork,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'ZHBEV_2STAGE(N,'//uplo// &
     &                      ')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL ZHEEVR_2STAGE('N','A',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m2,Wa2,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Rwork,Lrwork,Iwork(2*n+1),   &
     &                               Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'ZHEEVR_2STAGE(N,A,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL ZLACPY(' ',n,n,V,Ldu,A,Lda)
                  CALL ZHEEVR_2STAGE('N','I',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Rwork,Lrwork,Iwork(2*n+1),   &
     &                               Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'ZHEEVR_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL ZLACPY(' ',n,n,V,Ldu,A,Lda)
                  CALL ZHEEVR_2STAGE('N','V',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Rwork,Lrwork,Iwork(2*n+1),   &
     &                               Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'ZHEEVR_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
99001 FORMAT (' ZDRVST2STG: ',A,' returned INFO=',I6,/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
99002 FORMAT (' ZDRVST2STG: ',A,' returned INFO=',I6,/9X,'N=',I6,       &
     &        ', KD=',I6,', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
!
!     End of ZDRVST2STG
!
      END SUBROUTINE ZDRVST2STG
