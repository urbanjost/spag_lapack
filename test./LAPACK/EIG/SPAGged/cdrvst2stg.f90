!*==cdrvst2stg.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CDRVST2STG
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CDRVST2STG(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,&
...
!
                  ntest = ntest + 2
                  CALL CHEEVD_2STAGE('N',uplo,n,A,Ldu,D3,Work,Lwork,    &
     &                               Rwork,lrwedc,Iwork,liwedc,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'CHEEVD_2STAGE(N,'//uplo//&
     &                      ')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL CHEEVX_2STAGE('N','A',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m2,Wa2,Z,Ldu,Work,Lwork,    &
     &                               Rwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'CHEEVX_2STAGE(N,A,'//    &
...
!
                  ntest = ntest + 2
!
                  CALL CHEEVX_2STAGE('N','I',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Work,Lwork,    &
     &                               Rwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'CHEEVX_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
!
                  CALL CHEEVX_2STAGE('N','V',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Work,Lwork,    &
     &                               Rwork,Iwork,Iwork(5*n+1),iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'CHEEVX_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL CHBEVD_2STAGE('N',uplo,n,kd,V,Ldu,D3,Z,Ldu,Work, &
     &                               Lwork,Rwork,lrwedc,Iwork,liwedc,   &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'CHBEVD_2STAGE(N,'//uplo//&
     &                      ')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  ENDIF
!
                  CALL CHBEVX_2STAGE('N','A',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m2,Wa2,Z,Ldu,Work, &
     &                               Lwork,Rwork,Iwork,Iwork(5*n+1),    &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'CHBEVX_2STAGE(N,A,'//    &
...
!
                  ...
                  ELSE
                     ...
                     ENDDO
                  ENDIF
                  CALL CHBEVX_2STAGE('N','I',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m3,Wa3,Z,Ldu,Work, &
     &                               Lwork,Rwork,Iwork,Iwork(5*n+1),    &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'CHBEVX_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ...
                  ELSE
                     ...
                     ENDDO
                  ENDIF
                  CALL CHBEVX_2STAGE('N','V',uplo,n,kd,V,Ldu,U,Ldu,vl,  &
     &                               vu,il,iu,abstol,m3,Wa3,Z,Ldu,Work, &
     &                               Lwork,Rwork,Iwork,Iwork(5*n+1),    &
     &                               iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'CHBEVX_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL CHEEV_2STAGE('N',uplo,n,A,Ldu,D3,Work,Lwork,     &
     &                              Rwork,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'CHEEV_2STAGE(N,'//uplo// &
     &                      ')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL CHBEV_2STAGE('N',uplo,n,kd,V,Ldu,D3,Z,Ldu,Work,  &
     &                              Lwork,Rwork,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99002) 'CHBEV_2STAGE(N,'//uplo// &
     &                      ')' , iinfo , n , kd , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL CHEEVR_2STAGE('N','A',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m2,Wa2,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Rwork,Lrwork,Iwork(2*n+1),   &
     &                               Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'CHEEVR_2STAGE(N,A,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL CLACPY(' ',n,n,V,Ldu,A,Lda)
                  CALL CHEEVR_2STAGE('N','I',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Rwork,Lrwork,Iwork(2*n+1),   &
     &                               Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'CHEEVR_2STAGE(N,I,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
                  ntest = ntest + 2
                  CALL CLACPY(' ',n,n,V,Ldu,A,Lda)
                  CALL CHEEVR_2STAGE('N','V',uplo,n,A,Ldu,vl,vu,il,iu,  &
     &                               abstol,m3,Wa3,Z,Ldu,Iwork,Work,    &
     &                               Lwork,Rwork,Lrwork,Iwork(2*n+1),   &
     &                               Liwork-2*n,iinfo)
                  IF ( iinfo/=0 ) THEN
                     WRITE (Nounit,FMT=99001) 'CHEEVR_2STAGE(N,V,'//    &
     &                      uplo//')' , iinfo , n , jtype , ioldsd
                     Info = ABS(iinfo)
                     IF ( iinfo<0 ) THEN
                        RETURN
...
!
99001 FORMAT (' CDRVST2STG: ',A,' returned INFO=',I6,/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
99002 FORMAT (' CDRVST2STG: ',A,' returned INFO=',I6,/9X,'N=',I6,       &
     &        ', KD=',I6,', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
!
!     End of CDRVST2STG
!
      END SUBROUTINE CDRVST2STG
