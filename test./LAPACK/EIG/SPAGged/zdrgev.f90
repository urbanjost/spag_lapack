!*==zdrgev.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZDRGEV
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZDRGEV(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
...
!     .. Array Arguments ..
      ...
      INTEGER Iseed(4) , Nn(*)
      DOUBLE PRECISION Result(*) , Rwork(*)
      COMPLEX*16 A(Lda,*) , Alpha(*) , Alpha1(*) , B(Lda,*) , Beta(*) , &
...
!
                     IF ( ABS(katype(jtype))==3 ) THEN
                        in = 2*((n-1)/2) + 1
                        IF ( in/=n ) CALL ZLASET('Full',n,n,CZERO,CZERO,&
     &                       A,Lda)
                     ELSE
                        in = n
                     ENDIF
...
!
                        DO jc = 1 , n
                           DO jr = 1 , n
                              A(jr,jc) = Work(2*n+jr)                   &
     &                           *DCONJG(Work(3*n+jc))*A(jr,jc)
                              B(jr,jc) = Work(2*n+jr)                   &
     &                           *DCONJG(Work(3*n+jc))*B(jr,jc)
                           ENDDO
                        ENDDO
                        CALL ZUNM2R('L','N',n,n,n-1,Q,Ldq,Work,A,Lda,   &
     &                              Work(2*n+1),ierr)
                        IF ( ierr==0 ) THEN
...
!
               ...
               CALL ZGGEV('V','V',n,S,Lda,T,Lda,Alpha,Beta,Q,Ldq,Z,Ldq, &
     &                    Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'ZGGEV1' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL ZGET52(.TRUE.,n,A,Lda,B,Lda,Q,Ldq,Alpha,Beta,Work,  &
     &                     Rwork,Result(1))
               IF ( Result(2)>Thresh ) WRITE (Nounit,FMT=99002) 'Left' ,&
     &              'ZGGEV1' , Result(2) , n , jtype , ioldsd
!
!           Do the tests (3) and (4)
!
               CALL ZGET52(.FALSE.,n,A,Lda,B,Lda,Z,Ldq,Alpha,Beta,Work, &
     &                     Rwork,Result(3))
               IF ( Result(4)>Thresh ) WRITE (Nounit,FMT=99002)         &
     &              'Right' , 'ZGGEV1' , Result(4) , n , jtype , ioldsd
!
!           Do test (5)
!
               CALL ZLACPY(' ',n,n,A,Lda,S,Lda)
               CALL ZLACPY(' ',n,n,B,Lda,T,Lda)
               CALL ZGGEV('N','N',n,S,Lda,T,Lda,Alpha1,Beta1,Q,Ldq,Z,   &
     &                    Ldq,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'ZGGEV2' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL ZLACPY(' ',n,n,A,Lda,S,Lda)
               CALL ZLACPY(' ',n,n,B,Lda,T,Lda)
               CALL ZGGEV('V','N',n,S,Lda,T,Lda,Alpha1,Beta1,Qe,Ldqe,Z, &
     &                    Ldq,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'ZGGEV3' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL ZLACPY(' ',n,n,A,Lda,S,Lda)
               CALL ZLACPY(' ',n,n,B,Lda,T,Lda)
               CALL ZGGEV('N','V',n,S,Lda,T,Lda,Alpha1,Beta1,Q,Ldq,Qe,  &
     &                    Ldqe,Work,Lwork,Rwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'ZGGEV4' , ierr , n , jtype ,&
     &                   ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
      RETURN
!
99001 FORMAT (' ZDRGEV: ',A,' returned INFO=',I6,'.',/3X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99002 FORMAT (' ZDRGEV: ',A,' Eigenvectors from ',A,' incorrectly ',    &
     &        'normalized.',/' Bits of error=',0P,G10.3,',',3X,'N=',I4, &
     &        ', JTYPE=',I3,', ISEED=(',3(I4,','),I5,')')
!
99003 FORMAT (/1X,A3,' -- Complex Generalized eigenvalue problem ',     &
     &        'driver')
!
99004 FORMAT (' Matrix types (see ZDRGEV for details): ')
!
