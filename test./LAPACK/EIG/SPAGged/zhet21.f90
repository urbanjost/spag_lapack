!*==zhet21.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZHET21
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZHET21(Itype,Uplo,N,Kband,A,Lda,D,E,U,Ldu,V,Ldv,Tau,   &
     &                  Work,Rwork,Result)
      IMPLICIT NONE
!*--ZHET21218
!
...
!
         IF ( N>1 .AND. Kband==1 ) THEN
            DO j = 2 , N - 1
               CALL ZHER2(cuplo,N,-DCMPLX(E(j)),U(1,j),1,U(1,j-1),1,    &
     &                    Work,N)
            ENDDO
         ENDIF
...
!
               vsave = V(j+1,j)
               V(j+1,j) = ONE
               CALL ZLARFY('L',N-j,V(j+1,j),1,Tau(j),Work((N+1)*j+1),N, &
     &                     Work(N**2+1))
               V(j+1,j) = vsave
               Work((N+1)*(j-1)+1) = D(j)
            ENDDO
...
!
         ...
         IF ( lower ) THEN
            CALL ZUNM2R('R','C',N,N-1,N-1,V(2,1),Ldv,Tau,Work(N+1),N,   &
     &                  Work(N**2+1),iinfo)
         ELSE
            CALL ZUNM2L('R','C',N,N-1,N-1,V(1,2),Ldv,Tau,Work,N,        &
     &                  Work(N**2+1),iinfo)
         ENDIF
         IF ( iinfo/=0 ) THEN
            Result(1) = TEN/ulp
