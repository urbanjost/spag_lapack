!*==zdrges.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZDRGES
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZDRGES(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
...
!
                  ...
                  CALL ZGGES('V','V',sort,ZLCTES,n,S,Lda,T,Lda,sdim,    &
     &                       Alpha,Beta,Q,Ldq,Z,Ldq,Work,Lwork,Rwork,   &
     &                       Bwork,iinfo)
                  IF ( iinfo/=0 .AND. iinfo/=n+2 ) THEN
                     Result(1+rsub+isort) = ulpinv
                     WRITE (Nounit,FMT=99001) 'ZGGES' , iinfo , n ,     &
     &                      jtype , ioldsd
                     Info = ABS(iinfo)
                     EXIT
                  ENDIF
...
!
      RETURN
!
99001 FORMAT (' ZDRGES: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',4(I4,','),I5,')')
!
99002 FORMAT (' ZDRGES: S not in Schur form at eigenvalue ',I6,'.',/9X, &
     &        'N=',I6,', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99003 FORMAT (/1X,A3,' -- Complex Generalized Schur from problem ',     &
     &        'driver')
!
99004 FORMAT (' Matrix types (see ZDRGES for details): ')
!
