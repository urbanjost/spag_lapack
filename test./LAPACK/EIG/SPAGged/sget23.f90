!*==sget23.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SGET23
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SGET23(Comp,Balanc,Jtype,Thresh,Iseed,Nounit,N,A,Lda,H,&
...
!     .. Local Scalars ..
     ...
     &     ulpinv , v , vimin , vmax , vmx , vrmin , vrmx , vtst
!     ..
!     .. Local Arrays ..
      CHARACTER sens(2)
      REAL dum(1) , res(2)
...
!
      ...
      CALL SGEEVX(Balanc,'V','V',sense,N,H,Lda,Wr,Wi,Vl,Ldvl,Vr,Ldvr,   &
     &            ilo,ihi,Scale,abnrm,Rconde,Rcondv,Work,Lwork,Iwork,   &
     &            iinfo)
      IF ( iinfo/=0 ) THEN
         Result(1) = ulpinv
         IF ( Jtype/=22 ) THEN
            WRITE (Nounit,FMT=99002) 'SGEEVX1' , iinfo , N , Jtype ,    &
     &                               Balanc , Iseed
         ELSE
            WRITE (Nounit,FMT=99001) 'SGEEVX1' , iinfo , N , Iseed(1)
         ENDIF
         Info = ABS(iinfo)
         RETURN
      ENDIF
...
!
         CALL SLACPY('F',N,N,A,Lda,H,Lda)
         CALL SGEEVX(Balanc,'N','N',sense,N,H,Lda,Wr1,Wi1,dum,1,dum,1,  &
     &               ilo1,ihi1,Scale1,abnrm1,Rcnde1,Rcndv1,Work,Lwork,  &
     &               Iwork,iinfo)
         IF ( iinfo/=0 ) THEN
            Result(1) = ulpinv
            IF ( Jtype/=22 ) THEN
               WRITE (Nounit,FMT=99002) 'SGEEVX2' , iinfo , N , Jtype , &
     &                                  Balanc , Iseed
            ELSE
               WRITE (Nounit,FMT=99001) 'SGEEVX2' , iinfo , N , Iseed(1)
            ENDIF
            Info = ABS(iinfo)
            CYCLE
         ENDIF
...
!
         CALL SLACPY('F',N,N,A,Lda,H,Lda)
         CALL SGEEVX(Balanc,'N','V',sense,N,H,Lda,Wr1,Wi1,dum,1,Lre,    &
     &               Ldlre,ilo1,ihi1,Scale1,abnrm1,Rcnde1,Rcndv1,Work,  &
     &               Lwork,Iwork,iinfo)
         IF ( iinfo/=0 ) THEN
            Result(1) = ulpinv
            IF ( Jtype/=22 ) THEN
               WRITE (Nounit,FMT=99002) 'SGEEVX3' , iinfo , N , Jtype , &
     &                                  Balanc , Iseed
            ELSE
               WRITE (Nounit,FMT=99001) 'SGEEVX3' , iinfo , N , Iseed(1)
            ENDIF
            Info = ABS(iinfo)
            CYCLE
         ENDIF
...
!
         CALL SLACPY('F',N,N,A,Lda,H,Lda)
         CALL SGEEVX(Balanc,'V','N',sense,N,H,Lda,Wr1,Wi1,Lre,Ldlre,dum,&
     &               1,ilo1,ihi1,Scale1,abnrm1,Rcnde1,Rcndv1,Work,Lwork,&
     &               Iwork,iinfo)
         IF ( iinfo/=0 ) THEN
            Result(1) = ulpinv
            IF ( Jtype/=22 ) THEN
               WRITE (Nounit,FMT=99002) 'SGEEVX4' , iinfo , N , Jtype , &
     &                                  Balanc , Iseed
            ELSE
               WRITE (Nounit,FMT=99001) 'SGEEVX4' , iinfo , N , Iseed(1)
            ENDIF
            Info = ABS(iinfo)
            CYCLE
         ENDIF
...
!
      IF ( Comp ) THEN
         CALL SLACPY('F',N,N,A,Lda,H,Lda)
         CALL SGEEVX('N','V','V','B',N,H,Lda,Wr,Wi,Vl,Ldvl,Vr,Ldvr,ilo, &
     &               ihi,Scale,abnrm,Rconde,Rcondv,Work,Lwork,Iwork,    &
     &               iinfo)
         IF ( iinfo/=0 ) THEN
            Result(1) = ulpinv
            WRITE (Nounit,FMT=99001) 'SGEEVX5' , iinfo , N , Iseed(1)
            Info = ABS(iinfo)
            GOTO 99999
         ENDIF
!
...
!
      ENDIF
!
99001 FORMAT (' SGET23: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', INPUT EXAMPLE NUMBER = ',I4)
99002 FORMAT (' SGET23: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
