!*==ddrgev3.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DDRGEV3
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DDRGEV3(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A, &
...
!
               ...
               CALL DGGEV3('V','V',n,S,Lda,T,Lda,Alphar,Alphai,Beta,Q,  &
     &                     Ldq,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'DGGEV31' , ierr , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
     ...
     &              'DGGEV31' , Result(2) , n , jtype , ioldsd
!
...
!
     ...
     &              'Right' , 'DGGEV31' , Result(4) , n , jtype , ioldsd
!
!           Do the test (5)
!
               CALL DLACPY(' ',n,n,A,Lda,S,Lda)
               CALL DLACPY(' ',n,n,B,Lda,T,Lda)
               CALL DGGEV3('N','N',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Q, &
     &                     Ldq,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'DGGEV32' , ierr , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL DLACPY(' ',n,n,A,Lda,S,Lda)
               CALL DLACPY(' ',n,n,B,Lda,T,Lda)
               CALL DGGEV3('V','N',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Qe,&
     &                     Ldqe,Z,Ldq,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'DGGEV33' , ierr , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
               CALL DLACPY(' ',n,n,A,Lda,S,Lda)
               CALL DLACPY(' ',n,n,B,Lda,T,Lda)
               CALL DGGEV3('N','V',n,S,Lda,T,Lda,Alphr1,Alphi1,Beta1,Q, &
     &                     Ldq,Qe,Ldqe,Work,Lwork,ierr)
               IF ( ierr/=0 .AND. ierr/=n+1 ) THEN
                  Result(1) = ulpinv
                  WRITE (Nounit,FMT=99001) 'DGGEV34' , ierr , n ,       &
     &                   jtype , ioldsd
                  Info = ABS(ierr)
                  GOTO 10
               ENDIF
...
!
      RETURN
!
99001 FORMAT (' DDRGEV3: ',A,' returned INFO=',I6,'.',/3X,'N=',I6,      &
     &        ', JTYPE=',I6,', ISEED=(',4(I4,','),I5,')')
!
99002 FORMAT (' DDRGEV3: ',A,' Eigenvectors from ',A,                   &
...
!
99004 FORMAT (' Matrix types (see DDRGEV3 for details): ')
!
