!*==cchkgg.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CCHKGG
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CCHKGG(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Tstdif,    &
...
!     .. Array Arguments ..
      ...
      INTEGER Iseed(4) , Nn(*)
      REAL Result(15) , Rwork(*)
      COMPLEX A(Lda,*) , Alpha1(*) , Alpha3(*) , B(Lda,*) , Beta1(*) ,  &
     &        Beta3(*) , Evectl(Ldu,*) , Evectr(Ldu,*) , H(Lda,*) ,     &
     &        P1(Lda,*) , P2(Lda,*) , Q(Ldu,*) , S1(Lda,*) , S2(Lda,*) ,&
     &        T(Lda,*) , U(Ldu,*) , V(Ldu,*) , Work(*) , Z(Ldu,*)
!     ..
!
!  =====================================================================
!
...
!
                  DO jc = 1 , n
                     DO jr = 1 , n
                        A(jr,jc) = Work(2*n+jr)*CONJG(Work(3*n+jc))     &
     &                             *A(jr,jc)
                        B(jr,jc) = Work(2*n+jr)*CONJG(Work(3*n+jc))     &
     &                             *B(jr,jc)
                     ENDDO
                  ENDDO
                  CALL CUNM2R('L','N',n,n,n-1,U,Ldu,Work,A,Lda,         &
     &                        Work(2*n+1),iinfo)
                  IF ( iinfo/=0 ) GOTO 20
                  CALL CUNM2R('R','C',n,n,n-1,V,Ldu,Work(n+1),A,Lda,    &
     &                        Work(2*n+1),iinfo)
                  IF ( iinfo/=0 ) GOTO 20
                  CALL CUNM2R('L','N',n,n,n-1,U,Ldu,Work,B,Lda,         &
     &                        Work(2*n+1),iinfo)
                  IF ( iinfo/=0 ) GOTO 20
                  CALL CUNM2R('R','C',n,n,n-1,V,Ldu,Work(n+1),B,Lda,    &
     &                        Work(2*n+1),iinfo)
                  IF ( iinfo/=0 ) GOTO 20
               ENDIF
            ELSE
...
!
 40         CALL CLACPY(' ',n,n,A,Lda,H,Lda)
...
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CGEQR2' , iinfo , n , jtype ,  &
     &                                  ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CUNM2R' , iinfo , n , jtype ,  &
     &                                  ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
...
!
            ...
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CGGHRD' , iinfo , n , jtype ,  &
     &                                  ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
...
!           Do tests 1--4
!
            CALL CGET51(1,n,A,Lda,H,Lda,U,Ldu,V,Ldu,Work,Rwork,Result(1)&
     &                  )
            CALL CGET51(1,n,B,Lda,T,Lda,U,Ldu,V,Ldu,Work,Rwork,Result(2)&
     &                  )
            CALL CGET51(3,n,B,Lda,T,Lda,U,Ldu,U,Ldu,Work,Rwork,Result(3)&
     &                  )
            CALL CGET51(3,n,B,Lda,T,Lda,V,Ldu,V,Ldu,Work,Rwork,Result(4)&
     &                  )
!
...
!
            ...
            Result(5) = ulpinv
!
            CALL CHGEQZ('E','N','N',n,1,n,S2,Lda,P2,Lda,Alpha3,Beta3,Q, &
     &                  Ldu,Z,Ldu,Work,Lwork,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CHGEQZ(E)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
...
!
            ...
            CALL CLACPY(' ',n,n,T,Lda,P2,Lda)
!
            CALL CHGEQZ('S','N','N',n,1,n,S2,Lda,P2,Lda,Alpha1,Beta1,Q, &
     &                  Ldu,Z,Ldu,Work,Lwork,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CHGEQZ(S)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
...
!
            CALL CHGEQZ('S','I','I',n,1,n,S1,Lda,P1,Lda,Alpha1,Beta1,Q, &
     &                  Ldu,Z,Ldu,Work,Lwork,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CHGEQZ(V)' , iinfo , n ,       &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
...
!           Do Tests 5--8
!
            CALL CGET51(1,n,H,Lda,S1,Lda,Q,Ldu,Z,Ldu,Work,Rwork,        &
     &                  Result(5))
            CALL CGET51(1,n,T,Lda,P1,Lda,Q,Ldu,Z,Ldu,Work,Rwork,        &
     &                  Result(6))
            CALL CGET51(3,n,T,Lda,P1,Lda,Q,Ldu,Q,Ldu,Work,Rwork,        &
     &                  Result(7))
            CALL CGET51(3,n,T,Lda,P1,Lda,Z,Ldu,Z,Ldu,Work,Rwork,        &
     &                  Result(8))
!
...
!
            ...
            ENDDO
!
            CALL CTGEVC('L','S',Llwork,n,S1,Lda,P1,Lda,Evectl,Ldu,      &
     &                  cdumma,Ldu,n,in,Work,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CTGEVC(L,S1)' , iinfo , n ,    &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
!
            ...
            ENDDO
!
            CALL CTGEVC('L','S',Llwork,n,S1,Lda,P1,Lda,Evectl(1,i1+1),  &
     &                  Ldu,cdumma,Ldu,n,in,Work,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CTGEVC(L,S2)' , iinfo , n ,    &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
!
            CALL CGET52(.TRUE.,n,S1,Lda,P1,Lda,Evectl,Ldu,Alpha1,Beta1, &
     &                  Work,Rwork,dumma(1))
            Result(9) = dumma(1)
            IF ( dumma(2)>Thrshn ) WRITE (Nounit,FMT=99002) 'Left' ,    &
     &           'CTGEVC(HOWMNY=S)' , dumma(2) , n , jtype , ioldsd
!
...
!
            ...
            CALL CTGEVC('L','B',Llwork,n,S1,Lda,P1,Lda,Evectl,Ldu,      &
     &                  cdumma,Ldu,n,in,Work,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CTGEVC(L,B)' , iinfo , n ,     &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
!
            CALL CGET52(.TRUE.,n,H,Lda,T,Lda,Evectl,Ldu,Alpha1,Beta1,   &
     &                  Work,Rwork,dumma(1))
            Result(10) = dumma(1)
            IF ( dumma(2)>Thrshn ) WRITE (Nounit,FMT=99002) 'Left' ,    &
     &           'CTGEVC(HOWMNY=B)' , dumma(2) , n , jtype , ioldsd
!
...
!
            ...
            ENDDO
!
            CALL CTGEVC('R','S',Llwork,n,S1,Lda,P1,Lda,cdumma,Ldu,      &
     &                  Evectr,Ldu,n,in,Work,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CTGEVC(R,S1)' , iinfo , n ,    &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
...
!
     ...
     &                  Evectr(1,i1+1),Ldu,n,in,Work,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CTGEVC(R,S2)' , iinfo , n ,    &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
!
            CALL CGET52(.FALSE.,n,S1,Lda,P1,Lda,Evectr,Ldu,Alpha1,Beta1,&
     &                  Work,Rwork,dumma(1))
            Result(11) = dumma(1)
...
!
            ...
            CALL CTGEVC('R','B',Llwork,n,S1,Lda,P1,Lda,cdumma,Ldu,      &
     &                  Evectr,Ldu,n,in,Work,Rwork,iinfo)
            IF ( iinfo/=0 ) THEN
               WRITE (Nounit,FMT=99001) 'CTGEVC(R,B)' , iinfo , n ,     &
     &                                  jtype , ioldsd
               Info = ABS(iinfo)
               GOTO 60
            ENDIF
!
            CALL CGET52(.FALSE.,n,H,Lda,T,Lda,Evectr,Ldu,Alpha1,Beta1,  &
     &                  Work,Rwork,dumma(1))
            Result(12) = dumma(1)
            IF ( dumma(2)>Thresh ) WRITE (Nounit,FMT=99002) 'Right' ,   &
     &           'CTGEVC(HOWMNY=B)' , dumma(2) , n , jtype , ioldsd
...
!              Do Tests 13--14
!
               CALL CGET51(2,n,S1,Lda,S2,Lda,Q,Ldu,Z,Ldu,Work,Rwork,    &
     &                     Result(13))
               CALL CGET51(2,n,P1,Lda,P2,Lda,Q,Ldu,Z,Ldu,Work,Rwork,    &
     &                     Result(14))
!
!              Do Test 15
!
...
!
      ...
      RETURN
!
99001 FORMAT (' CCHKGG: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99002 FORMAT (' CCHKGG: ',A,' Eigenvectors from ',A,' incorrectly ',    &
     &        'normalized.',/' Bits of error=',0P,G10.3,',',9X,'N=',I6, &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99003 FORMAT (1X,A3,' -- Complex Generalized eigenvalue problem')
!
99004 FORMAT (' Matrix types (see CCHKGG for details): ')
!
