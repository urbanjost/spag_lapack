!*==dget52.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DGET52
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DGET52(Left,N,A,Lda,B,Ldb,E,Lde,Alphar,Alphai,Beta,    &
     &                  Work,Result)
      IMPLICIT NONE
!*--DGET52203
!
...
!     .. Local Scalars ..
      ...
      INTEGER j , jvec
      DOUBLE PRECISION abmax , acoef , alfmax , anorm , bcoefi ,        &
     &                 bcoefr , betmax , bnorm , enorm , enrmer ,       &
     &                 errnrm , safmax , safmin , salfi , salfr ,       &
     &                 sbeta , scale , temp1 , ulp
!     ..
!     .. External Functions ..
...
!
     ...
     &              abmax<ONE ) THEN
               ...
               acoef = scale*sbeta
               bcoefr = scale*salfr
               CALL DGEMV(trans,N,N,acoef,A,Lda,E(1,jvec),1,ZERO,       &
     &                    Work(N*(jvec-1)+1),1)
               CALL DGEMV(trans,N,N,-bcoefr,B,Lda,E(1,jvec),1,ONE,      &
     &                    Work(N*(jvec-1)+1),1)
            ELSE
!
...
!
     ...
     &                    Work(N*(jvec-1)+1),1)
               CALL DGEMV(trans,N,N,-bcoefr,B,Lda,E(1,jvec),1,ONE,      &
     &                    Work(N*(jvec-1)+1),1)
               CALL DGEMV(trans,N,N,bcoefi,B,Lda,E(1,jvec+1),1,ONE,     &
     &                    Work(N*(jvec-1)+1),1)
!
               CALL DGEMV(trans,N,N,acoef,A,Lda,E(1,jvec+1),1,ZERO,     &
     &                    Work(N*jvec+1),1)
               CALL DGEMV(trans,N,N,-bcoefi,B,Lda,E(1,jvec),1,ONE,      &
     &                    Work(N*jvec+1),1)
               CALL DGEMV(trans,N,N,-bcoefr,B,Lda,E(1,jvec+1),1,ONE,    &
     &                    Work(N*jvec+1),1)
            ENDIF
         ENDIF
      ENDDO
