!*==zdrges3.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!
!  =====================================================================
      SUBROUTINE ZDRGES3(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A, &
...
!     .. Array Arguments ..
      ...
      INTEGER Iseed(4) , Nn(*)
      DOUBLE PRECISION Result(13) , Rwork(*)
      COMPLEX*16 A(Lda,*) , Alpha(*) , B(Lda,*) , Beta(*) , Q(Ldq,*) ,  &
...
!
                  ...
                  CALL ZGGES3('V','V',sort,ZLCTES,n,S,Lda,T,Lda,sdim,   &
     &                        Alpha,Beta,Q,Ldq,Z,Ldq,Work,Lwork,Rwork,  &
     &                        Bwork,iinfo)
                  IF ( iinfo/=0 .AND. iinfo/=n+2 ) THEN
                     Result(1+rsub+isort) = ulpinv
                     WRITE (Nounit,FMT=99001) 'ZGGES3' , iinfo , n ,    &
     &                      jtype , ioldsd
                     Info = ABS(iinfo)
                     EXIT
                  ENDIF
...
!
                  IF ( isort==0 ) THEN
                     CALL ZGET51(1,n,A,Lda,S,Lda,Q,Ldq,Z,Ldq,Work,Rwork,&
     &                           Result(1))
                     CALL ZGET51(1,n,B,Lda,T,Lda,Q,Ldq,Z,Ldq,Work,Rwork,&
     &                           Result(2))
                  ELSE
                     CALL ZGET54(n,A,Lda,B,Lda,S,Lda,T,Lda,Q,Ldq,Z,Ldq, &
     &                           Work,Result(2+rsub))
                  ENDIF
!
                  CALL ZGET51(3,n,B,Lda,T,Lda,Q,Ldq,Q,Ldq,Work,Rwork,   &
     &                        Result(3+rsub))
                  CALL ZGET51(3,n,B,Lda,T,Lda,Z,Ldq,Z,Ldq,Work,Rwork,   &
     &                        Result(4+rsub))
!
...
!
      RETURN
!
99001 FORMAT (' ZDRGES3: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,      &
     &        ', JTYPE=',I6,', ISEED=(',4(I4,','),I5,')')
!
99002 FORMAT (' ZDRGES3: S not in Schur form at eigenvalue ',I6,'.',/9X,&
     &        'N=',I6,', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99003 FORMAT (/1X,A3,' -- Complex Generalized Schur from problem ',     &
     &        'driver')
!
99004 FORMAT (' Matrix types (see ZDRGES3 for details): ')
!
