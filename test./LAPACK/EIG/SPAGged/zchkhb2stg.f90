!*==zchkhb2stg.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZCHKHBSTG
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZCHKHB2STG(Nsizes,Nn,Nwdths,Kk,Ntypes,Dotype,Iseed,    &
...
!     .. Array Arguments ..
      LOGICAL Dotype(*)
      INTEGER Iseed(4) , Kk(*) , Nn(*)
      DOUBLE PRECISION Result(*) , Rwork(*) , Sd(*) , Se(*) , D1(*) ,   &
     &                 D2(*) , D3(*)
      COMPLEX*16 A(Lda,*) , U(Ldu,*) , Work(*)
!     ..
!
...
!
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'ZHBTRD(U)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                           RETURN
...
!              Do tests 1 and 2
!
                     CALL ZHBT21('Upper',n,k,1,A,Lda,Sd,Se,U,Ldu,Work,  &
     &                           Rwork,Result(1))
!
...
!
     ...
     &                           iinfo)
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'ZSTEQR(N)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                           RETURN
...
!
                     ...
                     lh = MAX(1,4*n)
                     lw = Lwork - lh
                     CALL ZHETRD_HB2ST('N','N',"U",n,k,U,Ldu,Sd,Se,Work,&
...
!
                     IF ( iinfo/=0 ) THEN
                        WRITE (Nounit,FMT=99001) 'ZHBTRD(L)' , iinfo ,  &
     &                         n , jtype , ioldsd
                        Info = ABS(iinfo)
                        IF ( iinfo<0 ) THEN
                           RETURN
...
!              Do tests 3 and 4
!
                     CALL ZHBT21('Lower',n,k,1,A,Lda,Sd,Se,U,Ldu,Work,  &
     &                           Rwork,Result(3))
!
...
!
                     ...
                     lh = MAX(1,4*n)
                     lw = Lwork - lh
                     CALL ZHETRD_HB2ST('N','N',"L",n,k,U,Ldu,Sd,Se,Work,&
     &                                 lh,Work(lh+1),lw,iinfo)
!
...
!
      ...
      RETURN
!
99001 FORMAT (' ZCHKHBSTG: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,    &
     &        ', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
99002 FORMAT (/1X,A3,                                                   &
     &     ' -- Complex Hermitian Banded Tridiagonal Reduction Routines'&
     &     )
99003 FORMAT (' Matrix types (see DCHK23 for details): ')
!
