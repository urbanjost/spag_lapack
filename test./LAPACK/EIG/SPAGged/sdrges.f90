!*==sdrges.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SDRGES
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SDRGES(Nsizes,Nn,Ntypes,Dotype,Iseed,Thresh,Nounit,A,  &
     &                  Lda,B,S,T,Q,Ldq,Z,Alphar,Alphai,Beta,Work,Lwork,&
     &                  Result,Bwork,Info)
      IMPLICIT NONE
!*--SDRGES406
!
...
!     .. Array Arguments ..
      LOGICAL Bwork(*) , Dotype(*)
      INTEGER Iseed(4) , Nn(*)
      REAL A(Lda,*) , Alphai(*) , Alphar(*) , B(Lda,*) , Beta(*) ,      &
     &     Q(Ldq,*) , Result(13) , S(Lda,*) , T(Lda,*) , Work(*) ,      &
     &     Z(Ldq,*)
!     ..
!
!  =====================================================================
...
!
                  ...
                  CALL SGGES('V','V',sort,SLCTES,n,S,Lda,T,Lda,sdim,    &
     &                       Alphar,Alphai,Beta,Q,Ldq,Z,Ldq,Work,Lwork, &
     &                       Bwork,iinfo)
                  IF ( iinfo/=0 .AND. iinfo/=n+2 ) THEN
                     Result(1+rsub+isort) = ulpinv
                     WRITE (Nounit,FMT=99001) 'SGGES' , iinfo , n ,     &
     &                      jtype , ioldsd
                     Info = ABS(iinfo)
                     EXIT
                  ENDIF
...
!
     ...
     &                           Result(2))
                  ELSE
                     CALL SGET54(n,A,Lda,B,Lda,S,Lda,T,Lda,Q,Ldq,Z,Ldq, &
     &                           Work,Result(7))
                  ENDIF
...
!
                     ELSE
                        ...
                        ENDIF
                        IF ( .NOT.ilabad ) THEN
                           CALL SGET53(S(i1,i1),Lda,T(i1,i1),Lda,Beta(j)&
...
!
                     ...
                     knteig = 0
                     DO i = 1 , n
                        IF ( SLCTES(Alphar(i),Alphai(i),Beta(i)) .OR.   &
     &                       SLCTES(Alphar(i),-Alphai(i),Beta(i)) )     &
     &                       knteig = knteig + 1
                        IF ( i<n ) THEN
                           IF ( (SLCTES(Alphar(i+1),Alphai(i+1),Beta(i+1&
     &                          )) .OR.                                 &
     &                          SLCTES(Alphar(i+1),-Alphai(i+1),Beta    &
     &                          (i+1))) .AND.                           &
...
!
      RETURN
!
99001 FORMAT (' SDRGES: ',A,' returned INFO=',I6,'.',/9X,'N=',I6,       &
     &        ', JTYPE=',I6,', ISEED=(',4(I4,','),I5,')')
!
99002 FORMAT (' SDRGES: SGET53 returned INFO=',I1,' for eigenvalue ',I6,&
     &        '.',/9X,'N=',I6,', JTYPE=',I6,', ISEED=(',4(I4,','),I5,   &
     &        ')')
!
99003 FORMAT (' SDRGES: S not in Schur form at eigenvalue ',I6,'.',/9X, &
     &        'N=',I6,', JTYPE=',I6,', ISEED=(',3(I5,','),I5,')')
!
99004 FORMAT (/1X,A3,' -- Real Generalized Schur form driver')
!
99005 FORMAT (' Matrix types (see SDRGES for details): ')
!
