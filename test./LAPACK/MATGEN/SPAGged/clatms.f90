!*==clatms.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CLATMS
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CLATMS(M,N,Dist,Iseed,Sym,D,Mode,Cond,Dmax,Kl,Ku,Pack, &
     &                  A,Lda,Work,Info)
      IMPLICIT NONE
!*--CLATMS336
!
...
!
                  DO jr = 1 , MIN(M+jku,N) + jkl - 1
                     ...
                     IF ( jr<M ) THEN
                        ...
                        CALL CLAROT(.TRUE.,jr>jkl,.FALSE.,il,c,s,       &
...
!
                     ...
                     DO jch = jr - jkl , 1 , -jkl - jku
                        IF ( ir<M ) THEN
                           CALL CLARTG(A(ir+1-iskew*(ic+1)+ioffst,ic+1),&
     &                                 extra,realc,s,dummy)
     ...
     &                              ctemp,extra)
                        IF ( iltemp ) THEN
                           CALL CLARTG(A(irow+1-iskew*(ic+1)+ioffst,ic+1&
     &                                 ),ctemp,realc,s,dummy)
                           dummy = CLARND(5,Iseed)
                           c = CONJG(realc*dummy)
                           s = CONJG(-s*dummy)
!
                           ...
                           il = ic + 2 - icol
                           extra = CZERO
                           CALL CLAROT(.TRUE.,jch>jku+jkl,.TRUE.,il,c,s,&
...
!
                  DO jc = 1 , MIN(N+jkl,M) + jku - 1
                     ...
                     IF ( jc<N ) THEN
                        ...
                        CALL CLAROT(.FALSE.,jc>jku,.FALSE.,il,c,s,      &
...
!
     ...
     &                              ctemp,extra)
                        IF ( iltemp ) THEN
                           CALL CLARTG(A(ir+1-iskew*(icol+1)+ioffst,icol&
     &                                 +1),ctemp,realc,s,dummy)
                           dummy = CLARND(5,Iseed)
                           c = CONJG(realc*dummy)
                           s = CONJG(-s*dummy)
                           ...
                           il = ir + 2 - irow
                           extra = CZERO
                           CALL CLAROT(.FALSE.,jch>jkl+jku,.TRUE.,il,c, &
...
!
                  ...
                  DO jc = MIN(M+jku,N) - 1 , 1 - jkl , -1
                     ...
                     IF ( jc>0 ) THEN
                        ...
                        CALL CLAROT(.FALSE.,.FALSE.,jc+jkl<M,il,c,s,    &
     &                              A(irow-iskew*jc+ioffst,jc),ilda,    &
     &                              dummy,extra)
                     ENDIF
!
...
!
                     ...
                     DO jch = jc + jkl , iendch , jkl + jku
                        ilextr = ic>0
                        IF ( ilextr ) THEN
                           CALL CLARTG(A(jch-iskew*ic+ioffst,ic),extra, &
     &                                 realc,s,dummy)
                           dummy = CLARND(5,Iseed)
                           c = realc*dummy
                           s = s*dummy
                        ...
                        iltemp = jch + jku<N
                        ctemp = CZERO
                        CALL CLAROT(.TRUE.,ilextr,iltemp,icol+2-ic,c,s, &
     &                              A(jch-iskew*ic+ioffst,ic),ilda,     &
     &                              extra,ctemp)
                        IF ( iltemp ) THEN
                           CALL CLARTG(A(jch-iskew*icol+ioffst,icol),   &
     &                                 ctemp,realc,s,dummy)
                           dummy = CLARND(5,Iseed)
                           c = realc*dummy
                           s = s*dummy
...
!
                  ...
                  DO jr = MIN(N+jkl,M) - 1 , 1 - jku , -1
                     ...
                     IF ( jr>0 ) THEN
                        ...
                        CALL CLAROT(.TRUE.,.FALSE.,jr+jku<N,il,c,s,     &
     &                              A(jr-iskew*icol+ioffst,icol),ilda,  &
     &                              dummy,extra)
                     ENDIF
!
...
!
                     ...
                     DO jch = jr + jku , iendch , jkl + jku
                        ilextr = ir>0
                        IF ( ilextr ) THEN
                           CALL CLARTG(A(ir-iskew*jch+ioffst,jch),extra,&
     &                                 realc,s,dummy)
                           dummy = CLARND(5,Iseed)
                           c = realc*dummy
                           s = s*dummy
                        ...
                        iltemp = jch + jkl<M
                        ctemp = CZERO
                        CALL CLAROT(.FALSE.,ilextr,iltemp,irow+2-ir,c,s,&
...
!
     ...
     &                           ctemp)
                     CALL CLAROT(.TRUE.,.TRUE.,.FALSE.,MIN(k,N-jc)+1,ct,&
     &                           st,A((1-iskew)*jc+ioffg,jc),ilda,ctemp,&
     &                           dummy)
!
...
!
                     icol = jc
                     DO jch = jc - k , 1 , -k
                        CALL CLARTG(A(jch+1-iskew*(icol+1)+ioffg,icol+1)&
     &                              ,extra,realc,s,dummy)
                        dummy = CLARND(5,Iseed)
                        c = CONJG(realc*dummy)
                        s = CONJG(-s*dummy)
     ...
     &                              ctemp,extra)
                        ...
                        il = MIN(jch+1,k+2)
                        extra = CZERO
                        CALL CLAROT(.FALSE.,jch>k,.TRUE.,il,ct,st,      &
...
!
     ...
     &                           A((1-iskew)*jc+ioffg,jc),ilda,ctemp,   &
     &                           extra)
                     icol = MAX(1,jc-k+1)
                     CALL CLAROT(.TRUE.,.FALSE.,.TRUE.,jc+2-icol,ct,st, &
...
!
                     icol = jc
                     DO jch = jc + k , N - 1 , k
                        CALL CLARTG(A(jch-iskew*icol+ioffg,icol),extra, &
     &                              realc,s,dummy)
                        dummy = CLARND(5,Iseed)
                        c = realc*dummy
                        s = s*dummy
     ...
     &                              extra,ctemp)
                        il = MIN(N+1-jch,k+2)
                        extra = CZERO
                        CALL CLAROT(.FALSE.,.TRUE.,N-jch>k,il,ct,st,    &
     &                              A((1-iskew)*jch+ioffg,jch),ilda,    &
     &                              ctemp,extra)
                        icol = jch
                     ENDDO
