!*==zlaror.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZLAROR
!
!  =========== DOCUMENTATION ===========
!
...
!
            CALL ZGEMV('C',ixfrm,N,CONE,A(kbeg,1),Lda,X(kbeg),1,CZERO,  &
     &                 X(2*nxfrm+1),1)
            CALL ZGERC(ixfrm,N,-DCMPLX(factor),X(kbeg),1,X(2*nxfrm+1),1,&
     &                 A(kbeg,1),Lda)
!
         ENDIF
!
...
!
            IF ( itype==4 ) CALL ZLACGV(ixfrm,X(kbeg),1)
!
            CALL ZGEMV('N',M,ixfrm,CONE,A(1,kbeg),Lda,X(kbeg),1,CZERO,  &
     &                 X(2*nxfrm+1),1)
            CALL ZGERC(M,ixfrm,-DCMPLX(factor),X(2*nxfrm+1),1,X(kbeg),1,&
     &                 A(1,kbeg),Lda)
!
         ENDIF
      ENDDO
