!*==clatmr.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CLATMR
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CLATMR(M,N,Dist,Iseed,Sym,D,Mode,Cond,Dmax,Rsign,Grade,&
...
!
      ...
      ELSEIF ( LSAME(Grade,'E') ) THEN
         igrade = 4
      ELSEIF ( LSAME(Grade,'H') ) THEN
         igrade = 5
      ELSEIF ( LSAME(Grade,'S') ) THEN
         igrade = 6
      ELSE
         igrade = -1
      ENDIF
...
!
      IF ( M<0 ) THEN
         Info = -1
      ELSEIF ( M/=N .AND. (isym==0 .OR. isym==2) ) THEN
         Info = -1
      ELSEIF ( N<0 ) THEN
         Info = -2
      ELSEIF ( idist==-1 ) THEN
     ...
     &         igrade==4 .OR. igrade==5) .AND. isym==2) ) THEN
      ...
      ELSEIF ( igrade==4 .AND. dzero ) THEN
         Info = -12
      ELSEIF ( (igrade==1 .OR. igrade==3 .OR. igrade==4 .OR.            &
     &         igrade==5 .OR. igrade==6) .AND. (Model<-6 .OR. Model>6) )&
     &         THEN
         Info = -13
      ELSEIF ( (igrade==1 .OR. igrade==3 .OR. igrade==4 .OR.            &
     ...
     &         ((ipvtng==1 .OR. ipvtng==2) .AND. (isym==0 .OR. isym==2))&
     &         ) THEN
         Info = -18
      ...
      ELSEIF ( Kl<0 ) THEN
         Info = -20
      ELSEIF ( Ku<0 .OR. ((isym==0 .OR. isym==2) .AND. Kl/=Ku) ) THEN
         Info = -21
...
!
      IF ( igrade==1 .OR. igrade==3 .OR. igrade==4 .OR. igrade==5 .OR.  &
     &     igrade==6 ) THEN
...
!
         IF ( ipack==0 ) THEN
            IF ( isym==0 ) THEN
               DO j = 1 , N
                  DO i = 1 , j
                     ctemp = CLATM3(M,N,i,j,isub,jsub,Kl,Ku,idist,Iseed,&
...
!
            DO j = 1 , N
               DO i = 1 , j
                  ctemp = CLATM3(M,N,i,j,isub,jsub,Kl,Ku,idist,Iseed,D, &
...
!
      ELSEIF ( ipack==0 ) THEN
         IF ( isym==0 ) THEN
            DO j = 1 , N
               DO i = 1 , j
                  A(i,j) = CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,igrade,Dl,&
...
!
         DO j = 1 , N
            DO i = 1 , j
               A(i,j) = CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,igrade,Dl,Dr,&
     &                  ipvtng,Iwork,Sparse)
               IF ( i/=j ) A(j,i) = CZERO
            ENDDO
         ENDDO
...
!
         DO j = 1 , N
            DO i = 1 , j
               IF ( isym==0 ) THEN
                  A(j,i) = CONJG(CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,    &
     &                     igrade,Dl,Dr,ipvtng,Iwork,Sparse))
               ELSE
                  A(j,i) = CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,igrade,Dl,&
     &                     Dr,ipvtng,Iwork,Sparse)
               ENDIF
               IF ( i/=j ) A(i,j) = CZERO
            ENDDO
         ...
         DO j = 1 , N
            DO i = 1 , j
               ...
               IF ( isub>Lda ) THEN
                  ...
                  jsub = jsub + 1
               ENDIF
               A(isub,jsub) = CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,igrade,&
...
!
                  ...
                  isub = k - Lda*(jsub-1)
!
                  A(isub,jsub) = CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,    &
     &                           igrade,Dl,Dr,ipvtng,Iwork,Sparse)
         ...
         DO j = 1 , N
            DO i = j - kuu , j
               IF ( i<1 ) THEN
                  A(j-i+1,i+N) = CZERO
               ELSEIF ( isym==0 ) THEN
                  A(j-i+1,i) = CONJG(CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,&
     &                         igrade,Dl,Dr,ipvtng,Iwork,Sparse))
               ELSE
                  A(j-i+1,i) = CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,      &
     &                         igrade,Dl,Dr,ipvtng,Iwork,Sparse)
               ENDIF
            ENDDO
         ENDDO
...
!
         DO j = 1 , N
            DO i = j - kuu , j
               A(i-j+kuu+1,j) = CLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,     &
     &                          igrade,Dl,Dr,ipvtng,Iwork,Sparse)
            ENDDO
         ENDDO
!
