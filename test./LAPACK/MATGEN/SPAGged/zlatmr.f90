!*==zlatmr.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b ZLATMR
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE ZLATMR(M,N,Dist,Iseed,Sym,D,Mode,Cond,Dmax,Rsign,Grade,&
...
!
         IF ( ipack==0 ) THEN
            IF ( isym==0 ) THEN
               DO j = 1 , N
                  DO i = 1 , j
                     ctemp = ZLATM3(M,N,i,j,isub,jsub,Kl,Ku,idist,Iseed,&
...
!
         DO j = 1 , N
            DO i = 1 , j
               IF ( isym==0 ) THEN
                  A(j,i) = DCONJG(ZLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,   &
     &                     igrade,Dl,Dr,ipvtng,Iwork,Sparse))
               ELSE
         ...
         DO j = 1 , N
            DO i = 1 , j
               ...
               IF ( isub>Lda ) THEN
                  ...
                  jsub = jsub + 1
               ENDIF
               A(isub,jsub) = ZLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,igrade,&
...
!
                  ...
                  isub = k - Lda*(jsub-1)
!
                  A(isub,jsub) = ZLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D,    &
     &                           igrade,Dl,Dr,ipvtng,Iwork,Sparse)
         ...
         DO j = 1 , N
            DO i = j - kuu , j
               IF ( i<1 ) THEN
                  A(j-i+1,i+N) = CZERO
               ELSEIF ( isym==0 ) THEN
                  A(j-i+1,i) = DCONJG(ZLATM2(M,N,i,j,Kl,Ku,idist,Iseed,D&
     &                         ,igrade,Dl,Dr,ipvtng,Iwork,Sparse))
               ELSE
