!*==slagge.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SLAGGE
!
!  =========== DOCUMENTATION ===========
!
...
!
            CALL SGEMV('Transpose',M-i+1,N-i+1,ONE,A(i,i),Lda,Work,1,   &
     &                 ZERO,Work(M+1),1)
            CALL SGER(M-i+1,N-i+1,-tau,Work,1,Work(M+1),1,A(i,i),Lda)
         ENDIF
         IF ( i<N ) THEN
!
!           generate random reflection
...
!
            CALL SGEMV('No transpose',M-i+1,N-i+1,ONE,A(i,i),Lda,Work,1,&
     &                 ZERO,Work(N+1),1)
            CALL SGER(M-i+1,N-i+1,-tau,Work(N+1),1,Work,1,A(i,i),Lda)
         ENDIF
      ENDDO
!
...
!
               CALL SGEMV('Transpose',M-Kl-i+1,N-i,ONE,A(Kl+i,i+1),Lda, &
     &                    A(Kl+i,i),1,ZERO,Work,1)
...
!
               CALL SGEMV('No transpose',M-i,N-Ku-i+1,ONE,A(i+1,Ku+i),  &
     &                    Lda,A(i,Ku+i),Lda,ZERO,Work,1)
               CALL SGER(M-i,N-Ku-i+1,-tau,Work,1,A(i,Ku+i),Lda,        &
     &                   A(i+1,Ku+i),Lda)
               A(i,Ku+i) = -wa
            ENDIF
         ELSE
