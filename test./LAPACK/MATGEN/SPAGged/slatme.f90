!*==slatme.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SLATME
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE SLATME(N,Dist,Iseed,D,Mode,Cond,Dmax,Ei,Rsign,Upper,   &
...
!
            ...
            Work(1) = ONE
!
            CALL SGEMV('T',irows,icols,ONE,A(jcr,ic+1),Lda,Work,1,ZERO, &
     &                 Work(irows+1),1)
            CALL SGER(irows,icols,-tau,Work,1,Work(irows+1),1,          &
     &                A(jcr,ic+1),Lda)
!
            CALL SGEMV('N',N,irows,ONE,A(1,jcr),Lda,Work,1,ZERO,        &
     &                 Work(irows+1),1)
            CALL SGER(N,irows,-tau,Work(irows+1),1,Work,1,A(1,jcr),Lda)
!
            A(jcr,ic) = xnorms
...
!
            ...
            Work(1) = ONE
!
            CALL SGEMV('N',irows,icols,ONE,A(ir+1,jcr),Lda,Work,1,ZERO, &
     &                 Work(icols+1),1)
            CALL SGER(irows,icols,-tau,Work(icols+1),1,Work,1,          &
     &                A(ir+1,jcr),Lda)
!
            CALL SGEMV('C',icols,N,ONE,A(jcr,1),Lda,Work,1,ZERO,        &
     &                 Work(icols+1),1)
            CALL SGER(icols,N,-tau,Work,1,Work(icols+1),1,A(jcr,1),Lda)
!
            A(ir,jcr) = xnorms
