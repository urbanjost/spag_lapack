!*==dlatmt.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b DLATMT
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE DLATMT(M,N,Dist,Iseed,Sym,D,Mode,Cond,Dmax,Rank,Kl,Ku, &
...
!     .. External Subroutines ..
      EXTERNAL DLATM7 , DCOPY , DLAGGE , DLAGSY , DLAROT , DLARTG ,     &
     &         DLASET , DSCAL , XERBLA
!     ..
!     .. Intrinsic Functions ..
...
!
                  DO jr = 1 , MIN(M+jku,N) + jkl - 1
                     ...
                     IF ( jr<M ) THEN
                        ...
                        CALL DLAROT(.TRUE.,jr>jkl,.FALSE.,il,c,s,       &
...
!
     ...
     &                              temp,extra)
                        IF ( iltemp ) THEN
                           CALL DLARTG(A(irow+1-iskew*(ic+1)+ioffst,ic+1&
     &                                 ),temp,c,s,dummy)
                           icol = MAX(1,jch-jku-jkl)
                           il = ic + 2 - icol
                           extra = ZERO
                           CALL DLAROT(.TRUE.,jch>jku+jkl,.TRUE.,il,c,  &
...
!
                  DO jc = 1 , MIN(N+jkl,M) + jku - 1
                     ...
                     IF ( jc<N ) THEN
                        ...
                        CALL DLAROT(.FALSE.,jc>jku,.FALSE.,il,c,s,      &
...
!
     ...
     &                              temp,extra)
                        IF ( iltemp ) THEN
                           CALL DLARTG(A(ir+1-iskew*(icol+1)+ioffst,icol&
     &                                 +1),temp,c,s,dummy)
                           irow = MAX(1,jch-jkl-jku)
                           il = ir + 2 - irow
                           extra = ZERO
                           CALL DLAROT(.FALSE.,jch>jkl+jku,.TRUE.,il,c, &
     &                                 -s,A(irow-iskew*icol+ioffst,icol)&
     &                                 ,ilda,extra,temp)
                           ic = icol
                           ir = irow
...
!
                  ...
                  DO jc = MIN(M+jku,N) - 1 , 1 - jkl , -1
                     ...
                     IF ( jc>0 ) THEN
                        ...
                        CALL DLAROT(.FALSE.,.FALSE.,jc+jkl<M,il,c,s,    &
     &                              A(irow-iskew*jc+ioffst,jc),ilda,    &
     &                              dummy,extra)
                     ENDIF
!
...
!
     ...
     &                       extra,c,s,dummy)
                        ...
                        iltemp = jch + jku<N
                        temp = ZERO
                        CALL DLAROT(.TRUE.,ilextr,iltemp,icol+2-ic,c,s, &
     &                              A(jch-iskew*ic+ioffst,ic),ilda,     &
     &                              extra,temp)
                        IF ( iltemp ) THEN
                           CALL DLARTG(A(jch-iskew*icol+ioffst,icol),   &
     &                                 temp,c,s,dummy)
...
!
                  ...
                  DO jr = MIN(N+jkl,M) - 1 , 1 - jku , -1
                     ...
                     IF ( jr>0 ) THEN
                        ...
                        CALL DLAROT(.TRUE.,.FALSE.,jr+jku<N,il,c,s,     &
     &                              A(jr-iskew*icol+ioffst,icol),ilda,  &
     &                              dummy,extra)
                     ENDIF
!
...
!
     ...
     &                       extra,c,s,dummy)
                        ...
                        iltemp = jch + jkl<M
                        temp = ZERO
                        CALL DLAROT(.FALSE.,ilextr,iltemp,irow+2-ir,c,s,&
...
!
               DO k = 1 , uub
                  DO jc = 1 , N - 1
                     ...
                     c = COS(angle)
                     s = SIN(angle)
                     CALL DLAROT(.FALSE.,jc>k,.TRUE.,il,c,s,            &
     &                           A(irow-iskew*jc+ioffg,jc),ilda,extra,  &
     &                           temp)
                     CALL DLAROT(.TRUE.,.TRUE.,.FALSE.,MIN(k,N-jc)+1,c, &
...
!
                     icol = jc
                     DO jch = jc - k , 1 , -k
                        CALL DLARTG(A(jch+1-iskew*(icol+1)+ioffg,icol+1)&
     &                              ,extra,c,s,dummy)
     ...
     &                              temp,extra)
                        ...
                        il = MIN(jch+1,k+2)
                        extra = ZERO
                        CALL DLAROT(.FALSE.,jch>k,.TRUE.,il,c,-s,       &
...
!
               DO k = 1 , uub
                  DO jc = N - 1 , 1 , -1
                     ...
                     c = COS(angle)
                     s = -SIN(angle)
                     CALL DLAROT(.FALSE.,.TRUE.,N-jc>k,il,c,s,          &
     &                           A((1-iskew)*jc+ioffg,jc),ilda,temp,    &
     &                           extra)
                     icol = MAX(1,jc-k+1)
                     CALL DLAROT(.TRUE.,.FALSE.,.TRUE.,jc+2-icol,c,s,   &
...
!
                     icol = jc
                     DO jch = jc + k , N - 1 , k
                        CALL DLARTG(A(jch-iskew*icol+ioffg,icol),extra, &
     &                              c,s,dummy)
     ...
     &                              extra,temp)
                        il = MIN(N+1-jch,k+2)
                        extra = ZERO
                        CALL DLAROT(.FALSE.,.TRUE.,N-jch>k,il,c,s,      &
     &                              A((1-iskew)*jch+ioffg,jch),ilda,    &
     &                              temp,extra)
                        icol = jch
                     ENDDO
