!*==clagge.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CLAGGE
!
!  =========== DOCUMENTATION ===========
!
...
!
            CALL CGEMV('Conjugate transpose',M-i+1,N-i+1,ONE,A(i,i),Lda,&
     &                 Work,1,ZERO,Work(M+1),1)
            CALL CGERC(M-i+1,N-i+1,-tau,Work,1,Work(M+1),1,A(i,i),Lda)
         ENDIF
         IF ( i<N ) THEN
!
!           generate random reflection
...
!
            CALL CGEMV('No transpose',M-i+1,N-i+1,ONE,A(i,i),Lda,Work,1,&
     &                 ZERO,Work(N+1),1)
            CALL CGERC(M-i+1,N-i+1,-tau,Work(N+1),1,Work,1,A(i,i),Lda)
         ENDIF
      ENDDO
!
...
!
     ...
     &                    A(Kl+i,i+1),Lda,A(Kl+i,i),1,ZERO,Work,1)
               CALL CGERC(M-Kl-i+1,N-i,-tau,A(Kl+i,i),1,Work,1,         &
     &                    A(Kl+i,i+1),Lda)
               A(Kl+i,i) = -wa
            ENDIF
!
...
!
               ...
               CALL CGEMV('No transpose',M-i,N-Ku-i+1,ONE,A(i+1,Ku+i),  &
     &                    Lda,A(i,Ku+i),Lda,ZERO,Work,1)
               CALL CGERC(M-i,N-Ku-i+1,-tau,Work,1,A(i,Ku+i),Lda,       &
     &                    A(i+1,Ku+i),Lda)
               A(i,Ku+i) = -wa
            ENDIF
         ELSE
