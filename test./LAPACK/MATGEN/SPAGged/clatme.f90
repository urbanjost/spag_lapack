!*==clatme.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b CLATME
!
!  =========== DOCUMENTATION ===========
!
...
!
!  =====================================================================
      SUBROUTINE CLATME(N,Dist,Iseed,D,Mode,Cond,Dmax,Rsign,Upper,Sim,  &
...
!
            CALL CGEMV('C',irows,icols,CONE,A(jcr,ic+1),Lda,Work,1,     &
     &                 CZERO,Work(irows+1),1)
            CALL CGERC(irows,icols,-tau,Work,1,Work(irows+1),1,         &
     &                 A(jcr,ic+1),Lda)
!
            CALL CGEMV('N',N,irows,CONE,A(1,jcr),Lda,Work,1,CZERO,      &
     &                 Work(irows+1),1)
            CALL CGERC(N,irows,-CONJG(tau),Work(irows+1),1,Work,1,      &
     &                 A(1,jcr),Lda)
!
            A(jcr,ic) = xnorms
...
!
            ...
            alpha = CLARND(5,Iseed)
!
            CALL CGEMV('N',irows,icols,CONE,A(ir+1,jcr),Lda,Work,1,     &
     &                 CZERO,Work(icols+1),1)
            CALL CGERC(irows,icols,-tau,Work(icols+1),1,Work,1,         &
     &                 A(ir+1,jcr),Lda)
!
            CALL CGEMV('C',icols,N,CONE,A(jcr,1),Lda,Work,1,CZERO,      &
     &                 Work(icols+1),1)
            CALL CGERC(icols,N,-CONJG(tau),Work,1,Work(icols+1),1,      &
     &                 A(jcr,1),Lda)
!
            A(ir,jcr) = xnorms
