!*==sblat2.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
!> \brief \b SBLAT2
!
!  =========== DOCUMENTATION ===========
!
...
!     .. Local Arrays ..
      REAL a(NMAX,NMAX) , aa(NMAX*NMAX) , alf(NALMAX) , as(NMAX*NMAX) , &
     &     bet(NBEMAX) , g(NMAX) , x(NMAX) , xs(NMAX*INCMAX) ,          &
...
!     .. Data statements ..
     ...
     &     'STRMV ' , 'STBMV ' , 'STPMV ' , 'STRSV ' , 'STBSV ' ,       &
     &     'STPSV ' , 'SGER  ' , 'SSYR  ' , 'SSPR  ' , 'SSYR2 ' ,       &
     &     'SSPR2 '/
...
!           Test SSYMV, 03, SSBMV, 04, and SSPMV, 05.
               CALL SCHK2(snames(isnum),eps,thresh,nout,ntra,trace,rewi,&
...
!           STRSV, 09, STBSV, 10, and STPSV, 11.
               CALL SCHK3(snames(isnum),eps,thresh,nout,ntra,trace,rewi,&
     &                    fatal,nidim,idim,nkb,kb,ninc,inc,NMAX,INCMAX, &
     &                    a,aa,as,y,yy,ys,yt,g,z)
            ELSEIF ( isnum==12 ) THEN
!           Test SGER, 12.
               CALL SCHK4(snames(isnum),eps,thresh,nout,ntra,trace,rewi,&
     &                    fatal,nidim,idim,nalf,alf,ninc,inc,NMAX,      &
     &                    INCMAX,a,aa,as,x,xx,xs,y,yy,ys,yt,g,z)
            ELSEIF ( isnum==13 .OR. isnum==14 ) THEN
!           Test SSYR, 13, and SSPR, 14.
               CALL SCHK5(snames(isnum),eps,thresh,nout,ntra,trace,rewi,&
     &                    fatal,nidim,idim,nalf,alf,ninc,inc,NMAX,      &
     &                    INCMAX,a,aa,as,x,xx,xs,y,yy,ys,yt,g,z)
            ELSEIF ( isnum==15 .OR. isnum==16 ) THEN
!           Test SSYR2, 15, and SSPR2, 16.
               CALL SCHK6(snames(isnum),eps,thresh,nout,ntra,trace,rewi,&
     &                    fatal,nidim,idim,nalf,alf,ninc,inc,NMAX,      &
     &                    INCMAX,a,aa,as,x,xx,xs,y,yy,ys,yt,g,z)
            ELSE
!           Test SGEMV, 01, and SGBMV, 02.
               CALL SCHK1(snames(isnum),eps,thresh,nout,ntra,trace,rewi,&
     &                    fatal,nidim,idim,nkb,kb,nalf,alf,nbet,bet,    &
     &                    ninc,inc,NMAX,INCMAX,a,aa,as,x,xx,xs,y,yy,ys, &
     &                    yt,g)
            ENDIF
...
99006 FORMAT (' ABSOLUTE VALUE OF INCX OR INCY IS 0 OR GREATER THAN ',  &
     &        I2)
99007 FORMAT (' TESTS OF THE REAL             LEVEL 2 BLAS',//' THE F', &
...
99014 FORMAT (' SUBPROGRAM NAME ',A6,' NOT RECOGNIZED',/' ******* T',   &
     &        'ESTS ABANDONED *******')
99015 FORMAT (' ERROR IN SMVCH -  IN-LINE DOT PRODUCTS ARE BEING EVALU',&
...
!
      END PROGRAM SBLAT2
!*==schk1.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!     .. Scalar Arguments ..
     ...
     &        Ntra
      ...
      CHARACTER*6 Sname
!     .. Array Arguments ..
      REAL A(Nmax,Nmax) , Aa(Nmax*Nmax) , Alf(Nalf) , As(Nmax*Nmax) ,   &
...
!
                  transl = ZERO
                  CALL SMAKE(Sname(2:3),' ',' ',m,n,A,Nmax,Aa,lda,kl,ku,&
     &                       reset,transl)
!
                  DO ic = 1 , 3
                     trans = ich(ic:ic)
...
!
     ...
     &                                 alpha , lda , incx , beta , incy
                                    IF ( Rewi ) REWIND Ntra
                                    CALL SGEMV(trans,m,n,alpha,Aa,lda,  &
     ...
     &                                 incy
                                    IF ( Rewi ) REWIND Ntra
                                    CALL SGBMV(trans,m,n,kl,ku,alpha,Aa,&
...
!     End of SCHK1.
!
      END SUBROUTINE SCHK1
!*==schk2.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!
                  transl = ZERO
                  CALL SMAKE(Sname(2:3),uplo,' ',n,n,A,Nmax,Aa,lda,k,k, &
     &                       reset,transl)
!
                  DO ix = 1 , Ninc
                     incx = Inc(ix)
...
!
     ...
     &                                lda , incx , beta , incy
                                 IF ( Rewi ) REWIND Ntra
                                 CALL SSYMV(uplo,n,alpha,Aa,lda,Xx,incx,&
     &                              beta,Yy,incy)
                              ELSEIF ( banded ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99006)    &
     &                                nc , Sname , uplo , n , k ,       &
     &                                alpha , lda , incx , beta , incy
                                 IF ( Rewi ) REWIND Ntra
                                 CALL SSBMV(uplo,n,k,alpha,Aa,lda,Xx,   &
     &                              incx,beta,Yy,incy)
                              ELSEIF ( packed ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99005)    &
     &                                nc , Sname , uplo , n , alpha ,   &
...
!     End of SCHK2.
!
      END SUBROUTINE SCHK2
!*==schk3.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!     .. Scalar Arguments ..
      ...
      CHARACTER*6 Sname
!     .. Array Arguments ..
      REAL A(Nmax,Nmax) , Aa(Nmax*Nmax) , As(Nmax*Nmax) , G(Nmax) ,     &
...
!     .. External Functions ..
      ...
      EXTERNAL LSE , LSERES
!     .. External Subroutines ..
      EXTERNAL SMAKE , SMVCH , STBMV , STBSV , STPMV , STPSV , STRMV ,  &
     &         STRSV
!     .. Intrinsic Functions ..
      INTRINSIC ABS , MAX
!     .. Scalars in Common ..
...
!
                        transl = ZERO
                        CALL SMAKE(Sname(2:3),uplo,diag,n,n,A,Nmax,Aa,  &
...
!
     ...
     &                                n , lda , incx
                                 IF ( Rewi ) REWIND Ntra
                                 CALL STRMV(uplo,trans,diag,n,Aa,lda,Xx,&
     &                              incx)
                              ELSEIF ( banded ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99006)    &
     &                                nc , Sname , uplo , trans , diag ,&
     &                                n , k , lda , incx
                                 IF ( Rewi ) REWIND Ntra
                                 CALL STBMV(uplo,trans,diag,n,k,Aa,lda, &
     &                              Xx,incx)
                              ELSEIF ( packed ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99005)    &
     &                                nc , Sname , uplo , trans , diag ,&
     ...
     &                                n , lda , incx
                                 IF ( Rewi ) REWIND Ntra
                                 CALL STRSV(uplo,trans,diag,n,Aa,lda,Xx,&
     &                              incx)
                              ELSEIF ( banded ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99006)    &
     &                                nc , Sname , uplo , trans , diag ,&
     &                                n , k , lda , incx
                                 IF ( Rewi ) REWIND Ntra
                                 CALL STBSV(uplo,trans,diag,n,k,Aa,lda, &
     &                              Xx,incx)
                              ELSEIF ( packed ) THEN
                                 IF ( Trace ) WRITE (Ntra,FMT=99005)    &
     &                                nc , Sname , uplo , trans , diag ,&
...
!     End of SCHK3.
!
      END SUBROUTINE SCHK3
!*==schk4.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!
                  transl = HALF
                  CALL SMAKE('GE',' ',' ',1,m,X,1,Xx,ABS(incx),0,m-1,   &
     &                       reset,transl)
                  IF ( m>1 ) THEN
                     X(m/2) = ZERO
...
!
                        transl = ZERO
                        CALL SMAKE(Sname(2:3),' ',' ',m,n,A,Nmax,Aa,lda,&
...
!     End of SCHK4.
!
      END SUBROUTINE SCHK4
!*==schk5.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!
                     transl = ZERO
                     CALL SMAKE(Sname(2:3),uplo,' ',n,n,A,Nmax,Aa,lda,  &
...
!     End of SCHK5.
!
      END SUBROUTINE SCHK5
!*==schk6.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!
     ...
     &                          lda
                           IF ( Rewi ) REWIND Ntra
                           CALL SSYR2(uplo,n,alpha,Xx,incx,Yy,incy,Aa,  &
     &                                lda)
                        ELSEIF ( packed ) THEN
...
!
                        ...
                        IF ( null ) THEN
                           isame(8) = LSE(As,Aa,laa)
                        ELSE
                           isame(8) = LSERES(Sname(2:3),uplo,n,n,As,Aa, &
     &                                lda)
                        ENDIF
...
!
                           ...
                           DO j = 1 , n
                              ...
                              ELSE
                                 ...
                                 lj = n - j + 1
                              ENDIF
                              CALL SMVCH('N',lj,2,alpha,Z(jj,1),Nmax,w, &
...
!     End of SCHK6.
!
      END SUBROUTINE SCHK6
!*==schke.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!     End of SCHKE.
!
      END SUBROUTINE SCHKE
!*==smake.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!     End of SMAKE.
!
      END SUBROUTINE SMAKE
!*==smvch.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!*==lse.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!     End of LSE.
!
99999 END FUNCTION LSE
!*==lseres.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
...
!*==sbeg.f90  processed by SPAG 7.51RB at 20:37 on  3 Mar 2022
